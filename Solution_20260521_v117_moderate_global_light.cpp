...
void solve_job(){
    ...
    if(!huge_job){
        ...
        // Moderate structured jobs may benefit from a global-aware trajectory
        // before the heavier global_swap/LNS passes over-steer the solution.
        if(any_jm_le_r && fl_count<=4000 && best_eval.score_fg>g_r+1){
            run_greedy(m, 1, 3, 1);
            run_swap(m);
            run_jm_repair(m);
            TRY_STRATEGY();
