...
void solve_job(){
    ...
    // Greedy: per card, evaluate all ports, pick best
    // Use per-flow evaluation (fast) but account for all phases via pmask
    for(int leaf=0;leaf<g_l;++leaf){
        int fc=leaf*g_pr,lc=fc+g_pr;
        for(int card=fc;card<lc;++card){
            if(card_start[card+1]==card_start[card])continue;
            int has_inter=0;
            for(int j=card_start[card];j<card_start[card+1];++j){
                if(fl_dst[card_fl_idx[j]]/g_pr!=leaf){has_inter=1;break;}
            }
            ...
            for(int j=card_start[card];j<card_start[card+1];++j){
                int fi=card_fl_idx[j],dl=fl_dst[fi]/g_pr;
                if(dl==leaf){fl_port[fi]=-1;continue;}
                fl_port[fi]=(short)bp;
                unsigned int mask=fl_pmask[fi];
                while(mask){int ph=__builtin_ctz(mask);out_load[leaf][bp][ph]++;in_load[dl][bp][ph]++;mask&=mask-1;}
            }
        }
    }
