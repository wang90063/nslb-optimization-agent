...
void run_tabu_search(int m){
    int orig_jm=get_job_max(m), orig_fg=get_future_gmax(m);
    if(orig_jm<=g_r) return;
    memcpy(bk_out,out_load,g_l*sizeof(out_load[0]));
    memcpy(bk_in,in_load,g_l*sizeof(in_load[0]));
    memcpy(bk_port,fl_port,fl_count*sizeof(short));
    static short best_port[MAX_FLOWS];
    memcpy(best_port,fl_port,fl_count*sizeof(short));
    int best_jm=orig_jm,best_fg=orig_fg;
    int cur_jm=orig_jm,cur_fg=orig_fg;
    static short tabu[MAX_FLOWS];
    memset(tabu,0,fl_count*sizeof(short));
    int tenure=8;
    for(int iter=0;iter<300&&cur_jm>g_r;++iter){
        int mf=-1,mp=-1,ms=0x7fffffff;
        for(int i=0;i<fl_count;++i){
            int sl=fl_sl[i],dl=fl_dl[i],cp=fl_port[i];
            if(sl==dl||cp<0) continue;
            unsigned int mk=fl_pmask[i];
            int hot=0; unsigned int t=mk;
            while(t){int ph=__builtin_ctz(t);
                if(out_load[sl][cp][ph]>=cur_jm||
                   in_load[dl][cp][ph]>=cur_jm){hot=1;break;}
                t&=t-1;}
            if(!hot) continue;
            for(int pk=0;pk<g_p;++pk){
                if(pk==cp) continue;
                if(tabu[i]>iter) continue;
                int bad=0,score=0; unsigned int t2=mk;
                while(t2){int ph=__builtin_ctz(t2);
                    if(out_load[sl][pk][ph]+1>cur_jm||
                       in_load[dl][pk][ph]+1>cur_jm){bad=1;break;}
                    t2&=t2-1;}
                if(bad) continue;
                int mpo=0,mpi=0;
                for(int ph=0;ph<m;++ph){
                    int b=((mk>>ph)&1);
                    int v=out_load[sl][pk][ph]+b;if(v>mpo)mpo=v;
                    v=in_load[dl][pk][ph]+b;if(v>mpi)mpi=v;
                }
                int f1=global_out[sl][pk]+mpo;
                int f2=global_in[dl][pk]+mpi;
                if(f1>cur_fg||f2>cur_fg) continue;
                unsigned int t3=mk;
                while(t3){int ph=__builtin_ctz(t3);
                    if(out_load[sl][cp][ph]==cur_jm) score--;
                    if(in_load[dl][cp][ph]==cur_jm) score--;
                    t3&=t3-1;}
                if(score<ms){ms=score;mf=i;mp=pk;}
            }
        }
        if(mf<0) break;
        int sl=fl_sl[mf],dl=fl_dl[mf],cp=fl_port[mf];
        unsigned int mk=fl_pmask[mf];
        while(mk){int ph=__builtin_ctz(mk);
            out_load[sl][cp][ph]--;in_load[dl][cp][ph]--;
            out_load[sl][mp][ph]++;in_load[dl][mp][ph]++;
            mk&=mk-1;}
        fl_port[mf]=(short)mp;
        tabu[mf]=(short)(iter+tenure);
        cur_jm=get_job_max(m);
        cur_fg=get_future_gmax(m);
        if(cur_jm<best_jm||(cur_jm==best_jm&&cur_fg<best_fg)){
            best_jm=cur_jm;best_fg=cur_fg;
            memcpy(best_port,fl_port,fl_count*sizeof(short));
        }
    }
    if(best_jm<orig_jm||(best_jm==orig_jm&&best_fg<orig_fg)){
        memset(out_load,0,g_l*sizeof(out_load[0]));
        memset(in_load,0,g_l*sizeof(in_load[0]));
        memcpy(fl_port,best_port,fl_count*sizeof(short));
        for(int i=0;i<fl_count;++i){
            int s=fl_sl[i],d=fl_dl[i],p2=fl_port[i];
            if(s==d||p2<0) continue;
            unsigned int mk=fl_pmask[i];
            while(mk){int ph=__builtin_ctz(mk);
                out_load[s][p2][ph]++;in_load[d][p2][ph]++;
                mk&=mk-1;}
        }
    } else {
        memcpy(out_load,bk_out,g_l*sizeof(out_load[0]));
...
void solve_job(){
    ...
    // Tabu search: reduce jm without worsening fg
    if(fl_count<=4000&&get_job_max(m)>g_r) run_tabu_search(m);
