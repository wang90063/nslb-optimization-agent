...
void solve_job(){
    ...
    // Adaptive global weight: increases as more jobs are processed
    // Early jobs have little global info; later jobs should respect global state more
    int gw = 1 + g_job_idx; // global weight grows with job index

    // Find current global max per leaf (for hotspot penalty)
    int max_go[MAX_LEAFS], max_gi[MAX_LEAFS];
    for(int leaf=0;leaf<g_l;++leaf){
        max_go[leaf]=0; max_gi[leaf]=0;
        for(int pk=0;pk<g_p;++pk){
            if(global_out[leaf][pk]>max_go[leaf]) max_go[leaf]=global_out[leaf][pk];
            if(global_in[leaf][pk]>max_gi[leaf]) max_gi[leaf]=global_in[leaf][pk];
        }
    }
    ...
    for(int i=0;i<fl_count;++i){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            // Global cost: use max(out,in) since Maxmultir takes max
            int go=global_out[sl][pk];
            int gi=global_in[dl][pk];
            int gmax=go>gi?go:gi;
            int gcost=gmax*gw;
            // Hotspot penalty: extra cost if this port is at the global max
            if(go>=max_go[sl] && max_go[sl]>0) gcost+=go;
            if(gi>=max_gi[dl] && max_gi[dl]>0) gcost+=gi;
            cost=cost*4+gcost;
