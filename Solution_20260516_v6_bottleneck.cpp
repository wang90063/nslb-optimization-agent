...
// cards on each (leaf, port) for targeted search
static int port_cards[MAX_LEAFS*MAX_PORTS][128]; // cards assigned to each port
static short port_card_cnt[MAX_LEAFS*MAX_PORTS];
...
void solve_job(){
    ...
    // Build port_cards index for targeted search
    memset(port_card_cnt,0,g_l*g_p*sizeof(short));
    for(int i=0;i<n_active_total;++i){
        int card=active_list[i];
        int leaf=card/g_pr, pk=card_assigned_port[card];
        int pidx=leaf*MAX_PORTS+pk;
        if(port_card_cnt[pidx]<128)
            port_cards[pidx][port_card_cnt[pidx]++]=i; // store active_list index
    }

    // LOCAL SEARCH: minimize max load (bottleneck reduction)
    if(n_active_total>1){
        double deadline=t0+SEARCH_BUDGET_MS;
        int iter=0;
        while(1){
            if(++iter%128==0&&now_ms()>=deadline)break;

            // find bottleneck: highest load across all (leaf, port, phase)
            int bn_leaf=-1,bn_port=-1,bn_ph=-1,bn_val=0,bn_is_out=1;
            // sample random leafs to find bottleneck faster
            for(int s=0;s<8;++s){
                int leaf=xorshift()%g_l;
                for(int pk=0;pk<g_p;++pk){
                    for(int ph=0;ph<m;++ph){
                        if(out_load[leaf][pk][ph]>bn_val){bn_val=out_load[leaf][pk][ph];bn_leaf=leaf;bn_port=pk;bn_ph=ph;bn_is_out=1;}
                        if(in_load[leaf][pk][ph]>bn_val){bn_val=in_load[leaf][pk][ph];bn_leaf=leaf;bn_port=pk;bn_ph=ph;bn_is_out=0;}
                    }
                }
            }
            if(bn_leaf<0||bn_val<=g_r)continue;

            // pick a random card on the bottleneck port (outgoing side)
            int pidx=bn_leaf*MAX_PORTS+bn_port;
            if(bn_is_out&&port_card_cnt[pidx]>0){
                int ci=xorshift()%port_card_cnt[pidx];
                int idx=port_cards[pidx][ci];
                int card=active_list[idx];
                int leaf=card/g_pr,cp=card_assigned_port[card];
                short*oc=&card_out_count[card*MAX_PHASES];
                int ies=in_entry_start[idx],iee=in_entry_start[idx+1];

                // find best alternative port (minimize resulting max)
                int bpk=-1,best_new_max=bn_val;
                for(int pk=0;pk<g_p;++pk){
                    if(pk==cp)continue;
                    int new_max=0;
                    for(int ph=0;ph<m;++ph){
                        int cnt=oc[ph];if(!cnt)continue;
                        int ov=out_load[leaf][cp][ph]-cnt;
                        int nv=out_load[leaf][pk][ph]+cnt;
                        if(nv>new_max)new_max=nv;
                        if(ov>new_max)new_max=ov; // remaining load on old port
                    }
                    for(int ie=ies;ie<iee;++ie){
                        int dl=in_entries[ie].dst_leaf,ph=in_entries[ie].phase,cnt=in_entries[ie].count;
                        int nv=in_load[dl][pk][ph]+cnt;
                        if(nv>new_max)new_max=nv;
                    }
                    if(new_max<best_new_max){best_new_max=new_max;bpk=pk;}
                }
                ...
                if(bpk>=0){
                    ...
                    // update port_cards index
                    port_cards[pidx][ci]=port_cards[pidx][--port_card_cnt[pidx]];
                    int npidx=leaf*MAX_PORTS+bpk;
                    if(port_card_cnt[npidx]<128)port_cards[npidx][port_card_cnt[npidx]++]=idx;
                }
            }
        }
    }
