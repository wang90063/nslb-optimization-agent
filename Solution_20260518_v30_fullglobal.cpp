...
void solve_job(){
    ...
    // Pass 1: greedy with full global penalty
    // Consider both directions for each (leaf, port) pair
    for(int i=0;i<fl_count;++i){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            // Full global: consider both out and in for each leaf's port
            int gs=global_out[sl][pk]; // what matters for (sl,pk) is out
            int gd=global_in[dl][pk];  // what matters for (dl,pk) is in
            // Also consider the other direction as secondary
            int gs2=global_in[sl][pk]; // (sl,pk) might also have high in from other flows
            int gd2=global_out[dl][pk]; // (dl,pk) might also have high out from other flows
            int gv_sl=gs>gs2?gs:gs2; // max load on (sl,pk) in either direction
            int gv_dl=gd>gd2?gd:gd2; // max load on (dl,pk) in either direction
            int gv=gv_sl>gv_dl?gv_sl:gv_dl;
            int total=cost*2+gv;
