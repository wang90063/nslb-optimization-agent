...
void solve_job(){
    ...
    // REGREEDY
    // Pass 3: re-greedy — remove each flow and re-assign with full cost function
    for(int pass=0;pass<2;++pass){
        int start=pass==0?fl_count-1:0;
        int end=pass==0?-1:fl_count;
        int step=pass==0?-1:1;
        for(int i=start;i!=end;i+=step){
            ...
            int cp=fl_port[i];
            unsigned int mask=fl_pmask[i];
            // remove flow from current port
            unsigned int m2=mask;
            while(m2){int ph=__builtin_ctz(m2);out_load[sl][cp][ph]--;in_load[dl][cp][ph]--;m2&=m2-1;}
            // re-assign with full cost
            int bp=cp,bc=0x7fffffff;
            for(int pk=0;pk<g_p;++pk){
                int cost=0;
                m2=mask;
                while(m2){
