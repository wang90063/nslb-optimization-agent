    ...
    for(int iter=0;iter<5;++iter){
        ...
        for(int i=0;i<fl_count;++i){
            ...
            if(base_cbt>0&&cnt<=160){
                ...
                long long best_future_delta=0x7fffffffffffffffLL;
                for(int tp=0;tp<g_p;++tp){
                    ...
                    int can_move=0;
                    short src_removed[MAX_PORTS][MAX_PHASES]={};
                    int dst_pair_leaf[160];
                    int dst_pair_port[160];
                    short dst_removed[160][MAX_PHASES]={};
                    int dst_pair_cnt=0;
                    for(int k=0;k<cnt;++k){
                        int fi=pc_flows[k];
                        int cp=fl_port[fi];
                        ...
                        can_move++;
                        int dst_id=-1;
                        for(int id=0;id<dst_pair_cnt;++id){
                            if(dst_pair_leaf[id]==dl&&dst_pair_port[id]==cp){
                                dst_id=id;
                                break;
                            }
                        }
                        if(dst_id<0){
                            dst_id=dst_pair_cnt++;
                            dst_pair_leaf[dst_id]=dl;
                            dst_pair_port[dst_id]=cp;
                        }
                        m2=mask;
                        while(m2){
                            int ph=__builtin_ctz(m2);
                            src_removed[cp][ph]++;
                            dst_removed[dst_id][ph]++;
                            tmp_ppc[ph][cp]--;
                    ...
                    long long future_delta=0;
                    for(int cp=0;cp<g_p;++cp){
                        int touched=0;
                        for(int ph=0;ph<m;++ph) if(src_removed[cp][ph]) { touched=1; break; }
                        if(!touched) continue;
                        int old_mo=0,new_mo=0;
                        for(int ph=0;ph<m;++ph){
                            int cur=out_load[src_leaf][cp][ph];
                            if(cur>old_mo) old_mo=cur;
                            int nxt=cur-src_removed[cp][ph];
                            if(nxt>new_mo) new_mo=nxt;
                        }
                        long long old_f=(long long)global_out[src_leaf][cp]+old_mo;
                        long long new_f=(long long)global_out[src_leaf][cp]+new_mo;
                        future_delta+=new_f*new_f-old_f*old_f;
                    }
                    for(int id=0;id<dst_pair_cnt;++id){
                        int dl=dst_pair_leaf[id],cp=dst_pair_port[id];
                        int old_mi=0,new_mi=0;
                        for(int ph=0;ph<m;++ph){
                            int cur=in_load[dl][cp][ph];
                            if(cur>old_mi) old_mi=cur;
                            int nxt=cur-dst_removed[id][ph];
                            if(nxt>new_mi) new_mi=nxt;
                        }
                        long long old_f=(long long)global_in[dl][cp]+old_mi;
                        long long new_f=(long long)global_in[dl][cp]+new_mi;
                        future_delta+=new_f*new_f-old_f*old_f;
                    }
                    int after_cbt=calc_phase_mask_cbt(tmp_masks,m);
                    if(after_cbt<best_cbt||
                       (after_cbt==best_cbt&&
                        (future_delta<best_future_delta||
                         (future_delta==best_future_delta&&can_move>best_total)))){
                        best_cbt=after_cbt;
