...
void solve_job(){
    ...
    double t0=now_ms(); // timer starts AFTER reading input
    ...
    // LOCAL SEARCH - per-job budget, measured from after input read
    if(n_active_total>1){
        double deadline=t0+SEARCH_BUDGET_MS;
        int iter=0;
        while(1){
            if(++iter%256==0&&now_ms()>=deadline)break;
            ...
            for(int pk=0;pk<g_p;++pk){
                if(pk==cp)continue;long long d=0;
