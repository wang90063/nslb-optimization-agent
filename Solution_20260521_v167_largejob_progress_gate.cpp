...
void solve_job(){
    ...
    int medium_large_refine_shape=(job_work>50000&&job_work<=80000&&g_p>=16&&m>=17&&fl_count<=5000);

    run_port_consistency(m);
    int refine1_moved=run_port_consistency_perport_refine(m);
    int revisit1_moved=run_port_consistency(m);
    if(allow_extra_pc_chain){
        int allow_after_first=(refine1_moved>0&&revisit1_moved>0);
        if(!allow_after_first&&medium_large_refine_shape&&refine1_moved>0) allow_after_first=1;
        if(allow_after_first){
            int refine2_moved=run_port_consistency_perport_refine(m);
            if(refine2_moved>0){
                int revisit2_moved=run_port_consistency(m);
                int allow_after_second=(revisit2_moved>0);
                if(!allow_after_second&&medium_large_refine_shape) allow_after_second=1;
                if(allow_after_second){
                    int refine3_moved=run_port_consistency_perport_refine(m);
                    if(refine3_moved>0) run_port_consistency(m);
                }
            }
        }
    }
