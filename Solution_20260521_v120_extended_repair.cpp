...
static int diff_order[MAX_FLOWS];
void run_greedy_csp(int m, int seed_val=0){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    int cross_cnt=0;
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]){fl_port[i]=-1;continue;}
        diff_order[cross_cnt++]=i;
    }
    for(int a=0;a<cross_cnt-1;++a){
        int best=a;
        int best_ph=__builtin_popcount(fl_pmask[diff_order[a]]);
        for(int b=a+1;b<cross_cnt;++b){
            int ph=__builtin_popcount(fl_pmask[diff_order[b]]);
            if(ph>best_ph){best=b;best_ph=ph;}
        }
        if(best!=a){int t=diff_order[a];diff_order[a]=diff_order[best];diff_order[best]=t;}
    }
    if(seed_val>0){
        unsigned int rng=seed_val*2654435761u;
        for(int a=cross_cnt-1;a>cross_cnt/3;--a){
            rng=rng*1664525u+1013904223u;
            int range=cross_cnt-cross_cnt/3;
            int b=cross_cnt/3+(rng%range);
            int t=diff_order[a];diff_order[a]=diff_order[b];diff_order[b]=t;
        }
    }
    for(int ii=0;ii<cross_cnt;++ii){
        int i=diff_order[ii];
        int sl=fl_sl[i],dl=fl_dl[i];
        unsigned int mask=fl_pmask[i];
        int bp=0,bc=0x7fffffff;
...
void run_mc_repair(int m){
    ...
    for(int iter=0;iter<3000&&viol_cnt>0;++iter){
        ...
        if(best_delta<=0&&n_best>0){
            ...
            int prev_vc=viol_cnt; viol_cnt=0;
            for(int i=0;i<fl_count;++i){
                int s=fl_sl[i],d=fl_dl[i];
                if(s==d) continue;
                int p2=fl_port[i]; if(p2<0) continue;
                unsigned int m3=fl_pmask[i]; int ov=0;
                while(m3){int ph=__builtin_ctz(m3);
                    if(out_load[s][p2][ph]>g_r||in_load[d][p2][ph]>g_r)
                        {ov=1;break;}
                    m3&=m3-1;}
...
static int destroy_idx[MAX_FLOWS];
void run_destroy_rebuild(int m){
    int orig_max=get_job_max(m);
    if(orig_max<=g_r||orig_max>g_r+2) return;
    memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
    memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
    memcpy(bk_port, fl_port, fl_count*sizeof(short));
    unsigned int rng=fl_count*71+m*37+g_p*11;
    int best_found=0;
    for(int attempt=0;attempt<30;++attempt){
        ...
        for(int i=0;i<fl_count;++i){
            ...
            unsigned int m2=fl_pmask[i]; int on_ov=0;
            while(m2){int ph=__builtin_ctz(m2);
                if(out_load[sl][cp][ph]>g_r||in_load[dl][cp][ph]>g_r){on_ov=1;break;}
                m2&=m2-1;}
            if(on_ov) destroy_idx[dcnt++]=i;
        }
        if(dcnt==0) break;
        rng=rng*1664525u+1013904223u;
        int extra=(rng%dcnt)/4+1;
        for(int k=0;k<extra&&dcnt<fl_count-1;++k){
            rng=rng*1664525u+1013904223u;
            int ri=rng%fl_count;
            if(fl_sl[ri]==fl_dl[ri]) continue;
            destroy_idx[dcnt++]=ri;
        }
        ...
        rng=rng*1664525u+1013904223u;
        for(int a=dcnt-1;a>0;--a){
            rng=rng*1664525u+1013904223u;
            int b=rng%(a+1);
            int t=destroy_idx[a];destroy_idx[a]=destroy_idx[b];destroy_idx[b]=t;
        }
        for(int k=0;k<dcnt;++k){
            int i=destroy_idx[k];
            int sl=fl_sl[i],dl=fl_dl[i];
        ...
        if(new_max<orig_max){best_found=1;
...
static short best_port_ws[MAX_FLOWS];
static short best_out_ws[MAX_LEAFS][MAX_PORTS][MAX_PHASES];
static short best_in_ws[MAX_LEAFS][MAX_PORTS][MAX_PHASES];

__attribute__((noinline,cold)) void run_walksat_repair(int m){
    int orig_max=get_job_max(m);
    if(orig_max<=g_r||orig_max>g_r+1) return;
    memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
    memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
    memcpy(bk_port, fl_port, fl_count*sizeof(short));
    int best_viol=0x7fffffff;
    int n_restarts=(fl_count<=300)?20:8;
    int iter_budget=(fl_count<=300)?100000:50000;
    for(int restart=0;restart<n_restarts;++restart){
        if(restart==0){
            ...
            // CSP greedy with random ordering as warm start
            memset(out_load,0,g_l*sizeof(out_load[0]));
            memset(in_load,0,g_l*sizeof(in_load[0]));
            unsigned int seed=restart*2654435761u+fl_count*97+m*31;
            int cross_cnt=0;
            for(int i=0;i<fl_count;++i){
                if(fl_sl[i]==fl_dl[i]){fl_port[i]=-1;continue;}
                diff_order[cross_cnt++]=i;
            }
            // Sort by phase count descending, then shuffle within groups
            for(int a=0;a<cross_cnt-1;++a){
                int best=a;
                int bpc=__builtin_popcount(fl_pmask[diff_order[a]]);
                for(int b=a+1;b<cross_cnt;++b){
                    int pc=__builtin_popcount(fl_pmask[diff_order[b]]);
                    if(pc>bpc){best=b;bpc=pc;}
                }
                if(best!=a){int t=diff_order[a];diff_order[a]=diff_order[best];diff_order[best]=t;}
            }
            // Shuffle within same-phase-count groups
            seed=seed*1664525u+1013904223u;
            for(int a=cross_cnt-1;a>0;--a){
                seed=seed*1664525u+1013904223u;
                int b=seed%(a+1);
                if(__builtin_popcount(fl_pmask[diff_order[a]])==__builtin_popcount(fl_pmask[diff_order[b]])){
                    int t=diff_order[a];diff_order[a]=diff_order[b];diff_order[b]=t;
                }
            }
            for(int ii=0;ii<cross_cnt;++ii){
        ...
        for(int i=0;i<fl_count;++i){
            int sl=fl_sl[i],dl=fl_dl[i],cp=fl_port[i];
            if(sl==dl||cp<0) continue;
            unsigned int m2=fl_pmask[i]; int ov=0;
            while(m2){int ph=__builtin_ctz(m2);
                if(out_load[sl][cp][ph]>g_r||in_load[dl][cp][ph]>g_r){ov=1;break;}
                m2&=m2-1;}
            if(ov) viol_flows[viol_cnt++]=i;
        }
        if(viol_cnt==0){best_viol=0;
            memcpy(best_port_ws, fl_port, fl_count*sizeof(short));
            memcpy(best_out_ws, out_load, g_l*sizeof(out_load[0]));
            memcpy(best_in_ws, in_load, g_l*sizeof(in_load[0]));
            break;}
        if(viol_cnt<best_viol){
            best_viol=viol_cnt;
            memcpy(best_port_ws, fl_port, fl_count*sizeof(short));
            memcpy(best_out_ws, out_load, g_l*sizeof(out_load[0]));
            memcpy(best_in_ws, in_load, g_l*sizeof(in_load[0]));
        ...
        // Walksat
        unsigned int rng=restart*999983u+fl_count*53+m*19;
        for(int i=0;i<fl_count;++i) mc_tabu[i]=0;
        int no_progress=0;
        for(int iter=0;iter<iter_budget&&viol_cnt>0;++iter){
            if(no_progress>5000) break;
            rng=rng*1664525u+1013904223u;
            int fi=viol_flows[rng%viol_cnt];
            if(mc_tabu[fi]>(short)(iter&0x7fff)){no_progress++;continue;}
            int sl=fl_sl[fi],dl=fl_dl[fi],cp=fl_port[fi];
            unsigned int mask=fl_pmask[fi];
            int best_pk=-1,best_delta=2,n_best=0;
            int best_pks[32];
            for(int pk=0;pk<g_p;++pk){
                if(pk==cp) continue;
                int delta=0; unsigned int m2=mask;
            ...
            int accept=0;
            if(best_delta<=0&&n_best>0) accept=1;
            else if(n_best>0){
                rng=rng*1664525u+1013904223u;
                if(best_delta==1&&(rng&0xff)<50) accept=1;
                else if(best_delta==2&&(rng&0xff)<10) accept=1;
            }
            if(accept){
                rng=rng*1664525u+1013904223u;
                best_pk=best_pks[rng%n_best];
                ...
                while(m2){int ph=__builtin_ctz(m2);
                    ...
                    m2&=m2-1;}
                fl_port[fi]=(short)best_pk;
                mc_tabu[fi]=(short)((iter+20)&0x7fff);
                int prev_vc=viol_cnt; viol_cnt=0;
                for(int i=0;i<fl_count;++i){
                    int s=fl_sl[i],d=fl_dl[i];
                    if(s==d) continue;
                    ...
                    unsigned int m3=fl_pmask[i]; int ov=0;
                    while(m3){int ph=__builtin_ctz(m3);
                        if(out_load[s][p2][ph]>g_r||in_load[d][p2][ph]>g_r){ov=1;break;}
                        m3&=m3-1;}
                    if(ov) viol_flows[viol_cnt++]=i;
                }
                if(viol_cnt<best_viol){
                    best_viol=viol_cnt;no_progress=0;
                    memcpy(best_port_ws, fl_port, fl_count*sizeof(short));
                    memcpy(best_out_ws, out_load, g_l*sizeof(out_load[0]));
                    memcpy(best_in_ws, in_load, g_l*sizeof(in_load[0]));
                    if(viol_cnt==0) break;
    ...
    if(best_viol==0){
        memcpy(out_load, best_out_ws, g_l*sizeof(out_load[0]));
        memcpy(in_load, best_in_ws, g_l*sizeof(in_load[0]));
        memcpy(fl_port, best_port_ws, fl_count*sizeof(short));
    } else {
        memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
        memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
        memcpy(fl_port, bk_port, fl_count*sizeof(short));
...
__attribute__((noinline,cold)) void run_random_start_repair(int m){
    int orig_max=get_job_max(m);
    if(orig_max<=g_r) return;
    if(orig_max>g_r+2) return;
    memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
    memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
    memcpy(bk_port, fl_port, fl_count*sizeof(short));
    int found=0;
    ...
    int n_restarts;
    if(g_r<=2) n_restarts=(fl_count<=1000)?80:40;
    else n_restarts=(fl_count<=1000)?120:60;
    for(int restart=0;restart<n_restarts&&!found;++restart){
        memset(out_load,0,g_l*sizeof(out_load[0]));
        memset(in_load,0,g_l*sizeof(in_load[0]));
        ...
        for(int i=0;i<fl_count;++i){
            ...
            unsigned int m2=fl_pmask[i];
            while(m2){int ph=__builtin_ctz(m2);
                out_load[sl][pk][ph]++;in_load[dl][pk][ph]++;
                m2&=m2-1;}
        }
        for(int i=0;i<fl_count;++i) mc_tabu[i]=0;
        int viol_cnt=0;
        ...
        if(viol_cnt==0){found=1;break;}
        int no_progress=0;
        for(int iter=0;iter<5000&&viol_cnt>0;++iter){
            if(no_progress>500) break;
            rng=rng*1664525u+1013904223u;
            int fi=viol_flows[rng%viol_cnt];
            if(mc_tabu[fi]>iter){no_progress++;continue;}
            int sl=fl_sl[fi],dl=fl_dl[fi],cp=fl_port[fi];
...
void solve_job(){
    ...
    // CSP greedy: difficulty-ordered (most phases first) + hard constraint
    run_greedy_csp(m, 0);
    run_swap(m);
    run_jm_repair(m);
    TRY_STRATEGY();
    ...
    // Walksat repair (accepts occasional worsening moves to escape local minima)
    if(fl_count<=6000&&get_job_max(m)>g_r&&get_job_max(m)<=g_r+1) run_walksat_repair(m);

    // Destroy-rebuild repair for marginal overflows
    if(fl_count<=6000&&get_job_max(m)>g_r&&get_job_max(m)<=g_r+2) run_destroy_rebuild(m);

    // Random-start min-conflicts for tight cases (extended to all r)
    if(fl_count<=6000&&get_job_max(m)>g_r&&get_job_max(m)<=g_r+2) run_random_start_repair(m);
