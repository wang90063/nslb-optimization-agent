...
__attribute__((noinline,cold)) void run_lowr_metric_refine(int m){
    if(!(g_r==3&&g_p>=16)) return;
    EvalMetrics base_eval=collect_metrics(m);
    if(base_eval.jm!=g_r+1) return;
    if(base_eval.ci<=0||base_eval.ci>36) return;
    if(fl_count>2500||(long long)fl_count*m>10000) return;
    ...
    if(viol_cnt<=0||viol_cnt>96) return;

    EvalMetrics best_eval=base_eval;
    unsigned int rng=fl_count*131u+m*17u+g_job_idx*911u;
    int accepted=0;

    while(accepted<8&&viol_cnt>0){
        int probe_cnt=viol_cnt<10?viol_cnt:10;
        int best_fi=-1,best_pk=-1;
        EvalMetrics move_eval=best_eval;
        int found=0;

        for(int probe=0;probe<probe_cnt;++probe){
            rng=rng*1664525u+1013904223u;
            int fi=viol_flows[(rng>>16)%viol_cnt];
            int cp=fl_port[fi];
            if(cp<0) continue;
            int best_delta=0;
            ...
            if(better_metrics(cand_eval,move_eval)){
                move_eval=cand_eval;
                best_fi=fi;
                best_pk=pk;
                found=1;
    ...
    if(accepted<=0||!better_metrics(best_eval,base_eval)){
        memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
        memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
        memcpy(fl_port, bk_port, fl_count*sizeof(short));
    }
...
void solve_job(){
    ...
    // For `r=3,p>=16`, first try a lighter score-aware local refine from the
    // current solution before falling back to the more disruptive restart path.
    run_lowr_metric_refine(m);
