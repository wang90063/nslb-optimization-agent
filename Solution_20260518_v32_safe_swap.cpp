...
void solve_job(){
    ...
    // Pass 1: greedy with global penalty (same as v18/v22)
    for(int i=0;i<fl_count;++i){
    ...
    // Pass 2: SAFE bottleneck swap - only commit if global max actually decreases
    for(int iter=0;iter<20;++iter){
        int mx=get_job_max(m);
        if(mx<=g_r) break;  // already at or below capacity, no need to swap
        int improved=0;
        for(int i=0;i<fl_count&&!improved;++i){
            ...
            if(best_new>=0){
                ...
                // Verify global max actually decreased
                int new_mx=get_job_max(m);
                if(new_mx<mx){
                    improved=1;  // swap helped, keep it
                } else {
                    // Undo: swap didn't reduce global max
                    m2=mask;
                    while(m2){int ph=__builtin_ctz(m2);out_load[sl][best_new][ph]--;in_load[dl][best_new][ph]--;out_load[sl][cp][ph]++;in_load[dl][cp][ph]++;m2&=m2-1;}
                    fl_port[i]=(short)cp;
                }
            }
        }
