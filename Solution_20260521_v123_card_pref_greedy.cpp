...
// Constraint: Maxmultir-aware — allow max_phase_load increase if cumulative stays below global max
static int pc_flows[MAX_FLOWS];
static int pc_mark[MAX_FLOWS];
static int pc_headroom_out[MAX_LEAFS][MAX_PORTS];
static int pc_headroom_in[MAX_LEAFS][MAX_PORTS];

void run_port_consistency(int m){
    // Compute current job max_phase_load per (leaf, port)
    int job_max_out[MAX_LEAFS][MAX_PORTS]={};
    int job_max_in[MAX_LEAFS][MAX_PORTS]={};
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            int mo=0,mi=0;
            for(int ph=0;ph<m;++ph){
            ...
            job_max_out[leaf][pk]=mo;
            job_max_in[leaf][pk]=mi;
        }
    // Find max cumulative
    int max_cum=0;
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            int co=global_out[leaf][pk]+job_max_out[leaf][pk];
            int ci=global_in[leaf][pk]+job_max_in[leaf][pk];
            if(co>max_cum)max_cum=co;
            if(ci>max_cum)max_cum=ci;
        }
    // Compute headroom: how much can each (leaf,port) max increase
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            pc_headroom_out[leaf][pk]=max_cum-(global_out[leaf][pk]+job_max_out[leaf][pk]);
            pc_headroom_in[leaf][pk]=max_cum-(global_in[leaf][pk]+job_max_in[leaf][pk]);
        }
        ...
        for(int i=0;i<fl_count;++i){
            ...
            for(int tp=0;tp<g_p;++tp){
                ...
                for(int k=0;k<cnt;++k){
                    ...
                    // Check: would this move increase max_phase_load beyond headroom?
                    ...
                    int new_max_out=cur_max_out,new_max_in=cur_max_in;
                    unsigned int m2=mask; int ok=1;
                    while(m2){
                        int ph=__builtin_ctz(m2);
                        int no=out_load[sl][tp][ph]+1;
                        int ni=in_load[dl][tp][ph]+1;
                        if(no>new_max_out) new_max_out=no;
                        if(ni>new_max_in) new_max_in=ni;
                        m2&=m2-1;
                    }
                    int inc_out=new_max_out-cur_max_out;
                    int inc_in=new_max_in-cur_max_in;
                    if(inc_out>pc_headroom_out[sl][tp]||
                       inc_in>pc_headroom_in[dl][tp]){ok=0;}
                    if(ok) can_move++;
                }
            ...
            for(int k=0;k<cnt;++k){
                ...
                while(m2){
                    int ph=__builtin_ctz(m2);
                    int no=out_load[sl][best_tp][ph]+1;
                    int ni=in_load[dl][best_tp][ph]+1;
                    if(no>new_max_out) new_max_out=no;
                    if(ni>new_max_in) new_max_in=ni;
                    m2&=m2-1;
                }
                int inc_out=new_max_out-cur_max_out;
                int inc_in=new_max_in-cur_max_in;
                if(inc_out>pc_headroom_out[sl][best_tp]||
                   inc_in>pc_headroom_in[dl][best_tp]) continue;
                // Execute move
                m2=mask;
                ...
                // Update headroom
                pc_headroom_out[sl][best_tp]-=inc_out;
                pc_headroom_in[dl][best_tp]-=inc_in;
                moved++;
            }
        }
        if(!moved) break;
