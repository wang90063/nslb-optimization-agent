...
static int eq_pjm[MAX_PORTS],eq_pfg[MAX_PORTS];
...
inline int break_equal_cost_tie(int cand_cnt){
    ...
    for(int idx=1;idx<cand_cnt;++idx){
        int pk=eq_cands[idx];
        if(eq_pjm[pk]!=eq_pjm[best_pk]){
            if(eq_pjm[pk]<eq_pjm[best_pk]) best_pk=pk;
            continue;
        }
        if(eq_pfg[pk]!=eq_pfg[best_pk]){
            if(eq_pfg[pk]<eq_pfg[best_pk]) best_pk=pk;
            continue;
        }
...
void run_greedy_fgmax(int m, int hardcap, int rev=0){
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            eq_pjm[pk]=local_max>g_r?local_max:g_r;
            if(g_hist_max_jm>eq_pjm[pk]) eq_pjm[pk]=g_hist_max_jm;
            eq_pfg[pk]=fgv>g_r?fgv:g_r;
            eq_delta[pk]=fgv-(global_out[sl][pk]>global_in[dl][pk]?global_out[sl][pk]:global_in[dl][pk]);
            eq_over[pk]=0;
            if(fo>g_r) eq_over[pk]+=fo-g_r;
            if(fi>g_r) eq_over[pk]+=fi-g_r;
            eq_sq[pk]=(long long)fo*fo+(long long)fi*fi;
            eq_sum[pk]=fo+fi;
            if(cost<bc){
                bc=cost;bp=pk;
                eq_cands[0]=pk;
...
void run_greedy(int m, int local_w, int global_w, int hardcap, int rev=0){
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            if(exceeds) cost+=10000;
            eq_pjm[pk]=cost>=10000?g_r+1:g_r;
            // More faithful partial jm for weighted greedy: use actual local max.
            unsigned int m4=mask;
            int local_max=0;
            while(m4){
...
void run_greedy_ftrl(int m, int hardcap, int rev=0, int sc_div=2){
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            eq_pjm[pk]=local_max>g_r?local_max:g_r;
            if(g_hist_max_jm>eq_pjm[pk]) eq_pjm[pk]=g_hist_max_jm;
            eq_pfg[pk]=gv>g_r?gv:g_r;
            eq_delta[pk]=gv-(global_out[sl][pk]>global_in[dl][pk]?global_out[sl][pk]:global_in[dl][pk]);
            eq_over[pk]=0;
            if(fo>g_r) eq_over[pk]+=fo-g_r;
            if(fi>g_r) eq_over[pk]+=fi-g_r;
            eq_sq[pk]=(long long)fo*fo+(long long)fi*fi;
    ...
    for(int round=0;round<5;++round){
        ...
        for(int k=0;k<cnt;++k){
            ...
            for(int pk=0;pk<g_p;++pk){
                ...
                if(exceeds) cost+=10000;
                eq_pjm[pk]=local_max>g_r?local_max:g_r;
                if(pre_jmax>eq_pjm[pk]) eq_pjm[pk]=pre_jmax;
                eq_pfg[pk]=fgv>g_r?fgv:g_r;
                eq_delta[pk]=fgv-(global_out[sl][pk]>global_in[dl][pk]?global_out[sl][pk]:global_in[dl][pk]);
                eq_over[pk]=0;
                if(fo>g_r) eq_over[pk]+=fo-g_r;
