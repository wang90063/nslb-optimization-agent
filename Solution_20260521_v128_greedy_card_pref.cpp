...
static short greedy_card_pc[MAX_CARDS][MAX_PORTS];
...
void run_greedy_fgmax(int m, int hardcap, int rev=0){
    ...
    memset(greedy_card_pc,0,sizeof(greedy_card_pc));
    if(rev>=2){
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        int bp=0,bc=0x7fffffff,bcard=0;
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            int cpc=greedy_card_pc[src_card][pk];
            if(cost<bc||(cost==bc&&cpc>bcard)){bc=cost;bp=pk;bcard=cpc;}
        }
        fl_port[i]=(short)bp;
        greedy_card_pc[src_card][bp]++;
        unsigned int m2=mask;
