...
static int fl_hist_port[MAX_FLOWS];
...
void run_greedy_sticky(int m, int hardcap, int rev=0){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    if(rev>=2){
        unsigned int seed=rev*2654435761u;
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            if(exceeds) cost+=10000;
            if(cost<bc||(cost==bc&&pk==hp)){bc=cost;bp=pk;}
        }
        fl_port[i]=(short)bp;
        unsigned int m2=mask;
        while(m2){int ph=__builtin_ctz(m2);out_load[sl][bp][ph]++;in_load[dl][bp][ph]++;m2&=m2-1;}
...
void solve_job(){
    ...
    for(int i=0;i<fl_count;++i){
        ...
        fl_order[i]=i;
        if(fl_sl[i]!=fl_dl[i]) fl_hist_port[i]=hist_get(fl_src[i]*MAX_CARDS+fl_dst[i]);
        else fl_hist_port[i]=-1;
    }
    ...
    // Sticky greedy: prefer historical port on tie
    run_greedy_sticky(m, 1);
    run_swap(m);
    run_jm_repair(m);
    TRY_STRATEGY();

    run_greedy_sticky(m, 1, 1);
    run_swap(m);
    run_jm_repair(m);
    TRY_STRATEGY();
