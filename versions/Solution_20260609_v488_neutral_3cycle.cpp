...
// CB-aware greedy: gives consolidation bonus to ports where same card has flows
void run_greedy_cb_aware(int m, int local_w, int global_w, int cb_w, int hardcap, int rev=0){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    // Track which ports each card uses in each phase
    static unsigned int card_port_mask[MAX_CARDS][MAX_PHASES];
    memset(card_port_mask,0,sizeof(card_port_mask));
    if(rev>=2){
        unsigned int seed=rev*2654435761u;
        for(int a=fl_count-1;a>0;--a){
            seed=seed*1664525u+1013904223u;
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            int go=global_out[sl][pk],gi=global_in[dl][pk];
            int gv=go>gi?go:gi;
            // CB consolidation bonus: count adjacent phases where card already uses this port
            int cb_bonus=0;
            m2=mask;
            while(m2){
                int ph=__builtin_ctz(m2);
                if(ph>0 && (card_port_mask[ci][ph-1]&(1u<<pk))) cb_bonus++;
                if(ph<m-1 && (card_port_mask[ci][ph+1]&(1u<<pk))) cb_bonus++;
                m2&=m2-1;
            }
            int cost=local_max*local_w+gv*global_w-cb_bonus*cb_w;
            if(exceeds) cost+=10000;
            if(cost<bc){bc=cost;bp=pk;}
        }
        fl_port[i]=(short)bp;
        ...
        while(m2){int ph=__builtin_ctz(m2);
            out_load[sl][bp][ph]++;in_load[dl][bp][ph]++;
            card_port_mask[ci][ph]|=(1u<<bp);
            m2&=m2-1;}
    }
}
...
void run_neutral_swap(int m){
    ...
    int ns_max_iter=3;
    // v488: structural cap for the in-group neutral 3-cycle rotation pass.
    // 3-cycle is O(gsz^3) per group, so it only runs on small groups (<=NS3_MAX);
    // 32^3 ~= 33k/group worst case. This is a structural-size threshold, not a
    // case-level gate.
    const int NS3_MAX=32;
    for(int iter=0;iter<ns_max_iter;++iter){
        ...
        while(g_start<ns_valid_cnt){
            ...
            // v488: neutral 3-cycle rotation within the SAME (sl,dl,pmask)
            // equivalence group [g_start,g_end). Load-neutral: same pmask =>
            // relabeling ports among group members leaves per-(phase,port) flow
            // counts unchanged => port loads and MM strictly unchanged. Only
            // small groups (gsz<=NS3_MAX) run, to bound O(gsz^3); skipped under
            // time pressure (g_time_tight visible as file-scope static).
            if(!g_time_tight&&gsz>=3&&gsz<=NS3_MAX){
                for(int a=g_start;a<g_end;++a){
                    int i=ns_keys[a].idx;
                    int pi=fl_port[i],ci=fl_src[i];
                    if(pi<0) continue;
                    for(int b=a+1;b<g_end;++b){
                        int j=ns_keys[b].idx;
                        int pj=fl_port[j],cj=fl_src[j];
                        if(pj<0||ci==cj) continue;
                        for(int c=b+1;c<g_end;++c){
                            int k=ns_keys[c].idx;
                            int pk=fl_port[k],ck=fl_src[k];
                            if(pk<0||ck==ci||ck==cj) continue;
                            if(pi==pj&&pj==pk) continue;
                            int before=card_cbtphsc(ci,m)+card_cbtphsc(cj,m)+card_cbtphsc(ck,m);
                            // Rotate: i:pi->pj, j:pj->pk, k:pk->pi (cards pairwise distinct)
                            unsigned int mk=fl_pmask[i];
                            while(mk){int ph=__builtin_ctz(mk);
                                cppc[ci][ph][pi]--;if(!cppc[ci][ph][pi])cpm[ci][ph]&=~(1u<<pi);
                                cppc[ci][ph][pj]++;cpm[ci][ph]|=(1u<<pj);
                            ...
                            while(mk){int ph=__builtin_ctz(mk);
                                cppc[cj][ph][pj]--;if(!cppc[cj][ph][pj])cpm[cj][ph]&=~(1u<<pj);
                                cppc[cj][ph][pk]++;cpm[cj][ph]|=(1u<<pk);
                                mk&=mk-1;}
                            mk=fl_pmask[k];
                            while(mk){int ph=__builtin_ctz(mk);
                                cppc[ck][ph][pk]--;if(!cppc[ck][ph][pk])cpm[ck][ph]&=~(1u<<pk);
                                cppc[ck][ph][pi]++;cpm[ck][ph]|=(1u<<pi);
                                mk&=mk-1;}
                            int after=card_cbtphsc(ci,m)+card_cbtphsc(cj,m)+card_cbtphsc(ck,m);
                            if(after<before){
                                fl_port[i]=(short)pj;fl_port[j]=(short)pk;fl_port[k]=(short)pi;
                                pi=pj;pj=pk; improved++;
                            } else {
                                // Revert (exact inverse): i:pj->pi, j:pk->pj, k:pi->pk
                                mk=fl_pmask[i];
                                while(mk){int ph=__builtin_ctz(mk);
                                    cppc[ci][ph][pj]--;if(!cppc[ci][ph][pj])cpm[ci][ph]&=~(1u<<pj);
                                    cppc[ci][ph][pi]++;cpm[ci][ph]|=(1u<<pi);
                                ...
                                mk=fl_pmask[j];
                                while(mk){int ph=__builtin_ctz(mk);
                                    cppc[cj][ph][pk]--;if(!cppc[cj][ph][pk])cpm[cj][ph]&=~(1u<<pk);
                                    cppc[cj][ph][pj]++;cpm[cj][ph]|=(1u<<pj);
                                    mk&=mk-1;}
                                mk=fl_pmask[k];
                                while(mk){int ph=__builtin_ctz(mk);
                                    cppc[ck][ph][pi]--;if(!cppc[ck][ph][pi])cpm[ck][ph]&=~(1u<<pi);
                                    cppc[ck][ph][pk]++;cpm[ck][ph]|=(1u<<pk);
                                    mk&=mk-1;}
                            }
                        }
                    }
...
void solve_job(){
    ...
    // CB-aware greedy strategies: consolidation bonus during construction
    run_greedy_cb_aware(m, 2, 1, 3, 1);
    run_swap(m);
    run_jm_repair(m);
    TRY_STRATEGY();

    run_greedy_cb_aware(m, 2, 1, 5, 1);
    run_swap(m);
    run_jm_repair(m);
    TRY_STRATEGY();

    run_greedy_cb_aware(m, 2, 1, 3, 1, 1);
    run_swap(m);
    run_jm_repair(m);
    TRY_STRATEGY();

    run_greedy_cb_aware(m, 1, 1, 4, 1);
    run_swap(m);
    run_jm_repair(m);
    TRY_STRATEGY();
