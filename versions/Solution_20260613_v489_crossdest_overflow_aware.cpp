...
void run_cross_dest_swap(int m){
    ...
    const int CD_OVF_BIG=1<<20; // overflow-port demote weight; ovf<=2*m<=62 so ovf*BIG<<INT_MAX
    ...
    for(int iter=0;iter<cd_max_iter;++iter){
        ...
        for(int fi=0;fi<fl_count;++fi){
            ...
            for(int pk=0;pk<g_p;++pk){
                int cnt=dp_cnt[pk];
                int ovf=0;
                unsigned int mo=mask_a;
                while(mo){int ph=__builtin_ctz(mo);
                    if(out_load[sl][pk][ph]+1>cd_max_out[sl][pk])ovf++;
                    if(in_load[dl_a][pk][ph]+1>cd_max_in[dl_a][pk])ovf++;
                    mo&=mo-1;}
                int score=cnt*256-joint_price_bias(sl,dl_a,pk)-ovf*CD_OVF_BIG;
    ...
    for(int iter=0;iter<2;++iter){
        ...
        for(int fi=0;fi<fl_count;++fi){
            ...
            for(int pk=0;pk<g_p;++pk){
                int cnt=dp_cnt[pk];
                int ovf=0;
                unsigned int mo=mask;
                while(mo){int ph=__builtin_ctz(mo);
                    if(out_load[sl][pk][ph]+1>cd_max_out[sl][pk])ovf++;
                    if(in_load[dl][pk][ph]+1>cd_max_in[dl][pk])ovf++;
                    mo&=mo-1;}
                int score=cnt*256-joint_price_bias(sl,dl,pk)-ovf*CD_OVF_BIG;
                if(score>cand_score[0]){
                    cand_score[2]=cand_score[1];
                    cand_ports[2]=cand_ports[1];
                    cand_score[1]=cand_score[0];
