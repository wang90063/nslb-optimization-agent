...
void solve_job(){
    ...
    // v487: for large cases above the full-extra gate but within a bounded work
    // envelope, run extra port_consistency MAIN-CHAIN rounds only. The main-chain
    // run_port_consistency has no total_work gate and converges fast (early `break`
    // when no move), so it is cheap; we deliberately omit perport_refine, which
    // early-returns for large work anyway. Structural gate via job_work, not per-case.
    int allow_extra_pc_chain_only=0;
    if(!allow_extra_pc_chain && !g_time_tight &&
       job_work>100000 && job_work<=600000) allow_extra_pc_chain_only=1;
    ...
    if(allow_extra_pc_chain){
        ...
        run_port_consistency_perport_refine(m);
        run_port_consistency(m);
    } else if(allow_extra_pc_chain_only){
        run_port_consistency(m);
        run_port_consistency(m);
    }
    if(!g_time_tight){
