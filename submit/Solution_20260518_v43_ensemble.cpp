...
void solve_job(){
    ...
    // Strategy 1: standard v33 (local*2, global*1, no hardcap)
    run_greedy(m, 2, 1, 0);
    run_swap(m);
    long long best_score=eval_strategy(m);
    ...
    // Strategy 2: hardcap (local*2, global*1, hardcap=1)
    run_greedy(m, 2, 1, 1);
    run_swap(m);
    long long s2=eval_strategy(m);
    if(s2<best_score){
        best_score=s2;
        memcpy(sv_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(sv_in, in_load, g_l*sizeof(in_load[0]));
    ...
    // Strategy 3: reversed order + hardcap (explores different solution space)
    run_greedy(m, 2, 1, 1, 1);
    run_swap(m);
    long long s3=eval_strategy(m);
    if(s3<best_score){
        best_score=s3;
        memcpy(sv_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(sv_in, in_load, g_l*sizeof(in_load[0]));
