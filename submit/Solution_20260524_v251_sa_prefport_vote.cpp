...
void run_sa_composite(int m){
    ...
    for(int iter=0;;++iter){
        ...
        int py=-1;
        if((rng&3u)!=0){
            unsigned int pref_mask=0;
            int pref_vote[MAX_PORTS];
            for(int pk=0;pk<g_p;++pk) pref_vote[pk]=0;
            unsigned int mk=mask;
            while(mk){
                int ph=__builtin_ctz(mk);
                if(ph>0){
                    unsigned int adj=cpm[ci][ph-1];
                    pref_mask|=adj;
                    unsigned int tmp=adj;
                    while(tmp){
                        int pk=__builtin_ctz(tmp);
                        pref_vote[pk]++;
                        tmp&=tmp-1;
                    }
                }
            ...
            if(pref_mask){
                ...
                while(tmp){
                    int pk=__builtin_ctz(tmp);
                    if(pref_vote[pk]>best_vote){
                        best_vote=pref_vote[pk];
                        cnt=1;
                    } else if(pref_vote[pk]==best_vote){
                        cnt++;
                    }
                    tmp&=tmp-1;
                }
