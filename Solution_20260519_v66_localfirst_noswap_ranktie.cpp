...
static int lf_cands[MAX_PORTS],lf_over[MAX_PORTS],lf_sum[MAX_PORTS],lf_delta[MAX_PORTS];
static long long lf_sq[MAX_PORTS];
...
inline int break_localfirst_tie(int cand_cnt){
    if(cand_cnt<=1) return lf_cands[0];
    int best_pk=lf_cands[0];
    for(int idx=1;idx<cand_cnt;++idx){
        int pk=lf_cands[idx];
        if(lf_delta[pk]!=lf_delta[best_pk]){
            if(lf_delta[pk]<lf_delta[best_pk]) best_pk=pk;
            continue;
        }
        if(lf_over[pk]!=lf_over[best_pk]){
            if(lf_over[pk]<lf_over[best_pk]) best_pk=pk;
            continue;
        }
        if(lf_sq[pk]!=lf_sq[best_pk]){
            if(lf_sq[pk]<lf_sq[best_pk]) best_pk=pk;
            continue;
        }
        if(lf_sum[pk]!=lf_sum[best_pk]){
            if(lf_sum[pk]<lf_sum[best_pk]) best_pk=pk;
            continue;
        }
...
void run_greedy_localfirst(int m, int hardcap, int rev=0){
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            lf_delta[pk]=fgv-gv;
            lf_over[pk]=0;
            if(fo>g_r) lf_over[pk]+=fo-g_r;
            if(fi>g_r) lf_over[pk]+=fi-g_r;
            lf_sq[pk]=(long long)fo*fo+(long long)fi*fi;
            lf_sum[pk]=fo+fi;
            if(cost<bc){
                bc=cost;bp=pk;
                lf_cands[0]=pk;
                ...
                lf_cands[cand_cnt++]=pk;
            }
        }
        bp=break_localfirst_tie(cand_cnt);
        fl_port[i]=(short)bp;
        unsigned int m2=mask;
...
void solve_job(){
    ...
    run_greedy_localfirst(m, 1);
    TRY_STRATEGY();

    run_greedy_localfirst(m, 1, 1);
    TRY_STRATEGY();
