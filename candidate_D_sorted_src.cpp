...
static int ht_key[HT_SIZE],ht_val[HT_SIZE],ht_used[MAX_FLOWS],ht_used_cnt;
...
static int order[MAX_FLOWS];
int cmp_src(const void*a,const void*b){return fl_src[*(const int*)a]/g_pr - fl_src[*(const int*)b]/g_pr;}
void solve_job(){
    ...
    for(int ph=0;ph<m;++ph)
        for(int i=0;i<f;++i){
            ...
            if(!(seen_bits[by]&(1<<bi))){seen_bits[by]|=(1<<bi);fl_src[fl_count]=src;fl_dst[fl_count]=dst;fl_pmask[fl_count]=(1u<<ph);ht_insert(hi,fl_count);fl_count++;}
            else{fl_pmask[ht_find(hi)]|=(1u<<ph);}
        }
    ...
    // sort inter-leaf flows by src_leaf
    int ic=0;
    for(int i=0;i<fl_count;++i){
        if(fl_src[i]/g_pr==fl_dst[i]/g_pr){fl_port[i]=-1;continue;}
        order[ic++]=i;
    }
    qsort(order,ic,sizeof(int),cmp_src);
    for(int ii=0;ii<ic;++ii){
        ...
        for(int pk=0;pk<g_p;++pk){
            int cost=0;unsigned int m2=mask;
