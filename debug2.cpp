...
void solve_job(int job_idx){
    ...
    for(int i=0;i<fl_count;++i){fl_sl[i]=fl_src[i]/g_pr;fl_dl[i]=fl_dst[i]/g_pr;}
    for(int i=0;i<fl_count;++i){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            while(m2){int ph=__builtin_ctz(m2);int o=out_load[sl][pk][ph]+1;int iv=in_load[dl][pk][ph]+1;int v=o>iv?o:iv;if(v>cost)cost=v;m2&=m2-1;}
    ...
    int job_max=0;
    int bn_leaf=-1,bn_port=-1,bn_phase=-1,bn_dir=0;
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk)
            for(int ph=0;ph<m;++ph){
                if(out_load[leaf][pk][ph]>job_max){job_max=out_load[leaf][pk][ph];bn_leaf=leaf;bn_port=pk;bn_phase=ph;bn_dir=0;}
                if(in_load[leaf][pk][ph]>job_max){job_max=in_load[leaf][pk][ph];bn_leaf=leaf;bn_port=pk;bn_phase=ph;bn_dir=1;}
            }
    if(job_max>g_r){
        int ports_below=0;
        for(int pk=0;pk<g_p;++pk){
            int v=bn_dir==0?out_load[bn_leaf][pk][bn_phase]:in_load[bn_leaf][pk][bn_phase];
            if(v<g_r) ports_below++;
        }
        int movable=0;
        for(int i=0;i<fl_count;++i){
            if(fl_port[i]!=bn_port) continue;
            if(!(fl_pmask[i]&(1u<<bn_phase))) continue;
            int relevant=(bn_dir==0)?(fl_sl[i]==bn_leaf):(fl_dl[i]==bn_leaf);
            if(relevant) movable++;
        }
        fprintf(stderr,"Job%d: max=%d bn=(%d,p%d,ph%d,%s) ports_below_r=%d movable=%d\n",
            job_idx,job_max,bn_leaf,bn_port,bn_phase,bn_dir?"in":"out",ports_below,movable);
    }
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            int mo=0,mi=0;
