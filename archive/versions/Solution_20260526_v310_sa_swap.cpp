...
void run_sa_composite(int m){
    ...
    for(int iter=0;;++iter){
        ...
        int sl=fl_sl[fi],dl=fl_dl[fi],px=fl_port[fi],ci=fl_src[fi];
        rng=rng*1664525u+1013904223u;
        // 25% chance: try swap move with another flow on same source leaf
        if((rng&3)==0){
            rng=rng*1664525u+1013904223u;
            int fj;
            if(sa_focus_cnt>0) fj=sa_focus[rng%sa_focus_cnt];
            else fj=rng%fl_count;
            if(fj==fi||fl_sl[fj]!=sl||fl_sl[fj]==fl_dl[fj]||fl_port[fj]<0||fl_port[fj]==px){T*=cool;continue;}
            int sl2=fl_sl[fj],dl2=fl_dl[fj],py=fl_port[fj],cj=fl_src[fj];
            unsigned int mask_a=fl_pmask[fi],mask_b=fl_pmask[fj];
            // Check feasibility: A→py, B→px
            unsigned int m2=mask_a; int ok=1;
            while(m2){int ph=__builtin_ctz(m2);
                if(out_load[sl][py][ph]-(mask_b>>ph&1)+1>sa_max_out[sl][py]){ok=0;break;}
                if(in_load[dl][py][ph]+1>sa_max_in[dl][py]){ok=0;break;}
                m2&=m2-1;}
            if(!ok){T*=cool;continue;}
            m2=mask_b;
            while(m2){int ph=__builtin_ctz(m2);
                if(out_load[sl][px][ph]-(mask_a>>ph&1)+1>sa_max_out[sl][px]){ok=0;break;}
                if(in_load[dl2][px][ph]+1>sa_max_in[dl2][px]){ok=0;break;}
                m2&=m2-1;}
            if(!ok){T*=cool;continue;}
            // Compute CB delta for both cards
            int before_cbt=card_cbtphsc(ci,m)+(ci!=cj?card_cbtphsc(cj,m):0);
            m2=mask_a;while(m2){int ph=__builtin_ctz(m2);
                cppc[ci][ph][px]--;if(!cppc[ci][ph][px])cpm[ci][ph]&=~(1u<<px);
                cppc[ci][ph][py]++;cpm[ci][ph]|=(1u<<py);m2&=m2-1;}
            m2=mask_b;while(m2){int ph=__builtin_ctz(m2);
                cppc[cj][ph][py]--;if(!cppc[cj][ph][py])cpm[cj][ph]&=~(1u<<py);
                cppc[cj][ph][px]++;cpm[cj][ph]|=(1u<<px);m2&=m2-1;}
            int after_cbt=card_cbtphsc(ci,m)+(ci!=cj?card_cbtphsc(cj,m):0);
            int cbt_delta=after_cbt-before_cbt;
            int obj_delta=5*cbt_delta;
            int accept=0;
            if(obj_delta<=0) accept=1;
            else{
                rng=rng*1664525u+1013904223u;
                double r2=(rng&0xFFFF)/65536.0;
                if(r2<__builtin_exp(-(double)obj_delta/T)) accept=1;
            }
            if(accept){
                m2=mask_a;while(m2){int ph=__builtin_ctz(m2);
                    out_load[sl][px][ph]--;out_load[sl][py][ph]++;
                ...
                m2=mask_b;while(m2){int ph=__builtin_ctz(m2);
                    ...
                    in_load[dl2][py][ph]--;in_load[dl2][px][ph]++;m2&=m2-1;}
                fl_port[fi]=(short)py;fl_port[fj]=(short)px;
                sa_accepted++;
            } else {
                m2=mask_b;while(m2){int ph=__builtin_ctz(m2);
                    cppc[cj][ph][px]--;if(!cppc[cj][ph][px])cpm[cj][ph]&=~(1u<<px);
                    cppc[cj][ph][py]++;cpm[cj][ph]|=(1u<<py);m2&=m2-1;}
                m2=mask_a;while(m2){int ph=__builtin_ctz(m2);
                    cppc[ci][ph][py]--;if(!cppc[ci][ph][py])cpm[ci][ph]&=~(1u<<py);
...
void solve_job(){
    ...
    // Post-SA CT reduce: SA may have created new CT reduction opportunities
    if(!g_time_tight){
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
