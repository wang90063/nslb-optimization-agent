...
void run_sa_composite(int m, double init_T=2.0){
    ...
    // SA main loop — temperature from parameter
    double T=init_T;
    double cool=0.99995;
...
void solve_job(){
    ...
    // Three-pass SA with decreasing temperature: explore → converge
    if(!g_time_tight){
        static const double sa_temps[3]={3.0, 2.0, 1.0};
        for(int sa_pass=0;sa_pass<3;++sa_pass){
            memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
            memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
            memcpy(bk_port, fl_port, fl_count*sizeof(short));
            EvalMetrics sa_base=collect_metrics(m);
            run_sa_composite(m, sa_temps[sa_pass]);
