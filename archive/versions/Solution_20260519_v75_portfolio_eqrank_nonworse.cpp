...
static int eq_cands[MAX_PORTS],eq_over[MAX_PORTS],eq_sum[MAX_PORTS],eq_delta[MAX_PORTS];
static long long eq_sq[MAX_PORTS];
...
inline int nonworse_primary_metrics(const EvalMetrics &a,const EvalMetrics &b){
    return a.score_jm<=b.score_jm && a.score_fg<=b.score_fg;
}

inline int break_equal_cost_tie(int cand_cnt){
    if(cand_cnt<=1) return eq_cands[0];
    int best_pk=eq_cands[0];
    for(int idx=1;idx<cand_cnt;++idx){
        int pk=eq_cands[idx];
        if(eq_delta[pk]!=eq_delta[best_pk]){
            if(eq_delta[pk]<eq_delta[best_pk]) best_pk=pk;
            continue;
        }
        if(eq_over[pk]!=eq_over[best_pk]){
            if(eq_over[pk]<eq_over[best_pk]) best_pk=pk;
            continue;
        }
        if(eq_sq[pk]!=eq_sq[best_pk]){
            if(eq_sq[pk]<eq_sq[best_pk]) best_pk=pk;
            continue;
        }
        if(eq_sum[pk]!=eq_sum[best_pk]){
            if(eq_sum[pk]<eq_sum[best_pk]) best_pk=pk;
            continue;
        }
        if(pk<best_pk) best_pk=pk;
    }
    return best_pk;
}
...
void run_greedy_eq(int m, int local_w, int global_w, int hardcap, int rev=0){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    if(rev>=2){
        unsigned int seed=rev*2654435761u;
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            eq_delta[pk]=0;
            eq_over[pk]=0;
            if(go>g_r) eq_over[pk]+=go-g_r;
            if(gi>g_r) eq_over[pk]+=gi-g_r;
            eq_sq[pk]=(long long)go*go+(long long)gi*gi;
            eq_sum[pk]=go+gi;
            if(cost<bc){
                bc=cost;bp=pk;
                eq_cands[0]=pk;
                ...
                eq_cands[cand_cnt++]=pk;
            }
        }
        bp=break_equal_cost_tie(cand_cnt);
        fl_port[i]=(short)bp;
        unsigned int m2=mask;
        while(m2){int ph=__builtin_ctz(m2);out_load[sl][bp][ph]++;in_load[dl][bp][ph]++;m2&=m2-1;}
    }
...
void solve_job(){
    ...
    run_greedy(m, 2, 1, 1);
    run_swap(m);
    EvalMetrics base_s2=collect_metrics(m);
    TRY_STRATEGY();

    // S2e: hardcap + equal-cost rank tie-break + swap
    run_greedy_eq(m, 2, 1, 1);
    run_swap(m);
    EvalMetrics eq_s2=collect_metrics(m);
    if(nonworse_primary_metrics(eq_s2, base_s2)) TRY_STRATEGY();

    // S3: reversed order + hardcap + swap
    run_greedy(m, 2, 1, 1, 1);
    run_swap(m);
    EvalMetrics base_s3=collect_metrics(m);
    TRY_STRATEGY();

    // S3e: reversed hardcap + equal-cost rank tie-break + swap
    run_greedy_eq(m, 2, 1, 1, 1);
    run_swap(m);
    EvalMetrics eq_s3=collect_metrics(m);
    if(nonworse_primary_metrics(eq_s3, base_s3)) TRY_STRATEGY();

    // S4: hardcap, no swap
    run_greedy(m, 2, 1, 1);
    EvalMetrics base_s4=collect_metrics(m);
    TRY_STRATEGY();

    // S4e: hardcap, no swap + equal-cost rank tie-break
    run_greedy_eq(m, 2, 1, 1);
    EvalMetrics eq_s4=collect_metrics(m);
    if(nonworse_primary_metrics(eq_s4, base_s4)) TRY_STRATEGY();
    ...
    run_greedy(m, 3, 2, 1);
    run_swap(m);
    EvalMetrics base_s6=collect_metrics(m);
    TRY_STRATEGY();

    // S6e1: stronger global + equal-cost rank tie-break + swap
    run_greedy_eq(m, 3, 2, 1);
    run_swap(m);
    EvalMetrics eq_s6=collect_metrics(m);
    if(nonworse_primary_metrics(eq_s6, base_s6)) TRY_STRATEGY();
    ...
    run_global_swap(m);
    run_lns(m);
    EvalMetrics base_s11=collect_metrics(m);
    TRY_STRATEGY();

    // S11e: global-dominant + equal-cost rank tie-break + swap + global swap + lns
    run_greedy_eq(m, 1, 3, 1);
    run_swap(m);
    run_global_swap(m);
    run_lns(m);
    EvalMetrics eq_s11=collect_metrics(m);
    if(nonworse_primary_metrics(eq_s11, base_s11)) TRY_STRATEGY();
