...
void run_lns_card(int m){
    ...
    static int cb_vals[MAX_CARDS];
    int cb_cnt=0;
    static int card_done[MAX_CARDS];
    memset(card_done,0,sizeof(card_done));
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
        int c=fl_src[i];
        if(!card_done[c]){
            ...
            if(cv>0){cb_cards[cb_cnt]=c;cb_vals[cb_cnt]=cv;cb_cnt++;}
        }
    }
    if(cb_cnt==0) return;
    static int destroy_flows[MAX_FLOWS];
    int jm=get_job_max(m);
    for(int ci=0;ci<cb_cnt&&ci<200;++ci){
        int card=cb_cards[ci];
        int before_cb=cb_vals[ci];
        if(before_cb<=0) continue;
        int df_cnt=0;
        for(int i=0;i<fl_count;++i){
            if(fl_src[i]==card&&fl_sl[i]!=fl_dl[i]&&fl_port[i]>=0)
                destroy_flows[df_cnt++]=i;
        }
        if(df_cnt<2||df_cnt>30) continue;
        short old_ports[30];
        for(int d=0;d<df_cnt;++d){
            int fi=destroy_flows[d];
            old_ports[d]=fl_port[fi];
            int px=fl_port[fi],sl=fl_sl[fi],dl=fl_dl[fi];
            unsigned int mk=fl_pmask[fi];
            while(mk){int ph=__builtin_ctz(mk);
                out_load[sl][px][ph]--;in_load[dl][px][ph]--;mk&=mk-1;}
        }
        for(int d=0;d<df_cnt;++d){
            int fi=destroy_flows[d];
            int sl=fl_sl[fi],dl=fl_dl[fi];
            ...
            for(int pk=0;pk<g_p;++pk){
                unsigned int m2=mask;int feasible=1,local_max=0;
                while(m2){int ph=__builtin_ctz(m2);
                    int o=out_load[sl][pk][ph]+1;
                    int iv=in_load[dl][pk][ph]+1;
                    if(o>jm||iv>jm){feasible=0;break;}
                    int v=o>iv?o:iv;if(v>local_max)local_max=v;
                    m2&=m2-1;}
                if(!feasible) continue;
                int fo=global_out[sl][pk],fi2=global_in[dl][pk];
                int fgv=fo>fi2?fo:fi2;
                int cost=local_max*2+fgv;
                if(cost<bc){bc=cost;bp=pk;}
            }
            if(bp<0) bp=old_ports[d];
            fl_port[fi]=(short)bp;
            unsigned int m2=mask;
            while(m2){int ph=__builtin_ctz(m2);
                out_load[sl][bp][ph]++;in_load[dl][bp][ph]++;m2&=m2-1;}
        }
        int new_jm=get_job_max(m);
        memset(cpm,0,sizeof(cpm));memset(cppc,0,sizeof(cppc));
        for(int i=0;i<fl_count;++i){
            if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
            int c2=fl_src[i],p2=fl_port[i];
            unsigned int mk=fl_pmask[i];
            while(mk){int ph=__builtin_ctz(mk);cppc[c2][ph][p2]++;cpm[c2][ph]|=(1u<<p2);mk&=mk-1;}
        }
        int after_cb=card_cbtphsc(card,m);
        if(after_cb>=before_cb||new_jm>jm){
            for(int d=0;d<df_cnt;++d){
                int fi=destroy_flows[d];
...
void solve_job(){
    ...
    // LNS: destroy high-CB cards and re-assign
    if(!g_time_tight){
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        EvalMetrics lns_base=collect_metrics(m);
        run_lns_card(m);
        EvalMetrics lns_after=collect_metrics(m);
        if(lns_after.fg>lns_base.fg||(lns_after.fg==lns_base.fg&&lns_after.jm>lns_base.jm)){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));
        } else {
