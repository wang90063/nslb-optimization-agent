...
void solve_job(){
    ...
    // Pass 3: conflict-neutral reassignment
    // Move flows to card's dominant port ONLY if:
    //   1. Target port load stays <= r in ALL phases (no Cinphsc increase)
    //   2. Job max doesn't increase (no Maxsingler harm)
    //   3. Source port load was <= r (moving away doesn't help Cinphsc, so neutral)
    int job_max = get_job_max(m);
    // Build card->port frequency map
    static short card_port_cnt[MAX_CARDS][MAX_PORTS];
    memset(card_port_cnt, 0, g_pr * sizeof(card_port_cnt[0]));
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]) continue;
        if(fl_port[i]>=0) card_port_cnt[fl_src[i]][fl_port[i]]++;
    }
    ...
    for(int i=0;i<fl_count;++i){
        ...
        int dom_port=cp, dom_cnt=card_port_cnt[src_card][cp];
        for(int pk=0;pk<g_p;++pk){
            if(card_port_cnt[src_card][pk]>dom_cnt){
                dom_cnt=card_port_cnt[src_card][pk];
                dom_port=pk;
            }
        }
        if(dom_port==cp) continue;
        // Check constraint: target port load <= r in all affected phases
        // AND target port load <= job_max (preserve Maxsingler)
        unsigned int mask=fl_pmask[i];
        int ok=1;
        unsigned int m2=mask;
        while(m2){
            int ph=__builtin_ctz(m2);
            if(out_load[sl][dom_port][ph]+1 > g_r){ok=0;break;}
            if(in_load[dl][dom_port][ph]+1 > g_r){ok=0;break;}
            m2&=m2-1;
        }
        if(ok){
            // Commit the move
            m2=mask;
            while(m2){int ph=__builtin_ctz(m2);out_load[sl][cp][ph]--;in_load[dl][cp][ph]--;out_load[sl][dom_port][ph]++;in_load[dl][dom_port][ph]++;m2&=m2-1;}
            card_port_cnt[src_card][cp]--;
            card_port_cnt[src_card][dom_port]++;
            fl_port[i]=(short)dom_port;
        }
    }
