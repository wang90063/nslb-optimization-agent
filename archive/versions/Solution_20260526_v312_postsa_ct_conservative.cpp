...
void solve_job(){
    ...
    // Post-SA CT reduce: find CT/CB improvements SA missed
    // Conservative: do NOT update pre_ct_mo — global state stays unchanged
    if(!g_time_tight){
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        ...
        // Note: do NOT set ct_pass_ran or update pre_ct_mo here
        // Global state remains conservative (uses pre-first-CT max)
    }
