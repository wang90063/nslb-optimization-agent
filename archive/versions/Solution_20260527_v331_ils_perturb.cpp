...
void solve_job(){
    ...
    if(!g_time_tight){
        for(int sa_pass=0;sa_pass<2;++sa_pass){
            memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
            memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
            memcpy(bk_port, fl_port, fl_count*sizeof(short));
    ...
    // ILS: perturb + SA pass (escape local optima)
    if(!g_time_tight){
        // Save pre-perturbation solution
        static short ils_out[MAX_LEAFS][MAX_PORTS][MAX_PHASES];
        static short ils_in[MAX_LEAFS][MAX_PORTS][MAX_PHASES];
        static short ils_port[MAX_FLOWS];
        memcpy(ils_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(ils_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(ils_port, fl_port, fl_count*sizeof(short));
        EvalMetrics ils_base=collect_metrics(m);
        // Perturb: random load-neutral swaps
        unsigned int prng=fl_count*1664525u+g_job_idx*2654435761u+13u;
        int n_perturb=20;
        if(fl_count<500) n_perturb=5;
        else if(fl_count<2000) n_perturb=10;
        for(int pi=0;pi<n_perturb;++pi){
            prng=prng*1664525u+1013904223u;
            int fa=prng%fl_count;
            if(fl_sl[fa]==fl_dl[fa]||fl_port[fa]<0) continue;
            int sl_a=fl_sl[fa],pa=fl_port[fa];
            // Find a swap partner: same source leaf, different port
            prng=prng*1664525u+1013904223u;
            int fb=prng%fl_count;
            if(fb==fa||fl_sl[fb]!=sl_a||fl_sl[fb]==fl_dl[fb]||fl_port[fb]<0) continue;
            int pb=fl_port[fb];
            if(pa==pb) continue;
            // Execute swap (no feasibility check — just perturb)
            unsigned int ma=fl_pmask[fa],mb=fl_pmask[fb];
            int dl_a=fl_dl[fa],dl_b=fl_dl[fb];
            unsigned int m2=ma;
            while(m2){int ph=__builtin_ctz(m2);
                out_load[sl_a][pa][ph]--;out_load[sl_a][pb][ph]++;
                in_load[dl_a][pa][ph]--;in_load[dl_a][pb][ph]++;
                m2&=m2-1;}
            m2=mb;
            while(m2){int ph=__builtin_ctz(m2);
                out_load[sl_a][pb][ph]--;out_load[sl_a][pa][ph]++;
                in_load[dl_b][pb][ph]--;in_load[dl_b][pa][ph]++;
                m2&=m2-1;}
            fl_port[fa]=(short)pb;
            fl_port[fb]=(short)pa;
        }
        ...
        run_relaxed_swap(m);
        run_cross_dest_swap(m);
        // Compare with pre-perturbation
        EvalMetrics ils_after=collect_metrics(m);
        if(ils_after.fg>ils_base.fg||(ils_after.fg==ils_base.fg&&ils_after.jm>ils_base.jm)){
            memcpy(out_load, ils_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, ils_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, ils_port, fl_count*sizeof(short));
        }
    }
