            ...
            if(base_cbt>0&&cnt<=160){
                ...
                for(int tp=0;tp<g_p;++tp){
                    ...
                    for(int k=0;k<cnt;++k){
                        ...
                        if(!useful) continue;
                        can_move++;
                        pressure+=(long long)joint_price_bias(sl,dl,tp)-joint_price_bias(sl,dl,cp);
                        m2=mask;
                        while(m2){
                            int ph=__builtin_ctz(m2);
                            tmp_ppc[ph][cp]--;
            ...
            if(gsz>=2&&gsz<=200){
                for(int a=g_start;a<g_end;++a){
                    ...
                    for(int b=a+1;b<g_end;++b){
                        ...
                        int sl=fl_sl[i],dl=fl_dl[i];
                        int take_swap=(after<before);
                        if(!take_swap&&after==before){
                            int price_pj=joint_price_bias(sl,dl,pj);
                            int price_pi=joint_price_bias(sl,dl,pi);
                            if(price_pj<price_pi) take_swap=1;
                        }
                        if(take_swap){
            ...
            if(gsz>=2&&gsz<=rs_gsz_cap){
                for(int a=g_start;a<g_end;++a){
                    ...
                    for(int b=a+1;b<g_end;++b){
                        ...
                        int take_swap=(after<before);
                        if(!take_swap&&after==before){
                            int price_pj=joint_price_bias(sl,dl_i,pj)+joint_price_bias(sl,dl_j,pj);
                            int price_pi=joint_price_bias(sl,dl_i,pi)+joint_price_bias(sl,dl_j,pi);
                            if(price_pj<price_pi) take_swap=1;
                        }
                        if(take_swap){
                            // Commit: update loads
