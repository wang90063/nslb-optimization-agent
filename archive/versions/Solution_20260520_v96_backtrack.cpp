...
void run_mc_repair(int m){
    ...
    for(int iter=0;iter<3000&&viol_cnt>0;++iter){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            while(m2){int ph=__builtin_ctz(m2);
                if(out_load[sl][cp][ph]==g_r+1) delta--;
                if(out_load[sl][pk][ph]==g_r) delta++;
                if(in_load[dl][cp][ph]==g_r+1) delta--;
...
void solve_job(){
    ...
    // Min-conflicts repair for marginal Maxsingler violations
    if(g_l<=32&&fl_count<=2000&&get_job_max(m)==g_r+1) run_mc_repair(m);
