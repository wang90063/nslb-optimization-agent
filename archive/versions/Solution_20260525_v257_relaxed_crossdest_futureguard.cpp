...
void run_relaxed_swap(int m){
    ...
    int use_future_guard=(g_r==4&&g_p>=16&&!g_time_tight);
            ...
            if(gsz>=2&&gsz<=rs_gsz_cap){
                for(int a=g_start;a<g_end;++a){
                    ...
                    for(int b=a+1;b<g_end;++b){
                        ...
                        int after=card_cbtphsc(ci,m)+card_cbtphsc(cj,m);
                        int accept_swap=(after<before);
                        if(accept_swap&&use_future_guard){
                            int net=before-after;
                            LocalFutureDelta fd=eval_makeroom_future_delta(m,i,pi,pj,j,pi);
                            if(fd.fg_delta>0||(net==1&&fd.over_delta>0)) accept_swap=0;
                        }
                        if(accept_swap){
                            // Commit: update loads
                            if(dl_i==dl_j){
...
void run_cross_dest_swap(int m){
    ...
    for(int iter=0;iter<cd_max_iter;++iter){
        ...
        for(int fi=0;fi<fl_count;++fi){
            ...
            for(int bi=bstart;bi<bend;++bi){
                ...
                if(wt<best_wt) continue;
                LocalFutureDelta fd=eval_makeroom_future_delta(m,fi,px,py,j,px);
                if(fd.fg_delta>0) continue;
                if(net==1&&fd.over_delta>0) continue;
                if(wt>best_wt||
                   (wt==best_wt&&fd.fg_delta<best_fg_delta)||
                   (wt==best_wt&&fd.fg_delta==best_fg_delta&&fd.over_delta<best_over_delta)||
                   (wt==best_wt&&fd.fg_delta==best_fg_delta&&fd.over_delta==best_over_delta&&fd.sq_delta<best_sq_delta)){
                    best_wt=wt;
                    best_j=j;
    ...
    for(int iter=0;iter<2;++iter){
        ...
        for(int fi=0;fi<fl_count;++fi){
            ...
            int net=before-after;
            int wt=5*net-12*ov_delta;
            int accept_move=(wt>0);
            if(accept_move&&use_future_guard){
                LocalFutureDelta fd=eval_makeroom_future_delta(m,fi,px,py,-1,-1);
                if(fd.fg_delta>0||(net==1&&fd.over_delta>0)) accept_move=0;
            }
            if(accept_move){
                mk=mask;while(mk){int ph=__builtin_ctz(mk);
