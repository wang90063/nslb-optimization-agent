...
void run_card_phase_group(int m){
    static short gp_max_out[MAX_LEAFS][MAX_PORTS];
    static short gp_max_in[MAX_LEAFS][MAX_PORTS];
    static int gp_flows[MAX_FLOWS];
    static short gp_save_port[MAX_FLOWS];
    static short gp_delta_out[MAX_PHASES];
    static short gp_delta_in[MAX_LEAFS][MAX_PHASES];
    static int gp_dls[MAX_LEAFS];
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            short mo=0,mi=0;
            for(int ph=0;ph<m;++ph){
            ...
            gp_max_out[leaf][pk]=mo;
            gp_max_in[leaf][pk]=mi;
        }
    memset(cpm,0,sizeof(cpm));
    ...
    for(int iter=0;iter<3;++iter){
        int any_improved=0;
        for(int cc=0;cc<card_cnt;++cc){
            int ci=cbt_eval_cards[cc];
            int cb_before=card_cbtphsc(ci,m);
            if(cb_before==0) continue;
            int dp=-1,dp_best=0;
            for(int pk=0;pk<g_p;++pk){
                int cnt=0;
                for(int ph=0;ph<m;++ph) cnt+=cppc[ci][ph][pk];
                if(cnt>dp_best){dp_best=cnt;dp=pk;}
            }
            if(dp<0) continue;
            int sl_card=ci/(g_p*g_r);
            int gcnt=0;
            for(int i=0;i<fl_count;++i){
                if(fl_src[i]!=ci) continue;
                if(fl_sl[i]==fl_dl[i]||fl_port[i]<0||fl_port[i]==dp) continue;
                gp_flows[gcnt]=i;
                gp_save_port[gcnt]=(short)fl_port[i];
                gcnt++;
            }
            if(gcnt==0) continue;
            memset(gp_delta_out,0,m*sizeof(short));
            int n_dl=0;
            for(int gi=0;gi<gcnt;++gi){
                ...
                int found=0;
                for(int d=0;d<n_dl;++d) if(gp_dls[d]==dl){found=1;break;}
                if(!found) gp_dls[n_dl++]=dl;
            }
            for(int d=0;d<n_dl;++d) memset(gp_delta_in[gp_dls[d]],0,m*sizeof(short));
            for(int gi=0;gi<gcnt;++gi){
                int fi=gp_flows[gi];
                int dl=fl_dl[fi];
                unsigned int mk=fl_pmask[fi];
            ...
            for(int ph=0;ph<m&&ok;++ph){
                if(gp_delta_out[ph]>0&&out_load[sl_card][dp][ph]+gp_delta_out[ph]>gp_max_out[sl_card][dp]) ok=0;
            }
            for(int d=0;d<n_dl&&ok;++d){
                int dl=gp_dls[d];
                for(int ph=0;ph<m&&ok;++ph){
                    if(gp_delta_in[dl][ph]>0&&in_load[dl][dp][ph]+gp_delta_in[dl][ph]>gp_max_in[dl][dp]) ok=0;
                }
            }
            if(!ok) continue;
            for(int gi=0;gi<gcnt;++gi){
                int fi=gp_flows[gi];
                int old_p=fl_port[fi];
                int dl=fl_dl[fi];
                unsigned int mk=fl_pmask[fi];
                while(mk){
                    int ph=__builtin_ctz(mk);
                    out_load[sl_card][old_p][ph]--;
                    in_load[dl][old_p][ph]--;
                    out_load[sl_card][dp][ph]++;
                    in_load[dl][dp][ph]++;
                    cppc[ci][ph][old_p]--;
                    if(cppc[ci][ph][old_p]==0) cpm[ci][ph]&=~(1u<<old_p);
                    cppc[ci][ph][dp]++;
                    cpm[ci][ph]|=(1u<<dp);
                    mk&=mk-1;
                }
            ...
            int cb_after=card_cbtphsc(ci,m);
            if(cb_after>=cb_before){
                for(int gi=0;gi<gcnt;++gi){
                    int fi=gp_flows[gi];
                    int old_p=gp_save_port[gi];
                    int dl=fl_dl[fi];
                    unsigned int mk=fl_pmask[fi];
                    while(mk){
                        int ph=__builtin_ctz(mk);
                        ...
                        in_load[dl][old_p][ph]++;
                        cppc[ci][ph][dp]--;
                        if(cppc[ci][ph][dp]==0) cpm[ci][ph]&=~(1u<<dp);
                        cppc[ci][ph][old_p]++;
                        cpm[ci][ph]|=(1u<<old_p);
                        mk&=mk-1;
                    }
                    fl_port[fi]=(short)old_p;
                }
            } else {
                any_improved=1;
            }
    ...
    for(int cc=0;cc<card_cnt;++cc) cbt_eval_seen[cbt_eval_cards[cc]]=0;
}

void run_card_phase_group_if_safe(int m){
    EvalMetrics base_eval=collect_metrics(m);
    int base_cb=total_cbtphsc_from_ports(fl_port,m);
    memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
    memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
    memcpy(bk_port, fl_port, fl_count*sizeof(short));
    run_card_phase_group(m);
    EvalMetrics cand_eval=collect_metrics(m);
    int cand_cb=total_cbtphsc_from_ports(fl_port,m);
    int keep=0;
    if(better_metrics(cand_eval, base_eval)){
        keep=1;
    } else if(cand_eval.score_jm==base_eval.score_jm &&
              cand_eval.score_fg==base_eval.score_fg &&
              cand_eval.ci==base_eval.ci &&
              cand_cb<base_cb){
        keep=1;
    }
    if(!keep){
...
void solve_job(){
    ...
    if(g_p>=32&&g_r>=4&&fl_count<=7000){
        ...
        run_swap(m);
        run_card_phase_group_if_safe(m);
        TRY_STRATEGY();
