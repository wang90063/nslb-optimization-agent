...
void solve_job(){
    ...
    // Pre-PC SA pass: explore from greedy state before PC locks in CB structure
    if(!g_time_tight){
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        EvalMetrics pre_pc_sa_base=collect_metrics(m);
        run_sa_composite(m);
        EvalMetrics pre_pc_sa_after=collect_metrics(m);
        if(pre_pc_sa_after.fg>pre_pc_sa_base.fg||(pre_pc_sa_after.fg==pre_pc_sa_base.fg&&pre_pc_sa_after.jm>pre_pc_sa_base.jm)){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));
        }
