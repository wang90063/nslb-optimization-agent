...
// Dedicated best-state buffer for local batch rebuild operators.
static short fp_out[MAX_LEAFS][MAX_PORTS][MAX_PHASES];
static short fp_in[MAX_LEAFS][MAX_PORTS][MAX_PHASES];
static short fp_port[MAX_FLOWS];
struct EvalMetrics{
    int score_jm;
    int score_fg;
    int jm;
...
static int future_port_flows[MAX_FLOWS];
static int future_port_score[MAX_FLOWS];
...
// Targeted p32/r4 future repair: rebuild a bounded subset of flows from the
// hottest future leaf-port, keeping jm fixed while trying to reduce overflow.
__attribute__((noinline,cold)) void run_p32_future_port_rebuild(int m){
    if(g_p<32||g_r<4) return;
    if(fl_count>7000||(long long)fl_count*m>140000) return;

    EvalMetrics base_eval=collect_metrics(m);
    if(base_eval.jm>g_r||base_eval.future_over<=0) return;

    memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
    memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
    memcpy(bk_port, fl_port, fl_count*sizeof(short));

    FutureHotPort hot_ports[8];
    int hot_cnt=0;
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            int mo=0,mi=0;
            ...
            int fi=global_in[leaf][pk]+mi;
            if(fo>g_r){
                int score=(fo-g_r)*8192+mo*64+(fo==base_eval.fg?2048:0);
                insert_future_hot_port(hot_ports,hot_cnt,8,leaf,pk,0,fo,mo,score);
            }
            if(fi>g_r){
                int score=(fi-g_r)*8192+mi*64+(fi==base_eval.fg?2048:0);
                insert_future_hot_port(hot_ports,hot_cnt,8,leaf,pk,1,fi,mi,score);
            }
        }
    if(hot_cnt<=0) return;
    ...
    int improved=0;
    int max_hot=hot_cnt<2?hot_cnt:2;
    for(int hi=0;hi<max_hot;++hi){
        FutureHotPort hp=hot_ports[hi];
        int cand_cnt=0;
        for(int fi=0;fi<fl_count;++fi){
            if(fl_sl[fi]==fl_dl[fi]) continue;
            if(bk_port[fi]!=hp.port||bk_port[fi]<0) continue;
            if((hp.dir==0&&fl_sl[fi]!=hp.leaf)||(hp.dir==1&&fl_dl[fi]!=hp.leaf)) continue;
            int score=__builtin_popcount(fl_pmask[fi])*64;
            unsigned int mk=fl_pmask[fi];
            while(mk){
                int ph=__builtin_ctz(mk);
                int cur=(hp.dir==0)?bk_out[hp.leaf][hp.port][ph]:bk_in[hp.leaf][hp.port][ph];
                if(cur==hp.local_max) score+=4096;
                else if(cur+1>=hp.local_max) score+=1024;
                else if(cur+2>=hp.local_max) score+=256;
                mk&=mk-1;
            }
            future_port_flows[cand_cnt]=fi;
            future_port_score[fi]=score;
            cand_cnt++;
        }
        if(cand_cnt<=1) continue;

        int top_cap=cand_cnt;
        if(top_cap>36) top_cap=36;
        for(int i=0;i<top_cap;++i){
            int best=i;
            for(int j=i+1;j<cand_cnt;++j)
                if(future_port_score[future_port_flows[j]]>future_port_score[future_port_flows[best]])
                    best=j;
            if(best!=i){
                int t=future_port_flows[i];
                future_port_flows[i]=future_port_flows[best];
                future_port_flows[best]=t;
            }
        }

        int try_sizes[3];
        int try_cnt=0;
        const int size_caps[3]={12,24,36};
        for(int si=0;si<3;++si){
            int use_cnt=cand_cnt;
            if(use_cnt>size_caps[si]) use_cnt=size_caps[si];
            if(use_cnt>=2&&(try_cnt==0||use_cnt!=try_sizes[try_cnt-1]))
                try_sizes[try_cnt++]=use_cnt;
        }

        for(int si=0;si<try_cnt;++si){
            int use_cnt=try_sizes[si];
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));

            for(int i=0;i<use_cnt;++i){
                int fi=future_port_flows[i];
            ...
            int failed=0;
            for(int i=0;i<use_cnt&&!failed;++i){
                int fi=future_port_flows[i];
                int sl=fl_sl[fi],dl=fl_dl[fi];
                int cur_pk=bk_port[fi];
                unsigned int mask=fl_pmask[fi];
                int best_pk=-1,best_local=0x7fffffff,best_hot=0x7fffffff;
                int best_add_over=0x7fffffff,best_fg=0x7fffffff,best_other=0x7fffffff;
                int best_future_over=0x7fffffff,best_press=0x7fffffff,best_stay=0x7fffffff;
                ...
                for(int pk=0;pk<g_p;++pk){
                    ...
                    while(mk){
                        ...
                        if(o>g_r) add_over+=o-g_r;
                        if(iv>g_r) add_over+=iv-g_r;
                        if(o>mo) mo=o;
                        if(iv>mi) mi=iv;
                        mk&=mk-1;
                    }
                    if(local_peak>g_r) continue;
                    for(int ph=0;ph<m;++ph){
                    ...
                    if(future_fg>base_eval.fg) continue;
                    int hot_future=(hp.dir==0)?fo:fi_v;
                    int other_future=(hp.dir==0)?fi_v:fo;
                    int future_over=0;
                    if(fo>g_r) future_over+=fo-g_r;
                    if(fi_v>g_r) future_over+=fi_v-g_r;
                    long long future_sq=(long long)fo*fo+(long long)fi_v*fi_v;
                    ...
                    if(local_peak<best_local||
                       ...
                       (local_peak==best_local&&hot_future==best_hot&&add_over==best_add_over&&future_fg<best_fg)||
                       (local_peak==best_local&&hot_future==best_hot&&add_over==best_add_over&&future_fg==best_fg&&other_future<best_other)||
                       (local_peak==best_local&&hot_future==best_hot&&add_over==best_add_over&&future_fg==best_fg&&other_future==best_other&&future_over<best_future_over)||
                       (local_peak==best_local&&hot_future==best_hot&&add_over==best_add_over&&future_fg==best_fg&&other_future==best_other&&future_over==best_future_over&&future_sq<best_sq)||
                       (local_peak==best_local&&hot_future==best_hot&&add_over==best_add_over&&future_fg==best_fg&&other_future==best_other&&future_over==best_future_over&&future_sq==best_sq&&press<best_press)||
                       (local_peak==best_local&&hot_future==best_hot&&add_over==best_add_over&&future_fg==best_fg&&other_future==best_other&&future_over==best_future_over&&future_sq==best_sq&&press==best_press&&stay<best_stay)){
                        best_pk=pk;
            ...
            if(better_metrics(cand_eval,best_eval)){
                best_eval=cand_eval;
                improved=1;
                memcpy(fp_out, out_load, g_l*sizeof(out_load[0]));
                memcpy(fp_in, in_load, g_l*sizeof(in_load[0]));
                memcpy(fp_port, fl_port, fl_count*sizeof(short));
            }
        }
    }

    if(improved){
        memcpy(out_load, fp_out, g_l*sizeof(out_load[0]));
        memcpy(in_load, fp_in, g_l*sizeof(in_load[0]));
        memcpy(fl_port, fp_port, fl_count*sizeof(short));
    } else {
        memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
        memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
        memcpy(fl_port, bk_port, fl_count*sizeof(short));
...
void solve_job(){
    ...
    if(g_p>=32&&g_r>=4&&fl_count<=7000){
        ...
        // Dedicated batch future repair on top of the stronger GP seed.
        cur_uses_global_price=1;
        run_greedy_global_price(m, 1, 0, 1, 2, 1);
        run_swap(m);
        if(get_job_max(m)<=g_r) run_p32_future_port_rebuild(m);
        TRY_STRATEGY();
