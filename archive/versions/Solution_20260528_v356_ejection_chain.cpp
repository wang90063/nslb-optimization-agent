...
void run_ejection_chain(int m){
    static short ej_max_out[MAX_LEAFS][MAX_PORTS];
    static short ej_max_in[MAX_LEAFS][MAX_PORTS];
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            short mo=0,mi=0;
            for(int ph=0;ph<m;++ph){
            ...
            ej_max_out[leaf][pk]=mo;ej_max_in[leaf][pk]=mi;
        }
    memset(cpm,0,sizeof(cpm));memset(cppc,0,sizeof(cppc));
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
    ...
    // Build per-(leaf,port,phase) flow index
    static int lpp_flows[MAX_LEAFS][MAX_PORTS][MAX_PHASES][8];
    static int lpp_cnt[MAX_LEAFS][MAX_PORTS][MAX_PHASES];
    memset(lpp_cnt,0,sizeof(lpp_cnt));
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
        int sl=fl_sl[i],p=fl_port[i];
        unsigned int mk=fl_pmask[i];
        while(mk){int ph=__builtin_ctz(mk);
            if(lpp_cnt[sl][p][ph]<8) lpp_flows[sl][p][ph][lpp_cnt[sl][p][ph]]=i;
            lpp_cnt[sl][p][ph]++;
            mk&=mk-1;}
    }
    int moved=0;
    for(int fi=0;fi<fl_count&&moved<200;++fi){
        int sl=fl_sl[fi],dl=fl_dl[fi],px=fl_port[fi],ci=fl_src[fi];
        if(sl==dl||px<0) continue;
        if(card_cbtphsc(ci,m)==0) continue;
        // Find majority port for this card
        int port_cnt[32]={};
        for(int j=0;j<fl_count;++j)
            if(fl_src[j]==ci&&fl_sl[j]!=fl_dl[j]&&fl_port[j]>=0) port_cnt[fl_port[j]]++;
        int mp=-1,mp_cnt=0;
        for(int pk=0;pk<g_p;++pk) if(port_cnt[pk]>mp_cnt){mp_cnt=port_cnt[pk];mp=pk;}
        if(mp<0||px==mp) continue;
        // Check if direct move to mp is blocked
        unsigned int mask_f=fl_pmask[fi];
        int blocked_ph=-1;
        unsigned int m2=mask_f;
        while(m2){int ph=__builtin_ctz(m2);
            if(out_load[sl][mp][ph]>=ej_max_out[sl][mp]||
               in_load[dl][mp][ph]>=ej_max_in[dl][mp]){blocked_ph=ph;break;}
            m2&=m2-1;}
        if(blocked_ph<0) continue; // not blocked, PC should handle
        // Find blocker on (sl, mp, blocked_ph)
        if(lpp_cnt[sl][mp][blocked_ph]<1||lpp_cnt[sl][mp][blocked_ph]>8) continue;
        int best_gi=-1,best_q=-1,best_net=0;
        for(int gi_idx=0;gi_idx<lpp_cnt[sl][mp][blocked_ph]&&gi_idx<8;++gi_idx){
            int gi=lpp_flows[sl][mp][blocked_ph][gi_idx];
            if(gi==fi) continue;
            int dl_g=fl_dl[gi],ci_g=fl_src[gi];
            unsigned int mask_g=fl_pmask[gi];
            for(int q=0;q<g_p;++q){
                if(q==mp) continue;
                // Check feasibility of moving g from mp to q
                unsigned int m3=mask_g;int ok=1;
                while(m3){int ph=__builtin_ctz(m3);
                    if(out_load[sl][q][ph]+1>ej_max_out[sl][q]){ok=0;break;}
                    if(in_load[dl_g][q][ph]+1>ej_max_in[dl_g][q]){ok=0;break;}
                    m3&=m3-1;}
                if(!ok) continue;
                // Check if after moving g, f can move to mp
                m3=mask_f;ok=1;
                while(m3){int ph2=__builtin_ctz(m3);
                    int new_out=out_load[sl][mp][ph2]-((mask_g>>ph2)&1)+1;
                    if(new_out>ej_max_out[sl][mp]){ok=0;break;}
                    if(in_load[dl][mp][ph2]+1>ej_max_in[dl][mp]){ok=0;break;}
                    m3&=m3-1;}
                if(!ok) continue;
                // Compute net CB change
                int before_cb=card_cbtphsc(ci,m)+card_cbtphsc(ci_g,m);
                // Simulate move g: mp->q
                unsigned int mk=mask_g;
                while(mk){int ph=__builtin_ctz(mk);
                    cppc[ci_g][ph][mp]--;if(!cppc[ci_g][ph][mp])cpm[ci_g][ph]&=~(1u<<mp);
                    cppc[ci_g][ph][q]++;cpm[ci_g][ph]|=(1u<<q);mk&=mk-1;}
                // Simulate move f: px->mp
                mk=mask_f;
                while(mk){int ph=__builtin_ctz(mk);
                    cppc[ci][ph][px]--;if(!cppc[ci][ph][px])cpm[ci][ph]&=~(1u<<px);
                    cppc[ci][ph][mp]++;cpm[ci][ph]|=(1u<<mp);mk&=mk-1;}
                int after_cb=card_cbtphsc(ci,m)+card_cbtphsc(ci_g,m);
                int net=before_cb-after_cb;
                // Undo simulations
                mk=mask_f;
                while(mk){int ph=__builtin_ctz(mk);
                    cppc[ci][ph][mp]--;if(!cppc[ci][ph][mp])cpm[ci][ph]&=~(1u<<mp);
                    cppc[ci][ph][px]++;cpm[ci][ph]|=(1u<<px);mk&=mk-1;}
                mk=mask_g;
                while(mk){int ph=__builtin_ctz(mk);
                    cppc[ci_g][ph][q]--;if(!cppc[ci_g][ph][q])cpm[ci_g][ph]&=~(1u<<q);
                    cppc[ci_g][ph][mp]++;cpm[ci_g][ph]|=(1u<<mp);mk&=mk-1;}
                if(net>best_net){best_net=net;best_gi=gi;best_q=q;}
            }
        }
        if(best_gi<0||best_net<=0) continue;
        // Execute: move g from mp to best_q, move f from px to mp
        int gi=best_gi,q=best_q;
        int dl_g=fl_dl[gi],ci_g=fl_src[gi];
        unsigned int mask_g=fl_pmask[gi];
        unsigned int mk=mask_g;
        while(mk){int ph=__builtin_ctz(mk);
            out_load[sl][mp][ph]--;out_load[sl][q][ph]++;
            in_load[dl_g][mp][ph]--;in_load[dl_g][q][ph]++;
            cppc[ci_g][ph][mp]--;if(!cppc[ci_g][ph][mp])cpm[ci_g][ph]&=~(1u<<mp);
            cppc[ci_g][ph][q]++;cpm[ci_g][ph]|=(1u<<q);mk&=mk-1;}
        fl_port[gi]=(short)q;
        mk=mask_f;
        while(mk){int ph=__builtin_ctz(mk);
            out_load[sl][px][ph]--;out_load[sl][mp][ph]++;
            in_load[dl][px][ph]--;in_load[dl][mp][ph]++;
            cppc[ci][ph][px]--;if(!cppc[ci][ph][px])cpm[ci][ph]&=~(1u<<px);
            cppc[ci][ph][mp]++;cpm[ci][ph]|=(1u<<mp);mk&=mk-1;}
        fl_port[fi]=(short)mp;
        moved++;
...
void solve_job(){
    ...
    // Ejection chain: make room on target port to consolidate cards
    if(!g_time_tight){
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        EvalMetrics ej_base=collect_metrics(m);
        run_ejection_chain(m);
        EvalMetrics ej_after=collect_metrics(m);
        if(ej_after.fg>ej_base.fg||(ej_after.fg==ej_base.fg&&ej_after.jm>ej_base.jm)){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));
        }
