...
#define HIST_HT_SIZE (1<<20)
#define HIST_HT_MASK (HIST_HT_SIZE-1)
static int hist_port_key[HIST_HT_SIZE];
static signed char hist_port_val[HIST_HT_SIZE];
inline void hist_init(){memset(hist_port_key,-1,sizeof(hist_port_key));}
inline int hist_get(int k){unsigned h=(unsigned)k;h=((h>>16)^h)*0x45d9f3b;h=((h>>16)^h)*0x45d9f3b;h=(h>>16)^h;int p=h&HIST_HT_MASK;while(1){if(hist_port_key[p]==k)return hist_port_val[p];if(hist_port_key[p]==-1)return -1;p=(p+1)&HIST_HT_MASK;}}
inline void hist_set(int k,int v){unsigned h=(unsigned)k;h=((h>>16)^h)*0x45d9f3b;h=((h>>16)^h)*0x45d9f3b;h=(h>>16)^h;int p=h&HIST_HT_MASK;while(hist_port_key[p]!=-1&&hist_port_key[p]!=k)p=(p+1)&HIST_HT_MASK;hist_port_key[p]=k;hist_port_val[p]=(signed char)v;}
...
void run_task_consistency(int m){
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]) continue;
        int cp=fl_port[i]; if(cp<0) continue;
        int key=fl_src[i]*MAX_CARDS+fl_dst[i];
        int hp=hist_get(key);
        if(hp<0||hp==cp) continue;
        int sl=fl_sl[i],dl=fl_dl[i];
        unsigned int mask=fl_pmask[i];
        int cur_max_out=0,cur_max_in=0;
        for(int ph=0;ph<m;++ph){
            if(out_load[sl][hp][ph]>cur_max_out)cur_max_out=out_load[sl][hp][ph];
            if(in_load[dl][hp][ph]>cur_max_in)cur_max_in=in_load[dl][hp][ph];
        }
        int ok=1;
        unsigned int m2=mask;
        while(m2){
            int ph=__builtin_ctz(m2);
            if(out_load[sl][hp][ph]+1>cur_max_out){ok=0;break;}
            if(in_load[dl][hp][ph]+1>cur_max_in){ok=0;break;}
            m2&=m2-1;
        }
        if(!ok) continue;
        m2=mask;
        while(m2){int ph=__builtin_ctz(m2);out_load[sl][cp][ph]--;in_load[dl][cp][ph]--;out_load[sl][hp][ph]++;in_load[dl][hp][ph]++;m2&=m2-1;}
        fl_port[i]=(short)hp;
    }
}
...
void solve_job(){
    ...
    // Update port history for task consistency
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
        hist_set(fl_src[i]*MAX_CARDS+fl_dst[i], fl_port[i]);
    }
