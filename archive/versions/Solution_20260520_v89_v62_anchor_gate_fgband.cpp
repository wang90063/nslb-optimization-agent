...
static short legacy_out[MAX_LEAFS][MAX_PORTS][MAX_PHASES];
static short legacy_in[MAX_LEAFS][MAX_PORTS][MAX_PHASES];
static short legacy_port[MAX_FLOWS];
...
inline int better_legacy_metrics(const EvalMetrics &a,const EvalMetrics &b){
    int solved_cap=g_r>g_hist_max_jm?g_r:g_hist_max_jm;
    int a_solved=a.jm<=solved_cap;
    int b_solved=b.jm<=solved_cap;
...
inline int should_use_proxy_metrics(const EvalMetrics &proxy_eval,int proxy_flags,
                                    const EvalMetrics &legacy_eval,int huge_job){
    if(better_legacy_metrics(proxy_eval,legacy_eval)) return 1;
    if(huge_job||g_p>16) return 0;
    if(proxy_eval.score_jm!=legacy_eval.score_jm) return 0;
    if(proxy_eval.score_fg!=legacy_eval.score_fg) return 0;
    if(proxy_eval.jm>legacy_eval.jm) return 0;
    if(proxy_eval.ci>legacy_eval.ci) return 0;
    if(proxy_flags&CAND_LOCALFIRST){
        if(proxy_eval.ci<legacy_eval.ci) return 1;
        if(proxy_eval.future_over<legacy_eval.future_over) return 1;
        if(proxy_eval.future_over!=legacy_eval.future_over) return 0;
        return proxy_eval.future_sq<legacy_eval.future_sq;
    }
    if(legacy_eval.fg<g_r+4) return 0;
    if(proxy_eval.ci<legacy_eval.ci) return 1;
    if(proxy_eval.future_over<legacy_eval.future_over) return 1;
    if(proxy_eval.future_over!=legacy_eval.future_over) return 0;
    return proxy_eval.future_sq<legacy_eval.future_sq;
...
void solve_job(){
    ...
    // Track both the v62 proxy-aware winner and a conservative legacy winner.
    EvalMetrics best_eval,best_legacy_eval;
    int first=1,legacy_first=1;
    int best_flags=0;
    int any_jm_le_r=0;

    #define TRY_STRATEGY(flags) do { \
        ...
        if(legacy_first||better_legacy_metrics(cur_eval,best_legacy_eval)){ \
            best_legacy_eval=cur_eval; legacy_first=0; \
            memcpy(legacy_out, out_load, g_l*sizeof(out_load[0])); \
            memcpy(legacy_in, in_load, g_l*sizeof(in_load[0])); \
            memcpy(legacy_port, fl_port, fl_count*sizeof(short)); \
        } \
    } while(0)
    ...
    // Default to the conservative winner, and only let the proxy-aware path
    // override it when it either wins on the legacy objective or when a
    // local-first candidate gives a strictly safer equal-fg refinement.
    if(should_use_proxy_metrics(best_eval,best_flags,best_legacy_eval,huge_job)){
        ...
        memcpy(out_load, legacy_out, g_l*sizeof(out_load[0]));
        memcpy(in_load, legacy_in, g_l*sizeof(in_load[0]));
        memcpy(fl_port, legacy_port, fl_count*sizeof(short));
    }
