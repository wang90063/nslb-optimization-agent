...
// CB-aware greedy: adds port affinity for card's adjacent-phase ports
void run_greedy_cb(int m, int hardcap){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    // Track card-phase-port usage
    static unsigned int card_ph_ports[MAX_CARDS][MAX_PHASES];
    memset(card_ph_ports,0,sizeof(card_ph_ports));
    // Sort flows by card then by earliest phase
    static int cb_order[MAX_FLOWS];
    for(int i=0;i<fl_count;++i) cb_order[i]=i;
    // Simple card-grouped sort (stable within card by original order)
    static int card_start[MAX_CARDS+1];
    memset(card_start,0,sizeof(card_start));
    for(int i=0;i<fl_count;++i) card_start[fl_src[i]+1]++;
    for(int i=1;i<=g_l*g_p*g_r;++i) card_start[i]+=card_start[i-1];
    static int cb_tmp[MAX_FLOWS];
    for(int i=0;i<fl_count;++i) cb_tmp[card_start[fl_src[i]]++]=i;
    memcpy(cb_order,cb_tmp,fl_count*sizeof(int));
    for(int ii=0;ii<fl_count;++ii){
        int i=cb_order[ii];
        int sl=fl_sl[i],dl=fl_dl[i];
        if(sl==dl){fl_port[i]=-1;continue;}
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            int go=global_out[sl][pk],gi=global_in[dl][pk];
            int gv=go>gi?go:gi;
            int cost=local_max*2+gv*1;
            if(exceeds) cost+=10000;
            // CB bonus: reward ports used by this card in adjacent phases
            int cb_bonus=0;
            m2=mask;
            while(m2){int ph=__builtin_ctz(m2);
                if(ph>0&&(card_ph_ports[card][ph-1]&(1u<<pk))) cb_bonus++;
                if(ph<m-1&&(card_ph_ports[card][ph+1]&(1u<<pk))) cb_bonus++;
                if(card_ph_ports[card][ph]&(1u<<pk)) cb_bonus+=2;
                m2&=m2-1;}
            cost-=cb_bonus;
            if(cost<bc){bc=cost;bp=pk;}
        }
...
void solve_job(){
    ...
    // S19: CB-aware greedy (card-grouped, phase affinity) + swap + global swap + lns
    run_greedy_cb(m, 1);
    run_swap(m);
    run_global_swap(m);
    run_lns(m);
