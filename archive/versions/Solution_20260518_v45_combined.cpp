...
// ban_leaf/ban_port/ban_cnt: banned (leaf,port) pairs get extra penalty
static int ban_leaf[4], ban_port_arr[4], ban_cnt;
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            for(int b=0;b<ban_cnt;++b)
                if((sl==ban_leaf[b]||dl==ban_leaf[b])&&pk==ban_port_arr[b])
                    cost+=50000;
...
void solve_job(){
    ...
    // S7: re-greedy with banned bottleneck port + swap
    {
        run_greedy(m, 2, 1, 1);
        int jm=get_job_max(m);
        if(jm>g_r){
            // Find bottleneck and ban it
            for(int leaf=0;leaf<g_l;++leaf)
                for(int pk=0;pk<g_p;++pk)
                    for(int ph=0;ph<m;++ph)
                        if(out_load[leaf][pk][ph]>=jm||in_load[leaf][pk][ph]>=jm){
                            ban_leaf[0]=leaf; ban_port_arr[0]=pk; ban_cnt=1;
                            goto found_bn;
                        }
            found_bn:
            if(ban_cnt>0){
