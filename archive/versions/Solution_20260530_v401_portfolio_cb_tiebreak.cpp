...
inline int total_cbtphsc_from_ports(const short *ports, int m){
...
void solve_job(){
    ...
    int first=1;
    int any_jm_le_r=0;
    int p32_cb_tiebreak=(g_p>=32&&g_r>=4&&fl_count<=7000);
    int best_total_cbt=-1;
    ...
    #define TRY_STRATEGY() do { \
        ...
        if(first||better_metrics(cur_eval,best_eval)){ \
            ...
            cur_total_cbt=total_cbtphsc_from_ports(fl_port,m); \
            if(best_total_cbt<0) best_total_cbt=total_cbtphsc_from_ports(sv_port,m); \
            if(cur_total_cbt<best_total_cbt) take_strategy=1; \
        } \
        if(take_strategy){ \
            best_eval=cur_eval; first=0; \
            best_total_cbt=(cur_total_cbt>=0)?cur_total_cbt:-1; \
            memcpy(sv_out, out_load, g_l*sizeof(out_load[0])); \
            memcpy(sv_in, in_load, g_l*sizeof(in_load[0])); \
            memcpy(sv_port, fl_port, fl_count*sizeof(short)); \
        } \
