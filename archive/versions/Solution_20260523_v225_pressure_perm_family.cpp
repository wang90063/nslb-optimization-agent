...
void run_greedy_fgmax(int m, int hardcap, int rev=0){
    ...
    int use_port_perm=(rev<=-10);
    int order_mode=use_port_perm?rev+10:rev;
    if(order_mode>=2){
        unsigned int seed=order_mode*2654435761u;
    ...
    for(int ii=0;ii<fl_count;++ii){
        int i;
        if(order_mode==0) i=ii;
        else if(order_mode==1) i=fl_count-1-ii;
        else i=fl_order[ii];
        int sl=fl_sl[i],dl=fl_dl[i];
        ...
        if(use_port_perm)
            base_seed=((((unsigned)sl*1315423911u)^((unsigned)dl*2654435761u)^
                        ((unsigned)g_job_idx*2246822519u)^(mask*374761393u))%((unsigned)g_p));
        for(int pk_idx=0;pk_idx<g_p;++pk_idx){
            int pk=use_port_perm?(int)((base_seed+(unsigned)pk_idx)%((unsigned)g_p)):pk_idx;
...
void solve_job(){
    ...
    if(allow_pressure_order){
        prepare_flow_order_pressure(m);

        run_greedy_ftrl(m, 1, -11, 2);
        run_swap(m);
        run_global_swap(m);
        run_lns(m);
        TRY_STRATEGY();

        run_greedy_fgmax(m, 1, -11);
        run_swap(m);
        run_global_swap(m);
        run_lns(m);
        TRY_STRATEGY();
