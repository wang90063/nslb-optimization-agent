...
static int fl_count;
// Two-pass forecast buffering (compact: short src/dst, reduced size)
#define MAX_TOTAL_FLOWS 520000
static short buf_src[MAX_TOTAL_FLOWS], buf_dst[MAX_TOTAL_FLOWS];
static unsigned int buf_pmask[MAX_TOTAL_FLOWS];
static short buf_port[MAX_TOTAL_FLOWS];
static int job_offset[40], job_flcount[40], job_m[40];
static int buf_total_flows;
static int forecast_out[40][MAX_LEAFS][MAX_PORTS];
static int forecast_in[40][MAX_LEAFS][MAX_PORTS];
static int forecast_total_out[MAX_LEAFS][MAX_PORTS];
static int forecast_total_in[MAX_LEAFS][MAX_PORTS];
static int g_pass; // 0=forecast pass, 1=full pass
...
static double g_forecast_elapsed;
#ifdef __APPLE__
#include <mach/mach_time.h>
static uint64_t g_start_time;
static double g_timebase;
static inline void init_timer(){mach_timebase_info_data_t tb;mach_timebase_info(&tb);g_timebase=(double)tb.numer/tb.denom/1e9;g_start_time=mach_absolute_time();}
static inline double elapsed_sec(){return (double)(mach_absolute_time()-g_start_time)*g_timebase;}
#else
#include <sys/time.h>
static struct timeval g_start_tv;
static inline void init_timer(){gettimeofday(&g_start_tv,0);}
static inline double elapsed_sec(){struct timeval tv;gettimeofday(&tv,0);return (tv.tv_sec-g_start_tv.tv_sec)+(tv.tv_usec-g_start_tv.tv_usec)*1e-6;}
#endif
...
void run_sa_composite(int m){
    ...
    double sa_budget=(g_forecast_elapsed>0.01)?0.013:0.017;
    if(fl_count>5000) sa_budget=(g_forecast_elapsed>0.01)?0.007:0.01;
...
void read_job_to_buffer(int job_idx){
    int m=fast_read_int(),f=fast_read_int();
    job_m[job_idx]=m;
    int off=buf_total_flows;
    job_offset[job_idx]=off;
    fl_count=0; ht_clear();
    for(int ph=0;ph<m;++ph)
        for(int i=0;i<f;++i){
            ...
            if(!(seen_bits[by]&(1<<bi))){
                seen_bits[by]|=(1<<bi);
                buf_src[off+fl_count]=src;buf_dst[off+fl_count]=dst;
                buf_pmask[off+fl_count]=(1u<<ph);
                ht_insert(hi,fl_count);
                fl_count++;
            } else {
                int fi=ht_find(hi);
                buf_pmask[off+fi]|=(1u<<ph);
            }
        }
    for(int i=0;i<ht_used_cnt;++i){int hi=ht_key[ht_used[i]];seen_bits[hi>>3]&=~(1<<(hi&7));}
    job_flcount[job_idx]=fl_count;
    buf_total_flows+=fl_count;
}

void solve_job(){
    int m=job_m[g_job_idx];
    int off=job_offset[g_job_idx];
    fl_count=job_flcount[g_job_idx];
    for(int i=0;i<fl_count;++i){
        fl_src[i]=buf_src[off+i];fl_dst[i]=buf_dst[off+i];
        fl_pmask[i]=buf_pmask[off+i];
    ...
    int time_tight = 0;
    if(g_job_idx>0){
        double el=elapsed_sec()-g_forecast_elapsed;
        double proj=el/g_job_idx*g_n;
        if(proj>4.5) time_tight=1;
        if(el>3.0&&fl_count>20000) time_tight=1;
    }
    // Hard time limit: if total elapsed > 4.0s, force time_tight
    if(elapsed_sec()>4.0) time_tight=1;
    ...
    // Buffer output
    int ooff=job_offset[g_job_idx];
    for(int i=0;i<fl_count;++i) buf_port[ooff+i]=fl_port[i];
}

void forecast_job(int ji){
    int m=job_m[ji];
    int off=job_offset[ji];
    fl_count=job_flcount[ji];
    for(int i=0;i<fl_count;++i){
        fl_src[i]=buf_src[off+i];fl_dst[i]=buf_dst[off+i];
        fl_pmask[i]=buf_pmask[off+i];
        fl_sl[i]=fl_src[i]/g_pr;fl_dl[i]=fl_dst[i]/g_pr;
        fl_order[i]=i;
    }
    // Run lightweight greedy (S2: hardcap, no swap) for forecast
    run_greedy(m, 2, 1, 1);
    // Record max_phase_load per (leaf, port) as forecast
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            int mo=0,mi=0;
            for(int ph=0;ph<m;++ph){
            ...
            forecast_out[ji][leaf][pk]=mo;
            forecast_in[ji][leaf][pk]=mi;
            forecast_total_out[leaf][pk]+=mo;
            forecast_total_in[leaf][pk]+=mi;
            global_out[leaf][pk]+=mo;
            global_in[leaf][pk]+=mi;
        }
}
...
int main(){
    ...
    // Read all jobs
    buf_total_flows=0;
    for(int i=0;i<n;++i) read_job_to_buffer(i);
    // Two-pass forecast only beneficial for larger cases with manageable size
    if(n>=30 && buf_total_flows<150000){
        // Pass 1: forecast (greedy only)
        memset(global_out,0,sizeof(global_out));
        memset(global_in,0,sizeof(global_in));
        memset(forecast_total_out,0,sizeof(forecast_total_out));
        memset(forecast_total_in,0,sizeof(forecast_total_in));
        for(int i=0;i<n;++i) forecast_job(i);
        g_forecast_elapsed=elapsed_sec();
        // Pass 2: full solve with forecast-informed global state
        memcpy(global_out, forecast_total_out, sizeof(global_out));
        memcpy(global_in, forecast_total_in, sizeof(global_in));
        g_hist_max_jm=0;
        for(int i=0;i<n;++i){
            for(int leaf=0;leaf<g_l;++leaf)
                for(int pk=0;pk<g_p;++pk){
                    global_out[leaf][pk]-=forecast_out[i][leaf][pk];
                    global_in[leaf][pk]-=forecast_in[i][leaf][pk];
                }
            g_job_idx=i;
            solve_job();
        }
    } else {
        // Small cases: solve normally without forecast
        memset(global_out,0,sizeof(global_out));
        memset(global_in,0,sizeof(global_in));
    ...
    for(int i=0;i<n;++i){
        int off=job_offset[i];
        int fc=job_flcount[i];
        fast_write(fc);write_char('\n');
        for(int j=0;j<fc;++j){
            fast_write(buf_src[off+j]);write_char(' ');
            fast_write(buf_dst[off+j]);write_char(' ');
            fast_write(buf_port[off+j]);
            if(j!=fc-1)write_char(' ');
        }
        write_char('\n');
    }
