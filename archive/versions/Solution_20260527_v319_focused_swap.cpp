...
void run_sa_swap(int m){
    ...
    // Build focused list: flows from cards with CB > 0
    static int swap_focus[MAX_FLOWS];
    int swap_focus_cnt=0;
    {
        static int card_cb_done2[MAX_CARDS];
        static int card_cb_val2[MAX_CARDS];
        for(int i=0;i<fl_count;++i){
            if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
            int c=fl_src[i];
            if(card_cb_done2[c]!=g_job_idx+1){
                card_cb_done2[c]=g_job_idx+1;
                card_cb_val2[c]=card_cbtphsc(c,m);
            }
            if(card_cb_val2[c]>0) swap_focus[swap_focus_cnt++]=i;
        }
    }
    double T=2.0;
    double cool=0.99995;
    ...
    for(int iter=0;;++iter){
        ...
        rng=rng*1664525u+1013904223u;
        int fi;
        if(swap_focus_cnt>0) fi=swap_focus[rng%swap_focus_cnt];
        else fi=rng%fl_count;
        if(fl_sl[fi]==fl_dl[fi]||fl_port[fi]<0){T*=cool;continue;}
...
void solve_job(){
    ...
    // SA swap pass: 2-opt neighborhood (skip if running late)
    double swap_el=elapsed_sec();
    int swap_ok=(!g_time_tight && swap_el<3.5);
    if(swap_ok && g_job_idx>0){
        double swap_proj=swap_el/g_job_idx*g_n;
        if(swap_proj>3.8) swap_ok=0;
    }
    if(swap_ok){
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
