...
void solve_job(){
    ...
    // Pass 3: Cbtphsc reduction - move flows to card's dominant port if max doesn't increase
    ...
    for(int i=0;i<fl_count;++i){
        ...
        // Find the most common port for this source card
        // (scan other flows from same card)
        int port_count[MAX_PORTS]={};
        for(int j=0;j<fl_count;++j){
            if(fl_src[j]==src_card && fl_sl[j]!=fl_dl[j] && fl_port[j]>=0)
                port_count[fl_port[j]]++;
        }
        int dom_port=cp, dom_cnt=0;
        for(int pk=0;pk<g_p;++pk)
            if(port_count[pk]>dom_cnt){dom_cnt=port_count[pk];dom_port=pk;}
        if(dom_port==cp) continue;
        // Try moving to dominant port - check if max doesn't increase
        unsigned int mask=fl_pmask[i];
        int ok=1;
        unsigned int m2=mask;
        while(m2){
            int ph=__builtin_ctz(m2);
            if(out_load[sl][dom_port][ph]+1>job_max){ok=0;break;}
            if(in_load[dl][dom_port][ph]+1>job_max){ok=0;break;}
            m2&=m2-1;
        }
        if(ok){
            m2=mask;
