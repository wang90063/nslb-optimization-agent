...
void solve_job(){
    ...
    // Pre-PC SA: run SA before PC chain to explore from post-greedy state
    if(!g_time_tight){
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        EvalMetrics prepc_base=collect_metrics(m);
        run_sa_composite(m);
        EvalMetrics prepc_after=collect_metrics(m);
        if(prepc_after.fg>prepc_base.fg||(prepc_after.fg==prepc_base.fg&&prepc_after.jm>prepc_base.jm)){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));
        }
