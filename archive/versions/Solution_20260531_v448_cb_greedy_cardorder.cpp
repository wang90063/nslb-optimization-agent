...
void run_greedy_cb_aware(int m, int local_w, int global_w, int cb_w, int hardcap, int rev=0){
    ...
    if(rev==10||rev==11){
        // Card-sorted order: group flows by card for maximum consolidation
        for(int i=0;i<fl_count;++i) fl_order[i]=i;
        // Simple insertion sort by card (stable)
        for(int i=1;i<fl_count;++i){
            int key=fl_order[i];
            int kc=fl_src[key];
            int j=i-1;
            while(j>=0&&fl_src[fl_order[j]]>kc){
                fl_order[j+1]=fl_order[j];j--;}
            fl_order[j+1]=key;
        }
        if(rev==11){
            for(int a=0,b=fl_count-1;a<b;a++,b--){
                int t=fl_order[a];fl_order[a]=fl_order[b];fl_order[b]=t;}
        }
    } else if(rev>=2){
        unsigned int seed=rev*2654435761u;
...
void solve_job(){
    ...
    // Card-sorted order: flows grouped by card for max consolidation
    run_greedy_cb_aware(m, 2, 1, 4, 1, 10);
    run_swap(m);
    run_jm_repair(m);
    TRY_STRATEGY();

    run_greedy_cb_aware(m, 2, 1, 6, 1, 10);
    run_swap(m);
    run_jm_repair(m);
    TRY_STRATEGY();

    run_greedy_cb_aware(m, 1, 1, 5, 1, 10);
    run_swap(m);
    run_jm_repair(m);
    TRY_STRATEGY();

    run_greedy_cb_aware(m, 2, 1, 4, 1, 11);
    run_swap(m);
    run_jm_repair(m);
    TRY_STRATEGY();
