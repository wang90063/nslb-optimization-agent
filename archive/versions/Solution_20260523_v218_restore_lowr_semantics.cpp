...
void solve_job(){
    ...
    if(enable_makeroom){
        ...
        if(total_cbt_before>0){
            for(int mr_iter=0;mr_iter<3;++mr_iter){
                ...
                for(int fi=0;fi<fl_count;++fi){
                    ...
                    if(card_cbtphsc(ci,m)==0) continue;
                    // Keep the historical low-r behavior on `r<=3`, and only
                    // widen to top-2 dominant ports inside the narrow r=4 gate.
                    int dp_cnt[32]={};
                    for(int ph=0;ph<m;++ph)
                    ...
                    int target_cap=enable_r4_makeroom?2:1;
                    for(int pk=0;pk<g_p;++pk){
                        int score=dp_cnt[pk];
                    ...
                    for(int dpk=0;dpk<target_cap;++dpk){
                        ...
                        if(!blocked){
                            if(!enable_r4_makeroom) continue;
        ...
        int revert=0;
        if(enable_r4_makeroom){
            if(mr_after.fg>mr_base.fg||(mr_after.fg==mr_base.fg&&mr_after.jm>mr_base.jm))
                revert=1;
        }else{
            if(!better_metrics(mr_after,mr_base)&&
               (mr_after.fg!=mr_base.fg||mr_after.jm!=mr_base.jm||mr_after.ci!=mr_base.ci))
                revert=1;
        }
        if(revert){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
