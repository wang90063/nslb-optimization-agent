        ...
        if(total_cbt_before>0){
            for(int mr_iter=0;mr_iter<3;++mr_iter){
                ...
                for(int fi=0;fi<fl_count;++fi){
                    ...
                    long long best_pressure=(1LL<<60);
                    for(int dpk=0;dpk<2;++dpk){
                        ...
                        if(!blocked){
                            ...
                            long long pressure=(long long)global_out[sl][dp]-global_out[sl][cp]
                                              +(long long)global_in[dl][dp]-global_in[dl][cp];
                            ...
                            if(g_r<=3&&pressure>0&&net<=1) continue;
                            if(net>best_net||(net==best_net&&pressure<best_pressure)){
                                best_net=net;
                                best_pressure=pressure;
                                best_bfi=-1;
                                best_bap=-1;
                        ...
                        for(int bfi=0;bfi<fl_count;++bfi){
                            ...
                            for(int ap=0;ap<g_p;++ap){
                                ...
                                long long pressure=(long long)global_out[sl][dp]-global_out[sl][cp]
                                                  +(long long)global_in[dl][dp]-global_in[dl][cp]
                                                  +(long long)global_out[bsl][ap]-global_out[bsl][dp]
                                                  +(long long)global_in[bdl][ap]-global_in[bdl][dp];
                                if(g_r<=3&&pressure>0&&net<=1) continue;
                                if(net>best_net||(net==best_net&&pressure<best_pressure)){
                                    best_net=net;
                                    best_pressure=pressure;
