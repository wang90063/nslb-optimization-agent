...
static inline int r4_hotleaf_flow_priority(int fi,int m,const int *port_vals){
    int cp=fl_port[fi];
    int press=0;
    if(cp>=0&&fl_sl[fi]!=fl_dl[fi])
        press=global_out[fl_sl[fi]][cp]+global_in[fl_dl[fi]][cp];
    return port_vals[cp]*256+card_cbtphsc(fl_src[fi],m)*64+
           press*8+__builtin_popcount(fl_pmask[fi])*16;
}
...
__attribute__((noinline,cold)) void run_r4_hotleaf_card_rebuild(int m){
    if(g_r!=4||g_p<16) return;
    if(g_time_tight||fl_count>5000||(long long)fl_count*m>80000) return;

    StageAcceptEval base_eval=collect_stage_accept_eval(m);
    if(base_eval.em.fg<=g_r||base_eval.em.future_over<=0) return;

    memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
    memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
    memcpy(bk_port, fl_port, fl_count*sizeof(short));

    int best_dir=-1,best_leaf=-1,best_top=-1,best_sum3=-1;
    int best_ports[32];
    memset(best_ports,0,sizeof(best_ports));
    for(int dir=0;dir<2;++dir){
        for(int leaf=0;leaf<g_l;++leaf){
            ...
            int sum3=top1+top2+top3;
            if(top1>best_top||(top1==best_top&&sum3>best_sum3)){
                best_top=top1;
                best_sum3=sum3;
                best_dir=dir;
                best_leaf=leaf;
                memcpy(best_ports,vals,g_p*sizeof(int));
            }
        }
    }
    if(best_dir<0||best_top<base_eval.em.fg) return;

    StageAcceptEval best_eval=base_eval;
    int improved=0;

    for(int use_ports=1;use_ports<=2;++use_ports){
        unsigned char hot_used[32]={};
        for(int take=0;take<use_ports;++take){
            int best_pk=-1;
            for(int pk=0;pk<g_p;++pk){
                if(hot_used[pk]) continue;
                if(best_pk<0||best_ports[pk]>best_ports[best_pk]) best_pk=pk;
            }
            if(best_pk>=0) hot_used[best_pk]=1;
        }
        ...
        for(int i=0;i<fl_count;++i){
            if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
            if(best_dir==0&&fl_sl[i]!=best_leaf) continue;
            if(best_dir==1&&fl_dl[i]!=best_leaf) continue;
            if(!hot_used[fl_port[i]]) continue;
            int c=fl_src[i];
            int score=r4_hotleaf_flow_priority(i,m,best_ports);
            cell_core_score[i]=score;
            if(!core_card_seen[c]){
                core_card_seen[c]=1;
                core_card_score[c]=0;
                core_cards[cand_card_cnt++]=c;
            }
            core_card_score[c]+=score;
        }
        if(cand_card_cnt<=0) continue;

        for(int i=0;i<cand_card_cnt;++i){
            int best=i;
            for(int j=i+1;j<cand_card_cnt;++j)
                if(core_card_score[core_cards[j]]>core_card_score[core_cards[best]])
                    best=j;
            if(best!=i){
                int t=core_cards[i];
        ...
        for(int i=0;i<cand_card_cnt;++i) core_card_seen[core_cards[i]]=0;

        int max_cards=6;
        if(cand_card_cnt<max_cards) max_cards=cand_card_cnt;
        int total_sel=0;
        for(int ci=0;ci<max_cards;++ci){
            int src_card=core_cards[ci];
            for(int i=0;i<fl_count;++i){
                if(fl_src[i]!=src_card||fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
                if(best_dir==0&&fl_sl[i]!=best_leaf) continue;
                if(best_dir==1&&fl_dl[i]!=best_leaf) continue;
                if(!hot_used[fl_port[i]]) continue;
                if(!cell_core_seen[i]){
                    cell_core_seen[i]=1;
                    pc_flows[total_sel++]=i;
                }
            }
        }
        if(total_sel<=1){
            for(int i=0;i<total_sel;++i) cell_core_seen[pc_flows[i]]=0;
            continue;
        }

        for(int i=0;i<total_sel;++i){
            int best=i;
            for(int j=i+1;j<total_sel;++j)
                if(cell_core_score[pc_flows[j]]>cell_core_score[pc_flows[best]])
                    best=j;
            if(best!=i){
                int t=pc_flows[i];
        ...
        int flow_cap=(use_ports==1)?48:72;
        int sel_cnt=total_sel;
        if(sel_cnt>flow_cap) sel_cnt=flow_cap;
        ...
        for(int mode=0;mode<3;++mode){
            ...
            for(int i=0;i<sel_cnt;++i){
                int best=i;
                for(int j=i+1;j<sel_cnt;++j){
                    int fi=pc_flows[j],fb=pc_flows[best];
                    int sj,sb;
                    if(mode==0){
                        sj=r4_hotleaf_flow_priority(fi,m,best_ports);
                        sb=r4_hotleaf_flow_priority(fb,m,best_ports);
                    } else if(mode==1){
                        sj=card_cbtphsc(fl_src[fi],m)*256+best_ports[fl_port[fi]]*64+
                           __builtin_popcount(fl_pmask[fi])*32;
                        sb=card_cbtphsc(fl_src[fb],m)*256+best_ports[fl_port[fb]]*64+
                           __builtin_popcount(fl_pmask[fb])*32;
                    } else {
                        sj=(global_out[fl_sl[fi]][fl_port[fi]]+global_in[fl_dl[fi]][fl_port[fi]])*64+
                           best_ports[fl_port[fi]]*128+__builtin_popcount(fl_pmask[fi])*32;
                        sb=(global_out[fl_sl[fb]][fl_port[fb]]+global_in[fl_dl[fb]][fl_port[fb]])*64+
                           best_ports[fl_port[fb]]*128+__builtin_popcount(fl_pmask[fb])*32;
                    }
                    if(sj>sb) best=j;
                }
                if(best!=i){
            ...
            for(int i=0;i<sel_cnt;++i){
                ...
                while(mk){
                    ...
                    in_load[dl][cp][ph]--;
                    cppc[ci][ph][cp]--;
                    if(!cppc[ci][ph][cp]) cpm[ci][ph]&=~(1u<<cp);
                    mk&=mk-1;
                }
            }
            ...
            for(int i=0;i<sel_cnt;++i){
                int fi=pc_flows[i];
                int sl=fl_sl[fi],dl=fl_dl[fi],cur_pk=bk_port[fi],ci=fl_src[fi];
                unsigned int mask=fl_pmask[fi];

                int best_pk=cur_pk;
                int best_hot=0x7fffffff,best_over=0x7fffffff,best_local=0x7fffffff;
                int best_cbt=0x7fffffff,best_support=-0x7fffffff;
                int best_other=0x7fffffff,best_press=0x7fffffff,best_change=0x7fffffff;

                for(int pk=0;pk<g_p;++pk){
                    int mo=0,mi=0,local_peak=0,future_over=0,support=0;
                    unsigned int tmp_masks[MAX_PHASES];
                    memcpy(tmp_masks,cpm[ci],m*sizeof(unsigned int));
                    unsigned int mk=mask;
                    while(mk){
                        int ph=__builtin_ctz(mk);
                        int o=out_load[sl][pk][ph]+1;
                        ...
                        if(o>local_peak) local_peak=o;
                        if(iv>local_peak) local_peak=iv;
                        if(out_load[sl][pk][ph]>=g_r) future_over++;
                        if(in_load[dl][pk][ph]>=g_r) future_over++;
                        if(cpm[ci][ph]&(1u<<pk)) support++;
                        tmp_masks[ph]|=(1u<<pk);
                        mk&=mk-1;
                    }
                    ...
                    int fo=global_out[sl][pk]+mo;
                    int fi_fg=global_in[dl][pk]+mi;
                    int hot_future=(best_dir==0)?fo:fi_fg;
                    int other_future=(best_dir==0)?fi_fg:fo;
                    int total_over=0;
                    if(fo>g_r) total_over+=fo-g_r;
                    if(fi_fg>g_r) total_over+=fi_fg-g_r;
                    int after_cbt=calc_phase_mask_cbt(tmp_masks,m);
                    int press=global_out[sl][pk]+global_in[dl][pk];
                    int change=(pk!=cur_pk);
                    ...
                    int better=0;
                    if(mode==0){
                        if(hot_future<best_hot) better=1;
                        else if(hot_future==best_hot&&total_over<best_over) better=1;
                        else if(hot_future==best_hot&&total_over==best_over&&after_cbt<best_cbt) better=1;
                        else if(hot_future==best_hot&&total_over==best_over&&after_cbt==best_cbt&&local_peak<best_local) better=1;
                    } else if(mode==1){
                        if(after_cbt<best_cbt) better=1;
                        else if(after_cbt==best_cbt&&total_over<best_over) better=1;
                        else if(after_cbt==best_cbt&&total_over==best_over&&hot_future<best_hot) better=1;
                        else if(after_cbt==best_cbt&&total_over==best_over&&hot_future==best_hot&&local_peak<best_local) better=1;
                    } else {
                        if(local_peak<best_local) better=1;
                        else if(local_peak==best_local&&hot_future<best_hot) better=1;
                        else if(local_peak==best_local&&hot_future==best_hot&&total_over<best_over) better=1;
                        else if(local_peak==best_local&&hot_future==best_hot&&total_over==best_over&&after_cbt<best_cbt) better=1;
                    }
                    if(!better&&hot_future==best_hot&&total_over==best_over&&after_cbt==best_cbt&&local_peak==best_local){
                        if(support>best_support) better=1;
                        else if(support==best_support&&other_future<best_other) better=1;
                        else if(support==best_support&&other_future==best_other&&press<best_press) better=1;
                        else if(support==best_support&&other_future==best_other&&press==best_press&&change<best_change) better=1;
                    }
                    if(better){
                        best_pk=pk;
                        best_hot=hot_future;
                ...
                while(mk){
                    ...
                    in_load[dl][best_pk][ph]++;
                    cppc[ci][ph][best_pk]++;
                    cpm[ci][ph]|=(1u<<best_pk);
                    mk&=mk-1;
                }
            }

            StageAcceptEval cand_eval=collect_stage_accept_eval(m);
            if(better_stage_accept_eval(cand_eval,best_eval)){
                best_eval=cand_eval;
                improved=1;
                memcpy(sv_out, out_load, g_l*sizeof(out_load[0]));
                memcpy(sv_in, in_load, g_l*sizeof(in_load[0]));
