...
void run_sa_composite(int m){
    ...
    // Identify fg bottleneck: only protect the top (leaf,port) pair
    int fg_bn_leaf=-1,fg_bn_port=-1,fg_bn_val=0;
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            int vo=global_out[leaf][pk]+sa_max_out[leaf][pk];
            int vi=global_in[leaf][pk]+sa_max_in[leaf][pk];
            int v=vo>vi?vo:vi;
            if(v>fg_bn_val){fg_bn_val=v;fg_bn_leaf=leaf;fg_bn_port=pk;}
        }
    memset(cpm,0,sizeof(cpm));memset(cppc,0,sizeof(cppc));
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
    ...
    for(int iter=0;;++iter){
        ...
        unsigned int m2=mask; int ok=1;
        while(m2){int ph=__builtin_ctz(m2);
            int out_lim=((sl==fg_bn_leaf&&py==fg_bn_port)?sa_max_out[sl][py]:g_r);
            int in_lim=((dl==fg_bn_leaf&&py==fg_bn_port)?sa_max_in[dl][py]:g_r);
            if(out_load[sl][py][ph]+1>out_lim){ok=0;break;}
            if(in_load[dl][py][ph]+1>in_lim){ok=0;break;}
            m2&=m2-1;}
