...
static int g_maxjm; // max job_max seen so far across all jobs
...
// Score-aware: only penalizes jm when it would set a new Maxsingler record
inline long long eval_strategy(int m){
    int jm=get_job_max(m);
    int fg=get_future_gmax(m);
    int ci=get_cinphsc(m);
    // Effective Maxsingler contribution: max(jm, g_maxjm) determines final Maxsingler
    int eff_jm = jm > g_maxjm ? jm : g_maxjm;
    // Score terms (higher is better, so negate for minimization):
    // 40*r / max(eff_jm, r) + 40*r / max(fg, r) - 12*ci/total_flows
    // Multiply by 10000 to use integers
    int d1 = eff_jm > g_r ? eff_jm : g_r;
    int d2 = fg > g_r ? fg : g_r;
    // Minimize: -40*r/d1 - 40*r/d2 + ci_penalty
    // Equivalent to minimize: -(400000*r/d1) - (400000*r/d2) + ci*12
    long long score = -(long long)(400000*g_r)/d1 - (long long)(400000*g_r)/d2 + (long long)ci*12;
    return score;
}
...
void solve_job(){
    ...
    // Update global state
    int jm_final=get_job_max(m);
    if(jm_final>g_maxjm) g_maxjm=jm_final;
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            int mo=0,mi=0;
