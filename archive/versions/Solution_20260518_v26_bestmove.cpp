...
void solve_job(){
    ...
    // PASS2_PLACEHOLDER
    // Pass 2: best-move bottleneck swap with global tiebreaker
    for(int iter=0;iter<40;++iter){
        ...
        if(mx<=1) break;
        // scan ALL flows on bottleneck, pick best (flow, port) pair
        int best_i=-1,best_pk=-1,best_max=mx,best_gv=0x7fffffff;
        for(int i=0;i<fl_count;++i){
            int sl=fl_sl[i],dl=fl_dl[i];
            if(sl==dl) continue;
            int cp=fl_port[i];
            ...
            for(int pk=0;pk<g_p;++pk){
                ...
                if(nm<best_max||(nm==best_max&&nm<mx)){
                    int go=global_out[sl][pk],gi=global_in[dl][pk];
                    int gv=go>gi?go:gi;
                    if(nm<best_max||(nm==best_max&&gv<best_gv)){
                        best_max=nm;best_i=i;best_pk=pk;best_gv=gv;
                    }
                }
            }
        }
        if(best_i<0) break;
        int sl=fl_sl[best_i],dl=fl_dl[best_i];
        int cp=fl_port[best_i];
        unsigned int mask=fl_pmask[best_i];
        unsigned int m2=mask;
