...
void run_cross_dest_swap(int m){
    ...
    for(int iter=0;iter<5;++iter){
        ...
        for(int fi=0;fi<fl_count;++fi){
            ...
            int tgt[2]={-1,-1};int tv[2]={0,0};
            for(int pk=0;pk<g_p;++pk){if(pk==px)continue;
                if(dp_cnt[pk]>tv[0]){tgt[1]=tgt[0];tv[1]=tv[0];tgt[0]=pk;tv[0]=dp_cnt[pk];}
                else if(dp_cnt[pk]>tv[1]){tgt[1]=pk;tv[1]=dp_cnt[pk];}}
            if(tgt[0]<0) continue;
            unsigned int mask_a=fl_pmask[fi];
            int best_wt=0,best_j=-1,best_py=-1;
            for(int ti=0;ti<2;++ti){
            int py=tgt[ti];if(py<0)continue;
            ...
            for(int bi=bstart;bi<bend;++bi){
                ...
                if(wt>best_wt){best_wt=wt;best_j=j;best_py=py;}
            }
            }// end ti loop
