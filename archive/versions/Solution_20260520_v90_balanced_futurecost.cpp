...
// Future-cost greedy: price the port by the future testcase-level fg it creates,
// instead of the current global load only. This is meant for moderate p<=16
// regimes where jm is often already near floor and small fg moves matter.
void run_greedy_futurecost(int m, int local_w, int global_w, int hardcap, int rev=0){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    if(rev>=2){
        unsigned int seed=rev*2654435761u;
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            int fi=global_in[dl][pk]+mi;
            int fgv=fo>fi?fo:fi;
            int cost=local_max*local_w+fgv*global_w;
            if(exceeds) cost+=10000;
            int cand_over=0;
...
void solve_job(){
    ...
    // Moderate balanced candidate: use future fg directly in the greedy cost
    // so p<=16 cases can improve testcase-level fg without extra selector gates.
    if(g_p<=16){
        run_greedy_futurecost(m, 2, 1, 1);
        run_swap(m);
        TRY_STRATEGY();

        run_greedy_futurecost(m, 2, 1, 1, 1);
        run_swap(m);
        TRY_STRATEGY();
    }
