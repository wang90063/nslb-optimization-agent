...
void run_sa_composite(int m){
    ...
    for(int iter=0;;++iter){
        ...
        // CT heuristic: bonus for moving away from high-CT ports
        int ct_delta=0;
        int ct_src_out=global_out[sl][px]+sa_max_out[sl][px];
        int ct_src_in=global_in[dl][px]+sa_max_in[dl][px];
        int ct_dst_out=global_out[sl][py]+sa_max_out[sl][py];
        int ct_dst_in=global_in[dl][py]+sa_max_in[dl][py];
        if(ct_src_out>g_pr&&ct_dst_out<=g_pr) ct_delta--;
        if(ct_src_in>g_pr&&ct_dst_in<=g_pr) ct_delta--;
        if(ct_dst_out>g_pr&&ct_src_out<=g_pr) ct_delta++;
        if(ct_dst_in>g_pr&&ct_src_in<=g_pr) ct_delta++;
