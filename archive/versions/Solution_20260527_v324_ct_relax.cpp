...
void run_ct_relax_post(int m){
    if(g_r<=2) return;
    static short ct_mo[MAX_LEAFS][MAX_PORTS];
    static short ct_mi[MAX_LEAFS][MAX_PORTS];
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            short mo=0,mi=0;
            for(int ph=0;ph<m;++ph){
            ...
            ct_mo[leaf][pk]=mo;ct_mi[leaf][pk]=mi;
        }
    for(int iter=0;iter<8;++iter){
        ...
        for(int fi=0;fi<fl_count;++fi){
            ...
            if(sl==dl||px<0) continue;
            int cur_ct_sl=(global_out[sl][px]+ct_mo[sl][px]>g_r)?(global_out[sl][px]+ct_mo[sl][px]-g_r):0;
            int cur_ct_dl=(global_in[dl][px]+ct_mi[dl][px]>g_r)?(global_in[dl][px]+ct_mi[dl][px]-g_r):0;
            if(cur_ct_sl==0&&cur_ct_dl==0) continue;
            int at_max_o=0,at_max_i=0;
            unsigned int m2=fl_pmask[fi];
            while(m2){int ph=__builtin_ctz(m2);
                if(out_load[sl][px][ph]==ct_mo[sl][px])at_max_o=1;
                if(in_load[dl][px][ph]==ct_mi[dl][px])at_max_i=1;
                m2&=m2-1;}
            if(!at_max_o&&!at_max_i) continue;
            int new_mo_px=0,new_mi_px=0;
            if(at_max_o){for(int ph=0;ph<m;++ph){
                int v=out_load[sl][px][ph]-((fl_pmask[fi]>>ph)&1);
                if(v>new_mo_px)new_mo_px=v;}
            } else new_mo_px=ct_mo[sl][px];
            if(at_max_i){for(int ph=0;ph<m;++ph){
                int v=in_load[dl][px][ph]-((fl_pmask[fi]>>ph)&1);
                if(v>new_mi_px)new_mi_px=v;}
            } else new_mi_px=ct_mi[dl][px];
            if(new_mo_px==ct_mo[sl][px]&&new_mi_px==ct_mi[dl][px]) continue;
            int lmo=0,lmi=0;
            for(int pk=0;pk<g_p;++pk){
                if(ct_mo[sl][pk]>lmo)lmo=ct_mo[sl][pk];
                if(ct_mi[dl][pk]>lmi)lmi=ct_mi[dl][pk];
            }
            int src_ct_reduce=(cur_ct_sl-((global_out[sl][px]+new_mo_px>g_r)?(global_out[sl][px]+new_mo_px-g_r):0))
                             +(cur_ct_dl-((global_in[dl][px]+new_mi_px>g_r)?(global_in[dl][px]+new_mi_px-g_r):0));
            int best_py=-1,best_wt=0;
            for(int py=0;py<g_p;++py){
                if(py==px) continue;
                unsigned int m3=fl_pmask[fi];
                int nmo_py=ct_mo[sl][py],nmi_py=ct_mi[dl][py];
                while(m3){int ph=__builtin_ctz(m3);
                    int nol=out_load[sl][py][ph]+1;
                    int nil=in_load[dl][py][ph]+1;
                    if(nol>nmo_py)nmo_py=nol;
                    if(nil>nmi_py)nmi_py=nil;
                    m3&=m3-1;}
                if(nmo_py>lmo||nmi_py>lmi) continue;
                int old_ct_py=(global_out[sl][py]+ct_mo[sl][py]>g_r)?(global_out[sl][py]+ct_mo[sl][py]-g_r):0;
                int old_ct_py_d=(global_in[dl][py]+ct_mi[dl][py]>g_r)?(global_in[dl][py]+ct_mi[dl][py]-g_r):0;
                int new_ct_py=(global_out[sl][py]+nmo_py>g_r)?(global_out[sl][py]+nmo_py-g_r):0;
                int new_ct_py_d=(global_in[dl][py]+nmi_py>g_r)?(global_in[dl][py]+nmi_py-g_r):0;
                int dest_inc=(new_ct_py-old_ct_py)+(new_ct_py_d-old_ct_py_d);
                int ct_net=src_ct_reduce-dest_inc;
                if(ct_net<=0) continue;
                int ov_delta=0;
                m3=fl_pmask[fi];
                while(m3){int ph=__builtin_ctz(m3);
                    if(out_load[sl][py][ph]>=g_r)ov_delta++;
            ...
            while(mk){int ph=__builtin_ctz(mk);
                ...
                in_load[dl][px][ph]--;in_load[dl][py][ph]++;mk&=mk-1;}
            fl_port[fi]=(short)py;
            int nmo_px2=0,nmi_px2=0,nmo_py2=0,nmi_py2=0;
            for(int ph=0;ph<m;++ph){
                if(out_load[sl][px][ph]>nmo_px2)nmo_px2=out_load[sl][px][ph];
                if(in_load[dl][px][ph]>nmi_px2)nmi_px2=in_load[dl][px][ph];
                if(out_load[sl][py][ph]>nmo_py2)nmo_py2=out_load[sl][py][ph];
                if(in_load[dl][py][ph]>nmi_py2)nmi_py2=in_load[dl][py][ph];
            }
            ct_mo[sl][px]=(short)nmo_px2;ct_mi[dl][px]=(short)nmi_px2;
            ct_mo[sl][py]=(short)nmo_py2;ct_mi[dl][py]=(short)nmi_py2;
            improved++;}
        }
        if(!improved) break;
    }
...
void solve_job(){
    ...
    // Post-SA relaxed CT reduce with MM guard
    {
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        ...
        if(ct2_after.ci>ct2_base.ci||ct2_after.fg>ct2_base.fg||ct2_after.jm>ct2_base.jm){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));
        }
