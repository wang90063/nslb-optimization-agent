...
static int cba_order[MAX_FLOWS];
static int cba_key[MAX_FLOWS];
int cba_cmp(const void*a,const void*b){return cba_key[*(const int*)a]-cba_key[*(const int*)b];}

void run_greedy_cb_aware(int m, int hardcap){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    static short card_ph_port[12800][32];
    memset(card_ph_port,-1,12800*32*sizeof(short));
    for(int i=0;i<fl_count;++i){
        cba_order[i]=i;
        int mp=fl_sl[i]==fl_dl[i]?9999:__builtin_ctz(fl_pmask[i]);
        cba_key[i]=fl_src[i]*32+mp;
    }
    qsort(cba_order,fl_count,sizeof(int),cba_cmp);
    for(int ii=0;ii<fl_count;++ii){
        int i=cba_order[ii];
        int sl=fl_sl[i],dl=fl_dl[i];
        if(sl==dl){fl_port[i]=-1;continue;}
        ...
        int adj_port=-1;
        if(min_ph>0&&card_ph_port[card][min_ph-1]>=0)
            adj_port=card_ph_port[card][min_ph-1];
        if(adj_port<0&&max_ph<m-1&&card_ph_port[card][max_ph+1]>=0)
            adj_port=card_ph_port[card][max_ph+1];
        int bp=0,bc=0x7fffffff;
        for(int pk=0;pk<g_p;++pk){
            int local_max=0,exceeds=0;
            int mo=0,mi=0;
            ...
            int cost=local_max+fgv*3;
            if(exceeds) cost+=10000;
            if(adj_port>=0&&pk==adj_port) cost-=1;
            if(cost<bc){bc=cost;bp=pk;}
        }
        fl_port[i]=(short)bp;
        unsigned int m2=mask;
        while(m2){int ph=__builtin_ctz(m2);out_load[sl][bp][ph]++;in_load[dl][bp][ph]++;card_ph_port[card][ph]=bp;m2&=m2-1;}
    }
}
...
void solve_job(){
    ...
    // CB-aware greedy: sort by card/phase, prefer adjacent-phase port
    run_greedy_cb_aware(m, 1);
    run_swap(m);
    run_jm_repair(m);
    TRY_STRATEGY();
