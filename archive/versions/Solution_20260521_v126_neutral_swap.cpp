...
// Swapping equivalent flows doesn't change any load — only card-port associations change
static int ns_order[MAX_FLOWS];
...
void run_neutral_swap(int m){
    static int card_pc[MAX_CARDS][MAX_PORTS];
    memset(card_pc, 0, sizeof(card_pc));
    ns_valid_cnt=0;
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
        card_pc[fl_src[i]][fl_port[i]]++;
            ...
            if(gsz>=2&&gsz<=200){
                for(int a=g_start;a<g_end;++a){
                    ...
                    for(int b=a+1;b<g_end;++b){
                        ...
                        int before=card_pc[ci][pi]+card_pc[cj][pj];
                        int after=card_pc[ci][pj]+1+card_pc[cj][pi]+1;
                        if(after>before){
                            fl_port[i]=(short)pj;fl_port[j]=(short)pi;
                            card_pc[ci][pi]--;card_pc[ci][pj]++;
                            card_pc[cj][pj]--;card_pc[cj][pi]++;
                            pi=pj; improved++;
                        }
                    }
                }
