...
void run_cross_dest_swap(int m){
    ...
    for(int iter=0;iter<2;++iter){
        ...
        for(int fi=0;fi<fl_count;++fi){
            ...
            int ct_delta=0;
            if(g_r>=3){
                int cur_fo_sl=global_out[sl][px]+cd_max_out[sl][px];
                int cur_fi_dl=global_in[dl][px]+cd_max_in[dl][px];
                int cur_ct_sl_px=(cur_fo_sl>g_r)?(cur_fo_sl-g_r):0;
                int cur_ct_dl_px=(cur_fi_dl>g_r)?(cur_fi_dl-g_r):0;
                int new_max_out_px=0,new_max_in_px=0;
                int new_max_out_py=cd_max_out[sl][py],new_max_in_py=cd_max_in[dl][py];
                for(int ph=0;ph<m;++ph){
                    int ob=((mask>>ph)&1u);
                    int o=out_load[sl][px][ph]-ob;
                    if(o>new_max_out_px)new_max_out_px=o;
                    int iv=in_load[dl][px][ph]-ob;
                    if(iv>new_max_in_px)new_max_in_px=iv;
                    int o2=out_load[sl][py][ph]+ob;
                    if(o2>new_max_out_py)new_max_out_py=o2;
                    int iv2=in_load[dl][py][ph]+ob;
                    if(iv2>new_max_in_py)new_max_in_py=iv2;
                }
                int new_fo_sl_px=global_out[sl][px]+new_max_out_px;
                int new_fi_dl_px=global_in[dl][px]+new_max_in_px;
                int new_ct_sl_px=(new_fo_sl_px>g_r)?(new_fo_sl_px-g_r):0;
                int new_ct_dl_px=(new_fi_dl_px>g_r)?(new_fi_dl_px-g_r):0;
                int cur_fo_sl_py=global_out[sl][py]+cd_max_out[sl][py];
                int cur_fi_dl_py=global_in[dl][py]+cd_max_in[dl][py];
                int cur_ct_sl_py=(cur_fo_sl_py>g_r)?(cur_fo_sl_py-g_r):0;
                int cur_ct_dl_py=(cur_fi_dl_py>g_r)?(cur_fi_dl_py-g_r):0;
                int new_fo_sl_py=global_out[sl][py]+new_max_out_py;
                int new_fi_dl_py=global_in[dl][py]+new_max_in_py;
                int new_ct_sl_py=(new_fo_sl_py>g_r)?(new_fo_sl_py-g_r):0;
                int new_ct_dl_py=(new_fi_dl_py>g_r)?(new_fi_dl_py-g_r):0;
                ct_delta=(cur_ct_sl_px-new_ct_sl_px)+(cur_ct_dl_px-new_ct_dl_px)
                        +(cur_ct_sl_py-new_ct_sl_py)+(cur_ct_dl_py-new_ct_dl_py);
            }
            ...
            int net=before-after;
            int wt=5*net+3*ct_delta-12*ov_delta;
            if(wt>0){
                mk=mask;while(mk){int ph=__builtin_ctz(mk);
