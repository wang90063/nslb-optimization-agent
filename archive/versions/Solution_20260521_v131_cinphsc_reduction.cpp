...
void run_port_consistency(int m){
    for(int iter=0;iter<5;++iter){
        ...
        for(int i=0;i<fl_count;++i){
            ...
            if(cnt<=1) continue;
            // Try each port as consolidation target
            ...
            for(int k=0;k<cnt;++k){
                ...
                if(!ok) continue;
                m2=mask;
                while(m2){int ph=__builtin_ctz(m2);out_load[sl][cp][ph]--;in_load[dl][cp][ph]--;out_load[sl][best_tp][ph]++;in_load[dl][best_tp][ph]++;m2&=m2-1;}
...
void run_cinphsc_reduction(int m){
    // Target Cbttskc: reduce max_phase_load of ports with high cumulative load
    // by moving flows from peak phases to ports with headroom
    int cur_gmax=get_future_gmax(m);
    static short max_out[MAX_LEAFS][MAX_PORTS];
    static short max_in[MAX_LEAFS][MAX_PORTS];
    for(int leaf=0;leaf<g_l;++leaf)
    ...
    for(int iter=0;iter<3;++iter){
        ...
        for(int i=0;i<fl_count;++i){
            ...
            unsigned int mask=fl_pmask[i];
            // Check if this flow is on a peak phase of its port
            int on_peak_out=0,on_peak_in=0;
            unsigned int m2=mask;
            while(m2){int ph=__builtin_ctz(m2);
                if(out_load[sl][cp][ph]==max_out[sl][cp]) on_peak_out=1;
                if(in_load[dl][cp][ph]==max_in[dl][cp]) on_peak_in=1;
                m2&=m2-1;}
            if(!on_peak_out&&!on_peak_in) continue;
            // Only worth it if cumulative is above r
            int cum_out=global_out[sl][cp]+max_out[sl][cp];
            int cum_in=global_in[dl][cp]+max_in[dl][cp];
            if(cum_out<=g_r&&cum_in<=g_r) continue;
            int best_tp=-1,best_score=0;
            for(int tp=0;tp<g_p;++tp){
                if(tp==cp) continue;
                int ok=1;
                m2=mask;
                while(m2){int ph=__builtin_ctz(m2);
                    if(out_load[sl][tp][ph]+1>max_out[sl][tp]||
                       in_load[dl][tp][ph]+1>max_in[dl][tp]){ok=0;break;}
                    m2&=m2-1;}
                if(!ok) continue;
                // Check global constraint
                int new_tp_max_out=max_out[sl][tp],new_tp_max_in=max_in[dl][tp];
                m2=mask;
                while(m2){int ph=__builtin_ctz(m2);
                    int no=out_load[sl][tp][ph]+1;
                    int ni=in_load[dl][tp][ph]+1;
                    if(no>new_tp_max_out) new_tp_max_out=no;
                    if(ni>new_tp_max_in) new_tp_max_in=ni;
                    m2&=m2-1;}
                int fv_out=global_out[sl][tp]+new_tp_max_out;
                int fv_in=global_in[dl][tp]+new_tp_max_in;
                if(fv_out>cur_gmax||fv_in>cur_gmax) continue;
                // Compute benefit: would source port's max decrease?
                int new_max_out_cp=0,new_max_in_cp=0;
                m2=mask;
                int all_ph_mask_bits=__builtin_popcount(mask);
                (void)all_ph_mask_bits;
                for(int ph2=0;ph2<m;++ph2){
                    int no=out_load[sl][cp][ph2]-((mask>>ph2)&1);
                    int ni=in_load[dl][cp][ph2]-((mask>>ph2)&1);
                    if(no>new_max_out_cp) new_max_out_cp=no;
                    if(ni>new_max_in_cp) new_max_in_cp=ni;
                }
                int delta_cum=0;
                if(cum_out>g_r) delta_cum+=(max_out[sl][cp]-new_max_out_cp);
                if(cum_in>g_r) delta_cum+=(max_in[dl][cp]-new_max_in_cp);
                if(delta_cum>best_score){best_score=delta_cum;best_tp=tp;}
            }
            if(best_tp<0||best_score<=0) continue;
            // Execute move
            m2=mask;
            while(m2){int ph=__builtin_ctz(m2);
                out_load[sl][cp][ph]--;in_load[dl][cp][ph]--;
                ...
                m2&=m2-1;}
            fl_port[i]=(short)best_tp;
            // Update max arrays
            short nmo=0,nmi=0;
            for(int ph=0;ph<m;++ph){
                if(out_load[sl][cp][ph]>nmo)nmo=out_load[sl][cp][ph];
                if(in_load[dl][cp][ph]>nmi)nmi=in_load[dl][cp][ph];
            }
            max_out[sl][cp]=nmo; max_in[dl][cp]=nmi;
            nmo=0;nmi=0;
            for(int ph=0;ph<m;++ph){
                if(out_load[sl][best_tp][ph]>nmo)nmo=out_load[sl][best_tp][ph];
                if(in_load[dl][best_tp][ph]>nmi)nmi=in_load[dl][best_tp][ph];
            }
            max_out[sl][best_tp]=nmo; max_in[dl][best_tp]=nmi;
            improved++;
        }
        if(!improved) break;
    }
...
__attribute__((noinline,cold)) void run_random_start_repair(int m){
    ...
    int n_restarts=(fl_count<=1000)?80:40;
    ...
    if(found){
        int new_gmax=get_future_gmax(m);
        ...
        memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
        memcpy(fl_port, bk_port, fl_count*sizeof(short));
        int old_gmax=get_future_gmax(m);
        if(new_gmax<=old_gmax){
...
void solve_job(){
    ...
    // Random-start min-conflicts for small-r tight cases
    if(g_r<=2&&g_p>=16&&fl_count<=6000&&get_job_max(m)>g_r) run_random_start_repair(m);

    run_port_consistency(m);
    run_neutral_swap(m);
    run_relaxed_swap(m);
    run_cinphsc_reduction(m);
