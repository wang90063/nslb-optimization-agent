...
static inline void insert_final_hot_port(FinalHotPort *hots,int &cnt,int cap,
    ...
    for(int i=cnt-1;i>0&&hots[i].score>hots[i-1].score;--i){
        FinalHotPort t=hots[i];
        hots[i]=hots[i-1];
        hots[i-1]=t;
    }
}

// Final-stage coordinated repair: operate on the restored end state so the
// search is cheap and does not perturb the portfolio/timing path.
__attribute__((noinline,cold)) void run_final_coord_repair(int m){
    if(g_time_tight||g_p<8) return;
    long long job_work=(long long)fl_count*(long long)m;
    if(fl_count>6500||job_work>110000) return;

    EvalMetrics base_eval=collect_metrics(m);
    if(base_eval.future_over<=0) return;
    if(base_eval.jm>g_r+1) return;
    ...
    for(int round=0;round<2;++round){
        ...
        for(int leaf=0;leaf<g_l;++leaf)
            for(int pk=0;pk<g_p;++pk){
                ...
                if(fo>g_r){
                    ...
                    insert_final_hot_port(hot_ports,hot_cnt,6,leaf,pk,0,fo,mo,score);
                }
                if(fi>g_r){
                    int score=(fi-g_r)*8192+mi*128+(fi==best_eval.fg?2048:0);
                    insert_final_hot_port(hot_ports,hot_cnt,6,leaf,pk,1,fi,mi,score);
                }
            }
        if(hot_cnt<=0) break;

        memset(cell_core_seen,0,fl_count*sizeof(unsigned char));
        int flow_cnt=0;
        int use_hot=hot_cnt<2?hot_cnt:2;
        for(int hi=0;hi<use_hot;++hi){
            FinalHotPort hp=hot_ports[hi];
            for(int fi=0;fi<fl_count;++fi){
                if(fl_sl[fi]==fl_dl[fi]||fl_port[fi]!=hp.port||fl_port[fi]<0) continue;
                if((hp.dir==0&&fl_sl[fi]!=hp.leaf)||(hp.dir==1&&fl_dl[fi]!=hp.leaf)) continue;
                int score=card_cbtphsc(fl_src[fi],m)*512+__builtin_popcount(fl_pmask[fi])*64;
                int hit_hot=0;
                unsigned int mk=fl_pmask[fi];
                while(mk){
                    int ph=__builtin_ctz(mk);
                    int cur=(hp.dir==0)?out_load[hp.leaf][hp.port][ph]:in_load[hp.leaf][hp.port][ph];
                    if(cur==hp.local_max){score+=4096;hit_hot=1;}
                ...
                if(!cell_core_seen[fi]){
                    cell_core_seen[fi]=1;
                    pc_flows[flow_cnt++]=fi;
                    cell_core_score[fi]=score;
                } else if(score>cell_core_score[fi]){
                    cell_core_score[fi]=score;
                }
            }
        }
        ...
        EvalMetrics round_best=best_eval;
        int best_fi=-1,best_cp=-1,best_tp=-1;
        int best_bfi=-1,best_bcp=-1,best_bap=-1;

        for(int ii=0;ii<flow_cap;++ii){
            int fi=pc_flows[ii];
            int cp=fl_port[fi];
            if(cp<0) continue;
            int sl=fl_sl[fi],dl=fl_dl[fi],card=fl_src[fi];
            unsigned int mask=fl_pmask[fi];

            int dp_cnt[32]={};
            for(int ph=0;ph<m;++ph)
                for(int pk=0;pk<g_p;++pk)
                    dp_cnt[pk]+=cppc[card][ph][pk];

            int targ_ports[3],targ_scores[3];
            unsigned int targ_src_block[3],targ_dst_block[3];
            int targ_cnt=0;
            for(int pk=0;pk<g_p;++pk){
                if(pk==cp) continue;
                int local_peak=0;
                int mo=0,mi=0;
                unsigned int block_src=0,block_dst=0;
                unsigned int mk=mask;
                while(mk){
                    int ph=__builtin_ctz(mk);
                    int o=out_load[sl][pk][ph]+1;
                    ...
                    if(o>mo) mo=o;
                    if(iv>mi) mi=iv;
                    if(o>allow_jm) block_src|=(1u<<ph);
                    if(iv>allow_jm) block_dst|=(1u<<ph);
                    mk&=mk-1;
                }
                if(local_peak>allow_jm+1) continue;
                for(int ph=0;ph<m;++ph){
                ...
                if(future_fg>best_eval.fg+1) continue;
                int future_over=0;
                if(fo>g_r) future_over+=fo-g_r;
                if(fi_v>g_r) future_over+=fi_v-g_r;
                int score=__builtin_popcount(block_src|block_dst)*1000000+
                          future_fg*4096+future_over*256+joint_price_bias(sl,dl,pk)-
                          dp_cnt[pk]*128;
                if(targ_cnt<3){
                    targ_ports[targ_cnt]=pk;
                    targ_scores[targ_cnt]=score;
                    targ_src_block[targ_cnt]=block_src;
                    targ_dst_block[targ_cnt]=block_dst;
                    targ_cnt++;
                } else if(score<targ_scores[targ_cnt-1]){
                    targ_ports[targ_cnt-1]=pk;
                    targ_scores[targ_cnt-1]=score;
                    targ_src_block[targ_cnt-1]=block_src;
                    targ_dst_block[targ_cnt-1]=block_dst;
                } else continue;
                for(int i=targ_cnt-1;i>0&&targ_scores[i]<targ_scores[i-1];--i){
                    int tp=targ_ports[i];targ_ports[i]=targ_ports[i-1];targ_ports[i-1]=tp;
                    int ts=targ_scores[i];targ_scores[i]=targ_scores[i-1];targ_scores[i-1]=ts;
                    unsigned int tm=targ_src_block[i];targ_src_block[i]=targ_src_block[i-1];targ_src_block[i-1]=tm;
                    tm=targ_dst_block[i];targ_dst_block[i]=targ_dst_block[i-1];targ_dst_block[i-1]=tm;
                }
            }

            for(int ti=0;ti<targ_cnt;++ti){
                int tp=targ_ports[ti];
                unsigned int block_src=targ_src_block[ti];
                unsigned int block_dst=targ_dst_block[ti];

                if(!block_src&&!block_dst){
                    apply_flow_move(fi,cp,tp);
                    EvalMetrics cand_eval=collect_metrics(m);
                    apply_flow_move(fi,tp,cp);
                    if(better_metrics(cand_eval,round_best)){
                        round_best=cand_eval;
                        best_fi=fi; best_cp=cp; best_tp=tp;
                        best_bfi=-1; best_bcp=-1; best_bap=-1;
                    }
                    continue;
                }

                int blocker_idx[3],blocker_score[3],blocker_cnt=0;
                for(int bj=0;bj<fl_count;++bj){
                    if(bj==fi||fl_sl[bj]==fl_dl[bj]||fl_port[bj]!=tp||fl_port[bj]<0) continue;
                    unsigned int ov_src=(fl_sl[bj]==sl)?(fl_pmask[bj]&block_src):0u;
                    unsigned int ov_dst=(fl_dl[bj]==dl)?(fl_pmask[bj]&block_dst):0u;
                    int ov_hits=__builtin_popcount(ov_src|ov_dst);
                    if(!ov_hits) continue;
                    int score=ov_hits*4096+card_cbtphsc(fl_src[bj],m)*256-__builtin_popcount(fl_pmask[bj])*32;
                    if(blocker_cnt<3){
                        blocker_idx[blocker_cnt]=bj;
                        blocker_score[blocker_cnt]=score;
                        blocker_cnt++;
                    } else if(score>blocker_score[blocker_cnt-1]){
                        blocker_idx[blocker_cnt-1]=bj;
                        blocker_score[blocker_cnt-1]=score;
                    } else continue;
                    for(int i=blocker_cnt-1;i>0&&blocker_score[i]>blocker_score[i-1];--i){
                        int tj=blocker_idx[i];blocker_idx[i]=blocker_idx[i-1];blocker_idx[i-1]=tj;
                        int ts=blocker_score[i];blocker_score[i]=blocker_score[i-1];blocker_score[i-1]=ts;
                    }
                }
                ...
                for(int bi=0;bi<blocker_cnt;++bi){
                    ...
                    int bcp=tp;
                    int bsl=fl_sl[bfi],bdl=fl_dl[bfi],bcard=fl_src[bfi];
                    unsigned int bmask=fl_pmask[bfi];
                    int bdp_cnt[32]={};
                    for(int ph=0;ph<m;++ph)
                        for(int pk=0;pk<g_p;++pk)
                            bdp_cnt[pk]+=cppc[bcard][ph][pk];

                    int alt_ports[2],alt_scores[2],alt_cnt=0;
                    for(int ap=0;ap<g_p;++ap){
                        if(ap==bcp) continue;
                        int local_peak=0;
                        int mo=0,mi=0;
                        unsigned int mk=bmask;
                        while(mk){
                            int ph=__builtin_ctz(mk);
                            int o=out_load[bsl][ap][ph]+1;
                            int iv=in_load[bdl][ap][ph]+1;
                            if(o>local_peak) local_peak=o;
                            if(iv>local_peak) local_peak=iv;
                            if(o>mo) mo=o;
                            if(iv>mi) mi=iv;
                        ...
                        if(local_peak>allow_jm) continue;
                        for(int ph=0;ph<m;++ph){
                        ...
                        if(fo>g_r) future_over+=fo-g_r;
                        if(fi_v>g_r) future_over+=fi_v-g_r;
                        int score=future_fg*4096+future_over*256+joint_price_bias(bsl,bdl,ap)-bdp_cnt[ap]*128;
                        if(alt_cnt<2){
                            alt_ports[alt_cnt]=ap;
                            alt_scores[alt_cnt]=score;
                            alt_cnt++;
                        } else if(score<alt_scores[alt_cnt-1]){
                            alt_ports[alt_cnt-1]=ap;
                            alt_scores[alt_cnt-1]=score;
                        } else continue;
                        for(int i=alt_cnt-1;i>0&&alt_scores[i]<alt_scores[i-1];--i){
                            int tp2=alt_ports[i];alt_ports[i]=alt_ports[i-1];alt_ports[i-1]=tp2;
                            int ts=alt_scores[i];alt_scores[i]=alt_scores[i-1];alt_scores[i-1]=ts;
                        }
                    }

                    for(int ai=0;ai<alt_cnt;++ai){
                        int ap=alt_ports[ai];
                        ...
                        while(mk){
                            int ph=__builtin_ctz(mk);
                            if(out_load[sl][tp][ph]+1>allow_jm||in_load[dl][tp][ph]+1>allow_jm){
                                ok=0;
                                break;
                            }
                            mk&=mk-1;
                        ...
                        if(ok){
                            ...
                            if(better_metrics(cand_eval,round_best)){
                                round_best=cand_eval;
                                best_fi=fi; best_cp=cp; best_tp=tp;
                                best_bfi=bfi; best_bcp=bcp; best_bap=ap;
                            }
                        }
        ...
        if(best_fi<0) break;
        if(best_bfi>=0) apply_flow_move(best_bfi,best_bcp,best_bap);
        apply_flow_move(best_fi,best_cp,best_tp);
        best_eval=round_best;
        improved=1;
    }
