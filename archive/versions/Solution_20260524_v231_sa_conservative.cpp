...
void run_sa_composite(int m){
    ...
    double sa_budget=0.08;
    if(fl_count>5000) sa_budget=0.05;
...
void solve_job(){
    ...
    static short pre_sa_mo[MAX_LEAFS][MAX_PORTS];
    static short pre_sa_mi[MAX_LEAFS][MAX_PORTS];
    int sa_pass_ran=0;
    if(!g_time_tight){
        for(int leaf=0;leaf<g_l;++leaf)
            for(int pk=0;pk<g_p;++pk){
                ...
                pre_sa_mo[leaf][pk]=mo;pre_sa_mi[leaf][pk]=mi;
            }
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
    ...
    // Update global state conservatively after passes that may reduce current-job max_phase.
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            if(ct_pass_ran){
                ...
                global_out[leaf][pk]+=pre_sa_mo[leaf][pk];
                global_in[leaf][pk]+=pre_sa_mi[leaf][pk];
            } else {
                int mo=0,mi=0;
