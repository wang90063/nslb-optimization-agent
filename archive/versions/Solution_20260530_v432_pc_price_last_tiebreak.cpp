    ...
    for(int iter=0;iter<max_iter;++iter){
        ...
        for(int i=0;i<fl_count;++i){
            ...
            if(base_cbt>0&&cnt<=160){
                ...
                long long best_price=0x7fffffffffffffffLL;
                for(int tp=0;tp<g_p;++tp){
                    ...
                    for(int k=0;k<cnt;++k){
                        ...
                        price+=(long long)price_term(sl,tp)-price_term(sl,cp);
                        price+=(long long)price_term(dl,tp)-price_term(dl,cp);
                        m2=mask;
                        while(m2){
                            int ph=__builtin_ctz(m2);
                            tmp_ppc[ph][cp]--;
                    ...
                    if(after_cbt<best_cbt||
                       (after_cbt==best_cbt&&
                        (pressure<best_pressure||
                         (pressure==best_pressure&&
                          (can_move>best_total||
                           (can_move==best_total&&price<best_price)))))){
                        best_cbt=after_cbt;
