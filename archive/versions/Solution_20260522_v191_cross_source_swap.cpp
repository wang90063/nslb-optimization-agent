...
void run_cross_source_swap(int m){
    // Symmetric to cross_dest_swap: bucket by (dl, port), swap flows sharing same dest leaf
    static short cs_max_out[MAX_LEAFS][MAX_PORTS];
    static short cs_max_in[MAX_LEAFS][MAX_PORTS];
    for(int iter=0;iter<3;++iter){
        for(int leaf=0;leaf<g_l;++leaf)
            for(int pk=0;pk<g_p;++pk){
                short mo=0,mi=0;
                ...
                cs_max_out[leaf][pk]=mo;cs_max_in[leaf][pk]=mi;
            }
        memset(cpm,0,sizeof(cpm));memset(cppc,0,sizeof(cppc));
        for(int i=0;i<fl_count;++i){
            if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
        ...
        for(int i=0;i<fl_count;++i){
            if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
            cd_bucket_cnt[fl_dl[i]*g_p+fl_port[i]]++;
        }
        cd_off[0]=0;
        ...
        for(int i=0;i<fl_count;++i){
            if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
            int b=fl_dl[i]*g_p+fl_port[i];
            cd_idx[cd_off[b]+cd_bucket_cnt[b]]=i;
            cd_bucket_cnt[b]++;
        }
        int improved=0;
        for(int fi=0;fi<fl_count;++fi){
            int sl_a=fl_sl[fi],dl=fl_dl[fi],px=fl_port[fi],ci=fl_src[fi];
            if(sl_a==dl||px<0) continue;
            ...
            for(int bi=bstart;bi<bend;++bi){
                ...
                // Feasibility on dest leaf (shared): check in_load
                if(sl_a==sl_b){
                    m2=only_a;while(m2){int ph=__builtin_ctz(m2);
                        if(in_load[dl][py][ph]+1>cs_max_in[dl][py]){ok=0;break;}m2&=m2-1;}
                    if(!ok) continue;
                    m2=only_b;while(m2){int ph=__builtin_ctz(m2);
                        if(in_load[dl][px][ph]+1>cs_max_in[dl][px]){ok=0;break;}m2&=m2-1;}
                } else {
                ...
                if(!ok) continue;
                // Feasibility on source leaves
                if(sl_a==sl_b){
                    m2=only_a;while(m2){int ph=__builtin_ctz(m2);
                        if(out_load[sl_a][py][ph]+1>cs_max_out[sl_a][py]){ok=0;break;}m2&=m2-1;}
                    if(!ok) continue;
                    m2=only_b;while(m2){int ph=__builtin_ctz(m2);
                        if(out_load[sl_a][px][ph]+1>cs_max_out[sl_a][px]){ok=0;break;}m2&=m2-1;}
                } else {
                    m2=mask_a;while(m2){int ph=__builtin_ctz(m2);
                        if(out_load[sl_a][py][ph]+1>cs_max_out[sl_a][py]){ok=0;break;}m2&=m2-1;}
                    if(!ok) continue;
                    m2=mask_b;while(m2){int ph=__builtin_ctz(m2);
                        if(out_load[sl_b][px][ph]+1>cs_max_out[sl_b][px]){ok=0;break;}m2&=m2-1;}
                }
                if(!ok) continue;
                // Overflow delta
                int ov_delta=0;
                if(sl_a==sl_b){
                    m2=only_a;while(m2){int ph=__builtin_ctz(m2);
                        if(out_load[sl_a][py][ph]>=g_r)ov_delta++;
                        if(out_load[sl_a][px][ph]>g_r)ov_delta--;
                    ...
                    m2=mask_a;while(m2){int ph=__builtin_ctz(m2);
                        ...
                        m2&=m2-1;}
                    m2=mask_b;while(m2){int ph=__builtin_ctz(m2);
                        if(out_load[sl_b][px][ph]>=g_r)ov_delta++;
                        if(out_load[sl_b][py][ph]>g_r)ov_delta--;
                        if(in_load[dl][px][ph]>=g_r)ov_delta++;
                        if(in_load[dl][py][ph]>g_r)ov_delta--;
                        m2&=m2-1;}
                }
                // Evaluate Cbtphsc change
                int before=card_cbtphsc(ci,m)+card_cbtphsc(cj,m);
                unsigned int mk;
            ...
            {
                int j=best_j,cj=fl_src[j],sl_b=fl_sl[j];
                unsigned int mask_b=fl_pmask[j];
                unsigned int only_a=mask_a&~mask_b,only_b=mask_b&~mask_a;
                unsigned int mk,m2;
                mk=mask_a;while(mk){int ph=__builtin_ctz(mk);
                ...
                if(sl_a==sl_b){
                    m2=only_a;while(m2){int ph=__builtin_ctz(m2);
                        out_load[sl_a][px][ph]--;out_load[sl_a][py][ph]++;m2&=m2-1;}
                    m2=only_b;while(m2){int ph=__builtin_ctz(m2);
                        out_load[sl_a][py][ph]--;out_load[sl_a][px][ph]++;m2&=m2-1;}
                    m2=only_a;while(m2){int ph=__builtin_ctz(m2);
                        in_load[dl][px][ph]--;in_load[dl][py][ph]++;m2&=m2-1;}
                    m2=only_b;while(m2){int ph=__builtin_ctz(m2);
                        in_load[dl][py][ph]--;in_load[dl][px][ph]++;m2&=m2-1;}
                } else {
                    m2=mask_a;while(m2){int ph=__builtin_ctz(m2);
                        out_load[sl_a][px][ph]--;out_load[sl_a][py][ph]++;
                        in_load[dl][px][ph]--;in_load[dl][py][ph]++;m2&=m2-1;}
                    m2=mask_b;while(m2){int ph=__builtin_ctz(m2);
                        out_load[sl_b][py][ph]--;out_load[sl_b][px][ph]++;
                        in_load[dl][py][ph]--;in_load[dl][px][ph]++;m2&=m2-1;}
                }
