...
// For pairwise swap: group flows by leaf pair
#define MAX_LP_FLOWS 4096
static int lp_flows[MAX_LP_FLOWS];
static int lp_count;
...
void solve_job(){
    ...
    // Pass 2: bottleneck swap (same as v22)
    ...
    // Pass 3: pairwise swap for same-leaf-pair flows
    for(int piter=0;piter<10;++piter){
        ...
        for(int i=0;i<fl_count&&!improved;++i){
            ...
            if(sl==dl) continue;
            int cp=fl_port[i];
            unsigned int maskA=fl_pmask[i];
            // check if flow i is on bottleneck
            int on_bn=0;
            unsigned int m2=maskA;
            while(m2){
                int ph=__builtin_ctz(m2);
                if(out_load[sl][cp][ph]==mx||in_load[dl][cp][ph]==mx){on_bn=1;break;}
                m2&=m2-1;
            ...
            if(!on_bn) continue;
            // try swapping with another flow on a different port, same leaf pair
            for(int j=0;j<fl_count&&!improved;++j){
                if(j==i) continue;
                if(fl_sl[j]!=sl||fl_dl[j]!=dl) continue;
                int pp=fl_port[j];
                if(pp==cp) continue;
                unsigned int maskB=fl_pmask[j];
                // compute new max if we swap: A goes to pp, B goes to cp
                int new_max=0;
                for(int ph=0;ph<m;++ph){
                    int dA=((maskA>>ph)&1), dB=((maskB>>ph)&1);
                    // port cp: loses A, gains B
                    int oc=out_load[sl][cp][ph]-dA+dB;
                    int ic=in_load[dl][cp][ph]-dA+dB;
                    // port pp: loses B, gains A
                    int op=out_load[sl][pp][ph]-dB+dA;
                    int ip=in_load[dl][pp][ph]-dB+dA;
                    if(oc>new_max)new_max=oc;
                    if(ic>new_max)new_max=ic;
                    if(op>new_max)new_max=op;
                    if(ip>new_max)new_max=ip;
                }
                if(new_max<mx){
                    // apply swap
                    for(int ph=0;ph<m;++ph){
                        int dA=((maskA>>ph)&1), dB=((maskB>>ph)&1);
                        out_load[sl][cp][ph]+=dB-dA;
                        in_load[dl][cp][ph]+=dB-dA;
                        out_load[sl][pp][ph]+=dA-dB;
                        in_load[dl][pp][ph]+=dA-dB;
                    }
                    fl_port[i]=(short)pp;
                    fl_port[j]=(short)cp;
                    improved=1;
