...
// Strong global greedy for r<=2: cubic penalty on future gmax, no scale divisor
void run_greedy_r2_global(int m, int rev=0){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    if(rev>=2){
        unsigned int seed=rev*2654435761u;
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        if(sl==dl){fl_port[i]=-1;continue;}
        unsigned int mask=fl_pmask[i];
        int bp=0; long long bc=0x7fffffffffffffffLL;
        for(int pk=0;pk<g_p;++pk){
            int local_max=0,exceeds=0;
            int mo=0,mi=0;
            unsigned int m2=mask;
            ...
            int fi=global_in[dl][pk]+mi;
            int gv=fo>fi?fo:fi;
            long long cost=(long long)local_max + (long long)gv*gv*gv;
            if(exceeds) cost+=100000;
            if(cost<bc){bc=cost;bp=pk;}
        }
        fl_port[i]=(short)bp;
        unsigned int m2=mask;
...
// Relaxed global swap for r<=2: allows jm to increase to r+1 for gmax reduction
void run_global_swap_r2(int m){
    int pre_gmax=get_future_gmax(m);
    if(pre_gmax<=g_r) return;
    int jm_limit=g_r+1;
    memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        ...
        for(int i=0;i<fl_count&&!improved;++i){
            ...
            for(int pk=0;pk<g_p;++pk){
                ...
                while(m2){
                    ...
                    int no=out_load[sl][pk][ph]+1;
                    int ni=in_load[dl][pk][ph]+1;
                    if(no>jm_limit||ni>jm_limit){ok=0;break;}
                    m2&=m2-1;
                }
                if(!ok) continue;
                m2=mask;
...
void solve_job(){
    ...
    // R2-specific: strong global strategies when r<=2
    if(g_r<=2 && any_jm_le_r){
        // R2-S1: cubic global greedy + swap + global_swap + lns
        run_greedy_r2_global(m, 0);
        run_swap(m);
        run_global_swap(m);
        run_lns(m);
        TRY_STRATEGY();

        // R2-S2: cubic global greedy reversed + swap + global_swap + lns
        run_greedy_r2_global(m, 1);
        run_swap(m);
        run_global_swap(m);
        run_lns(m);
        TRY_STRATEGY();

        // R2-S3: cubic global greedy shuffled + swap + global_swap + lns
        run_greedy_r2_global(m, 2);
        run_swap(m);
        run_global_swap(m);
        run_lns(m);
        TRY_STRATEGY();

        // R2-S4: cubic global greedy shuffled seed 3 + swap + global_swap + lns
        run_greedy_r2_global(m, 3);
        run_swap(m);
        run_global_swap(m);
        run_lns(m);
        TRY_STRATEGY();
    ...
    // For r<=2: run relaxed global swap that allows jm=r+1 for gmax reduction
    if(g_r<=2 && !time_tight) run_global_swap_r2(m);
