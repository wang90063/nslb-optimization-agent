    ...
    for(int iter=0;iter<2;++iter){
        ...
        for(int fi=0;fi<fl_count;++fi){
            ...
            for(int pk=0;pk<g_p;++pk){
                ...
                while(m2){int ph=__builtin_ctz(m2);
                    cppc[ci][ph][px]--;if(!cppc[ci][ph][px])cpm[ci][ph]&=~(1u<<px);
                    cppc[ci][ph][pk]++;cpm[ci][ph]|=(1u<<pk);m2&=m2-1;}
                int new_cb=card_cbtphsc(ci,m);
                int cb_delta=base_cb-new_cb;
                m2=mask;
                while(m2){int ph=__builtin_ctz(m2);
                    cppc[ci][ph][pk]--;if(!cppc[ci][ph][pk])cpm[ci][ph]&=~(1u<<pk);
                    cppc[ci][ph][px]++;cpm[ci][ph]|=(1u<<px);m2&=m2-1;}
                if(cb_delta<=0) continue;
                int score=cb_delta*256-joint_price_bias(sl,dl,pk);
                if(score>cand_score[0]){
                    cand_score[2]=cand_score[1];
                    cand_ports[2]=cand_ports[1];
                    cand_score[1]=cand_score[0];
