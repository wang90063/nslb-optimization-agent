...
void run_lahc_composite(int m){
    if(fl_count<10) return;
    clock_t lahc_start=clock();
    double lahc_budget=0.05;
    if(fl_count>5000) lahc_budget=0.03;
    ...
    // LAHC: maintain history of L recent objective values
    #define LAHC_L 50
    int hist[LAHC_L];
    int cur_obj=0;
    for(int i=0;i<fl_count;++i){
    ...
    // Compute initial objective: 12*CI + 5*CB
    int init_ci=get_cinphsc(m);
    int init_cb=0;
    {int max_card=0;
    for(int i=0;i<fl_count;++i){int c=fl_src[i];if(c>max_card)max_card=c;}
    for(int c=0;c<=max_card;++c) init_cb+=card_cbtphsc(c,m);}
    cur_obj=12*init_ci+5*init_cb;
    for(int i=0;i<LAHC_L;++i) hist[i]=cur_obj;
    unsigned int rng=fl_count*2654435761u+g_job_idx*1013904223u+17;
    int lahc_iter=0;
    for(;;++lahc_iter){
        if((lahc_iter&0xFF)==0){
            double elapsed=(double)(clock()-lahc_start)/CLOCKS_PER_SEC;
            if(elapsed>=lahc_budget) break;
        }
        ...
        int new_obj=cur_obj+obj_delta;
        int v_idx=lahc_iter%LAHC_L;
        int accept=(new_obj<=cur_obj||new_obj<=hist[v_idx]);
        if(accept){
            m2=mask;
            while(m2){int ph=__builtin_ctz(m2);
                out_load[sl][px][ph]--;out_load[sl][py][ph]++;
...
void solve_job(){
    ...
    // LAHC composite pass: escape local optima with late acceptance
    if(!g_time_tight){
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        EvalMetrics sa_base=collect_metrics(m);
        run_lahc_composite(m);
