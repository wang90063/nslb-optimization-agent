...
static int eq_phase_over[MAX_PORTS];
...
inline int break_equal_cost_tie(int cand_cnt){
    ...
    for(int idx=1;idx<cand_cnt;++idx){
        ...
        if(eq_phase_over[pk]!=eq_phase_over[best_pk]){
            if(eq_phase_over[pk]<eq_phase_over[best_pk]) best_pk=pk;
            continue;
        }
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            while(m2){
                ...
                if(o>mo)mo=o;
                if(iv>mi)mi=iv;
                if(o>g_r) phase_over+=o-g_r;
                if(iv>g_r) phase_over+=iv-g_r;
