...
void run_port_consistency(int m){
    for(int iter=0;iter<5;++iter){
        ...
        for(int i=0;i<fl_count;++i){
            ...
            short src_port_max_out[MAX_PORTS]={};
            for(int k=0;k<cnt;++k){
                int fi=pc_flows[k];
                int cp=fl_port[fi];
            ...
            for(int cp=0;cp<g_p;++cp)
                for(int ph=0;ph<m;++ph)
                    if(out_load[src_leaf][cp][ph]>src_port_max_out[cp])
                        src_port_max_out[cp]=out_load[src_leaf][cp][ph];
                ...
                for(int tp=0;tp<g_p;++tp){
                    ...
                    for(int k=0;k<cnt;++k){
                        ...
                        while(m2){
                            ...
                            if(out_load[sl][cp][ph]==src_max) src_peak=1;
                            if(in_load[dl][cp][ph]==dst_max) dst_peak=1;
                            m2&=m2-1;
                        }
                        if(!ok) continue;
                        can_move++;
                        long long source_relief=0;
                        if(src_peak) source_relief+=global_out[sl][cp];
                        if(dst_peak) source_relief+=global_in[dl][cp];
                        long long target_cost=(long long)global_out[sl][tp]+global_in[dl][tp];
                        pressure+=target_cost-source_relief;
                        m2=mask;
                        while(m2){
                            int ph=__builtin_ctz(m2);
                            tmp_ppc[ph][cp]--;
