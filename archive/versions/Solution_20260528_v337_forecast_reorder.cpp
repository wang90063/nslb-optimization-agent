...
static int fl_count;
// Two-pass forecast buffering
#define MAX_TOTAL_FLOWS 800000
static int buf_src[MAX_TOTAL_FLOWS], buf_dst[MAX_TOTAL_FLOWS];
...
int main(){
    ...
    if(n>=30 && buf_total_flows<150000){
        ...
        // Sort jobs by decreasing forecast contribution (heavy first)
        static int solve_order[40];
        for(int i=0;i<n;++i) solve_order[i]=i;
        for(int i=0;i<n-1;++i)
            for(int j=i+1;j<n;++j){
                int si=solve_order[i],sj=solve_order[j];
                int wi=0,wj=0;
                for(int leaf=0;leaf<g_l;++leaf)
                    for(int pk=0;pk<g_p;++pk){
                        wi+=forecast_out[si][leaf][pk]+forecast_in[si][leaf][pk];
                        wj+=forecast_out[sj][leaf][pk]+forecast_in[sj][leaf][pk];
                    }
                if(wj>wi){int t=solve_order[i];solve_order[i]=solve_order[j];solve_order[j]=t;}
            }
        // Pass 2: full solve in sorted order with forecast-informed global state
        memcpy(global_out, forecast_total_out, sizeof(global_out));
        memcpy(global_in, forecast_total_in, sizeof(global_in));
        g_hist_max_jm=0;
        for(int i=0;i<n;++i){
            int ji=solve_order[i];
            for(int leaf=0;leaf<g_l;++leaf)
                for(int pk=0;pk<g_p;++pk){
                    global_out[leaf][pk]-=forecast_out[ji][leaf][pk];
                    global_in[leaf][pk]-=forecast_in[ji][leaf][pk];
                }
            g_job_idx=ji;
            solve_job();
        }
