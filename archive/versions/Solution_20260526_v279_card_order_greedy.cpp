...
void run_greedy(int m, int local_w, int global_w, int hardcap, int rev=0){
    ...
    if(rev>=2){
        if(rev==100){
            // Sort by card (fl_src) for CB-aware ordering
            for(int a=0;a<fl_count;++a) fl_order[a]=a;
            // Use counting sort by fl_src for O(n) performance
            static int cnt_buf[MAX_CARDS+1];
            int max_card=0;
            for(int a=0;a<fl_count;++a){int c=fl_src[a];if(c>max_card)max_card=c;}
            memset(cnt_buf,0,(max_card+2)*sizeof(int));
            for(int a=0;a<fl_count;++a) cnt_buf[fl_src[a]+1]++;
            for(int a=1;a<=max_card+1;++a) cnt_buf[a]+=cnt_buf[a-1];
            static int tmp_order[MAX_FLOWS];
            for(int a=0;a<fl_count;++a) tmp_order[cnt_buf[fl_src[a]]++]=a;
            for(int a=0;a<fl_count;++a) fl_order[a]=tmp_order[a];
        } else {
            unsigned int seed=rev*2654435761u;
            for(int a=fl_count-1;a>0;--a){
                seed=seed*1664525u+1013904223u;
...
void solve_job(){
    ...
    // Card-order greedy: process flows grouped by card for lower initial CB
    if(!huge_job){
