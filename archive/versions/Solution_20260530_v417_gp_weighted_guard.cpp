...
inline long long gp_weighted_proxy(int cbt, int future_over){
    return 5LL*cbt + 3LL*future_over;
}
...
void solve_job(){
    ...
    #define TRY_STRATEGY() do { \
        ...
        if(first||better_metrics(cur_eval,best_eval)){ \
            take_strategy=1; \
            if(!first && p32_gp_tiebreak && cur_uses_global_price!=best_uses_global_price && \
               ...
               cur_eval.ci==best_eval.ci){ \
                ...
                int best_cb=total_cbtphsc_from_ports(sv_port,m); \
                long long cur_proxy=gp_weighted_proxy(cur_cb, cur_eval.future_over); \
                long long best_proxy=gp_weighted_proxy(best_cb, best_eval.future_over); \
                if(!cur_uses_global_price && best_uses_global_price && best_proxy<=cur_proxy){ \
                    take_strategy=0; \
                } \
            } \
            ...
            if(cur_proxy<best_proxy || \
               (cur_proxy==best_proxy && cur_cb<best_cb)){ \
                take_strategy=1; \
            } \
        } \
        if(take_strategy){ \
