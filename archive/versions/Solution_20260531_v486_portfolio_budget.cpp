...
void solve_job(){
    ...
    // Budget gate for expensive portfolio strategies
    int portfolio_budget_ok=1;
    if(g_job_idx>0){
        double pb_el=elapsed_sec();
        double pb_proj=pb_el/g_job_idx*g_n;
        if(pb_proj>6.5) portfolio_budget_ok=0;
    }
    ...
    // the per-job overflow.
    if(any_jm_le_r && portfolio_budget_ok){
