...
__attribute__((noinline,cold)) void run_lowr_hotcell_beam(int m){
    ...
    if(fl_count>2200||(long long)fl_count*m>42000) return;
...
void solve_job(){
    ...
    // If `jm` still exceeds a simple lower bound, try a tiny coordinated beam
    // around the hottest `jm=r+1` cells before broader rebuild operators.
