...
void run_fg_reduce(int m){
    int fg_leaf=-1,fg_port=-1,fg_val=0;
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            int mo=0,mi=0;
            for(int ph=0;ph<m;++ph){
            ...
            int fi=global_in[leaf][pk]+mi;
            int fv=fo>fi?fo:fi;
            if(fv>fg_val){fg_val=fv;fg_leaf=leaf;fg_port=pk;}
        }
    if(fg_leaf<0) return;
    int fg_is_out=1;
    {int mo=0,mi=0;
     for(int ph=0;ph<m;++ph){
         if(out_load[fg_leaf][fg_port][ph]>mo)mo=out_load[fg_leaf][fg_port][ph];
         if(in_load[fg_leaf][fg_port][ph]>mi)mi=in_load[fg_leaf][fg_port][ph];
     }
     int fo=global_out[fg_leaf][fg_port]+mo;
     int fi=global_in[fg_leaf][fg_port]+mi;
     fg_is_out=(fo>=fi);}
    int peak_ph=-1,peak_val=0;
    for(int ph=0;ph<m;++ph){
        int v=fg_is_out?out_load[fg_leaf][fg_port][ph]:in_load[fg_leaf][fg_port][ph];
        if(v>peak_val){peak_val=v;peak_ph=ph;}
    }
    if(peak_ph<0||peak_val<=1) return;
    for(int iter=0;iter<3;++iter){
        int moved=0;
        for(int i=0;i<fl_count;++i){
            int sl=fl_sl[i],dl=fl_dl[i],cp=fl_port[i];
            if(cp<0) continue;
            int on_bn=0;
            if(fg_is_out&&sl==fg_leaf&&cp==fg_port) on_bn=1;
            if(!fg_is_out&&dl==fg_leaf&&cp==fg_port) on_bn=1;
            if(!on_bn) continue;
            if(!((fl_pmask[i]>>peak_ph)&1)) continue;
            int best_pk=-1,best_new_fg=fg_val;
            for(int pk=0;pk<g_p;++pk){
                if(pk==cp) continue;
                if(!(fl_pmask[i]&((1u<<m)-1))) continue;
                int ok=1;
                unsigned int mk=fl_pmask[i];
                while(mk&&ok){int ph=__builtin_ctz(mk);
                    if(out_load[sl][pk][ph]+1>g_r&&out_load[sl][pk][ph]+1>out_load[sl][cp][ph])ok=0;
                    if(in_load[dl][pk][ph]+1>g_r&&in_load[dl][pk][ph]+1>in_load[dl][cp][ph])ok=0;
                    mk&=mk-1;}
                if(!ok) continue;
                int new_mo=0,new_mi=0;
                unsigned int mk2=fl_pmask[i];
                while(mk2){int ph=__builtin_ctz(mk2);
                    int no=out_load[sl][pk][ph]+1;if(no>new_mo)new_mo=no;
                    int ni=in_load[dl][pk][ph]+1;if(ni>new_mi)new_mi=ni;
                    mk2&=mk2-1;}
                int old_mo=0,old_mi=0;
                mk2=fl_pmask[i];
                while(mk2){int ph=__builtin_ctz(mk2);
                    int oo=out_load[sl][cp][ph]-1;if(oo>old_mo)old_mo=oo;
                    int oi=in_load[dl][cp][ph]-1;if(oi>old_mi)old_mi=oi;
                    mk2&=mk2-1;}
                for(int ph=0;ph<m;++ph){
                    if(!((fl_pmask[i]>>ph)&1)){
                        if(out_load[sl][cp][ph]>old_mo)old_mo=out_load[sl][cp][ph];
                        if(in_load[dl][cp][ph]>old_mi)old_mi=in_load[dl][cp][ph];
                        if(out_load[sl][pk][ph]>new_mo)new_mo=out_load[sl][pk][ph];
                        if(in_load[dl][pk][ph]>new_mi)new_mi=in_load[dl][pk][ph];
                    }
                }
                int new_fo=global_out[sl][pk]+new_mo;
                int new_fi=global_in[dl][pk]+new_mi;
                int new_fv=new_fo>new_fi?new_fo:new_fi;
                int old_fo=global_out[sl][cp]+old_mo;
                int old_fi=global_in[dl][cp]+old_mi;
                int old_fv_reduced=old_fo>old_fi?old_fo:old_fi;
                if(new_fv<=fg_val&&new_fv<best_new_fg){
                    best_new_fg=new_fv;best_pk=pk;
                }
            }
            if(best_pk>=0){
                unsigned int mk=fl_pmask[i];
...
void solve_job(){
    ...
    // MM-focused end pass: reduce fg bottleneck after all CB/CT optimization
    if(!g_time_tight){
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        EvalMetrics fg_base=collect_metrics(m);
        run_fg_reduce(m);
        EvalMetrics fg_after=collect_metrics(m);
        if(!better_metrics(fg_after,fg_base)){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));
        }
