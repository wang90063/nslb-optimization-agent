...
void solve_job(){
    ...
    // Sort flows by phase count descending (most constrained first)
    for(int i=0;i<fl_count;++i) fl_order[i]=i;
    // Simple counting sort by popcount (max 31 phases)
    int buckets[32]={0};
    for(int i=0;i<fl_count;++i) buckets[__builtin_popcount(fl_pmask[i])]++;
    int offsets[32]; offsets[31]=0;
    for(int b=30;b>=0;--b) offsets[b]=offsets[b+1]+buckets[b+1];
    static int tmp_order[MAX_FLOWS];
    for(int i=0;i<fl_count;++i) tmp_order[offsets[__builtin_popcount(fl_pmask[i])]++]=i;
    memcpy(fl_order, tmp_order, fl_count*sizeof(int));

    for(int idx=0;idx<fl_count;++idx){
        int i=fl_order[idx];
