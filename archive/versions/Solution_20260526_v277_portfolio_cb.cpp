...
static unsigned int eval_cpm[MAX_CARDS][MAX_PHASES];
...
inline EvalMetrics collect_metrics(int m){
    ...
    for(int i=0;i<fl_count;++i){
        int c=fl_src[i];
        if(c>=max_card) max_card=c+1;
    }
    memset(eval_cpm,0,max_card*sizeof(eval_cpm[0]));
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
        int c=fl_src[i],p=fl_port[i];
        unsigned int mk=fl_pmask[i];
        while(mk){int ph=__builtin_ctz(mk);eval_cpm[c][ph]|=(1u<<p);mk&=mk-1;}
    }
    for(int c=0;c<max_card;++c)
        for(int ph=0;ph<m-1;++ph)
            if(eval_cpm[c][ph]&&eval_cpm[c][ph+1]&&eval_cpm[c][ph]!=eval_cpm[c][ph+1])
                em.cbt++;
    return em;
}
