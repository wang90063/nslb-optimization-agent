...
inline int cross_dest_phaseaware_score(int card, int sl, int dl, unsigned int mask, int pk, int m){
    int exact_support=0;
    int total_support=0;
    for(int ph=0;ph<m;++ph){
        int cnt=cppc[card][ph][pk];
        total_support+=cnt;
        if(mask&(1u<<ph)) exact_support+=cnt;
    }
    return exact_support*512 + total_support*32 - joint_price_bias(sl, dl, pk);
}

inline int cross_dest_total_score(int card, int sl, int dl, int pk, int m){
    int total_support=0;
    for(int ph=0;ph<m;++ph) total_support+=cppc[card][ph][pk];
    return total_support*256 - joint_price_bias(sl, dl, pk);
}
...
void run_cross_dest_swap(int m){
    ...
    for(int iter=0;iter<cd_max_iter;++iter){
        ...
        for(int fi=0;fi<fl_count;++fi){
            ...
            for(int pk=0;pk<g_p;++pk){
                int score;
                if(g_p>=32&&g_r>=4) score=cross_dest_phaseaware_score(ci, sl, dl_a, mask_a, pk, m);
                else score=cross_dest_total_score(ci, sl, dl_a, pk, m);
    ...
    for(int iter=0;iter<2;++iter){
        ...
        for(int fi=0;fi<fl_count;++fi){
            ...
            int cand_limit=(g_p>=32)?3:((g_p>=16)?2:1);
            for(int pk=0;pk<g_p;++pk){
                int score=cross_dest_total_score(ci, sl, dl, pk, m);
                if(score>cand_score[0]){
                    cand_score[2]=cand_score[1];
                    cand_ports[2]=cand_ports[1];
                    cand_score[1]=cand_score[0];
