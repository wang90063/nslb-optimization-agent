...
// Try to move one flow off the bottleneck. Returns 1 if improved.
int try_bottleneck_move(int m, int mx){
...
// Lateral move: move a flow off bottleneck port to another port with same max but better global
int try_lateral_move(int m, int mx){
    for(int i=0;i<fl_count;++i){
        int sl=fl_sl[i],dl=fl_dl[i];
        if(sl==dl) continue;
        int cp=fl_port[i];
        ...
        if(!on_bn) continue;
        int cur_gv=global_out[sl][cp]>global_in[dl][cp]?global_out[sl][cp]:global_in[dl][cp];
        int best_new=-1,best_gv=cur_gv;
        for(int pk=0;pk<g_p;++pk){
            if(pk==cp) continue;
            int nm=0; m2=mask;
            while(m2){
            ...
            if(nm<=mx){
                ...
                if(gv<best_gv){best_gv=gv;best_new=pk;}
            }
        }
        if(best_new>=0){
            m2=mask;
...
void solve_job(){
    ...
    // Pass 2: bottleneck swap + plateau walking
    for(int round=0;round<3;++round){
        // Phase A: strict improvement
        for(int iter=0;iter<20;++iter){
            int mx=get_job_max(m);
            if(mx<=1) break;
            if(!try_bottleneck_move(m,mx)) break;
        }
        // Phase B: lateral moves (same max, better global)
        int mx=get_job_max(m);
        if(mx<=1) break;
        int lateral_done=0;
        for(int iter=0;iter<5&&!lateral_done;++iter){
            if(!try_lateral_move(m,mx)) lateral_done=1;
        }
        if(lateral_done&&round==0) break; // no lateral moves possible, stop
    }
