...
__attribute__((noinline,cold)) void run_overflow_repair(int m){
    if(g_r<=3) return;
    if(fl_count>6000||(long long)fl_count*m>100000) return;
    ...
    for(int round=0;round<6;++round){
        ...
        for(int fi=0;fi<fl_count;++fi){
            ...
            while(mk){
                int ph=__builtin_ctz(mk);
                if(out_load[sl][cp][ph]>=best_eval.jm||
                   in_load[dl][cp][ph]>=best_eval.jm){
                    touches_hot=1; break;
                }
                mk&=mk-1;
            }
            ...
            for(int pk=0;pk<g_p;++pk){
                ...
                while(mk){
                    int ph=__builtin_ctz(mk);
                    if(out_load[sl][pk][ph]+1>best_eval.jm||
                       in_load[dl][pk][ph]+1>best_eval.jm){
                        feasible=0; break;
                    }
                    mk&=mk-1;
                }
...
void solve_job(){
    ...
    // S11 and S14 run unconditionally — they use global-dominant/fgmax greedy
    // which can find better solutions even when jm > r on all prior strategies.
    ...
    // Overflow repair for r=4 cases where jm > r
    run_overflow_repair(m);
