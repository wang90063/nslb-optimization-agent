...
void solve_job(){
    ...
    if(fl_count<=6000){
                ...
                for(int fi=0;fi<fl_count;++fi){
                    ...
                    for(int bfi=0;bfi<fl_count;++bfi){
                        ...
                        for(int ap=0;ap<g_p;++ap){
                            ...
                            if(!bok) continue;
                            // Check blocker doesn't increase max-phase-load
                            if(g_p<16||g_r>=4){
                                int cmx_o=0,cmx_i=0;
                                for(int ph=0;ph<m;++ph){
        ...
        int mr_revert=0;
        if(g_r>=4){
            int total_cbt_after=0;
            for(int c=0;c<12800;++c) total_cbt_after+=card_cbtphsc(c,m);
            // The r=4 extension is only worth keeping if it buys real global CB.
            if(total_cbt_after>=total_cbt_before) mr_revert=1;
        }
        if(g_r>=4){
            // For r>=4, revert if any (leaf,port) max_phase_load increased
            for(int leaf=0;leaf<g_l&&!mr_revert;++leaf)
                for(int pk=0;pk<g_p&&!mr_revert;++pk){
                    int mo=0,mi=0,bo=0,bi=0;
                    for(int ph=0;ph<m;++ph){
                        if(out_load[leaf][pk][ph]>mo)mo=out_load[leaf][pk][ph];
                        if(in_load[leaf][pk][ph]>mi)mi=in_load[leaf][pk][ph];
                        if(bk_out[leaf][pk][ph]>bo)bo=bk_out[leaf][pk][ph];
                        if(bk_in[leaf][pk][ph]>bi)bi=bk_in[leaf][pk][ph];
                    }
                    if(mo>bo||mi>bi) mr_revert=1;
                }
        }
        if(!mr_revert&&!better_metrics(mr_after,mr_base)&&(mr_after.fg!=mr_base.fg||mr_after.jm!=mr_base.jm||mr_after.ci!=mr_base.ci))
            mr_revert=1;
        if(mr_revert){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
