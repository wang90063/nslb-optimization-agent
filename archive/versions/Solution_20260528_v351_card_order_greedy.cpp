...
void solve_job(){
    ...
    if(!huge_job){
        ...
        // Card-sorted greedy: group same-card flows together
        for(int i=0;i<fl_count;++i) fl_order[i]=i;
        for(int i=1;i<fl_count;++i){
            int j=i;
            while(j>0&&fl_src[fl_order[j]]<fl_src[fl_order[j-1]]){
                int t=fl_order[j];fl_order[j]=fl_order[j-1];fl_order[j-1]=t;j--;
            }
        }
