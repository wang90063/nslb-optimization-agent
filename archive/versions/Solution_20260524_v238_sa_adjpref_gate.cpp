...
void run_sa_composite(int m){
    ...
    for(int iter=0;;++iter){
        ...
        int before_cbt=card_cbtphsc(ci,m);
        int py=-1;
        if(before_cbt>0 && (rng&3u)==0){
            unsigned int pref_union=0;
            unsigned int pref_inter=0;
            int have_pref=0;
            unsigned int mk=mask;
            while(mk){
                int ph=__builtin_ctz(mk);
                unsigned int local_pref=0;
                if(ph>0 && cpm[ci][ph-1] && cpm[ci][ph-1]!=cpm[ci][ph]) local_pref|=cpm[ci][ph-1];
                if(ph+1<m && cpm[ci][ph+1] && cpm[ci][ph+1]!=cpm[ci][ph]) local_pref|=cpm[ci][ph+1];
                local_pref&=~(1u<<px);
                if(local_pref){
                    pref_union|=local_pref;
                    if(!have_pref){pref_inter=local_pref;have_pref=1;}
                    else pref_inter&=local_pref;
                }
                mk&=mk-1;
            }
            unsigned int pref_mask=pref_inter;
            if(!pref_mask && pref_union && __builtin_popcount(pref_union)==1){
                pref_mask=pref_union;
            }
            if(pref_mask){
                int cnt=0;
                unsigned int tmp=pref_mask;
                while(tmp){cnt++;tmp&=tmp-1;}
                rng=rng*1664525u+1013904223u;
                int kth=rng%cnt;
                tmp=pref_mask;
                while(kth--) tmp&=tmp-1;
                py=__builtin_ctz(tmp);
            }
        }
        if(py<0){
            rng=rng*1664525u+1013904223u;
            py=rng%(g_p-1); if(py>=px)py++;
        }
        unsigned int m2=mask; int ok=1;
        while(m2){int ph=__builtin_ctz(m2);
