...
static inline int port_hash_start(int fi,int mode){
    if(mode<=0||g_p<=1) return 0;
    unsigned int x=(unsigned int)fl_src[fi]*1000003u;
    x^=(unsigned int)fl_dst[fi]*9176u;
    x^=fl_pmask[fi]*2246822519u;
    x^=(unsigned int)g_job_idx*3266489917u;
    x^=(unsigned int)fl_sl[fi]*668265263u;
    x^=(unsigned int)fl_dl[fi]*374761393u;
    x^=(unsigned int)mode*2654435761u;
    x^=x>>16;
    x*=0x7feb352dU;
    x^=x>>15;
    x*=0x846ca68bU;
    x^=x>>16;
    return (int)(x%(unsigned int)g_p);
}
...
void run_greedy_porthash(int m, int local_w, int global_w, int hardcap, int rev=0, int mode=1){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    if(rev>=2){
        unsigned int seed=rev*2654435761u;
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        int bt_over=0x7fffffff,bt_sum=0x7fffffff;
        long long bt_sq=0x7fffffffffffffffLL;
        int start=port_hash_start(i,mode);
        for(int off=0;off<g_p;++off){
            int pk=(start+off)%g_p;
            int local_max=0;
            int exceeds=0;
            int use_future_tie=(hardcap&&g_p>=16&&g_hist_max_jm>g_r);
            int mo=0,mi=0;
...
void run_greedy_ftrl_porthash(int m, int hardcap, int rev=0, int sc_div=2, int local_w=2, int mode=1){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    if(rev>=2){
        unsigned int seed=rev*2654435761u;
...
void solve_job(){
    ...
    // S6k-S6n: rotate the equal-cost port scan by a flow/job hash. This is a
    // candidate-family-level decorrelation variant, not a pressure-order gate.
    if(g_r==4&&g_p>=16){
        run_greedy_porthash(m, 1000, 1, 1);
        run_swap(m);
        run_jm_repair(m);
        TRY_STRATEGY();

        run_greedy_porthash(m, 1000, 1, 1, 1);
        run_swap(m);
        run_jm_repair(m);
        TRY_STRATEGY();

        run_greedy_ftrl_porthash(m, 1);
        run_swap(m);
        run_jm_repair(m);
        TRY_STRATEGY();

        run_greedy_ftrl_porthash(m, 1, 1);
        run_swap(m);
        run_jm_repair(m);
        TRY_STRATEGY();
    }
