...
void run_sa_composite(int m){
    ...
    int sa_accepted=0;
    static short pre_out_from[MAX_PHASES],pre_out_to[MAX_PHASES];
    static short pre_in_from[MAX_PHASES],pre_in_to[MAX_PHASES];
    for(int iter=0;;++iter){
        if((iter&0xFF)==0){
        ...
        for(int ph=0;ph<m;++ph){
            pre_out_from[ph]=out_load[sl][px][ph];
            pre_out_to[ph]=out_load[sl][py][ph];
            pre_in_from[ph]=in_load[dl][px][ph];
            pre_in_to[ph]=in_load[dl][py][ph];
            if(pre_out_from[ph]>pre_mo_px) pre_mo_px=pre_out_from[ph];
            if(pre_out_to[ph]>pre_mo_py) pre_mo_py=pre_out_to[ph];
            if(pre_in_from[ph]>pre_mi_px) pre_mi_px=pre_in_from[ph];
            if(pre_in_to[ph]>pre_mi_py) pre_mi_py=pre_in_to[ph];
        }
        m2=mask;
        while(m2){int ph=__builtin_ctz(m2);
            ...
            m2&=m2-1;}
        for(int ph=0;ph<m;++ph){
            int out_from=pre_out_from[ph]-((mask>>ph)&1u);
            int out_to=pre_out_to[ph]+((mask>>ph)&1u);
            int in_from=pre_in_from[ph]-((mask>>ph)&1u);
            int in_to=pre_in_to[ph]+((mask>>ph)&1u);
            if(out_from>post_mo_px) post_mo_px=out_from;
            if(out_to>post_mo_py) post_mo_py=out_to;
            if(in_from>post_mi_px) post_mi_px=in_from;
            if(in_to>post_mi_py) post_mi_py=in_to;
        }
        int pre_ct=0,post_ct=0;
        ...
        if(pre_fo_px>g_r) pre_ct+=pre_fo_px-g_r;
        if(pre_fo_py>g_r) pre_ct+=pre_fo_py-g_r;
        if(pre_fi_px>g_r) pre_ct+=pre_fi_px-g_r;
        if(pre_fi_py>g_r) pre_ct+=pre_fi_py-g_r;
        if(post_fo_px>g_r) post_ct+=post_fo_px-g_r;
        if(post_fo_py>g_r) post_ct+=post_fo_py-g_r;
        if(post_fi_px>g_r) post_ct+=post_fi_px-g_r;
        if(post_fi_py>g_r) post_ct+=post_fi_py-g_r;
        ct_delta=post_ct-pre_ct;
        int before_cbt=card_cbtphsc(ci,m);
        m2=mask;
        while(m2){int ph=__builtin_ctz(m2);
            cppc[ci][ph][px]--;if(!cppc[ci][ph][px])cpm[ci][ph]&=~(1u<<px);
