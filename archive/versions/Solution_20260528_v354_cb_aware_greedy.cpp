...
inline EvalMetrics collect_metrics(int m){
    ...
    em.cbt=0;
    {
        static unsigned int tmp_cpm[MAX_CARDS][MAX_PHASES];
        memset(tmp_cpm,0,sizeof(tmp_cpm));
        for(int i=0;i<fl_count;++i){
            if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
            unsigned int mk=fl_pmask[i];int p=fl_port[i];
            while(mk){int ph=__builtin_ctz(mk);tmp_cpm[fl_src[i]][ph]|=(1u<<p);mk&=mk-1;}
        }
        for(int c=0;c<MAX_CARDS;++c)
            for(int ph=0;ph<m-1;++ph){
                unsigned int a=tmp_cpm[c][ph],b=tmp_cpm[c][ph+1];
                if(a&&b) em.cbt+=__builtin_popcount(a^(a&b))+__builtin_popcount(b^(a&b));
            }
    }
    return em;
}
...
void run_greedy_cb_aware(int m, int cb_w){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    static unsigned int card_port_mask[MAX_CARDS][MAX_PHASES];
    memset(card_port_mask,0,sizeof(card_port_mask));
    static int cb_order[MAX_FLOWS];
    static int card_cnt[MAX_CARDS+1];
    memset(card_cnt,0,(MAX_CARDS+1)*sizeof(int));
    for(int i=0;i<fl_count;++i) card_cnt[fl_src[i]+1]++;
    for(int i=1;i<=MAX_CARDS;++i) card_cnt[i]+=card_cnt[i-1];
    for(int i=0;i<fl_count;++i) cb_order[card_cnt[fl_src[i]]++]=i;
    for(int ii=0;ii<fl_count;++ii){
        int i=cb_order[ii];
        int sl=fl_sl[i],dl=fl_dl[i];
        if(sl==dl){fl_port[i]=-1;continue;}
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            while(m2){
                int ph=__builtin_ctz(m2);
                if(ph>0&&card_port_mask[card][ph-1]&&!(card_port_mask[card][ph-1]&(1u<<pk))) cb_pen++;
                if(ph<m-1&&card_port_mask[card][ph+1]&&!(card_port_mask[card][ph+1]&(1u<<pk))) cb_pen++;
                m2&=m2-1;
            }
            int cost=local_max*2+gv+cb_pen*cb_w;
            if(exceeds) cost+=10000;
            if(cost<bc){bc=cost;bp=pk;}
        }
        fl_port[i]=(short)bp;
        unsigned int m2=mask;
        while(m2){int ph=__builtin_ctz(m2);out_load[sl][bp][ph]++;in_load[dl][bp][ph]++;card_port_mask[card][ph]|=(1u<<bp);m2&=m2-1;}
    }
}
...
void solve_job(){
    ...
    if(!huge_job){
        ...
        // S6k: CB-aware greedy (different weights) + swap
        run_greedy_cb_aware(m, 3);
        run_swap(m);
        TRY_STRATEGY();

        run_greedy_cb_aware(m, 5);
        run_swap(m);
        TRY_STRATEGY();
