...
void solve_job(){
    ...
    if(!g_time_tight){
        ...
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        EvalMetrics second_stage_base=collect_metrics(m);
        run_relaxed_swap(m);
        run_neutral_swap(m);
        run_relaxed_swap(m);
        ...
        run_neutral_swap(m);
        run_relaxed_swap(m);
        EvalMetrics second_stage_after=collect_metrics(m);
        if(!better_metrics(second_stage_after,second_stage_base)){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));
        }
    ...
    if(!g_time_tight){
        ...
        if(better_metrics(ct_after,ct_base)) ct_pass_ran=1;
        else {
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));
