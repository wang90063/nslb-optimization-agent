...
void solve_job(){
    ...
    #define WORTH_POST() (first || get_job_max(m) <= best_eval.jm + SKIP_THRESHOLD)
    ...
    run_greedy(m, 2, 1, 1);
    if(WORTH_POST()){ run_swap(m); run_jm_repair(m); }
    TRY_STRATEGY();
    ...
    // S6e2-S6e5: local-first hardcap candidates.
