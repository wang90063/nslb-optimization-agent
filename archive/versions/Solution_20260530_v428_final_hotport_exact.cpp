...
__attribute__((noinline,cold)) void run_final_hotport_exact(int m){
    if(g_time_tight||g_p<8) return;
    long long job_work=(long long)fl_count*(long long)m;
    if(fl_count>5500||job_work>90000) return;
    ...
    for(int round=0;round<2;++round){
        ...
        for(int leaf=0;leaf<g_l;++leaf)
            for(int pk=0;pk<g_p;++pk){
                ...
                if(fo>g_r){
                    ...
                    insert_final_hot_port(hot_ports,hot_cnt,4,leaf,pk,0,fo,mo,score);
                }
                if(fi>g_r){
                    int score=(fi-g_r)*8192+mi*128+(fi==best_eval.fg?2048:0);
                    insert_final_hot_port(hot_ports,hot_cnt,4,leaf,pk,1,fi,mi,score);
                }
            }
        if(hot_cnt<=0) break;
        ...
        for(int hi=0;hi<use_hot;++hi){
            ...
            for(int fi=0;fi<fl_count;++fi){
                ...
                int score=card_cbtphsc(fl_src[fi],m)*256+__builtin_popcount(fl_pmask[fi])*64;
                int hit_hot=0;
                unsigned int mk=fl_pmask[fi];
                while(mk){
                    int ph=__builtin_ctz(mk);
        ...
        for(int ii=0;ii<flow_cap;++ii){
            ...
            for(int pk=0;pk<g_p;++pk){
                if(pk==cp) continue;
                int new_out_cp=0,new_out_pk=0,new_in_cp=0,new_in_pk=0;
                for(int ph=0;ph<m;++ph){
                    int take=(mask>>ph)&1u;
                    int out_cp=out_load[sl][cp][ph]-take;
                    int out_pk=out_load[sl][pk][ph]+take;
                    int in_cp=in_load[dl][cp][ph]-take;
                    int in_pk=in_load[dl][pk][ph]+take;
                    if(out_cp>new_out_cp) new_out_cp=out_cp;
                    if(out_pk>new_out_pk) new_out_pk=out_pk;
                    if(in_cp>new_in_cp) new_in_cp=in_cp;
                    if(in_pk>new_in_pk) new_in_pk=in_pk;
                }
                int local_peak=new_out_cp;
                if(new_out_pk>local_peak) local_peak=new_out_pk;
                if(new_in_cp>local_peak) local_peak=new_in_cp;
                if(new_in_pk>local_peak) local_peak=new_in_pk;
                if(local_peak>allow_jm) continue;

                int fo_cp=global_out[sl][cp]+new_out_cp;
                int fo_pk=global_out[sl][pk]+new_out_pk;
                int fi_cp=global_in[dl][cp]+new_in_cp;
                int fi_pk=global_in[dl][pk]+new_in_pk;
                int future_fg=fo_cp;
                if(fo_pk>future_fg) future_fg=fo_pk;
                if(fi_cp>future_fg) future_fg=fi_cp;
                if(fi_pk>future_fg) future_fg=fi_pk;
                if(future_fg>best_eval.fg) continue;

                int future_over=0;
                if(fo_cp>g_r) future_over+=fo_cp-g_r;
                if(fo_pk>g_r) future_over+=fo_pk-g_r;
                if(fi_cp>g_r) future_over+=fi_cp-g_r;
                if(fi_pk>g_r) future_over+=fi_pk-g_r;
                if(future_over>best_eval.future_over+2) continue;

                int bias=joint_price_bias(sl,dl,pk)-dp_cnt[pk]*128;
                if(bias>best_eval.future_over*2048&&future_over==best_eval.future_over) continue;
