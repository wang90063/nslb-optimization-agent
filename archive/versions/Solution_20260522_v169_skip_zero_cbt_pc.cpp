...
void run_port_consistency(int m){
    for(int iter=0;iter<5;++iter){
        ...
        for(int i=0;i<fl_count;++i){
            ...
            // Once a source-card is already phase-consistent, revisiting it in
            // the group-level PC chain can only reshuffle ports and may
            // reintroduce Cbtphsc on later passes.
            if(base_cbt<=0) continue;
            if(cnt<=160){
