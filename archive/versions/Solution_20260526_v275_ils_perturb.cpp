...
void run_ils_cb(int m){
    if(fl_count<50||g_time_tight) return;
    clock_t ils_start=clock();
    double ils_budget=0.03;
    unsigned int rng=fl_count*1013904223u+g_job_idx*2654435761u+7;
    ...
    int n_cards=0;
    static int card_cb[MAX_CARDS];
    for(int c=0;c<fl_count;++c) card_cb[c]=0;
    for(int i=0;i<fl_count;++i){
        int c=fl_src[i];
        if(c>=n_cards) n_cards=c+1;
    }
    for(int c=0;c<n_cards;++c) card_cb[c]=card_cbtphsc(c,m);
    for(int c=0;c<n_cards;++c) best_cb+=card_cb[c];
    int base_fg=get_future_gmax(m);
    int base_jm=get_job_max(m);
    int perturb_k=fl_count/20;
    if(perturb_k<5) perturb_k=5;
    if(perturb_k>50) perturb_k=50;
    for(int round=0;round<5;++round){
        double elapsed=(double)(clock()-ils_start)/CLOCKS_PER_SEC;
        if(elapsed>=ils_budget) break;
        ...
        if(cur_fg>base_fg||(cur_fg==base_fg&&cur_jm>base_jm)){
            ...
            for(int c=0;c<n_cards;++c) cur_cb+=card_cbtphsc(c,m);
            if(cur_cb<best_cb){
                best_cb=cur_cb;
                memcpy(ils_out, out_load, g_l*sizeof(out_load[0]));
                memcpy(ils_in, in_load, g_l*sizeof(in_load[0]));
...
void solve_job(){
    ...
    // ILS perturbation: escape local optima via random perturbation + CB recovery
    run_ils_cb(m);
