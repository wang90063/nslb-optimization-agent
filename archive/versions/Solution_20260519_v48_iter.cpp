...
// Constrained greedy: like run_greedy but hardcap at max_load instead of r
// Used to optimize Maxmultir without exceeding the best achievable Maxsingler
void run_greedy_capped(int m, int global_w, int cap, int rev=0){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    if(rev>=2){
        unsigned int seed=rev*2654435761u;
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            while(m2){
                ...
                int iv=in_load[dl][pk][ph]+1;
                int v=o>iv?o:iv;if(v>local_max)local_max=v;
                if(o>cap||iv>cap) exceeds=1;
                m2&=m2-1;
            }
            int go=global_out[sl][pk],gi=global_in[dl][pk];
            int gv=go>gi?go:gi;
            int cost=local_max+gv*global_w;
            if(exceeds) cost+=10000;
            if(cost<bc){bc=cost;bp=pk;}
        }
        fl_port[i]=(short)bp;
...
// Ports with high accumulated load get DISPROPORTIONATELY penalized
// This naturally encourages balance without cascading issues
void run_greedy_ftrl(int m, int hardcap, int rev=0){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    if(rev>=2){
        unsigned int seed=rev*2654435761u;
    ...
    // scale: controls when quadratic term becomes significant
    int scale = g_r * g_n / 2;
    if(scale < 1) scale = 1;
    for(int ii=0;ii<fl_count;++ii){
        int i;
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            int fi=global_in[dl][pk]+mi;
            int gv=fo>fi?fo:fi;
            // FTRL cost: linear + quadratic penalty
...
static int fl_pressure[MAX_FLOWS];
static int cmp_pressure(const void*a,const void*b){
    return fl_pressure[*(const int*)b]-fl_pressure[*(const int*)a];
}
void sort_flows_by_pressure(){
    for(int i=0;i<fl_count;++i){
        int sl=fl_sl[i],dl=fl_dl[i];
        int mx=0;
        for(int pk=0;pk<g_p;++pk){
            int v=global_out[sl][pk];if(v>mx)mx=v;
            v=global_in[dl][pk];if(v>mx)mx=v;
        }
        fl_pressure[i]=mx;
    }
    qsort(fl_order,fl_count,sizeof(int),cmp_pressure);
}

static int lns_idx[MAX_FLOWS];
void run_lns_w(int m, int gw){
    int pre_gmax=get_future_gmax(m);
    int pre_jmax=get_job_max(m);
    if(pre_gmax<=g_r) return;
    memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
...
inline void run_lns(int m){run_lns_w(m,5);}
...
void solve_job(){
    ...
    // S6b: FTRL-inspired greedy (quadratic global penalty) + swap
    run_greedy_ftrl(m, 1);
    run_swap(m);
    TRY_STRATEGY();
    ...
    // Compute best_jm from standard strategies
    int best_jm;
    {
        ...
        best_jm=get_job_max(m);
    }

    // Global strategies gate:
    // - When jm<=r: always run (same as v47)
    // - When jm>r: only run for last quarter of jobs (minimize cascading)
    if(best_jm <= g_r || g_job_idx >= g_n - g_n/4){

    // Use cap=best_jm when jm>r, cap=r when jm<=r
    int cap = best_jm <= g_r ? g_r : best_jm;

    // S11: apply global_swap + lns to best standard result (LAST JOB ONLY)
    if(g_job_idx == g_n-1){
        run_global_swap(m);
        run_lns(m);
        TRY_STRATEGY();
    ...
    // S11: capped global (gw=3) + swap + global swap + lns
    run_greedy_capped(m, 3, cap);
    run_swap(m);
    run_global_swap(m);
    run_lns(m);
    TRY_STRATEGY();

    // S12: capped global reversed (gw=3) + swap + global swap + lns
    run_greedy_capped(m, 3, cap, 1);
    run_swap(m);
    run_global_swap(m);
    run_lns(m);
    TRY_STRATEGY();

    // S13: capped extreme global (gw=5) + swap + global swap + lns
    run_greedy_capped(m, 5, cap);
    run_swap(m);
    run_global_swap(m);
    run_lns(m);
    TRY_STRATEGY();

    // S14: fgmax greedy + swap + global swap + lns
    ...
    // S15: fgmax greedy reversed + swap + global swap + lns
    ...
    // S16-S19: shuffled capped global (4 seeds)
    for(int seed=6;seed<=9;++seed){
        for(int i=0;i<fl_count;++i) fl_order[i]=i;
        run_greedy_capped(m, 3, cap, seed);
        run_swap(m);
        run_global_swap(m);
        run_lns(m);
        TRY_STRATEGY();
    ...
    } // end if(best_jm <= g_r || g_job_idx == g_n-1)
