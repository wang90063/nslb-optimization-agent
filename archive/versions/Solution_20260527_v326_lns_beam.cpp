...
// LNS card rebuild: destroy all flows from a card, try single-port assignment (CB=0)
#define LNS_MAX_DESTROY 32
static int lns_idx2[LNS_MAX_DESTROY];
static int lns_cnt2;

void run_lns_card_rebuild(int m){
    if(fl_count<10) return;
    clock_t lns_start=clock();
    double lns_budget=0.012;
    if(fl_count>5000) lns_budget=0.006;
    int pre_jm=get_job_max(m);
    // Build cpm/cppc
    memset(cpm,0,sizeof(cpm));memset(cppc,0,sizeof(cppc));
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
    ...
    // Find cards with CB > 0
    static int lb_cards[MAX_CARDS];
    static int lb_card_val[MAX_CARDS];
    int lb_card_cnt=0;
    static int lb_done[MAX_CARDS];
    static int lb_ep; lb_ep++;
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
        int c=fl_src[i];
        if(lb_done[c]==lb_ep) continue;
        lb_done[c]=lb_ep;
        int cv=card_cbtphsc(c,m);
        if(cv>0){lb_cards[lb_card_cnt]=c;lb_card_val[lb_card_cnt]=cv;lb_card_cnt++;}
    }
    for(int i=0;i<lb_card_cnt-1;++i)
        for(int j=i+1;j<lb_card_cnt;++j)
            if(lb_card_val[j]>lb_card_val[i]){
                int t=lb_cards[i];lb_cards[i]=lb_cards[j];lb_cards[j]=t;
                t=lb_card_val[i];lb_card_val[i]=lb_card_val[j];lb_card_val[j]=t;
            }
    for(int ci=0;ci<lb_card_cnt;++ci){
        double el=(double)(clock()-lns_start)/CLOCKS_PER_SEC;
        if(el>=lns_budget) break;
        int card=lb_cards[ci];
        lns_cnt2=0;
        for(int i=0;i<fl_count;++i){
            if(fl_src[i]!=card||fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
            if(lns_cnt2>=LNS_MAX_DESTROY) break;
            lns_idx2[lns_cnt2++]=i;
        }
        if(lns_cnt2<2) continue;
        // Save original
        static short orig_p[LNS_MAX_DESTROY];
        for(int k=0;k<lns_cnt2;++k) orig_p[k]=fl_port[lns_idx2[k]];
        int orig_cb=lb_card_val[ci];
        // Destroy
        for(int k=0;k<lns_cnt2;++k){
            int fi=lns_idx2[k],sl=fl_sl[fi],dl=fl_dl[fi],p=fl_port[fi];
            unsigned int mk=fl_pmask[fi];
            while(mk){int ph=__builtin_ctz(mk);
                out_load[sl][p][ph]--;in_load[dl][p][ph]--;
                cppc[card][ph][p]--;if(!cppc[card][ph][p])cpm[card][ph]&=~(1u<<p);
                mk&=mk-1;}
        }
        // Try each port as single-target (CB=0 if all fit)
        int best_port=-1,best_ci=0x7fffffff;
        for(int tp=0;tp<g_p;++tp){
            int ok=1,ci_sum=0,placed=0;
            for(int k=0;k<lns_cnt2;++k){
                int fi=lns_idx2[k],sl=fl_sl[fi],dl=fl_dl[fi];
                unsigned int mk=fl_pmask[fi];
                int feasible=1;
                while(mk){int ph=__builtin_ctz(mk);
                    int ov=out_load[sl][tp][ph]+1,iv=in_load[dl][tp][ph]+1;
                    if(ov>pre_jm||iv>pre_jm){feasible=0;break;}
                    if(ov>g_r)ci_sum++;if(iv>g_r)ci_sum++;
                    mk&=mk-1;}
                if(!feasible){ok=0;break;}
                unsigned int mk2=fl_pmask[fi];
                while(mk2){int ph=__builtin_ctz(mk2);
                    out_load[sl][tp][ph]++;in_load[dl][tp][ph]++;
                    mk2&=mk2-1;}
                placed++;
            }
            // Always undo all placed flows
            ...
            if(ok && 12*ci_sum<5*orig_cb && ci_sum<best_ci){
                best_ci=ci_sum;best_port=tp;
            }
        }
        if(best_port>=0){
            // Apply single-port assignment
            for(int k=0;k<lns_cnt2;++k){
                int fi=lns_idx2[k];fl_port[fi]=best_port;
                unsigned int mk=fl_pmask[fi];
                while(mk){int ph=__builtin_ctz(mk);
                    out_load[fl_sl[fi]][best_port][ph]++;
                    in_load[fl_dl[fi]][best_port][ph]++;
                    cppc[card][ph][best_port]++;cpm[card][ph]|=(1u<<best_port);
                    mk&=mk-1;}
            }
        } else {
            // Restore original
            for(int k=0;k<lns_cnt2;++k){
                int fi=lns_idx2[k],p=orig_p[k];fl_port[fi]=p;
                unsigned int mk=fl_pmask[fi];
                while(mk){int ph=__builtin_ctz(mk);
                    out_load[fl_sl[fi]][p][ph]++;in_load[fl_dl[fi]][p][ph]++;
                    cppc[card][ph][p]++;cpm[card][ph]|=(1u<<p);
                    mk&=mk-1;}
            }
        }
    }
...
void solve_job(){
    ...
    // LNS card rebuild: post-SA large neighborhood CB optimization
    if(!g_time_tight){
        run_lns_card_rebuild(m);
    }
