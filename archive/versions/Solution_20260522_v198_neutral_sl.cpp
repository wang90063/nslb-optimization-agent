...
void run_neutral_swap(int m){
    // Precompute max_in for cross-dl feasibility
    static short ns_max_in[MAX_LEAFS][MAX_PORTS];
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            short mi=0;
            for(int ph=0;ph<m;++ph)
                if(in_load[leaf][pk][ph]>mi)mi=in_load[leaf][pk][ph];
            ns_max_in[leaf][pk]=mi;
        }
        ...
        while(g_start<ns_valid_cnt){
            ...
            if(gsz>=2&&gsz<=200){
                for(int a=g_start;a<g_end;++a){
                    int i=ns_keys[a].idx;
                    int pi=fl_port[i],ci=fl_src[i],dl_i=fl_dl[i];
                    if(pi<0) continue;
                    for(int b=a+1;b<g_end;++b){
                        ...
                        int dl_j=fl_dl[j];
                        if(dl_i!=dl_j){
                            unsigned int mk2=fl_pmask[i];int ok=1;
                            while(mk2){int ph=__builtin_ctz(mk2);
                                if(in_load[dl_i][pj][ph]+1>ns_max_in[dl_i][pj]){ok=0;break;}
                                mk2&=mk2-1;}
                            if(!ok) continue;
                            mk2=fl_pmask[j];
                            while(mk2){int ph=__builtin_ctz(mk2);
                                if(in_load[dl_j][pi][ph]+1>ns_max_in[dl_j][pi]){ok=0;break;}
                                mk2&=mk2-1;}
                            if(!ok) continue;
                        }
                        ...
                        if(after<before){
                            if(dl_i!=dl_j){
                                ...
                                while(mk2){int ph=__builtin_ctz(mk2);
                                    in_load[dl_i][pi][ph]--;in_load[dl_i][pj][ph]++;
                                    mk2&=mk2-1;}
                                mk2=fl_pmask[j];
                                while(mk2){int ph=__builtin_ctz(mk2);
                                    in_load[dl_j][pj][ph]--;in_load[dl_j][pi][ph]++;
                                    mk2&=mk2-1;}
                            }
