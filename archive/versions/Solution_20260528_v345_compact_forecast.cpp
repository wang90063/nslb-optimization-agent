...
// Buffer-based input: read all stdin into raw_buf, parse from pointer
#define RAW_BUF_SIZE (5*1024*1024)
static char raw_buf[RAW_BUF_SIZE];
static const char *rp;
inline int fast_read_int(){while(*rp<'0'||*rp>'9')rp++;int x=0;while(*rp>='0'&&*rp<='9'){x=x*10+(*rp-'0');rp++;}return x;}
...
static const char *job_parse_start; // saved parse position after header
...
void solve_job_forecast(int ji){
    int m=fast_read_int(),f=fast_read_int();
    fl_count=0; ht_clear();
    for(int ph=0;ph<m;++ph)
        for(int i=0;i<f;++i){
    ...
    for(int i=0;i<ht_used_cnt;++i){int hi=ht_key[ht_used[i]];seen_bits[hi>>3]&=~(1<<(hi&7));}
    for(int i=0;i<fl_count;++i){
        fl_sl[i]=fl_src[i]/g_pr;fl_dl[i]=fl_dst[i]/g_pr;fl_order[i]=i;
    }
    run_greedy(m, 2, 1, 1);
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
...
int main(){
    ...
    // Read all stdin into raw_buf
    raw_buf[0]=0;
    int total_read=0;
    while(1){
        int r=fread(raw_buf+total_read,1,RAW_BUF_SIZE-total_read-1,stdin);
        if(r<=0)break;
        total_read+=r;
    }
    raw_buf[total_read]=0;
    rp=raw_buf;
    int n=fast_read_int();
    g_n=n; g_job_idx=0; g_hist_max_jm=0;
    g_l=fast_read_int();g_p=fast_read_int();g_r=fast_read_int();g_pr=g_p*g_r;
    job_parse_start=rp; // save position after header
    if(n>=30){
        // Pass 1: forecast (lightweight greedy only)
        ...
        for(int ji=0;ji<n;++ji){
            g_job_idx=ji;
            solve_job_forecast(ji);
        }
        // Pass 2: full solve with leave-one-out
        rp=job_parse_start;
