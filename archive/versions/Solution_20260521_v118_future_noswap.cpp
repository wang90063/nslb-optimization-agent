...
void solve_job(){
    ...
    if(!huge_job){
        ...
        if(any_jm_le_r && fl_count<=3000 && best_eval.score_fg>g_r+1){
            ...
            // Structured moderate jobs sometimes need the shuffled trajectory
            // itself, not another swap/jm_repair pass that collapses it back
            // toward the same local minimum.
            for(int seed=38;seed<=41;++seed){
                for(int i=0;i<fl_count;++i) fl_order[i]=i;
                run_greedy(m, 1000, 1, 1, seed);
                TRY_STRATEGY();
            }
