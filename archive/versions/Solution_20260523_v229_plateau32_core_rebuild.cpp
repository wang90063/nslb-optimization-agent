...
static unsigned char plateau_out_leaf[MAX_LEAFS];
static unsigned char plateau_in_leaf[MAX_LEAFS];
static int plateau_fg_lb;
static int plateau_active_rows;
struct EvalMetrics{
    int score_jm;
    int score_fg;
    int jm;
...
static inline unsigned plateau_mix(unsigned x){
    x^=x>>16;
    x*=0x7feb352du;
    x^=x>>15;
    x*=0x846ca68bu;
...
// Relaxed swap: keep same-(sl,dl) grouping on r<=2 jobs, while r>=3 keeps the
// wider source-leaf grouping that helped the main proxy line.
...
static int count_current_fg_cells(int m,int target_fg){
    int cnt=0;
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            int mo=0,mi=0;
            ...
            if(global_out[leaf][pk]+mo==target_fg) cnt++;
            if(global_in[leaf][pk]+mi==target_fg) cnt++;
        }
    return cnt;
}

__attribute__((noinline,cold)) void run_plateau32_rebalance(int m){
    if(plateau_active_rows<2) return;
    if(fl_count>7000||(long long)fl_count*m>120000) return;

    EvalMetrics start_eval=collect_metrics(m);
    if(start_eval.fg<=plateau_fg_lb) return;
    int start_cells=count_current_fg_cells(m,start_eval.fg);

    memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
    memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
    memcpy(bk_port, fl_port, fl_count*sizeof(short));

    EvalMetrics cur_eval=start_eval;
    int cur_cells=start_cells;

    for(int iter=0;iter<6;++iter){
        int best_fi=-1,best_pk=-1;
        EvalMetrics best_eval=cur_eval;
        int best_cells=cur_cells;

        for(int fi=0;fi<fl_count;++fi){
            int sl=fl_sl[fi],dl=fl_dl[fi],cp=fl_port[fi];
            if(sl==dl||cp<0) continue;
            if(!plateau_out_leaf[sl]&&!plateau_in_leaf[dl]) continue;

            int cp_hot=0;
            int cp_out_fg=0,cp_in_fg=0;
            if(plateau_out_leaf[sl]){
                int mo=0;
                for(int ph=0;ph<m;++ph)
                    if(out_load[sl][cp][ph]>mo) mo=out_load[sl][cp][ph];
                cp_out_fg=global_out[sl][cp]+mo;
                if(cp_out_fg==cur_eval.fg) cp_hot=1;
            }
            if(plateau_in_leaf[dl]){
                int mi=0;
                for(int ph=0;ph<m;++ph)
                    if(in_load[dl][cp][ph]>mi) mi=in_load[dl][cp][ph];
                cp_in_fg=global_in[dl][cp]+mi;
                if(cp_in_fg==cur_eval.fg) cp_hot=1;
            }
            if(!cp_hot) continue;

            int cand_ports[16];
            int cand_cnt=0;
            unsigned char used[MAX_PORTS]={0};
            if(plateau_out_leaf[sl]){
                int vals[MAX_PORTS];
                for(int pk=0;pk<g_p;++pk){
                    int mo=0;
                    ...
                    vals[pk]=global_out[sl][pk]+mo;
                }
                for(int pick=0;pick<4;++pick){
                    int best=-1;
                    for(int pk=0;pk<g_p;++pk){
                        if(pk==cp||used[pk]) continue;
                        if(best<0||vals[pk]<vals[best]) best=pk;
                    }
                    if(best>=0){
                        cand_ports[cand_cnt++]=best;
                        used[best]=1;
                    }
                }
            }
            ...
            for(int ci=0;ci<cand_cnt;++ci){
                ...
                int cand_cells=count_current_fg_cells(m,cand_eval.fg);
                int accept=0;
                if(cand_eval.score_jm<best_eval.score_jm) accept=1;
                else if(cand_eval.score_jm==best_eval.score_jm&&cand_eval.score_fg<best_eval.score_fg) accept=1;
                else if(cand_eval.score_jm==best_eval.score_jm&&cand_eval.score_fg==best_eval.score_fg&&cand_cells<best_cells) accept=1;
                else if(cand_eval.score_jm==best_eval.score_jm&&cand_eval.score_fg==best_eval.score_fg&&cand_cells==best_cells&&better_metrics(cand_eval,best_eval)) accept=1;
                apply_flow_move(fi,pk,cp);
                if(accept){
                    best_eval=cand_eval;
                    best_cells=cand_cells;
                    best_fi=fi;
                    best_pk=pk;
                }
            }
        ...
        if(best_fi<0) break;
        apply_flow_move(best_fi,fl_port[best_fi],best_pk);
        cur_eval=best_eval;
        cur_cells=best_cells;
        if(cur_eval.fg<start_eval.fg) break;
    }

    EvalMetrics final_eval=collect_metrics(m);
    if(final_eval.score_jm>start_eval.score_jm||
       final_eval.score_fg>start_eval.score_fg||
       (final_eval.score_jm==start_eval.score_jm&&final_eval.score_fg==start_eval.score_fg&&
        !better_metrics(final_eval,start_eval))){
        memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
        memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
        memcpy(fl_port, bk_port, fl_count*sizeof(short));
    }
...
__attribute__((noinline,cold)) void run_plateau32_core_rebuild(int m){
    if(plateau_active_rows<2) return;
    long long job_work=(long long)fl_count*(long long)m;
    if(g_time_tight||fl_count>6500||job_work>120000) return;

    EvalMetrics base_eval=collect_metrics(m);
    if(base_eval.jm>g_r) return;
    if(base_eval.fg<=plateau_fg_lb) return;

    memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
    memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
    memcpy(bk_port, fl_port, fl_count*sizeof(short));

    short out_peak[MAX_LEAFS][MAX_PORTS];
    short in_peak[MAX_LEAFS][MAX_PORTS];
    memset(out_peak,0,sizeof(out_peak));
    memset(in_peak,0,sizeof(in_peak));
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            for(int ph=0;ph<m;++ph){
                if(bk_out[leaf][pk][ph]>out_peak[leaf][pk]) out_peak[leaf][pk]=bk_out[leaf][pk][ph];
                if(bk_in[leaf][pk][ph]>in_peak[leaf][pk]) in_peak[leaf][pk]=bk_in[leaf][pk][ph];
            }
        }

    int core_cap=96;
    if(fl_count<=2500&&job_work<=50000) core_cap=128;
    else if(fl_count>4500||job_work>80000) core_cap=64;
    int core_fi[128];
    int core_score[128];
    int core_cnt=0;
    ...
    for(int fi=0;fi<fl_count;++fi){
        int sl=fl_sl[fi],dl=fl_dl[fi],cp=fl_port[fi];
        if(sl==dl||cp<0) continue;
        int touches=plateau_out_leaf[sl]+plateau_in_leaf[dl];
        if(!touches) continue;

        unsigned int mask=fl_pmask[fi];
        int hot_level=0,phase_hits=0,row_hist=0;
        if(plateau_out_leaf[sl]){
            int outv=global_out[sl][cp]+out_peak[sl][cp];
            row_hist+=global_out[sl][cp];
            if(outv>=base_eval.fg-1) hot_level+=2+(outv-(base_eval.fg-1));
            else if(outv>=base_eval.fg-2) hot_level+=1;
            unsigned int mk=mask;
            while(mk){
                int ph=__builtin_ctz(mk);
                if(out_peak[sl][cp]&&bk_out[sl][cp][ph]+1>=out_peak[sl][cp]) phase_hits+=2;
                mk&=mk-1;
            }
        }
        if(plateau_in_leaf[dl]){
            int inv=global_in[dl][cp]+in_peak[dl][cp];
            row_hist+=global_in[dl][cp];
            if(inv>=base_eval.fg-1) hot_level+=2+(inv-(base_eval.fg-1));
            else if(inv>=base_eval.fg-2) hot_level+=1;
            unsigned int mk=mask;
            while(mk){
                int ph=__builtin_ctz(mk);
                if(in_peak[dl][cp]&&bk_in[dl][cp][ph]+1>=in_peak[dl][cp]) phase_hits+=2;
                mk&=mk-1;
            }
        }
        if(hot_level<=0&&phase_hits<=0) continue;

        int score=hot_level*4096+touches*1024+phase_hits*128+
                  row_hist*4+__builtin_popcount(mask)*8;
        if(core_cnt<core_cap){
            core_fi[core_cnt]=fi;
            core_score[core_cnt]=score;
            core_cnt++;
        } else if(score>core_score[core_cnt-1]){
            core_fi[core_cnt-1]=fi;
            core_score[core_cnt-1]=score;
        } else {
            continue;
        }
        for(int i=core_cnt-1;i>0&&core_score[i]>core_score[i-1];--i){
            int tf=core_fi[i],ts=core_score[i];
            core_fi[i]=core_fi[i-1];
            core_score[i]=core_score[i-1];
            core_fi[i-1]=tf;
            core_score[i-1]=ts;
        }
    }
    if(core_cnt<12) return;

    EvalMetrics best_trial_eval=base_eval;
    int improved=0;
    for(int trial=0;trial<2;++trial){
        ...
        int core_used_out[MAX_LEAFS][MAX_PORTS];
        int core_used_in[MAX_LEAFS][MAX_PORTS];
        int core_port_used[MAX_PORTS];
        memset(core_used_out,0,sizeof(core_used_out));
        memset(core_used_in,0,sizeof(core_used_in));
        memset(core_port_used,0,sizeof(core_port_used));
        ...
        for(int ord=0;ord<core_cnt;++ord){
            int fi=core_fi[(trial==0)?ord:(core_cnt-1-ord)];
            int sl=fl_sl[fi],dl=fl_dl[fi],cur_pk=bk_port[fi];
            unsigned int mask=fl_pmask[fi];
            unsigned int seed_base=plateau_mix((unsigned)g_job_idx*2246822519u ^
                                               (unsigned)sl*1315423911u ^
                                               (unsigned)dl*2654435761u ^
                                               mask*374761393u ^
                                               (unsigned)trial*668265263u);

            int best_pk=-1;
            int best_local=0x7fffffff,best_fg=0x7fffffff,best_spike=0x7fffffff;
            int best_hot=0x7fffffff,best_phase=0x7fffffff,best_core=0x7fffffff;
            int best_row=0x7fffffff,best_port=0x7fffffff,best_stay=0x7fffffff;
            int best_bias=0x7fffffff;

            for(int pk=0;pk<g_p;++pk){
                int local_peak=0,phase_cost=0,bad=0;
                int mo=0,mi=0;
                unsigned int mk=mask;
                while(mk){
                    int ph=__builtin_ctz(mk);
                    int o=out_load[sl][pk][ph]+1;
                    int iv=in_load[dl][pk][ph]+1;
                    if(o>g_r||iv>g_r){ bad=1; break; }
                    if(o>local_peak) local_peak=o;
                    if(iv>local_peak) local_peak=iv;
                    if(o>mo) mo=o;
                    if(iv>mi) mi=iv;
                    if(plateau_out_leaf[sl]) phase_cost+=out_load[sl][pk][ph];
                    if(plateau_in_leaf[dl]) phase_cost+=in_load[dl][pk][ph];
                    mk&=mk-1;
                }
                if(bad) continue;
                for(int ph=0;ph<m;++ph){
                ...
                int out_future=global_out[sl][pk]+mo;
                int in_future=global_in[dl][pk]+mi;
                int future_fg=out_future>in_future?out_future:in_future;
                int spike=0,hot_hits=0,row_hist=0;
                int core_hits=core_port_used[pk];
                if(plateau_out_leaf[sl]){
                    row_hist+=global_out[sl][pk];
                    core_hits+=core_used_out[sl][pk];
                    if(out_future>base_eval.fg) spike+=(out_future-base_eval.fg)*8;
                    if(out_future>=base_eval.fg) hot_hits+=2;
                    else if(out_future==base_eval.fg-1) hot_hits++;
                }
                if(plateau_in_leaf[dl]){
                    row_hist+=global_in[dl][pk];
                    core_hits+=core_used_in[dl][pk];
                    if(in_future>base_eval.fg) spike+=(in_future-base_eval.fg)*8;
                    if(in_future>=base_eval.fg) hot_hits+=2;
                    else if(in_future==base_eval.fg-1) hot_hits++;
                }
                int stay=(pk==cur_pk);
                int bias=(int)(plateau_mix(seed_base^(unsigned)pk*3266489917u)&1023u);

                if(local_peak<best_local||
                   (local_peak==best_local&&future_fg<best_fg)||
                   (local_peak==best_local&&future_fg==best_fg&&spike<best_spike)||
                   (local_peak==best_local&&future_fg==best_fg&&spike==best_spike&&hot_hits<best_hot)||
                   (local_peak==best_local&&future_fg==best_fg&&spike==best_spike&&hot_hits==best_hot&&phase_cost<best_phase)||
                   (local_peak==best_local&&future_fg==best_fg&&spike==best_spike&&hot_hits==best_hot&&phase_cost==best_phase&&core_hits<best_core)||
                   (local_peak==best_local&&future_fg==best_fg&&spike==best_spike&&hot_hits==best_hot&&phase_cost==best_phase&&core_hits==best_core&&row_hist<best_row)||
                   (local_peak==best_local&&future_fg==best_fg&&spike==best_spike&&hot_hits==best_hot&&phase_cost==best_phase&&core_hits==best_core&&row_hist==best_row&&core_port_used[pk]<best_port)||
                   (local_peak==best_local&&future_fg==best_fg&&spike==best_spike&&hot_hits==best_hot&&phase_cost==best_phase&&core_hits==best_core&&row_hist==best_row&&core_port_used[pk]==best_port&&stay<best_stay)||
                   (local_peak==best_local&&future_fg==best_fg&&spike==best_spike&&hot_hits==best_hot&&phase_cost==best_phase&&core_hits==best_core&&row_hist==best_row&&core_port_used[pk]==best_port&&stay==best_stay&&bias<best_bias)){
                    best_local=local_peak;
                    best_fg=future_fg;
                    best_spike=spike;
                    best_hot=hot_hits;
                    best_phase=phase_cost;
                    best_core=core_hits;
                    best_row=row_hist;
                    best_port=core_port_used[pk];
                    best_stay=stay;
                    best_bias=bias;
                    best_pk=pk;
                }
            ...
            if(plateau_out_leaf[sl]) core_used_out[sl][best_pk]++;
            if(plateau_in_leaf[dl]) core_used_in[dl][best_pk]++;
            core_port_used[best_pk]+=plateau_out_leaf[sl]+plateau_in_leaf[dl];
        }

        EvalMetrics cand_eval=collect_metrics(m);
        if(better_metrics(cand_eval,best_trial_eval)){
            best_trial_eval=cand_eval;
            improved=1;
    ...
    memcpy(sv_out, bk_out, g_l*sizeof(out_load[0]));
    memcpy(sv_in, bk_in, g_l*sizeof(in_load[0]));
    memcpy(sv_port, bk_port, fl_count*sizeof(short));
}
...
static int detect_plateau32_shape(short state_out[MAX_LEAFS][MAX_PORTS][MAX_PHASES],
                                  short state_in[MAX_LEAFS][MAX_PORTS][MAX_PHASES],
                                  int m, int fg_value){
    memset(plateau_out_leaf,0,sizeof(plateau_out_leaf));
    memset(plateau_in_leaf,0,sizeof(plateau_in_leaf));
    plateau_fg_lb=lowr_future_fg_lower_bound(m);
    plateau_active_rows=0;
    if(g_r!=4||g_p<32) return 0;
    if(fg_value-plateau_fg_lb<2) return 0;

    int width_need=(g_p*3)/4;
    if(width_need<24) width_need=24;
    for(int leaf=0;leaf<g_l;++leaf){
        int row_max=0,row_width=0;
        for(int pk=0;pk<g_p;++pk){
            int mo=0;
            for(int ph=0;ph<m;++ph)
                if(state_out[leaf][pk][ph]>mo) mo=state_out[leaf][pk][ph];
            int fv=global_out[leaf][pk]+mo;
            if(fv>row_max) row_max=fv;
        }
        if(row_max==fg_value){
            for(int pk=0;pk<g_p;++pk){
                int mo=0;
                for(int ph=0;ph<m;++ph)
                    if(state_out[leaf][pk][ph]>mo) mo=state_out[leaf][pk][ph];
                int fv=global_out[leaf][pk]+mo;
                if(fv>=row_max-1) row_width++;
            }
            if(row_width>=width_need){
                plateau_out_leaf[leaf]=1;
                plateau_active_rows++;
            }
        }

        row_max=0;row_width=0;
        for(int pk=0;pk<g_p;++pk){
            int mi=0;
            for(int ph=0;ph<m;++ph)
                if(state_in[leaf][pk][ph]>mi) mi=state_in[leaf][pk][ph];
            int fv=global_in[leaf][pk]+mi;
            if(fv>row_max) row_max=fv;
        }
        if(row_max==fg_value){
            for(int pk=0;pk<g_p;++pk){
            ...
            if(row_width>=width_need){
                plateau_in_leaf[leaf]=1;
                plateau_active_rows++;
            }
        }
    }
    return plateau_active_rows>=2;
}

void run_greedy_plateau32(int m, int hardcap, int rev=0, int sc_div=2){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    if(rev>=2){
        unsigned int seed=rev*2654435761u;
    ...
    int scale = g_r * g_n / sc_div;
    if(scale < 1) scale = 1;
    int plateau_job_used[MAX_PORTS]={0};
    for(int ii=0;ii<fl_count;++ii){
        int i;
        if(rev==0) i=ii;
        else if(rev==1) i=fl_count-1-ii;
        ...
        if(sl==dl){fl_port[i]=-1;continue;}
        unsigned int mask=fl_pmask[i];
        int touches_plateau=(plateau_out_leaf[sl]||plateau_in_leaf[dl]);
        unsigned int seed_base=plateau_mix((unsigned)g_job_idx*2246822519u ^
                                           (unsigned)sl*1315423911u ^
                                           (unsigned)dl*2654435761u ^
                                           mask*374761393u);

        int bp=0,best_local=0x7fffffff,best_fg=0x7fffffff;
        int best_row_hist=0x7fffffff,best_hist=0x7fffffff;
        int best_job_used=0x7fffffff,best_bias=0x7fffffff;
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            int fo=global_out[sl][pk]+mo;
            int fi=global_in[dl][pk]+mi;
            int future_fg=fo>fi?fo:fi;
            int hist=global_out[sl][pk]+global_in[dl][pk];
            int row_hist=0;
            if(plateau_out_leaf[sl]) row_hist+=global_out[sl][pk];
            if(plateau_in_leaf[dl]) row_hist+=global_in[dl][pk];
            int bias=(int)(plateau_mix(seed_base^(unsigned)pk*3266489917u)&1023u);
            int cost=local_max*2 + future_fg + future_fg*future_fg/scale;
            if(exceeds) cost+=10000;

            if(!touches_plateau){
                if(cost<best_cost||
                   (cost==best_cost&&(hist<best_hist||
                   (hist==best_hist&&bias<best_bias)))){
                    best_cost=cost;
                    best_hist=hist;
                    best_bias=bias;
                    bp=pk;
            ...
            if(local_max<best_local||
               (local_max==best_local&&future_fg<best_fg)||
               (local_max==best_local&&future_fg==best_fg&&row_hist<best_row_hist)||
               (local_max==best_local&&future_fg==best_fg&&row_hist==best_row_hist&&plateau_job_used[pk]<best_job_used)||
               (local_max==best_local&&future_fg==best_fg&&row_hist==best_row_hist&&plateau_job_used[pk]==best_job_used&&hist<best_hist)||
               (local_max==best_local&&future_fg==best_fg&&row_hist==best_row_hist&&plateau_job_used[pk]==best_job_used&&hist==best_hist&&bias<best_bias)){
                best_local=local_max;
                best_fg=future_fg;
                best_row_hist=row_hist;
                best_job_used=plateau_job_used[pk];
                best_hist=hist;
                best_bias=bias;
                bp=pk;
            }
        ...
        fl_port[i]=(short)bp;
        if(touches_plateau)
            plateau_job_used[bp]+=plateau_out_leaf[sl]+plateau_in_leaf[dl];
        unsigned int m2=mask;
        while(m2){int ph=__builtin_ctz(m2);out_load[sl][bp][ph]++;in_load[dl][bp][ph]++;m2&=m2-1;}
    }
}
...
void solve_job(){
    ...
    // Plateau32 family: when a wide p32/r4 case is stuck on a broad fg
    // plateau, use cross-job anti-correlation on near-equivalent ports instead
    // of the generic pressure-order reshaping.
    if(detect_plateau32_shape(sv_out, sv_in, m, best_eval.fg)){
        run_greedy_plateau32(m, 1);
        run_swap(m);
        run_global_swap(m);
        run_lns(m);
        TRY_STRATEGY();

        run_greedy_plateau32(m, 1, 1);
        run_swap(m);
        run_global_swap(m);
        run_lns(m);
        TRY_STRATEGY();
        ...
        memcpy(in_load, sv_in, g_l*sizeof(in_load[0]));
        memcpy(fl_port, sv_port, fl_count*sizeof(short));
        EvalMetrics plateau_core_base=collect_metrics(m);
        if(detect_plateau32_shape(out_load, in_load, m, plateau_core_base.fg)){
            run_plateau32_core_rebuild(m);
            run_global_swap(m);
            run_lns(m);
            TRY_STRATEGY();
        }
    ...
    if(g_r<=3&&g_p>=16&&fl_count<=6000){
        // Backup state for rollback if MM worsens
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        EvalMetrics mr_base=collect_metrics(m);
        ...
        if(total_cbt_before>0){
            for(int mr_iter=0;mr_iter<3;++mr_iter){
                ...
                for(int fi=0;fi<fl_count;++fi){
                    ...
                    if(sl==dl||cp<0) continue;
                    if(card_cbtphsc(ci,m)==0) continue;
                    // Find dominant port for this card
                    int dp_cnt[32]={};
                    for(int ph=0;ph<m;++ph)
                        if(cpm[ci][ph]) for(int pk=0;pk<g_p;++pk) if(cppc[ci][ph][pk]) dp_cnt[pk]+=cppc[ci][ph][pk];
                    int dp=-1,dp_best=0;
                    for(int pk=0;pk<g_p;++pk) if(dp_cnt[pk]>dp_best){dp_best=dp_cnt[pk];dp=pk;}
                    if(dp<0||cp==dp) continue;
                    // Check which phases of fi block moving to dp
                    unsigned int mask=fl_pmask[fi];
                    ...
                    if(!blocked){
                        // Can move directly - but PC should have caught this; skip
                        continue;
                    }
                    // Try to find a single blocker flow on dp that covers at least one blocked phase
                    int best_net=-1,best_bfi=-1,best_bap=-1;
                    for(int bfi=0;bfi<fl_count;++bfi){
                        if(bfi==fi) continue;
                        int bsl=fl_sl[bfi],bdl=fl_dl[bfi],bcp=fl_port[bfi];
                        if(bsl==bdl||bcp<0) continue;
                        // Blocker must be on dp for the relevant leaf side
                        int on_dp_sl=(bsl==sl&&bcp==dp);
                        int on_dp_dl=(bdl==dl&&bcp==dp);
                        if(!on_dp_sl&&!on_dp_dl) continue;
                        unsigned int bmask=fl_pmask[bfi];
                        // Check overlap with blocked phases
                        unsigned int overlap=bmask&blocked;
                        if(!overlap) continue;
                        // Try moving blocker to each alternative port
                        for(int ap=0;ap<g_p;++ap){
                            ...
                            if(!bok) continue;
                            // After moving blocker, check if fi can now move to dp
                            ...
                            if(net>best_net){best_net=net;best_bfi=bfi;best_bap=ap;}
                        }
                        if(best_net>0) break;
                    }
                    if(best_net>0){
                        ...
                        // Move blocker from dp to best_bap
                        unsigned int m3=bmask;while(m3){int ph=__builtin_ctz(m3);
                            out_load[bsl][dp][ph]--;in_load[bdl][dp][ph]--;
                        ...
                        // Move fi from cp to dp
                        m3=mask;while(m3){int ph=__builtin_ctz(m3);
                            out_load[sl][cp][ph]--;in_load[dl][cp][ph]--;
                            out_load[sl][dp][ph]++;in_load[dl][dp][ph]++;
                            cppc[ci][ph][cp]--;if(!cppc[ci][ph][cp])cpm[ci][ph]&=~(1u<<cp);
                            cppc[ci][ph][dp]++;cpm[ci][ph]|=(1u<<dp);m3&=m3-1;}
                        fl_port[fi]=(short)dp;
                        mr_improved++;
    ...
    // Narrow r=4 make-room consistency: keep the low-r path untouched and
    // only try the wider top-2 candidate search on proxy-like r=4 jobs.
    if(g_r==4&&g_l>=16&&g_p>=16&&fl_count<=4500&&job_work<=40000&&!g_time_tight){
        EvalMetrics mr_gate_base=collect_metrics(m);
        if(mr_gate_base.fg>=32){
            memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
            memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
            memcpy(bk_port, fl_port, fl_count*sizeof(short));
