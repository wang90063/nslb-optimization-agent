...
void run_sa_composite(int m){
    ...
    double sa_budget=(g_forecast_elapsed>0.01)?0.010:0.017;
    if(fl_count>5000) sa_budget=(g_forecast_elapsed>0.01)?0.005:0.01;
...
void solve_job(){
    ...
    // Hard time limit: if total elapsed > 3.0s, force time_tight
    if(elapsed_sec()>3.0) time_tight=1;
