...
static short card_pref_port[MAX_CARDS];
static int card_pref_done[MAX_CARDS];
static int card_pref_gen;

void run_greedy_cb_aware(int m, int cb_w=1, int rev=0){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    card_pref_gen++;
    if(rev>=2){
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        unsigned int mask=fl_pmask[i];
        int bp=0,bc=0x7fffffff;
        int has_pref=(card_pref_done[card]==card_pref_gen);
        int pref_pk=has_pref?card_pref_port[card]:-1;
        for(int pk=0;pk<g_p;++pk){
            int local_max=0,exceeds=0;
            unsigned int m2=mask;
            while(m2){
            ...
            if(exceeds) cost+=10000;
            if(has_pref&&pk!=pref_pk) cost+=cb_w;
            if(cost<bc){bc=cost;bp=pk;}
        }
        fl_port[i]=(short)bp;
        if(!has_pref){
            card_pref_done[card]=card_pref_gen;
            card_pref_port[card]=(short)bp;
        }
        unsigned int m2=mask;
        while(m2){int ph=__builtin_ctz(m2);out_load[sl][bp][ph]++;in_load[dl][bp][ph]++;m2&=m2-1;}
    }
...
void solve_job(){
    ...
    if(!huge_job){
        ...
        // SCB1: CB-aware greedy (cb_w=1) + swap
        run_greedy_cb_aware(m, 1);
        run_swap(m);
        run_jm_repair(m);
        TRY_STRATEGY();

        // SCB2: CB-aware greedy (cb_w=2) + swap
        run_greedy_cb_aware(m, 2);
        run_swap(m);
        run_jm_repair(m);
        TRY_STRATEGY();

        // SCB3: CB-aware greedy reversed (cb_w=1) + swap
        run_greedy_cb_aware(m, 1, 1);
        run_swap(m);
        run_jm_repair(m);
        TRY_STRATEGY();
