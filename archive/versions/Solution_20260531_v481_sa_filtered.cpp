...
void run_sa_composite(int m){
    ...
    for(int iter=0;;++iter){
        ...
        int sl=fl_sl[fi],dl=fl_dl[fi],px=fl_port[fi],ci=fl_src[fi];
        unsigned int mask=fl_pmask[fi];
        int feas[32]; int feas_cnt=0;
        for(int pp=0;pp<g_p;pp++){
            if(pp==px) continue;
            unsigned int m2=mask; int ok=1;
            while(m2){int ph=__builtin_ctz(m2);
                if(out_load[sl][pp][ph]+1>sa_max_out[sl][pp]){ok=0;break;}
                if(in_load[dl][pp][ph]+1>sa_max_in[dl][pp]){ok=0;break;}
                m2&=m2-1;}
            if(ok) feas[feas_cnt++]=pp;
        }
        if(!feas_cnt){T*=cool;continue;}
        rng=rng*1664525u+1013904223u;
        int py=feas[rng%feas_cnt];
        int ci_delta=0;
        unsigned int m2=mask;
        while(m2){int ph=__builtin_ctz(m2);
