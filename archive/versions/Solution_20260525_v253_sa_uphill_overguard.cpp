...
void run_sa_composite(int m){
    ...
    for(int iter=0;;++iter){
        ...
        if(obj_delta<=0) accept=1;
            ...
            int pre_mo_px=0,pre_mo_py=0,pre_mi_px=0,pre_mi_py=0;
            int post_mo_px=0,post_mo_py=0,post_mi_px=0,post_mi_py=0;
            for(int ph=0;ph<m;++ph){
                int out_px=out_load[sl][px][ph];
                int out_py=out_load[sl][py][ph];
                int in_px=in_load[dl][px][ph];
                int in_py=in_load[dl][py][ph];
                if(out_px>pre_mo_px) pre_mo_px=out_px;
                if(out_py>pre_mo_py) pre_mo_py=out_py;
                if(in_px>pre_mi_px) pre_mi_px=in_px;
                if(in_py>pre_mi_py) pre_mi_py=in_py;
                int take=(mask>>ph)&1u;
                out_px-=take; out_py+=take;
                in_px-=take; in_py+=take;
                if(out_px>post_mo_px) post_mo_px=out_px;
                if(out_py>post_mo_py) post_mo_py=out_py;
                if(in_px>post_mi_px) post_mi_px=in_px;
                if(in_py>post_mi_py) post_mi_py=in_py;
            }
            int pre_over=0,post_over=0;
            int pre_fo_px=global_out[sl][px]+pre_mo_px;
            int pre_fo_py=global_out[sl][py]+pre_mo_py;
            int pre_fi_px=global_in[dl][px]+pre_mi_px;
            int pre_fi_py=global_in[dl][py]+pre_mi_py;
            int post_fo_px=global_out[sl][px]+post_mo_px;
            int post_fo_py=global_out[sl][py]+post_mo_py;
            int post_fi_px=global_in[dl][px]+post_mi_px;
            int post_fi_py=global_in[dl][py]+post_mi_py;
            if(pre_fo_px>g_r) pre_over+=pre_fo_px-g_r;
            if(pre_fo_py>g_r) pre_over+=pre_fo_py-g_r;
            if(pre_fi_px>g_r) pre_over+=pre_fi_px-g_r;
            if(pre_fi_py>g_r) pre_over+=pre_fi_py-g_r;
            if(post_fo_px>g_r) post_over+=post_fo_px-g_r;
            if(post_fo_py>g_r) post_over+=post_fo_py-g_r;
            if(post_fi_px>g_r) post_over+=post_fi_px-g_r;
            if(post_fi_py>g_r) post_over+=post_fi_py-g_r;
            if(post_over<=pre_over){
                long long pre_sq=(long long)pre_fo_px*pre_fo_px+(long long)pre_fo_py*pre_fo_py+
                                 (long long)pre_fi_px*pre_fi_px+(long long)pre_fi_py*pre_fi_py;
                long long post_sq=(long long)post_fo_px*post_fo_px+(long long)post_fo_py*post_fo_py+
                                  (long long)post_fi_px*post_fi_px+(long long)post_fi_py*post_fi_py;
                rng=rng*1664525u+1013904223u;
                double r=(rng&0xFFFF)/65536.0;
                double temp=T;
                if(post_over==pre_over&&post_sq>pre_sq) temp*=0.7;
                if(r<__builtin_exp(-(double)obj_delta/temp)) accept=1;
            }
        }
        if(accept){
            m2=mask;
