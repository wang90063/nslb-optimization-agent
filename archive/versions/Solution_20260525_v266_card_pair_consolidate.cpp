...
// v266: per-card batch consolidate. For each source card with cbtphsc>0,
// attempt to move ALL of the card's non-dominant-port flows onto the dominant
// port atomically. Single-flow operators can't find this when each individual
// move alone exceeds destination max_phase but the joint move stays inside
// existing max_phase (shared phases cancel). Accept by explicit 12:5:3 score
// weights; jm/fg are guarded by the surrounding stage_accept transaction.
static int cc_card_score[MAX_CARDS];
static int cc_cards[MAX_CARDS];
static int cc_flows[MAX_FLOWS];
static short cc_add_out_dp[MAX_PHASES];
static short cc_add_in_dst_dp[MAX_LEAFS][MAX_PHASES];
static short cc_sub_out_old[MAX_PORTS][MAX_PHASES];
static short cc_sub_in_dst_old[MAX_LEAFS][MAX_PORTS][MAX_PHASES];
void run_r4_card_consolidate(int m){
    static short cc_max_out[MAX_LEAFS][MAX_PORTS];
    static short cc_max_in[MAX_LEAFS][MAX_PORTS];
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            short mo=0,mi=0;
            for(int ph=0;ph<m;++ph){
            ...
            cc_max_out[leaf][pk]=mo;cc_max_in[leaf][pk]=mi;
        }
    memset(cpm,0,sizeof(cpm));memset(cppc,0,sizeof(cppc));
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
    ...
    int n_cards=0;
    for(int c=0;c<12800;++c){
        int s=card_cbtphsc(c,m);
        if(s>0){cc_card_score[n_cards]=s;cc_cards[n_cards]=c;n_cards++;}
    }
    for(int i=1;i<n_cards;++i){
        int kc=cc_cards[i],ks=cc_card_score[i],j=i-1;
        while(j>=0&&cc_card_score[j]<ks){cc_cards[j+1]=cc_cards[j];cc_card_score[j+1]=cc_card_score[j];--j;}
        cc_cards[j+1]=kc;cc_card_score[j+1]=ks;
    }
    for(int ci_idx=0;ci_idx<n_cards;++ci_idx){
        int ci=cc_cards[ci_idx];
        ...
        int n_mv=0,sl=-1;
        for(int i=0;i<fl_count;++i){
            if(fl_src[i]!=ci) continue;
            if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
            if(fl_port[i]==dp) continue;
            if(sl<0) sl=fl_sl[i];
            cc_flows[n_mv++]=i;
        }
        if(n_mv<=0||sl<0) continue;
        // Build delta accumulators. Track touched ports/leaves for cleanup.
        unsigned int touched_old_ports=0;
        int touched_dst_list[MAX_LEAFS]; int n_tdl=0;
        unsigned char touched_dst_seen[MAX_LEAFS]={0};
        for(int ph=0;ph<m;++ph) cc_add_out_dp[ph]=0;
        for(int k=0;k<n_mv;++k){
            int fi=cc_flows[k];
            int op=fl_port[fi];
            int dl=fl_dl[fi];
            if(!(touched_old_ports&(1u<<op))){
                for(int ph=0;ph<m;++ph) cc_sub_out_old[op][ph]=0;
                touched_old_ports|=(1u<<op);
            }
            if(!touched_dst_seen[dl]){
                touched_dst_seen[dl]=1;
                touched_dst_list[n_tdl++]=dl;
                for(int ph=0;ph<m;++ph) cc_add_in_dst_dp[dl][ph]=0;
            }
            unsigned int mk=fl_pmask[fi];
            while(mk){int ph=__builtin_ctz(mk);
                cc_add_out_dp[ph]++;
                cc_add_in_dst_dp[dl][ph]++;
                cc_sub_out_old[op][ph]++;
                mk&=mk-1;}
        }
        // Per-(dl, op) sub_in_dst_old: clear on first touch then fill.
        // Track touched (dl, op) pairs via a bitset per dl.
        unsigned int touched_dst_op[MAX_LEAFS];
        for(int ti=0;ti<n_tdl;++ti) touched_dst_op[touched_dst_list[ti]]=0;
        for(int k=0;k<n_mv;++k){
            int fi=cc_flows[k];
            int op=fl_port[fi];
            int dl=fl_dl[fi];
            if(!(touched_dst_op[dl]&(1u<<op))){
                for(int ph=0;ph<m;++ph) cc_sub_in_dst_old[dl][op][ph]=0;
                touched_dst_op[dl]|=(1u<<op);
            }
            unsigned int mk=fl_pmask[fi];
            while(mk){int ph=__builtin_ctz(mk);
                cc_sub_in_dst_old[dl][op][ph]++;
                mk&=mk-1;}
        }
        // Feasibility: post-move loads must not exceed pre-move max_phase.
        int feasible=1;
        for(int ph=0;ph<m&&feasible;++ph){
            int new_o=out_load[sl][dp][ph]+cc_add_out_dp[ph];
            if(new_o>cc_max_out[sl][dp]){feasible=0;break;}
        }
        if(feasible){
            for(int ti=0;ti<n_tdl&&feasible;++ti){
                int dl=touched_dst_list[ti];
                for(int ph=0;ph<m;++ph){
                    int new_i=in_load[dl][dp][ph]+cc_add_in_dst_dp[dl][ph];
                    if(new_i>cc_max_in[dl][dp]){feasible=0;break;}
                }
            }
        }
        if(!feasible){
            for(int ti=0;ti<n_tdl;++ti) touched_dst_seen[touched_dst_list[ti]]=0;
            continue;
        }
        // ci_delta: Cinphsc change.
        int ci_delta=0;
        for(int ph=0;ph<m;++ph){
            int o=out_load[sl][dp][ph],no=o+cc_add_out_dp[ph];
            ci_delta+=(no>g_r?no-g_r:0)-(o>g_r?o-g_r:0);
        }
        for(int op=0;op<32;++op){
            if(!(touched_old_ports&(1u<<op))) continue;
            for(int ph=0;ph<m;++ph){
                int sub=cc_sub_out_old[op][ph];if(!sub) continue;
                int o=out_load[sl][op][ph],no=o-sub;
                ci_delta+=(no>g_r?no-g_r:0)-(o>g_r?o-g_r:0);
            }
        }
        for(int ti=0;ti<n_tdl;++ti){
            int dl=touched_dst_list[ti];
            for(int ph=0;ph<m;++ph){
                int o=in_load[dl][dp][ph],no=o+cc_add_in_dst_dp[dl][ph];
                ci_delta+=(no>g_r?no-g_r:0)-(o>g_r?o-g_r:0);
            }
            for(int op=0;op<32;++op){
                if(!(touched_dst_op[dl]&(1u<<op))) continue;
                for(int ph=0;ph<m;++ph){
                    int sub=cc_sub_in_dst_old[dl][op][ph];if(!sub) continue;
                    int o=in_load[dl][op][ph],no=o-sub;
                    ci_delta+=(no>g_r?no-g_r:0)-(o>g_r?o-g_r:0);
                }
            }
        }
        // cb_delta via tentative cppc/cpm mutation.
        int before_cb=card_cbtphsc(ci,m);
        for(int k=0;k<n_mv;++k){
            int fi=cc_flows[k];int op=fl_port[fi];unsigned int mk=fl_pmask[fi];
            while(mk){int ph=__builtin_ctz(mk);
                cppc[ci][ph][op]--;if(!cppc[ci][ph][op])cpm[ci][ph]&=~(1u<<op);
                cppc[ci][ph][dp]++;cpm[ci][ph]|=(1u<<dp);mk&=mk-1;}
        }
        int after_cb=card_cbtphsc(ci,m);
        for(int k=0;k<n_mv;++k){
            int fi=cc_flows[k];int op=fl_port[fi];unsigned int mk=fl_pmask[fi];
            while(mk){int ph=__builtin_ctz(mk);
                cppc[ci][ph][dp]--;if(!cppc[ci][ph][dp])cpm[ci][ph]&=~(1u<<dp);
                cppc[ci][ph][op]++;cpm[ci][ph]|=(1u<<op);mk&=mk-1;}
        }
        int cb_delta=before_cb-after_cb;
        // ct_delta: Cbttskc change via per-port max_phase × global accumulation.
        int ct_delta=0;
        {
            int new_max=cc_max_out[sl][dp];
            for(int ph=0;ph<m;++ph){
                int v=out_load[sl][dp][ph]+cc_add_out_dp[ph];
                if(v>new_max)new_max=v;
            }
            int cur=(global_out[sl][dp]+cc_max_out[sl][dp]>g_r)?(global_out[sl][dp]+cc_max_out[sl][dp]-g_r):0;
            int nw=(global_out[sl][dp]+new_max>g_r)?(global_out[sl][dp]+new_max-g_r):0;
            ct_delta+=cur-nw;
        }
        for(int op=0;op<32;++op){
            if(!(touched_old_ports&(1u<<op))) continue;
            int new_max=0;
            for(int ph=0;ph<m;++ph){
                int v=out_load[sl][op][ph]-cc_sub_out_old[op][ph];
                if(v>new_max)new_max=v;
            }
            int cur=(global_out[sl][op]+cc_max_out[sl][op]>g_r)?(global_out[sl][op]+cc_max_out[sl][op]-g_r):0;
            int nw=(global_out[sl][op]+new_max>g_r)?(global_out[sl][op]+new_max-g_r):0;
            ct_delta+=cur-nw;
        }
        for(int ti=0;ti<n_tdl;++ti){
            int dl=touched_dst_list[ti];
            int new_max=cc_max_in[dl][dp];
            for(int ph=0;ph<m;++ph){
                int v=in_load[dl][dp][ph]+cc_add_in_dst_dp[dl][ph];
                if(v>new_max)new_max=v;
            }
            int cur=(global_in[dl][dp]+cc_max_in[dl][dp]>g_r)?(global_in[dl][dp]+cc_max_in[dl][dp]-g_r):0;
            int nw=(global_in[dl][dp]+new_max>g_r)?(global_in[dl][dp]+new_max-g_r):0;
            ct_delta+=cur-nw;
            for(int op=0;op<32;++op){
                if(!(touched_dst_op[dl]&(1u<<op))) continue;
                int new_max2=0;
                for(int ph=0;ph<m;++ph){
                    int v=in_load[dl][op][ph]-cc_sub_in_dst_old[dl][op][ph];
                    if(v>new_max2)new_max2=v;
                }
                int cur2=(global_in[dl][op]+cc_max_in[dl][op]>g_r)?(global_in[dl][op]+cc_max_in[dl][op]-g_r):0;
                int nw2=(global_in[dl][op]+new_max2>g_r)?(global_in[dl][op]+new_max2-g_r):0;
                ct_delta+=cur2-nw2;
            }
        }
        long long score=(long long)5*cb_delta+(long long)3*ct_delta-(long long)12*ci_delta;
        if(score<=0){
            for(int ti=0;ti<n_tdl;++ti) touched_dst_seen[touched_dst_list[ti]]=0;
            continue;
        }
        // Commit.
        for(int k=0;k<n_mv;++k){
            int fi=cc_flows[k];int op=fl_port[fi];int dl=fl_dl[fi];unsigned int mk=fl_pmask[fi];
            while(mk){int ph=__builtin_ctz(mk);
                cppc[ci][ph][op]--;if(!cppc[ci][ph][op])cpm[ci][ph]&=~(1u<<op);
                cppc[ci][ph][dp]++;cpm[ci][ph]|=(1u<<dp);
                out_load[sl][op][ph]--;out_load[sl][dp][ph]++;
                in_load[dl][op][ph]--;in_load[dl][dp][ph]++;
                mk&=mk-1;}
            fl_port[fi]=(short)dp;
        }
        {short mo=0;for(int ph=0;ph<m;++ph)if(out_load[sl][dp][ph]>mo)mo=out_load[sl][dp][ph];cc_max_out[sl][dp]=mo;}
        for(int op=0;op<32;++op){
            if(!(touched_old_ports&(1u<<op))) continue;
            short mo=0;for(int ph=0;ph<m;++ph)if(out_load[sl][op][ph]>mo)mo=out_load[sl][op][ph];cc_max_out[sl][op]=mo;
        }
        for(int ti=0;ti<n_tdl;++ti){
            int dl=touched_dst_list[ti];
            {short mi=0;for(int ph=0;ph<m;++ph)if(in_load[dl][dp][ph]>mi)mi=in_load[dl][dp][ph];cc_max_in[dl][dp]=mi;}
            for(int op=0;op<32;++op){
                if(!(touched_dst_op[dl]&(1u<<op))) continue;
                short mi=0;for(int ph=0;ph<m;++ph)if(in_load[dl][op][ph]>mi)mi=in_load[dl][op][ph];cc_max_in[dl][op]=mi;
            }
            touched_dst_seen[dl]=0;
        }
    }
}
