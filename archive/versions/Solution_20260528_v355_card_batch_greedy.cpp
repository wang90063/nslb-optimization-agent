...
void run_greedy_card_batch(int m, int hardcap){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    static int card_flow_list[MAX_CARDS][200];
    static int card_flow_cnt[MAX_CARDS];
    static int card_phase_load_out[MAX_CARDS][MAX_PHASES];
    static int active_cards[MAX_CARDS];
    int num_cards=0;
    memset(card_flow_cnt,0,sizeof(card_flow_cnt));
    memset(card_phase_load_out,0,sizeof(card_phase_load_out));
    for(int i=0;i<fl_count;++i){
        int sl=fl_sl[i],dl=fl_dl[i];
        if(sl==dl){fl_port[i]=-1;continue;}
        int c=fl_src[i];
        if(card_flow_cnt[c]<200) card_flow_list[c][card_flow_cnt[c]]=i;
        if(card_flow_cnt[c]==0) active_cards[num_cards++]=c;
        card_flow_cnt[c]++;
        unsigned int mk=fl_pmask[i];
        while(mk){int ph=__builtin_ctz(mk);card_phase_load_out[c][ph]++;mk&=mk-1;}
    }
    for(int ci=0;ci<num_cards;++ci){
        int c=active_cards[ci];
        int cnt=card_flow_cnt[c];
        if(cnt==0) continue;
        int sl=fl_sl[card_flow_list[c][0]];
        int max_pl=0;
        for(int ph=0;ph<m;++ph) if(card_phase_load_out[c][ph]>max_pl) max_pl=card_phase_load_out[c][ph];
        if(max_pl<=g_r){
            int bp=-1,bc=0x7fffffff;
            for(int pk=0;pk<g_p;++pk){
                int feasible=1,cost=0;
                for(int fi_idx=0;fi_idx<cnt&&fi_idx<200;++fi_idx){
                    int fi=card_flow_list[c][fi_idx];
                    int dl=fl_dl[fi];
                    unsigned int mk=fl_pmask[fi];
                    while(mk){int ph=__builtin_ctz(mk);
                        int o=out_load[sl][pk][ph]+card_phase_load_out[c][ph];
                        int iv=in_load[dl][pk][ph]+1;
                        if(hardcap&&(o>g_r||iv>g_r)){feasible=0;break;}
                        int v=o>iv?o:iv;if(v>cost)cost=v;
                        mk&=mk-1;}
                    if(!feasible) break;
                }
                if(!feasible) continue;
                int gv=global_out[sl][pk]>global_in[fl_dl[card_flow_list[c][0]]][pk]?global_out[sl][pk]:global_in[fl_dl[card_flow_list[c][0]]][pk];
                cost=cost*2+gv;
                if(cost<bc){bc=cost;bp=pk;}
            }
            if(bp>=0){
                for(int fi_idx=0;fi_idx<cnt&&fi_idx<200;++fi_idx){
                    ...
                    fl_port[fi]=(short)bp;
                    unsigned int mk=fl_pmask[fi];
                    while(mk){int ph=__builtin_ctz(mk);out_load[sl][bp][ph]++;in_load[fl_dl[fi]][bp][ph]++;mk&=mk-1;}
                }
                continue;
            }
        }
        for(int fi_idx=0;fi_idx<cnt&&fi_idx<200;++fi_idx){
            ...
            int bp2=0,bc2=0x7fffffff;
            for(int pk=0;pk<g_p;++pk){
                int local_max=0,exceeds=0;
                unsigned int m2=mask;
                while(m2){int ph=__builtin_ctz(m2);
                    int o=out_load[sl][pk][ph]+1;int iv=in_load[dl][pk][ph]+1;
                    int v=o>iv?o:iv;if(v>local_max)local_max=v;
                    if(hardcap&&(o>g_r||iv>g_r))exceeds=1;
                    m2&=m2-1;}
                int gv=global_out[sl][pk]>global_in[dl][pk]?global_out[sl][pk]:global_in[dl][pk];
                int cost2=local_max*2+gv;
                if(exceeds) cost2+=10000;
                if(cost2<bc2){bc2=cost2;bp2=pk;}
            }
            fl_port[fi]=(short)bp2;
            unsigned int m2=mask;
            while(m2){int ph=__builtin_ctz(m2);out_load[sl][bp2][ph]++;in_load[dl][bp2][ph]++;m2&=m2-1;}
        }
    }
}
...
void solve_job(){
    ...
    // Card-batch greedy: assign entire cards to ports for CB=0
    run_greedy_card_batch(m, 1);
    run_swap(m);
    TRY_STRATEGY();

    run_greedy_card_batch(m, 0);
    run_swap(m);
    TRY_STRATEGY();
