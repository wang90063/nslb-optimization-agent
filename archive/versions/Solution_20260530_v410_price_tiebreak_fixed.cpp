            ...
            if(gsz>=2&&gsz<=rs_gsz_cap){
                for(int a=g_start;a<g_end;++a){
                    ...
                    for(int b=a+1;b<g_end;++b){
                        ...
                        if(!take_swap&&after==before&&dl_i!=dl_j){
                            int price_after=joint_price_bias(sl,dl_i,pj)+joint_price_bias(sl,dl_j,pi);
                            int price_before=joint_price_bias(sl,dl_i,pi)+joint_price_bias(sl,dl_j,pj);
                            if(price_after<price_before) take_swap=1;
                        }
                        if(take_swap){
                            // Commit: update loads
                            if(dl_i==dl_j){
