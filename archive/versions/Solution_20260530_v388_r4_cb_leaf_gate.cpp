...
void solve_job(){
    ...
    // For r=4, target the CB-dominant non-overflow leaf directly instead of using
    // a high-fg proxy gate, which is mostly dead on the p16/p32 main window.
    EvalMetrics mr_gate_base=collect_metrics(m);
    int enable_r4_makeroom=(g_r==4&&g_p>=16&&fl_count<=4500&&job_work<=40000&&!g_time_tight&&mr_gate_base.jm<=g_r&&mr_gate_base.ci==0);
