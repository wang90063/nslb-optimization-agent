...
void run_cross_src_swap(int m){
    static short cs_mo[MAX_LEAFS][MAX_PORTS],cs_mi[MAX_LEAFS][MAX_PORTS];
    for(int iter=0;iter<3;++iter){
        for(int lf=0;lf<g_l;++lf)for(int pk=0;pk<g_p;++pk){short mo=0,mi=0;for(int ph=0;ph<m;++ph){if(out_load[lf][pk][ph]>mo)mo=out_load[lf][pk][ph];if(in_load[lf][pk][ph]>mi)mi=in_load[lf][pk][ph];}cs_mo[lf][pk]=mo;cs_mi[lf][pk]=mi;}
        memset(cpm,0,sizeof(cpm));memset(cppc,0,sizeof(cppc));
        for(int i=0;i<fl_count;++i){if(fl_sl[i]==fl_dl[i]||fl_port[i]<0)continue;int c=fl_src[i],p=fl_port[i];unsigned int mk=fl_pmask[i];while(mk){int ph=__builtin_ctz(mk);cppc[c][ph][p]++;cpm[c][ph]|=(1u<<p);mk&=mk-1;}}
        int moved=0;
        for(int fi=0;fi<fl_count;++fi){
            int sl_a=fl_sl[fi],dl=fl_dl[fi],px=fl_port[fi],ci=fl_src[fi];
            if(sl_a==dl||px<0)continue;
            if(card_cbtphsc(ci,m)==0)continue;
            int dp_cnt[32]={};for(int ph=0;ph<m;++ph)for(int pk=0;pk<g_p;++pk)dp_cnt[pk]+=cppc[ci][ph][pk];
            int dp=-1,dpb=0;for(int pk=0;pk<g_p;++pk)if(dp_cnt[pk]>dpb){dpb=dp_cnt[pk];dp=pk;}
            if(dp<0||px==dp)continue;
            int py=dp,best_wt=0,best_j=-1;
            for(int fj=fi+1;fj<fl_count;++fj){
                if(fl_dl[fj]!=dl||fl_port[fj]!=py||fl_sl[fj]==sl_a)continue;
                if(fl_sl[fj]==fl_dl[fj]||fl_port[fj]<0)continue;
                int cj=fl_src[fj];if(cj==ci)continue;
                int sl_b=fl_sl[fj];
                unsigned int ma=fl_pmask[fi],mb=fl_pmask[fj];
                unsigned int oa=ma&~mb,ob=mb&~ma;
                int ok=1;unsigned int m2;
                m2=oa;while(m2){int ph=__builtin_ctz(m2);if(out_load[sl_a][py][ph]+1>cs_mo[sl_a][py]){ok=0;break;}m2&=m2-1;}
                if(!ok)continue;
                m2=ob;while(m2){int ph=__builtin_ctz(m2);if(out_load[sl_b][px][ph]+1>cs_mo[sl_b][px]){ok=0;break;}m2&=m2-1;}
                if(!ok)continue;
                m2=oa;while(m2){int ph=__builtin_ctz(m2);if(in_load[dl][py][ph]+1>cs_mi[dl][py]){ok=0;break;}m2&=m2-1;}
                if(!ok)continue;
                m2=ob;while(m2){int ph=__builtin_ctz(m2);if(in_load[dl][px][ph]+1>cs_mi[dl][px]){ok=0;break;}m2&=m2-1;}
                if(!ok)continue;
                int bef=card_cbtphsc(ci,m)+card_cbtphsc(cj,m);
                m2=ma;while(m2){int ph=__builtin_ctz(m2);cppc[ci][ph][px]--;if(!cppc[ci][ph][px])cpm[ci][ph]&=~(1u<<px);cppc[ci][ph][py]++;cpm[ci][ph]|=(1u<<py);m2&=m2-1;}
                m2=mb;while(m2){int ph=__builtin_ctz(m2);cppc[cj][ph][py]--;if(!cppc[cj][ph][py])cpm[cj][ph]&=~(1u<<py);cppc[cj][ph][px]++;cpm[cj][ph]|=(1u<<px);m2&=m2-1;}
                int aft=card_cbtphsc(ci,m)+card_cbtphsc(cj,m);
                int wt=bef-aft;
                if(wt>best_wt){best_wt=wt;best_j=fj;}
                m2=mb;while(m2){int ph=__builtin_ctz(m2);cppc[cj][ph][px]--;if(!cppc[cj][ph][px])cpm[cj][ph]&=~(1u<<px);cppc[cj][ph][py]++;cpm[cj][ph]|=(1u<<py);m2&=m2-1;}
                m2=ma;while(m2){int ph=__builtin_ctz(m2);cppc[ci][ph][py]--;if(!cppc[ci][ph][py])cpm[ci][ph]&=~(1u<<py);cppc[ci][ph][px]++;cpm[ci][ph]|=(1u<<px);m2&=m2-1;}
            }
            if(best_j<0)continue;
            int fj=best_j,sl_b=fl_sl[fj],cj=fl_src[fj];
            unsigned int ma=fl_pmask[fi],mb=fl_pmask[fj];
            unsigned int m2=ma;while(m2){int ph=__builtin_ctz(m2);out_load[sl_a][px][ph]--;out_load[sl_a][py][ph]++;in_load[dl][px][ph]--;in_load[dl][py][ph]++;cppc[ci][ph][px]--;if(!cppc[ci][ph][px])cpm[ci][ph]&=~(1u<<px);cppc[ci][ph][py]++;cpm[ci][ph]|=(1u<<py);m2&=m2-1;}
            m2=mb;while(m2){int ph=__builtin_ctz(m2);out_load[sl_b][py][ph]--;out_load[sl_b][px][ph]++;in_load[dl][py][ph]--;in_load[dl][px][ph]++;cppc[cj][ph][py]--;if(!cppc[cj][ph][py])cpm[cj][ph]&=~(1u<<py);cppc[cj][ph][px]++;cpm[cj][ph]|=(1u<<px);m2&=m2-1;}
            fl_port[fi]=(short)py;fl_port[fj]=(short)px;
            moved++;
        }
        if(!moved)break;
    }
