...
static short card_dom_port[MAX_CARDS]; // dominant port per card during greedy
static short card_port_cnt[MAX_CARDS][MAX_PORTS]; // per-card port usage count

void run_greedy(int m, int local_w, int global_w, int hardcap, int rev=0){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    memset(card_dom_port,-1,sizeof(card_dom_port));
    memset(card_port_cnt,0,sizeof(card_port_cnt));
    if(rev>=2){
        unsigned int seed=rev*2654435761u;
        for(int a=fl_count-1;a>0;--a){
            seed=seed*1664525u+1013904223u;
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            if(cost<bc){
                ...
                int ci=fl_src[i];
                int cur_card_cnt=card_port_cnt[ci][bp];
                int new_card_cnt=card_port_cnt[ci][pk];
                if(new_card_cnt>cur_card_cnt){
                    bp=pk;
                    bt_over=cand_over;
                    bt_sq=cand_sq;
                    bt_sum=cand_sum;
                } else if(new_card_cnt==cur_card_cnt && use_future_tie){
