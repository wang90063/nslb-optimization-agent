...
static short card_home_port[MAX_CARDS];

void run_greedy_cb_first(int m){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    static int card_load_contrib[MAX_CARDS][MAX_PORTS];
    memset(card_load_contrib,0,sizeof(card_load_contrib));
    for(int i=0;i<fl_count;++i){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            while(mk){
                ...
                int go=global_out[fl_sl[i]][pk];
                int gi=global_in[fl_dl[i]][pk];
                int v=go>gi?go:gi;
                if(v>mx) mx=v;
                mk&=mk-1;
            }
            card_load_contrib[c][pk]+=mx;
        }
    }
    for(int c=0;c<g_l*g_p*g_r;++c){
        int bp=0,bc=0x7fffffff;
        for(int pk=0;pk<g_p;++pk){
            if(card_load_contrib[c][pk]<bc){
                bc=card_load_contrib[c][pk];bp=pk;
            }
        }
        card_home_port[c]=(short)bp;
    }
    for(int i=0;i<fl_count;++i){
        int sl=fl_sl[i],dl=fl_dl[i];
        if(sl==dl){fl_port[i]=-1;continue;}
        int bp=(int)card_home_port[fl_src[i]];
        fl_port[i]=(short)bp;
...
void solve_job(){
    ...
    if(!huge_job){
        ...
        // SCB_FIRST: CB-first greedy (all card flows on one port) + swap
        run_greedy_cb_first(m);
        run_swap(m);
        run_jm_repair(m);
        TRY_STRATEGY();
