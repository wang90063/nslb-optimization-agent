...
void run_cross_dest_swap(int m){
    ...
    for(int iter=0;iter<cd_max_iter;++iter){
        ...
        for(int fi=0;fi<fl_count;++fi){
            ...
            const int CD_MAXK=5;
            int cand_ports[CD_MAXK];
            int cand_score[CD_MAXK];
            for(int t=0;t<CD_MAXK;++t){cand_ports[t]=-1;cand_score[t]=-0x7fffffff;}
            int cand_limit=(g_p>=32)?5:((g_p>=16)?2:1);
            for(int pk=0;pk<g_p;++pk){
                int cnt=dp_cnt[pk];
                int score=cnt*256-joint_price_bias(sl,dl_a,pk);
                if(score<=cand_score[cand_limit-1]) continue;
                int t=cand_limit-1;
                while(t>0&&score>cand_score[t-1]){
                    cand_score[t]=cand_score[t-1];
                    cand_ports[t]=cand_ports[t-1];
                    t--;
                }
                cand_score[t]=score;
                cand_ports[t]=pk;
