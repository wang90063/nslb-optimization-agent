...
static int fl_count;
// Job reorder buffering
#define MAX_TOTAL_FLOWS 500000
...
void solve_job(){
    ...
    {int jj=job_order[g_job_idx];
    int oo=job_offset[jj];
    for(int i=0;i<fl_count;++i) buf_port[oo+i]=fl_port[i];}
}
...
int main(){
    ...
    // Read all jobs into buffer
    buf_total_flows=0;
    for(int i=0;i<n;++i) read_job_to_buffer(i);
    // Compute heaviness: max phase load across all (leaf,phase) for each job
    // Use total flows as proxy for heaviness (correlates with CT contribution)
    for(int i=0;i<n;++i) job_order[i]=i;
    // Sort by increasing fl_count (lighter jobs first establish baseline)
    for(int i=0;i<n-1;++i)
        for(int j=i+1;j<n;++j)
            if(job_flcount[job_order[j]]<job_flcount[job_order[i]]){
                int t=job_order[i];job_order[i]=job_order[j];job_order[j]=t;
            }
    // Solve in sorted order
