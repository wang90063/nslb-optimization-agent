...
void run_greedy_global_price(int m, int hardcap, int rev=0, int price_w=1, int local_w=2, int global_w=1, int src_price_only=0){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    if(rev>=2){
        unsigned int seed=rev*2654435761u;
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            if(src_price_only){
                price_bundle=price_term(sl, pk)*price_w+src_price;
            }
