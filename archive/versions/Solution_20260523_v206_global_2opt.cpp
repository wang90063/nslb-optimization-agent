    ...
    for(int iter=0;iter<15;++iter){
        ...
        for(int leaf=0;leaf<g_l;++leaf)
            for(int pk=0;pk<g_p;++pk){
                ...
                int fo=global_out[leaf][pk]+mo;
                int fi=global_in[leaf][pk]+mi;
                if(fo==cur_gmax){bn_leaf=leaf;bn_pk=pk;bn_dir=0;goto found_bn;}
                if(fi==cur_gmax){bn_leaf=leaf;bn_pk=pk;bn_dir=1;goto found_bn;}
            }
        found_bn:
        if(bn_leaf<0) break;
        int improved=0;
        for(int i=0;i<fl_count&&!improved;++i){
            ...
            if(cp!=bn_pk) continue;
            if(bn_dir==0&&sl!=bn_leaf) continue;
            if(bn_dir==1&&dl!=bn_leaf) continue;
            unsigned int mi_mask=fl_pmask[i];
            for(int j=0;j<fl_count&&!improved;++j){
                if(j==i) continue;
                int sl2=fl_sl[j],dl2=fl_dl[j],cp2=fl_port[j];
                if(sl2==dl2||cp2<0||cp2==cp) continue;
                ...
                while(m2){
                    int ph=__builtin_ctz(m2);
                    int no=out_load[sl][cp2][ph]+1;
                    int ni=in_load[dl][cp2][ph]+1;
                    if(no>pre_jmax||ni>pre_jmax){ok=0;break;}
                    m2&=m2-1;
                }
                if(!ok) continue;
                ...
                while(m2){
                    int ph=__builtin_ctz(m2);
                    int no=out_load[sl2][cp][ph]+1;
                    int ni=in_load[dl2][cp][ph]+1;
                    if(no>pre_jmax||ni>pre_jmax){ok=0;break;}
                    m2&=m2-1;
                }
                if(!ok) continue;
                ...
                while(m2){int ph=__builtin_ctz(m2);
                    out_load[sl][cp][ph]--;in_load[dl][cp][ph]--;
                    out_load[sl][cp2][ph]++;in_load[dl][cp2][ph]++;m2&=m2-1;}
                m2=mj_mask;
                while(m2){int ph=__builtin_ctz(m2);
                    out_load[sl2][cp2][ph]--;in_load[dl2][cp2][ph]--;
                    out_load[sl2][cp][ph]++;in_load[dl2][cp][ph]++;m2&=m2-1;}
                fl_port[i]=(short)cp2;fl_port[j]=(short)cp;
                int ng=get_future_gmax(m);
                if(ng<cur_gmax&&get_job_max(m)<=pre_jmax){
                    improved=1;
                } else {
                    m2=mi_mask;
                    while(m2){int ph=__builtin_ctz(m2);
                        out_load[sl][cp2][ph]--;in_load[dl][cp2][ph]--;
                        out_load[sl][cp][ph]++;in_load[dl][cp][ph]++;m2&=m2-1;}
                    m2=mj_mask;
                    while(m2){int ph=__builtin_ctz(m2);
                        out_load[sl2][cp][ph]--;in_load[dl2][cp][ph]--;
                        out_load[sl2][cp2][ph]++;in_load[dl2][cp2][ph]++;m2&=m2-1;}
                    fl_port[i]=(short)cp;fl_port[j]=(short)cp2;
                }
            }
        }
        if(!improved) break;
...
void run_relaxed_swap(int m){
    ...
    for(int iter=0;iter<rs_max_iter;++iter){
        ...
        while(g_start<rs_valid_cnt){
            int g_end=g_start+1;
            while(g_end<rs_valid_cnt&&rs_keys[g_end].sl==rs_keys[g_start].sl) g_end++;
            int gsz=g_end-g_start;
            if(gsz>=2&&gsz<=300){
                for(int a=g_start;a<g_end;++a){
                    int i=rs_keys[a].idx;
...
void solve_job(){
    ...
    // S11: global-dominant (local*1, global*3, hardcap) + swap + global swap + 2opt + lns
    run_greedy(m, 1, 3, 1);
    run_swap(m);
    run_global_swap(m);
    run_global_2opt(m);
    ...
    // S12: global-dominant reversed + swap + global swap + 2opt + lns
    ...
    // S13: extreme global (local*1, global*5, hardcap) + swap + global swap + 2opt + lns
    run_greedy(m, 1, 5, 1);
    run_swap(m);
    run_global_swap(m);
    run_global_2opt(m);
    ...
    // S14: future-gmax greedy (hardcap) + swap + global swap + 2opt + lns
    ...
    // S15: future-gmax greedy reversed (hardcap) + swap + global swap + 2opt + lns
    ...
    // S16: FTRL (sc_div=2) + global swap + 2opt + lns
    ...
    // S17: FTRL strong (sc_div=4) + global swap + 2opt + lns
    run_greedy_ftrl(m, 1, 0, 4);
    run_swap(m);
    run_global_swap(m);
    run_global_2opt(m);
    ...
    // S18: FTRL reversed (sc_div=2) + global swap + 2opt + lns
    ...
    if(!time_tight && (restored_jm<=g_r || job_work<=120000)) run_global_2opt(m);
