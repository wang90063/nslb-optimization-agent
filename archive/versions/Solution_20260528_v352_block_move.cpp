...
void run_block_move(int m){
    if(g_time_tight) return;
    // For each (source_leaf, port) pair, try moving all its flows to another port
    static int blk_flows[MAX_FLOWS];
    for(int sl=0;sl<g_l;++sl){
        for(int px=0;px<g_p;++px){
            // Collect all flows from (sl, px)
            int bcnt=0;
            for(int i=0;i<fl_count;++i){
                if(fl_sl[i]==sl&&fl_port[i]==px&&fl_sl[i]!=fl_dl[i])
                    blk_flows[bcnt++]=i;
            }
            if(bcnt<2||bcnt>200) continue;
            // Try each target port
            for(int py=0;py<g_p;++py){
                if(py==px) continue;
                // Check feasibility: all flows must fit on py without exceeding current max
                int feasible=1;
                // Compute load delta on (sl, py) for out_load
                // and on each (dl, py) for in_load
                unsigned int m2;
                for(int bi=0;bi<bcnt;++bi){
                    ...
                    while(m2){
                        int ph=__builtin_ctz(m2);
                        // We need cumulative check - simplified: just check current load
                        if(out_load[sl][py][ph]>=g_r){feasible=0;break;}
                        if(in_load[dl][py][ph]>=g_r){feasible=0;break;}
                        m2&=m2-1;
                    }
                    if(!feasible) break;
                }
                if(!feasible) continue;
                // Simulate: compute CB delta
                // Save card cbt before
                static int seen_cards[MAX_CARDS];
                static int card_list[1000];
                int card_cnt=0;
                for(int bi=0;bi<bcnt;++bi){
                    int c=fl_src[blk_flows[bi]];
                    if(!seen_cards[c]){seen_cards[c]=1;card_list[card_cnt++]=c;}
                }
                int cb_before=0;
                for(int ci=0;ci<card_cnt;++ci) cb_before+=card_cbtphsc(card_list[ci],m);
                // Apply move temporarily to cpm/cppc
                for(int bi=0;bi<bcnt;++bi){
                    int fi=blk_flows[bi],c=fl_src[fi];
                    m2=fl_pmask[fi];
                    while(m2){int ph=__builtin_ctz(m2);
                        cppc[c][ph][px]--;if(!cppc[c][ph][px])cpm[c][ph]&=~(1u<<px);
                        cppc[c][ph][py]++;cpm[c][ph]|=(1u<<py);m2&=m2-1;}
                }
                int cb_after=0;
                for(int ci=0;ci<card_cnt;++ci) cb_after+=card_cbtphsc(card_list[ci],m);
                int cb_delta=cb_after-cb_before;
                if(cb_delta<0){
                    // Check fg safety: compute new max on (sl,py) and compare
                    short new_max_out=0;
                    for(int ph=0;ph<m;++ph){
                        short v=out_load[sl][py][ph];
                        for(int bi=0;bi<bcnt;++bi)
                            if(fl_pmask[blk_flows[bi]]&(1u<<ph)) v++;
                        if(v>new_max_out) new_max_out=v;
                    }
                    int cur_fg_out=global_out[sl][py];
                    short cur_max_out=0;
                    for(int ph=0;ph<m;++ph)
                        if(out_load[sl][py][ph]>cur_max_out) cur_max_out=out_load[sl][py][ph];
                    if(cur_fg_out+new_max_out > cur_fg_out+cur_max_out){
                        // fg might worsen - check if it exceeds current global fg
                        int cur_fg=get_future_gmax(m);
                        // Only accept if new fg <= cur_fg
                        int test_fg=cur_fg_out+new_max_out;
                        if(test_fg>cur_fg) cb_delta=0; // block the move
                    }
                }
                if(cb_delta<0){
                    // Accept: apply load changes
                    for(int bi=0;bi<bcnt;++bi){
                        int fi=blk_flows[bi],dl=fl_dl[fi];
                        m2=fl_pmask[fi];
                        while(m2){int ph=__builtin_ctz(m2);
                            out_load[sl][px][ph]--;out_load[sl][py][ph]++;
                            in_load[dl][px][ph]--;in_load[dl][py][ph]++;m2&=m2-1;}
                    ...
                    for(int ci=0;ci<card_cnt;++ci) seen_cards[card_list[ci]]=0;
                    break; // move to next (sl, px) pair
                } else {
                    // Revert cpm/cppc
                    for(int bi=0;bi<bcnt;++bi){
                        int fi=blk_flows[bi],c=fl_src[fi];
                        m2=fl_pmask[fi];
                        while(m2){int ph=__builtin_ctz(m2);
                            cppc[c][ph][py]--;if(!cppc[c][ph][py])cpm[c][ph]&=~(1u<<py);
                            cppc[c][ph][px]++;cpm[c][ph]|=(1u<<px);m2&=m2-1;}
                    }
                    for(int ci=0;ci<card_cnt;++ci) seen_cards[card_list[ci]]=0;
                }
            }
...
void solve_job(){
    ...
    // Block move: try moving entire (leaf, port) blocks
    if(!g_time_tight){
        memset(cpm,0,sizeof(cpm));memset(cppc,0,sizeof(cppc));
        for(int i=0;i<fl_count;++i){
            if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
        ...
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        EvalMetrics bm_base=collect_metrics(m);
        run_block_move(m);
        EvalMetrics bm_after=collect_metrics(m);
        if(bm_after.fg>bm_base.fg||(bm_after.fg==bm_base.fg&&bm_after.jm>bm_base.jm)){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));
        }
