...
void run_sa_composite(int m){
    ...
    for(int iter=0;;++iter){
        ...
        // Global pressure: prefer moves to ports with lower global cumulative load
        int gp_src=global_out[sl][px]+global_in[dl][px];
        int gp_dst=global_out[sl][py]+global_in[dl][py];
        int gp_delta=(gp_dst>gp_src)?1:((gp_dst<gp_src)?-1:0);
        int obj_delta=12*ci_delta+5*cbt_delta+3*gp_delta;
        int accept=0;
