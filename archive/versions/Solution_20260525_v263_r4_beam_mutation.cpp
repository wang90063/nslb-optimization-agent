...
#define R4_BEAM_WIDTH 4
#define R4_BEAM_CAND 24
...
static short r4_beam_out[R4_BEAM_WIDTH][MAX_LEAFS][MAX_PORTS][MAX_PHASES];
static short r4_beam_in[R4_BEAM_WIDTH][MAX_LEAFS][MAX_PORTS][MAX_PHASES];
static short r4_beam_port[R4_BEAM_WIDTH][MAX_FLOWS];
static short r4_cand_out[R4_BEAM_CAND][MAX_LEAFS][MAX_PORTS][MAX_PHASES];
static short r4_cand_in[R4_BEAM_CAND][MAX_LEAFS][MAX_PORTS][MAX_PHASES];
static short r4_cand_port[R4_BEAM_CAND][MAX_FLOWS];
struct EvalMetrics{
    int score_jm;
    int score_fg;
    int jm;
...
struct LocalFutureDelta{
    int fg_delta;
    int over_delta;
    long long sq_delta;
};

static inline void add_unique_local_pair(int leaf,int port,int *leafs,int *ports,int &cnt){
    for(int i=0;i<cnt;++i)
        if(leafs[i]==leaf&&ports[i]==port) return;
    leafs[cnt]=leaf;
    ports[cnt]=port;
    cnt++;
}

static inline void collect_local_future_stats(int m,
                                              const int *out_leafs,const int *out_ports,int out_cnt,
                                              const int *in_leafs,const int *in_ports,int in_cnt,
                                              int &fg,int &over,long long &sq){
    fg=0;
    over=0;
    sq=0;
    for(int i=0;i<out_cnt;++i){
        int leaf=out_leafs[i],pk=out_ports[i];
        int mo=0;
        for(int ph=0;ph<m;++ph)
            if(out_load[leaf][pk][ph]>mo) mo=out_load[leaf][pk][ph];
        int fv=global_out[leaf][pk]+mo;
        if(fv>fg) fg=fv;
        if(fv>g_r) over+=fv-g_r;
        sq+=(long long)fv*fv;
    }
    for(int i=0;i<in_cnt;++i){
        int leaf=in_leafs[i],pk=in_ports[i];
        int mi=0;
        for(int ph=0;ph<m;++ph)
            if(in_load[leaf][pk][ph]>mi) mi=in_load[leaf][pk][ph];
...
static inline LocalFutureDelta eval_makeroom_future_delta(int m,int fi,int from_pk,int to_pk,int bfi,int bto_pk){
    int out_leafs[4],out_ports[4],out_cnt=0;
    int in_leafs[4],in_ports[4],in_cnt=0;
    int sl=fl_sl[fi],dl=fl_dl[fi];
    add_unique_local_pair(sl,from_pk,out_leafs,out_ports,out_cnt);
    add_unique_local_pair(sl,to_pk,out_leafs,out_ports,out_cnt);
    add_unique_local_pair(dl,from_pk,in_leafs,in_ports,in_cnt);
    add_unique_local_pair(dl,to_pk,in_leafs,in_ports,in_cnt);
    int bfrom_pk=-1;
    if(bfi>=0){
        int bsl=fl_sl[bfi],bdl=fl_dl[bfi];
        bfrom_pk=fl_port[bfi];
        add_unique_local_pair(bsl,bfrom_pk,out_leafs,out_ports,out_cnt);
        add_unique_local_pair(bsl,bto_pk,out_leafs,out_ports,out_cnt);
        add_unique_local_pair(bdl,bfrom_pk,in_leafs,in_ports,in_cnt);
        add_unique_local_pair(bdl,bto_pk,in_leafs,in_ports,in_cnt);
    }

    int pre_fg,pre_over,post_fg,post_over;
    long long pre_sq,post_sq;
    collect_local_future_stats(m,out_leafs,out_ports,out_cnt,in_leafs,in_ports,in_cnt,
                               pre_fg,pre_over,pre_sq);
    ...
    if(bfi>=0){
        ...
        while(bmask){
            int ph=__builtin_ctz(bmask);
            out_load[bsl][bfrom_pk][ph]--;
            out_load[bsl][bto_pk][ph]++;
            in_load[bdl][bfrom_pk][ph]--;
            in_load[bdl][bto_pk][ph]++;
            bmask&=bmask-1;
        }
    }

    collect_local_future_stats(m,out_leafs,out_ports,out_cnt,in_leafs,in_ports,in_cnt,
                               post_fg,post_over,post_sq);
    ...
    LocalFutureDelta fd;
    fd.fg_delta=post_fg-pre_fg;
    fd.over_delta=post_over-pre_over;
    fd.sq_delta=post_sq-pre_sq;
    return fd;
}
...
struct StageAcceptEval{
    EvalMetrics em;
    int cbt;
};
static StageAcceptEval r4_beam_eval[R4_BEAM_WIDTH];
static StageAcceptEval r4_cand_eval[R4_BEAM_CAND];

inline int collect_total_cbtphsc(int m){
    ...
    int total=0;
    for(int c=0;c<MAX_CARDS;++c) total+=card_cbtphsc(c,m);
    return total;
}

inline StageAcceptEval collect_stage_accept_eval(int m){
    StageAcceptEval sev;
    sev.em=collect_metrics(m);
    sev.cbt=collect_total_cbtphsc(m);
    return sev;
}

inline int better_stage_accept_eval(const StageAcceptEval &a,const StageAcceptEval &b){
    if(a.em.score_jm!=b.em.score_jm) return a.em.score_jm<b.em.score_jm;
    if(a.em.score_fg!=b.em.score_fg) return a.em.score_fg<b.em.score_fg;
    long long a_pen=12LL*a.em.ci+5LL*a.cbt+3LL*a.em.future_over;
    long long b_pen=12LL*b.em.ci+5LL*b.cbt+3LL*b.em.future_over;
    if(a_pen!=b_pen) return a_pen<b_pen;
    if(a.cbt!=b.cbt) return a.cbt<b.cbt;
    if(a.em.ci!=b.em.ci) return a.em.ci<b.em.ci;
    if(a.em.future_over!=b.em.future_over) return a.em.future_over<b.em.future_over;
    if(a.em.future_sq!=b.em.future_sq) return a.em.future_sq<b.em.future_sq;
    if(a.em.jm!=b.em.jm) return a.em.jm<b.em.jm;
    if(a.em.fg!=b.em.fg) return a.em.fg<b.em.fg;
    return 0;
}
...
static inline int r4_card_flow_priority(int fi){
    int cp=fl_port[fi];
    int press=0;
    if(cp>=0&&fl_sl[fi]!=fl_dl[fi])
        press=global_out[fl_sl[fi]][cp]+global_in[fl_dl[fi]][cp];
    return lowr_core_flow_priority(fi)+press*16+__builtin_popcount(fl_pmask[fi])*16;
}

static inline int run_r4_card_single_card(int m,int src_card,int mode){
    int cnt=0;
    for(int i=0;i<fl_count;++i){
        if(fl_src[i]==src_card&&fl_sl[i]!=fl_dl[i]&&fl_port[i]>=0)
            pc_flows[cnt++]=i;
    }
    if(cnt<=1||cnt>96) return 0;
    ...
    for(int i=0;i<cnt;++i){
        ...
        for(int j=i+1;j<cnt;++j){
            ...
            int sj,sb;
            if(mode==0){
                sj=r4_card_flow_priority(fj);
                sb=r4_card_flow_priority(fb);
            } else if(mode==1){
                sj=r4_card_flow_priority(fj)+__builtin_popcount(fl_pmask[fj])*32;
                sb=r4_card_flow_priority(fb)+__builtin_popcount(fl_pmask[fb])*32;
            } else {
                sj=__builtin_popcount(fl_pmask[fj])*256+r4_card_flow_priority(fj);
                sb=__builtin_popcount(fl_pmask[fb])*256+r4_card_flow_priority(fb);
            }
            if(sj>sb) best=j;
        }
        if(best!=i){
    ...
    for(int i=0;i<cnt;++i){
        int fi=pc_flows[i];
        int sl=fl_sl[fi],dl=fl_dl[fi];
        int cur_pk=fl_port[fi];
        unsigned int mask=fl_pmask[fi];

        int best_pk=-1;
        int best_local=0x7fffffff;
        int best_add_over=0x7fffffff;
        int best_support=-1;
        int best_cbt=0x7fffffff;
        int best_fg=0x7fffffff;
        int best_press=0x7fffffff;
        int best_change=0x7fffffff;

        for(int pk=0;pk<g_p;++pk){
            int local_peak=0,add_over=0,support=0;
            unsigned int tmp_masks[MAX_PHASES];
            memcpy(tmp_masks,src_masks,m*sizeof(unsigned int));
            ...
            if(mode==0){
                ...
                if(local_peak<best_local||
                   (local_peak==best_local&&add_over<best_add_over)||
                   (local_peak==best_local&&add_over==best_add_over&&support>best_support)||
                   (local_peak==best_local&&add_over==best_add_over&&support==best_support&&after_cbt<best_cbt)||
                   (local_peak==best_local&&add_over==best_add_over&&support==best_support&&after_cbt==best_cbt&&future_fg<best_fg)||
                   (local_peak==best_local&&add_over==best_add_over&&support==best_support&&after_cbt==best_cbt&&future_fg==best_fg&&press<best_press)||
                   (local_peak==best_local&&add_over==best_add_over&&support==best_support&&after_cbt==best_cbt&&future_fg==best_fg&&press==best_press&&change<best_change)){
                    best_local=local_peak;
                    best_add_over=add_over;
                    best_support=support;
                    best_cbt=after_cbt;
                ...
                if(future_fg<best_fg||
                   (future_fg==best_fg&&local_peak<best_local)||
                   (future_fg==best_fg&&local_peak==best_local&&add_over<best_add_over)||
                   (future_fg==best_fg&&local_peak==best_local&&add_over==best_add_over&&support>best_support)||
                   (future_fg==best_fg&&local_peak==best_local&&add_over==best_add_over&&support==best_support&&after_cbt<best_cbt)||
                   (future_fg==best_fg&&local_peak==best_local&&add_over==best_add_over&&support==best_support&&after_cbt==best_cbt&&press<best_press)||
                   (future_fg==best_fg&&local_peak==best_local&&add_over==best_add_over&&support==best_support&&after_cbt==best_cbt&&press==best_press&&change<best_change)){
                    best_local=local_peak;
                    best_add_over=add_over;
                    best_support=support;
                    best_cbt=after_cbt;
...
__attribute__((noinline,cold)) void run_r4_card_core_rebuild(int m){
    if(g_r!=4||g_p<16) return;
    if(g_time_tight||fl_count>4500||(long long)fl_count*m>70000) return;

    StageAcceptEval base_eval=collect_stage_accept_eval(m);
    if(base_eval.em.fg<32||base_eval.cbt<=0) return;
    ...
    for(int i=0;i<fl_count;++i){
        int fi=i;
        if(fl_sl[fi]==fl_dl[fi]||fl_port[fi]<0) continue;
        int c=fl_src[fi];
        if(card_cbtphsc(c,m)<=0) continue;
        if(!core_card_seen[c]){
            core_card_seen[c]=1;
            core_card_score[c]=0;
            core_cards[cand_cnt++]=c;
        }
        int score=r4_card_flow_priority(fi)+64*card_cbtphsc(c,m);
        core_card_score[c]+=score;
    }
    if(cand_cnt<=0) return;
    for(int c=0;c<MAX_CARDS;++c){
        if(!core_card_seen[c]) continue;
        core_card_seen[c]=0;
    }
    ...
    EvalMetrics best_eval=base_eval.em;
    int best_cbt=base_eval.cbt;
    int improved=0;
    int max_cards=4;
    if(cand_cnt<max_cards) max_cards=cand_cnt;
    int beam_cnt=1;
    memcpy(r4_beam_out[0], bk_out, g_l*sizeof(out_load[0]));
    memcpy(r4_beam_in[0], bk_in, g_l*sizeof(in_load[0]));
    memcpy(r4_beam_port[0], bk_port, fl_count*sizeof(short));
    r4_beam_eval[0]=base_eval;

    int beam_cards=max_cards<2?max_cards:2;
    for(int bi=0;bi<beam_cards;++bi){
        int src_card=core_cards[bi];
        int next_cnt=0;
        for(int state=0;state<beam_cnt;++state){
            for(int mode=0;mode<3;++mode){
                memcpy(out_load, r4_beam_out[state], g_l*sizeof(out_load[0]));
                memcpy(in_load, r4_beam_in[state], g_l*sizeof(in_load[0]));
                memcpy(fl_port, r4_beam_port[state], fl_count*sizeof(short));
                if(!run_r4_card_single_card(m,src_card,mode)) continue;
                StageAcceptEval cand_eval=collect_stage_accept_eval(m);
                if(!better_stage_accept_eval(cand_eval,r4_beam_eval[state])&&
                   !(cand_eval.em.score_jm==r4_beam_eval[state].em.score_jm&&
                     cand_eval.em.score_fg==r4_beam_eval[state].em.score_fg&&
                     cand_eval.em.ci==r4_beam_eval[state].em.ci&&
                     cand_eval.em.future_over==r4_beam_eval[state].em.future_over&&
                     cand_eval.cbt<r4_beam_eval[state].cbt))
                    continue;

                int placed=-1;
                if(next_cnt<R4_BEAM_CAND){
                    placed=next_cnt++;
                } else {
                    int worst=0;
                    for(int k=1;k<next_cnt;++k)
                        if(better_stage_accept_eval(r4_cand_eval[worst],r4_cand_eval[k]))
                            worst=k;
                    if(!better_stage_accept_eval(cand_eval,r4_cand_eval[worst])) continue;
                    placed=worst;
                }
                memcpy(r4_cand_out[placed], out_load, g_l*sizeof(out_load[0]));
                memcpy(r4_cand_in[placed], in_load, g_l*sizeof(in_load[0]));
                memcpy(r4_cand_port[placed], fl_port, fl_count*sizeof(short));
                r4_cand_eval[placed]=cand_eval;
            }
        }
        if(next_cnt<=0) continue;

        int keep=next_cnt<R4_BEAM_WIDTH?next_cnt:R4_BEAM_WIDTH;
        for(int k=0;k<keep;++k){
            int best=k;
            for(int j=k+1;j<next_cnt;++j)
                if(better_stage_accept_eval(r4_cand_eval[j],r4_cand_eval[best]))
                    best=j;
            if(best!=k){
                StageAcceptEval te=r4_cand_eval[k];
                r4_cand_eval[k]=r4_cand_eval[best];
                r4_cand_eval[best]=te;

                memcpy(out_load, r4_cand_out[k], g_l*sizeof(out_load[0]));
                memcpy(r4_cand_out[k], r4_cand_out[best], g_l*sizeof(out_load[0]));
                memcpy(r4_cand_out[best], out_load, g_l*sizeof(out_load[0]));

                memcpy(in_load, r4_cand_in[k], g_l*sizeof(in_load[0]));
                memcpy(r4_cand_in[k], r4_cand_in[best], g_l*sizeof(in_load[0]));
                memcpy(r4_cand_in[best], in_load, g_l*sizeof(in_load[0]));

                memcpy(fl_port, r4_cand_port[k], fl_count*sizeof(short));
                memcpy(r4_cand_port[k], r4_cand_port[best], fl_count*sizeof(short));
                memcpy(r4_cand_port[best], fl_port, fl_count*sizeof(short));
            }
        }
        beam_cnt=keep;
        for(int k=0;k<beam_cnt;++k){
            memcpy(r4_beam_out[k], r4_cand_out[k], g_l*sizeof(out_load[0]));
            memcpy(r4_beam_in[k], r4_cand_in[k], g_l*sizeof(in_load[0]));
            memcpy(r4_beam_port[k], r4_cand_port[k], fl_count*sizeof(short));
            r4_beam_eval[k]=r4_cand_eval[k];
        }
    }

    for(int state=0;state<beam_cnt;++state){
        StageAcceptEval cand_eval=r4_beam_eval[state];
        int cand_cbt=cand_eval.cbt;
        if(better_stage_accept_eval(cand_eval,base_eval)||
           (cand_eval.em.score_jm==best_eval.score_jm&&cand_eval.em.score_fg==best_eval.score_fg&&
            cand_eval.em.ci==best_eval.ci&&cand_eval.em.future_over==best_eval.future_over&&
            cand_eval.cbt<best_cbt)){
            base_eval=cand_eval;
            best_eval=cand_eval.em;
            best_cbt=cand_cbt;
            improved=1;
            memcpy(sv_out, r4_beam_out[state], g_l*sizeof(out_load[0]));
            memcpy(sv_in, r4_beam_in[state], g_l*sizeof(in_load[0]));
            memcpy(sv_port, r4_beam_port[state], fl_count*sizeof(short));
        }
    }
...
void solve_job(){
    ...
    int enable_r4_stage_accept=(!g_time_tight&&g_r==4&&g_p>=16&&fl_count<=5000&&job_work<=50000);
    if(enable_r4_stage_accept){
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        StageAcceptEval second_stage_base=collect_stage_accept_eval(m);
        run_r4_card_core_rebuild(m);
        run_relaxed_swap(m);
        run_neutral_swap(m);
        run_relaxed_swap(m);
        ...
        run_neutral_swap(m);
        run_relaxed_swap(m);
        StageAcceptEval second_stage_after=collect_stage_accept_eval(m);
        if(!better_stage_accept_eval(second_stage_after,second_stage_base)){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));
        }
        ...
        if(total_cbt_before>0){
            ...
            for(int mr_iter=0;mr_iter<3;++mr_iter){
                ...
                for(int fi=0;fi<fl_count;++fi){
                    ...
                    int best_fg_delta=0x7fffffff,best_over_delta=0x7fffffff;
                    long long best_sq_delta=0x7fffffffffffffffLL;
                    for(int dpk=0;dpk<2;++dpk){
                        ...
                        if(!blocked){
                            ...
                            LocalFutureDelta fd=eval_makeroom_future_delta(m,fi,cp,dp,-1,-1);
                            if(fd.fg_delta>0) continue;
                            if(net>best_net||
                               (net==best_net&&fd.fg_delta<best_fg_delta)||
                               (net==best_net&&fd.fg_delta==best_fg_delta&&fd.over_delta<best_over_delta)||
                               (net==best_net&&fd.fg_delta==best_fg_delta&&fd.over_delta==best_over_delta&&fd.sq_delta<best_sq_delta)){
                                best_net=net;
                                best_bfi=-1;
                                best_bap=-1;
                                best_dp=dp;
                                best_fg_delta=fd.fg_delta;
                                best_over_delta=fd.over_delta;
                                best_sq_delta=fd.sq_delta;
                            }
                            continue;
                        }
                        ...
                        for(int bfi=0;bfi<fl_count;++bfi){
                            ...
                            for(int ap=0;ap<g_p;++ap){
                                ...
                                LocalFutureDelta fd=eval_makeroom_future_delta(m,fi,cp,dp,bfi,ap);
                                if(fd.fg_delta>0) continue;
                                if(net>best_net||
                                   (net==best_net&&fd.fg_delta<best_fg_delta)||
                                   (net==best_net&&fd.fg_delta==best_fg_delta&&fd.over_delta<best_over_delta)||
