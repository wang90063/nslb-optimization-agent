...
void solve_job(){
    ...
    if(!huge_job){
        ...
        // When jm is already solved, the remaining gains tend to come from
        // choosing a better future trajectory, not from another local repair.
        // Expand only the candidate families that historically benefitted from
        // testcase-level state evolution: local-first and FTRL.
        if(any_jm_le_r && fl_count<=3000 && best_eval.score_fg>g_r+1){
            for(int seed=30;seed<=33;++seed){
                for(int i=0;i<fl_count;++i) fl_order[i]=i;
                run_greedy(m, 1000, 1, 1, seed);
                run_swap(m);
                run_jm_repair(m);
                run_fo_swap(m);
                TRY_STRATEGY();
            ...
            for(int seed=34;seed<=35;++seed){
                for(int i=0;i<fl_count;++i) fl_order[i]=i;
                run_greedy_ftrl(m, 1, seed, 2);
                run_swap(m);
                run_jm_repair(m);
                run_fo_swap(m);
                TRY_STRATEGY();
            ...
            if(g_p<=16){
                for(int seed=36;seed<=37;++seed){
                    for(int i=0;i<fl_count;++i) fl_order[i]=i;
                    run_greedy_ftrl(m, 1, seed, 2, 6);
                    run_swap(m);
                    run_jm_repair(m);
                    run_fo_swap(m);
                    TRY_STRATEGY();
