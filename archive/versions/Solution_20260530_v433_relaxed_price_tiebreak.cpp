            ...
            if(gsz>=2&&gsz<=rs_gsz_cap){
                for(int a=g_start;a<g_end;++a){
                    ...
                    for(int b=a+1;b<g_end;++b){
                        ...
                        if(after<before){
                            do_commit=1;
                        } else if(after==before&&dl_i!=dl_j){
                            // Cbtphsc-neutral: break tie by dest-side price.
                            // Source ports stay occupied on sl (just swapped owners),
                            // so src-side price delta cancels; only dest side changes.
                            long long pbefore=(long long)price_term(dl_i,pi)+price_term_dst(dl_i,pi)
                                +price_term(dl_j,pj)+price_term_dst(dl_j,pj);
                            long long pafter=(long long)price_term(dl_i,pj)+price_term_dst(dl_i,pj)
                                +price_term(dl_j,pi)+price_term_dst(dl_j,pi);
                            if(pafter<pbefore) do_commit=1;
                        }
                        if(do_commit){
                            // Commit: update loads
                            if(dl_i==dl_j){
