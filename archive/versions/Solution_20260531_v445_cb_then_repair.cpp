...
void run_cb_consolidate_repair(int m){
    if(fl_count<10) return;
    static short orig_max_out[MAX_LEAFS][MAX_PORTS];
    static short orig_max_in[MAX_LEAFS][MAX_PORTS];
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            short mo=0,mi=0;
            for(int ph=0;ph<m;++ph){
            ...
            orig_max_out[leaf][pk]=mo;orig_max_in[leaf][pk]=mi;
        }
    memset(cpm,0,sizeof(cpm));memset(cppc,0,sizeof(cppc));
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
    ...
    // Phase 1: CB consolidation — allow sa_max to grow by +1
    for(int iter=0;iter<3;++iter){
        int any_move=0;
        for(int fi=0;fi<fl_count;++fi){
            int sl=fl_sl[fi],dl=fl_dl[fi],px=fl_port[fi],ci=fl_src[fi];
            if(sl==dl||px<0) continue;
            int cur_cb=card_cbtphsc(ci,m);
            if(cur_cb<=0) continue;
            int best_py=-1,best_cb_gain=0;
            for(int py=0;py<g_p;++py){
                if(py==px) continue;
                unsigned int m2=fl_pmask[fi]; int ok=1;
                while(m2){int ph=__builtin_ctz(m2);
                    if(out_load[sl][py][ph]+1>orig_max_out[sl][py]+1){ok=0;break;}
                    if(in_load[dl][py][ph]+1>orig_max_in[dl][py]+1){ok=0;break;}
                    m2&=m2-1;}
                if(!ok) continue;
                int ci_delta=0;
                m2=fl_pmask[fi];
                while(m2){int ph=__builtin_ctz(m2);
                    ...
                    m2&=m2-1;}
                if(ci_delta>0) continue;
                unsigned int mk=fl_pmask[fi];
                while(mk){int ph=__builtin_ctz(mk);
                    cppc[ci][ph][px]--;if(!cppc[ci][ph][px])cpm[ci][ph]&=~(1u<<px);
                    cppc[ci][ph][py]++;cpm[ci][ph]|=(1u<<py);mk&=mk-1;}
                int new_cb=card_cbtphsc(ci,m);
                int cb_gain=cur_cb-new_cb;
                mk=fl_pmask[fi];
                while(mk){int ph=__builtin_ctz(mk);
                    cppc[ci][ph][py]--;if(!cppc[ci][ph][py])cpm[ci][ph]&=~(1u<<py);
                    cppc[ci][ph][px]++;cpm[ci][ph]|=(1u<<px);mk&=mk-1;}
                if(cb_gain>best_cb_gain){best_cb_gain=cb_gain;best_py=py;}
            }
            if(best_py<0) continue;
            int py=best_py;
            unsigned int mk=fl_pmask[fi];
    ...
    // Phase 2: Repair — move non-CB-critical flows off ports that exceeded orig_max
    // Rebuild cppc/cpm
    memset(cpm,0,sizeof(cpm));memset(cppc,0,sizeof(cppc));
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
    ...
    for(int iter=0;iter<5;++iter){
        ...
        for(int fi=0;fi<fl_count;++fi){
            int sl=fl_sl[fi],dl=fl_dl[fi],px=fl_port[fi],ci=fl_src[fi];
            if(sl==dl||px<0) continue;
            // Check if this port is overloaded (above orig_max)
            int overloaded=0;
            unsigned int m2=fl_pmask[fi];
            while(m2){int ph=__builtin_ctz(m2);
                if(out_load[sl][px][ph]>orig_max_out[sl][px]){overloaded=1;break;}
                if(in_load[dl][px][ph]>orig_max_in[dl][px]){overloaded=1;break;}
                m2&=m2-1;}
            if(!overloaded) continue;
            // Only move if this flow is NOT CB-critical (moving it won't increase CB)
            int best_py=-1,best_wt=0;
            for(int py=0;py<g_p;++py){
                if(py==px) continue;
                unsigned int m3=fl_pmask[fi]; int ok=1;
                while(m3){int ph=__builtin_ctz(m3);
                    if(out_load[sl][py][ph]+1>orig_max_out[sl][py]){ok=0;break;}
                    if(in_load[dl][py][ph]+1>orig_max_in[dl][py]){ok=0;break;}
                    m3&=m3-1;}
                if(!ok) continue;
                // Check CB impact
                unsigned int mk=fl_pmask[fi];
                ...
                while(mk){int ph=__builtin_ctz(mk);
                    cppc[ci][ph][py]--;if(!cppc[ci][ph][py])cpm[ci][ph]&=~(1u<<py);
                    cppc[ci][ph][px]++;cpm[ci][ph]|=(1u<<px);mk&=mk-1;}
                int orig_cb=card_cbtphsc(ci,m);
                int cb_delta=new_cb-orig_cb;
                if(cb_delta>0) continue; // Don't worsen CB
                int ci_delta=0;
                m3=fl_pmask[fi];
                while(m3){int ph=__builtin_ctz(m3);
                    if(out_load[sl][py][ph]>=g_r)ci_delta++;
                    ...
                    if(in_load[dl][px][ph]>g_r)ci_delta--;
                    m3&=m3-1;}
                int wt=-ci_delta*12-cb_delta*5+1; // prefer moves that also reduce CI/CB
                if(wt>best_wt){best_wt=wt;best_py=py;}
            }
            if(best_py<0) continue;
            int py=best_py;
...
void solve_job(){
    ...
    if(!g_time_tight){
        ...
        // CB consolidate-then-repair: relax sa_max for CB, then repair MM
    if(!g_time_tight){
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        EvalMetrics cr_base=collect_metrics(m);
        run_cb_consolidate_repair(m);
        EvalMetrics cr_after=collect_metrics(m);
        if(cr_after.fg>cr_base.fg||(cr_after.fg==cr_base.fg&&cr_after.jm>cr_base.jm)){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));
        } else {
