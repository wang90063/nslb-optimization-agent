...
void solve_job(){
    ...
    // Second SA+CB cycle: only when time budget allows
    if(!g_time_tight && g_job_idx>0){
        double el2=elapsed_sec();
        double proj2=el2/g_job_idx*g_n;
        if(proj2<3.5){
            memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
            memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
            memcpy(bk_port, fl_port, fl_count*sizeof(short));
            EvalMetrics sa2_base=collect_metrics(m);
            run_sa_composite(m);
            EvalMetrics sa2_after=collect_metrics(m);
            if(sa2_after.fg>sa2_base.fg||(sa2_after.fg==sa2_base.fg&&sa2_after.jm>sa2_base.jm)){
                memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
                memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
                memcpy(fl_port, bk_port, fl_count*sizeof(short));
            } else {
