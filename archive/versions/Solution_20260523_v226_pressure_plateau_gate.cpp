...
static unsigned char fl_popcnt[MAX_FLOWS];
...
static int po_leaf_outmax[MAX_LEAFS],po_leaf_inmax[MAX_LEAFS];
static int po_leaf_outsum[MAX_LEAFS],po_leaf_insum[MAX_LEAFS];
static int po_leaf_goutmax[MAX_LEAFS],po_leaf_ginmax[MAX_LEAFS];
static int po_leaf_goutsum[MAX_LEAFS],po_leaf_ginsum[MAX_LEAFS];

static int pressure_order_cmp(const void *pa,const void *pb){
    int a=*(const int*)pa,b=*(const int*)pb;
    int asl=fl_sl[a],adl=fl_dl[a];
    int bsl=fl_sl[b],bdl=fl_dl[b];

    int a_src_peak=po_leaf_goutmax[asl]*64+po_leaf_outmax[asl]*4;
    int a_dst_peak=po_leaf_ginmax[adl]*64+po_leaf_inmax[adl]*4;
    int b_src_peak=po_leaf_goutmax[bsl]*64+po_leaf_outmax[bsl]*4;
    int b_dst_peak=po_leaf_ginmax[bdl]*64+po_leaf_inmax[bdl]*4;

    int a_primary=a_src_peak>a_dst_peak?a_src_peak:a_dst_peak;
    int b_primary=b_src_peak>b_dst_peak?b_src_peak:b_dst_peak;
    if(a_primary!=b_primary) return b_primary-a_primary;

    int a_secondary=a_src_peak+a_dst_peak;
    int b_secondary=b_src_peak+b_dst_peak;
    if(a_secondary!=b_secondary) return b_secondary-a_secondary;

    int a_global_sum=po_leaf_goutsum[asl]+po_leaf_ginsum[adl];
    int b_global_sum=po_leaf_goutsum[bsl]+po_leaf_ginsum[bdl];
    if(a_global_sum!=b_global_sum) return b_global_sum-a_global_sum;

    int a_job_sum=po_leaf_outsum[asl]+po_leaf_insum[adl];
    int b_job_sum=po_leaf_outsum[bsl]+po_leaf_insum[bdl];
    if(a_job_sum!=b_job_sum) return b_job_sum-a_job_sum;

    if(fl_popcnt[a]!=fl_popcnt[b]) return (int)fl_popcnt[b]-(int)fl_popcnt[a];
    if(asl!=bsl) return asl-bsl;
    if(adl!=bdl) return adl-bdl;
    return a-b;
}

void prepare_flow_order_pressure(int m){
    static short phase_out[MAX_LEAFS][MAX_PHASES];
    static short phase_in[MAX_LEAFS][MAX_PHASES];
    memset(phase_out,0,sizeof(phase_out));
    memset(phase_in,0,sizeof(phase_in));
    memset(po_leaf_outmax,0,sizeof(po_leaf_outmax));
    memset(po_leaf_inmax,0,sizeof(po_leaf_inmax));
    memset(po_leaf_outsum,0,sizeof(po_leaf_outsum));
    memset(po_leaf_insum,0,sizeof(po_leaf_insum));
    memset(po_leaf_goutmax,0,sizeof(po_leaf_goutmax));
    memset(po_leaf_ginmax,0,sizeof(po_leaf_ginmax));
    memset(po_leaf_goutsum,0,sizeof(po_leaf_goutsum));
    memset(po_leaf_ginsum,0,sizeof(po_leaf_ginsum));

    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            int go=global_out[leaf][pk],gi=global_in[leaf][pk];
            po_leaf_goutsum[leaf]+=go;
            po_leaf_ginsum[leaf]+=gi;
            if(go>po_leaf_goutmax[leaf]) po_leaf_goutmax[leaf]=go;
            if(gi>po_leaf_ginmax[leaf]) po_leaf_ginmax[leaf]=gi;
        }
    ...
    for(int leaf=0;leaf<g_l;++leaf)
        for(int ph=0;ph<m;++ph){
            if(phase_out[leaf][ph]>po_leaf_outmax[leaf]) po_leaf_outmax[leaf]=phase_out[leaf][ph];
            if(phase_in[leaf][ph]>po_leaf_inmax[leaf]) po_leaf_inmax[leaf]=phase_in[leaf][ph];
        }

    qsort(fl_order,fl_count,sizeof(int),pressure_order_cmp);
}

int count_state_fg_cells(short state_out[MAX_LEAFS][MAX_PORTS][MAX_PHASES],
...
void solve_job(){
    ...
    for(int i=0;i<fl_count;++i){
        ...
        fl_order[i]=i;
        fl_popcnt[i]=(unsigned char)__builtin_popcount(fl_pmask[i]);
    }
    ...
    int allow_pressure_order = (!time_tight && !huge_job && g_p>=16 &&
                                fl_count<=120000 && job_work<=200000);
    ...
    int pressure_fg_lb=lowr_future_fg_lower_bound(m);
    int pressure_plateau_cells=count_state_fg_cells(sv_out, sv_in, m, best_eval.fg);
    int enable_pressure_family=(allow_pressure_order &&
                                best_eval.fg>pressure_fg_lb &&
                                pressure_plateau_cells<=g_p*4);

    if(enable_pressure_family){
        prepare_flow_order_pressure(m);
