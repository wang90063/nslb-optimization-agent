...
static short alt_out[MAX_LEAFS][MAX_PORTS][MAX_PHASES];
static short alt_in[MAX_LEAFS][MAX_PORTS][MAX_PHASES];
static short alt_port[MAX_FLOWS];
static short traj_out[MAX_LEAFS][MAX_PORTS][MAX_PHASES];
static short traj_in[MAX_LEAFS][MAX_PORTS][MAX_PHASES];
static short traj_port[MAX_FLOWS];
static short traj_ct_pre_out[MAX_LEAFS][MAX_PORTS];
static short traj_ct_pre_in[MAX_LEAFS][MAX_PORTS];
static short alt_ct_pre_out[MAX_LEAFS][MAX_PORTS];
static short alt_ct_pre_in[MAX_LEAFS][MAX_PORTS];
...
inline void save_current_state(short dst_out[MAX_LEAFS][MAX_PORTS][MAX_PHASES],
                               short dst_in[MAX_LEAFS][MAX_PORTS][MAX_PHASES],
                               short dst_port[MAX_FLOWS]){
    memcpy(dst_out, out_load, g_l*sizeof(out_load[0]));
    memcpy(dst_in, in_load, g_l*sizeof(in_load[0]));
    memcpy(dst_port, fl_port, fl_count*sizeof(short));
}

inline void restore_state(const short src_out[MAX_LEAFS][MAX_PORTS][MAX_PHASES],
                          const short src_in[MAX_LEAFS][MAX_PORTS][MAX_PHASES],
                          const short src_port[MAX_FLOWS]){
    memcpy(out_load, src_out, g_l*sizeof(out_load[0]));
    memcpy(in_load, src_in, g_l*sizeof(in_load[0]));
    memcpy(fl_port, src_port, fl_count*sizeof(short));
}

inline int assignment_diff_limit(const short *a,const short *b,int limit){
    int diff=0;
    for(int i=0;i<fl_count;++i){
        if(a[i]!=b[i]&&++diff>=limit) break;
    }
    return diff;
}

inline int should_run_trajectory_duel(const EvalMetrics &best_eval,
                                      const EvalMetrics &alt_eval,
                                      int alt_valid,
                                      long long job_work,
                                      int time_tight){
    if(time_tight||!alt_valid) return 0;
    if(g_r<4||g_p<16) return 0;
    if(fl_count>6000||job_work>90000) return 0;
    if(best_eval.jm>g_r+1) return 0;
    if(alt_eval.score_jm!=best_eval.score_jm) return 0;
    if(alt_eval.score_fg!=best_eval.score_fg) return 0;
    if(alt_eval.ci>best_eval.ci+64) return 0;
    if(alt_eval.future_over>best_eval.future_over+128) return 0;
    return assignment_diff_limit(sv_port,alt_port,8)>=8;
}
...
void solve_job(){
    ...
    #define TRY_STRATEGY() do { \
        ...
        if(first||better_metrics(cur_eval,best_eval)){ \
            if(!first){ \
                alt_eval=best_eval; \
                alt_valid=1; \
                memcpy(alt_out, sv_out, g_l*sizeof(alt_out[0])); \
                memcpy(alt_in, sv_in, g_l*sizeof(alt_in[0])); \
                memcpy(alt_port, sv_port, fl_count*sizeof(short)); \
            } \
            best_eval=cur_eval; first=0; \
            memcpy(sv_out, out_load, g_l*sizeof(out_load[0])); \
            memcpy(sv_in, in_load, g_l*sizeof(in_load[0])); \
            memcpy(sv_port, fl_port, fl_count*sizeof(short)); \
        } else if(!alt_valid||better_metrics(cur_eval,alt_eval)){ \
            alt_eval=cur_eval; \
            alt_valid=1; \
            memcpy(alt_out, out_load, g_l*sizeof(alt_out[0])); \
            memcpy(alt_in, in_load, g_l*sizeof(alt_in[0])); \
            memcpy(alt_port, fl_port, fl_count*sizeof(short)); \
        } \
    } while(0)
    ...
    int run_dual_trajectory=should_run_trajectory_duel(best_eval,alt_eval,alt_valid,job_work,time_tight);
    ...
    int final_ct_pass_ran=ct_pass_ran;
    if(ct_pass_ran){
        memcpy(traj_ct_pre_out, pre_ct_mo, g_l*sizeof(traj_ct_pre_out[0]));
        memcpy(traj_ct_pre_in, pre_ct_mi, g_l*sizeof(traj_ct_pre_in[0]));
    }
    ...
    if(run_dual_trajectory){
        memcpy(traj_out, out_load, g_l*sizeof(traj_out[0]));
        memcpy(traj_in, in_load, g_l*sizeof(traj_in[0]));
        memcpy(traj_port, fl_port, fl_count*sizeof(short));
    }

    if(run_dual_trajectory){
        int keep_best=1;

        restore_state(alt_out, alt_in, alt_port);

        int alt_restored_jm=get_job_max(m);
        if(!time_tight && (alt_restored_jm<=g_r || job_work<=120000)) run_global_swap(m);
        if(fl_count<=6000&&get_job_max(m)>g_r) run_mc_repair(m);
        run_lowr_hotcell_exact(m);
        run_lowr_cell_core_rebuild(m);
        run_lowr_hotleaf_rebuild(m);
        run_lowr_card_core_rebuild(m);
        int alt_lowr_jm=get_job_max(m);
        int alt_enable_lowr_repair=((g_r==2&&g_p>=32)||(g_r==3&&g_p>=16));
        if(alt_enable_lowr_repair&&fl_count<=4500&&job_work<=70000&&alt_lowr_jm==g_r+1)
            run_random_start_repair(m);

        int alt_allow_extra_pc_chain=1;
        if(job_work>100000) alt_allow_extra_pc_chain=0;
        else if(job_work>80000&&!(g_p>=16&&m>=17&&fl_count<=5500)) alt_allow_extra_pc_chain=0;
        if(job_work>50000&&fl_count>5000) alt_allow_extra_pc_chain=0;
        if(g_time_tight) alt_allow_extra_pc_chain=0;

        run_port_consistency(m);
        run_port_consistency_perport_refine(m);
        run_port_consistency(m);
        if(alt_allow_extra_pc_chain){
            run_port_consistency_perport_refine(m);
            run_port_consistency(m);
            run_port_consistency_perport_refine(m);
            run_port_consistency(m);
        ...
        if(!g_time_tight){
            ...
            if(ct_after.fg>ct_base.fg||(ct_after.fg==ct_base.fg&&ct_after.jm>ct_base.jm)){
                ...
                alt_ct_pass_ran=1;
                memcpy(alt_ct_pre_out, pre_ct_mo, g_l*sizeof(alt_ct_pre_out[0]));
                memcpy(alt_ct_pre_in, pre_ct_mi, g_l*sizeof(alt_ct_pre_in[0]));
                run_port_consistency(m);
                run_neutral_swap(m);
                run_relaxed_swap(m);
                run_cross_dest_swap(m);
        ...
        EvalMetrics alt_final_eval=collect_metrics(m);
        if(better_metrics(alt_final_eval,final_eval)){
            keep_best=0;
            final_eval=alt_final_eval;
            final_ct_pass_ran=alt_ct_pass_ran;
            if(alt_ct_pass_ran){
                memcpy(traj_ct_pre_out, alt_ct_pre_out, g_l*sizeof(traj_ct_pre_out[0]));
                memcpy(traj_ct_pre_in, alt_ct_pre_in, g_l*sizeof(traj_ct_pre_in[0]));
            }
        }

        if(keep_best){
            restore_state(traj_out, traj_in, traj_port);
        }
    }
    ...
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            int inc_o=0,inc_i=0;
            if(final_ct_pass_ran){
                global_out[leaf][pk]+=traj_ct_pre_out[leaf][pk];
                global_in[leaf][pk]+=traj_ct_pre_in[leaf][pk];
                inc_o=traj_ct_pre_out[leaf][pk];
                inc_i=traj_ct_pre_in[leaf][pk];
            } else {
                int mo=0,mi=0;
