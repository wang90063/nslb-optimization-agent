...
void solve_job(){
    ...
    #define TRY_STRATEGY() do { \
        ...
        if(first||better_metrics(cur_eval,best_eval)){ \
            take_strategy=1; \
        } else if(!better_metrics(best_eval,cur_eval)){ \
            int cur_cb=total_cbtphsc_from_ports(fl_port,m); \
            if(best_cb<0) best_cb=total_cbtphsc_from_ports(sv_port,m); \
            if(cur_cb<best_cb) take_strategy=1; \
        ...
        if(take_strategy){ \
            ...
            best_cb=total_cbtphsc_from_ports(fl_port,m); \
            memcpy(sv_out, out_load, g_l*sizeof(out_load[0])); \
            memcpy(sv_in, in_load, g_l*sizeof(in_load[0])); \
            memcpy(sv_port, fl_port, fl_count*sizeof(short)); \
        } \
