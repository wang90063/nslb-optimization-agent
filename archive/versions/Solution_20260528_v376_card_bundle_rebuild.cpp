...
static int bundle_cards[MAX_CARDS];
static int bundle_card_score[MAX_CARDS];
static int bundle_card_flowcnt[MAX_CARDS];
static unsigned char bundle_card_seen[MAX_CARDS];
static unsigned char bundle_card_slot[MAX_CARDS];
static int bundle_flows[MAX_FLOWS];
static int bundle_core_cards[MAX_CARDS];
static int bundle_core_cardcnt;
static int bundle_sel_cards[6];
static int bundle_sel_cnt;
...
static inline int bundle_card_cbt(int card, int m){
    return card_cbtphsc(card, m);
}
...
__attribute__((noinline,cold)) void run_card_bundle_rebuild(int m){
    if(g_time_tight||g_p<16) return;
    long long work=(long long)fl_count*(long long)m;
    if(fl_count>5500||work>85000) return;
    ...
    for(int c=0;c<MAX_CARDS;++c){
        int s=bundle_card_cbt(c,m);
        if(s<=0) continue;
        bundle_cards[card_cnt]=c;
        bundle_card_score[c]=s;
        bundle_card_flowcnt[c]=0;
        bundle_card_seen[c]=0;
        bundle_card_slot[c]=0;
        bundle_core_cards[card_cnt]=c;
        card_cnt++;
    }
    if(card_cnt<2) return;

    for(int i=0;i<card_cnt;++i){
        int c=bundle_cards[i];
        for(int fi=0;fi<fl_count;++fi)
            if(fl_src[fi]==c&&fl_sl[fi]!=fl_dl[fi]&&fl_port[fi]>=0)
                bundle_card_flowcnt[c]++;
    }
    for(int i=1;i<card_cnt;++i){
        int c=bundle_cards[i],sc=bundle_card_score[c],j=i-1;
        while(j>=0&&bundle_card_score[bundle_cards[j]]<sc){
            bundle_cards[j+1]=bundle_cards[j];
            --j;
        }
        bundle_cards[j+1]=c;
    }
    ...
    for(int trial=0;trial<2;++trial){
        ...
        int selected_cnt=0;
        int selected_leaf=-1;
        int selected_flowcnt=0;

        for(int i=0;i<card_cnt&&selected_cnt<4;++i){
            int c=bundle_cards[i];
            if(bundle_card_seen[c]) continue;
            int leaf=c/g_pr;
            if(selected_leaf<0) selected_leaf=leaf;
            if(leaf!=selected_leaf) continue;
            if(bundle_card_score[c]<=0) continue;
            if(selected_flowcnt+bundle_card_flowcnt[c]>220&&selected_cnt>=2) continue;
            bundle_card_seen[c]=1;
            bundle_sel_cards[selected_cnt++]=c;
            selected_flowcnt+=bundle_card_flowcnt[c];
            if(selected_flowcnt>=220) break;
        }
        if(selected_cnt<2){
            for(int i=0;i<selected_cnt;++i) bundle_card_seen[bundle_sel_cards[i]]=0;
            continue;
        }
        ...
        for(int fi=0;fi<fl_count;++fi){
            ...
            bundle_flows[flow_cnt++]=fi;
        }
        if(flow_cnt<2||flow_cnt>220){
            for(int i=0;i<selected_cnt;++i) bundle_card_seen[bundle_sel_cards[i]]=0;
            continue;
        }

        int leaf=selected_leaf;
        int leaf_score=0;
        for(int i=0;i<selected_cnt;++i) leaf_score+=bundle_card_score[bundle_sel_cards[i]];
        if(leaf_score<=0){
            for(int i=0;i<selected_cnt;++i) bundle_card_seen[bundle_sel_cards[i]]=0;
            continue;
        }
        ...
        int flow_order[MAX_FLOWS];
        for(int i=0;i<flow_cnt;++i) flow_order[i]=bundle_flows[i];
        for(int i=0;i<flow_cnt;++i){
            int best=i;
            int best_key=-1;
            for(int j=i;j<flow_cnt;++j){
                int fi=flow_order[j];
                int c=fl_src[fi];
                int key=bundle_card_score[c]*1024+bundle_card_flowcnt[c]*32+__builtin_popcount(fl_pmask[fi])*8;
                if(key>best_key){best_key=key;best=j;}
            }
            if(best!=i){
                int t=flow_order[i];flow_order[i]=flow_order[best];flow_order[best]=t;
            }
        }

        int flow_slot[MAX_FLOWS];
        for(int i=0;i<flow_cnt;++i){
            int fi=flow_order[i];
            int c=fl_src[fi];
            int slot=0;
            for(int k=0;k<selected_cnt;++k) if(bundle_sel_cards[k]==c){slot=k;break;}
            flow_slot[fi]=slot;
        }

        unsigned int bundle_masks[4][MAX_PHASES];
        memset(bundle_masks,0,sizeof(bundle_masks));
        for(int i=0;i<flow_cnt;++i){
            int fi=flow_order[i];
            int sl=fl_sl[fi],dl=fl_dl[fi],cp=fl_port[fi];
            unsigned int mask=fl_pmask[fi];
            int slot=flow_slot[fi];
            unsigned int tmp[MAX_PHASES];
            memcpy(tmp,bundle_masks[slot],sizeof(tmp));

            int best_pk=cp;
            int best_fg=0x7fffffff,best_over=0x7fffffff,best_cbt=0x7fffffff;
            int best_local=0x7fffffff,best_press=0x7fffffff,best_change=0x7fffffff;
            long long best_sq=0x7fffffffffffffffLL;
            for(int pk=0;pk<g_p;++pk){
                int local_peak=0,add_over=0,ok=1;
                memcpy(tmp,bundle_masks[slot],sizeof(tmp));
                unsigned int mk=mask;
                while(mk){
                    int ph=__builtin_ctz(mk);
                ...
                if(!ok) continue;
                int after_cbt=calc_phase_mask_cbt(tmp,m);
                int mo=0,mi=0;
                for(int ph=0;ph<m;++ph){
                ...
                long long future_sq=(long long)fo*fo+(long long)fi*fi;
                int press=global_out[sl][pk]+global_in[dl][pk];
                int change=(pk!=cp);
                if(future_fg<best_fg||
                   (future_fg==best_fg&&add_over<best_over)||
                   (future_fg==best_fg&&add_over==best_over&&after_cbt<best_cbt)||
                   (future_fg==best_fg&&add_over==best_over&&after_cbt==best_cbt&&future_sq<best_sq)||
                   (future_fg==best_fg&&add_over==best_over&&after_cbt==best_cbt&&future_sq==best_sq&&local_peak<best_local)||
                   (future_fg==best_fg&&add_over==best_over&&after_cbt==best_cbt&&future_sq==best_sq&&local_peak==best_local&&press<best_press)||
                   (future_fg==best_fg&&add_over==best_over&&after_cbt==best_cbt&&future_sq==best_sq&&local_peak==best_local&&press==best_press&&change<best_change)){
                    best_pk=pk;
                    best_fg=future_fg;
                    best_over=add_over;
                    best_cbt=after_cbt;
            ...
            while(mk){
                ...
                bundle_masks[slot][ph]&=~(1u<<cp);
                bundle_masks[slot][ph]|=(1u<<best_pk);
                mk&=mk-1;
            }
...
void solve_job(){
    ...
    // Small-card bundle rebuild: coordinate a few high-CB cards on the same leaf
    // and rebuild them jointly under a strict fg/jm rollback gate.
    run_card_bundle_rebuild(m);
