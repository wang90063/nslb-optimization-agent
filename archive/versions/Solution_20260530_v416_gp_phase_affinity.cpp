...
static unsigned int gp_card_pm[MAX_CARDS][MAX_PHASES];
static unsigned char gp_card_seen[MAX_CARDS];
static int gp_card_list[MAX_CARDS];
...
inline int better_global_price_choice_p32r4(const GlobalPriceChoice &a, const GlobalPriceChoice &b){
    ...
    if(a.future_over!=b.future_over) return a.future_over<b.future_over;
    if(a.card_phase_penalty!=b.card_phase_penalty) return a.card_phase_penalty<b.card_phase_penalty;
...
void run_greedy_global_price(int m, int hardcap, int rev=0, int price_w=1, int local_w=2, int global_w=1, int card_affinity_mode=0){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    int gp_card_cnt=0;
    if(rev>=2){
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        if(card_affinity_mode && !gp_card_seen[src_card]){
            gp_card_seen[src_card]=1;
            gp_card_list[gp_card_cnt++]=src_card;
            memset(gp_card_pm[src_card],0,m*sizeof(gp_card_pm[src_card][0]));
        }
        int sl=fl_sl[i],dl=fl_dl[i];
        if(sl==dl){fl_port[i]=-1;continue;}
        unsigned int mask=fl_pmask[i];
        ...
        int bt_over=0x7fffffff,bt_sum=0x7fffffff;
        long long bt_sq=0x7fffffffffffffffLL;
        GlobalPriceChoice best_choice = {0x7fffffff,0x7fffffff,0x7fffffff,0x7fffffff,0x7fffffff,0x7fffffff,0x7fffffffffffffffLL,0x7fffffff,0x7fffffff,0x7fffffff};
        for(int pk=0;pk<g_p;++pk){
            int local_max=0;
            int exceeds=0;
            int use_future_tie=(hardcap&&g_p>=16&&g_hist_max_jm>g_r);
            int local_over=0;
            int card_phase_penalty=0;
            int mo=0,mi=0;
            unsigned int m2=mask;
            while(m2){
                int ph=__builtin_ctz(m2);
                ...
                if(card_affinity_mode && use_p32_lex){
                    unsigned int pm=gp_card_pm[src_card][ph];
                    if(pm&(1u<<pk)) card_phase_penalty -= 1;
                    else if(pm) card_phase_penalty += 2;
                }
                if(hardcap&&g_p>=16&&v>g_r) use_future_tie=1;
            ...
            if(use_p32_lex){
                ...
                cur_choice.card_phase_penalty = card_phase_penalty;
        ...
        while(m2){
            ...
            out_load[sl][bp][ph]++;
            in_load[dl][bp][ph]++;
            if(card_affinity_mode) gp_card_pm[src_card][ph]|=(1u<<bp);
            m2&=m2-1;
        }
    }
    if(card_affinity_mode){
        for(int i=0;i<gp_card_cnt;++i) gp_card_seen[gp_card_list[i]]=0;
    }
}
...
void solve_job(){
    ...
    if(g_p>=32&&g_r>=4&&fl_count<=7000){
        ...
        cur_uses_global_price=1;
        run_greedy_global_price(m, 1, 1, 1, 2, 1, 1);
        run_swap(m);
        TRY_STRATEGY();
