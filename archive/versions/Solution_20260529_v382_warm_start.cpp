...
static int g_hist_max_jm;
static short prev_port_pref[MAX_LEAFS][MAX_LEAFS];
static int prev_port_valid;
...
void run_greedy_warm(int m, int hardcap, int rev=0){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    for(int ii=0;ii<fl_count;++ii){
        int i=(rev==0)?ii:(fl_count-1-ii);
        int sl=fl_sl[i],dl=fl_dl[i];
        if(sl==dl){fl_port[i]=-1;continue;}
        unsigned int mask=fl_pmask[i];
        int bp=0,bc=0x7fffffff;
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            while(m2){
                ...
                if(hardcap&&(o>g_r||iv>g_r)) exceeds=1;
                if(hardcap&&g_p>=16&&v>g_r) use_future_tie=1;
                if(use_future_tie){if(o>mo)mo=o;if(iv>mi)mi=iv;}
                m2&=m2-1;
            }
            int go=global_out[sl][pk],gi=global_in[dl][pk];
            int gv=go>gi?go:gi;
            int cost=local_max*2+gv;
            if(exceeds) cost+=10000;
            int cand_over=0,cand_sum=0;
            long long cand_sq=0;
            if(use_future_tie){
            ...
            int is_pref=(pref>=0&&pk==pref);
            int better=0;
            if(cost<bc) better=1;
            else if(cost==bc){
                if(use_future_tie){
                    if(cand_over<bt_over) better=1;
                    else if(cand_over==bt_over&&cand_sq<bt_sq) better=1;
                    else if(cand_over==bt_over&&cand_sq==bt_sq&&cand_sum<bt_sum) better=1;
                    else if(cand_over==bt_over&&cand_sq==bt_sq&&cand_sum==bt_sum&&is_pref&&!bp_is_pref) better=1;
                } else {
                    if(is_pref&&!bp_is_pref) better=1;
                }
            }
            if(better){
                bc=cost;bp=pk;bt_over=cand_over;bt_sq=cand_sq;bt_sum=cand_sum;bp_is_pref=is_pref;
            }
        }
        fl_port[i]=(short)bp;
        unsigned int m2=mask;
...
void solve_job(){
    ...
    // S_warm: warm-start from previous job's port preference
    if(prev_port_valid){
        run_greedy_warm(m, 1);
        run_swap(m);
        run_jm_repair(m);
    ...
    if(g_r<=3&&fl_count<=6000){
                ...
                for(int fi=0;fi<fl_count;++fi){
                    ...
                    for(int bfi=0;bfi<fl_count;++bfi){
                        ...
                        for(int ap=0;ap<g_p;++ap){
                            ...
                            if(!bok) continue;
                            // For small-p cases, also check blocker doesn't increase max-phase-load
                            if(g_p<16){
                                int cmx_o=0,cmx_i=0;
                                for(int ph=0;ph<m;++ph){
                                    if(out_load[bsl][ap][ph]>cmx_o)cmx_o=out_load[bsl][ap][ph];
                                    if(in_load[bdl][ap][ph]>cmx_i)cmx_i=in_load[bdl][ap][ph];
                                }
                                m2=bmask;int mok=1;
                                while(m2){int ph=__builtin_ctz(m2);
                                    if(out_load[bsl][ap][ph]+1>cmx_o){mok=0;break;}
                                    if(in_load[bdl][ap][ph]+1>cmx_i){mok=0;break;}
                                    m2&=m2-1;}
                                if(!mok) continue;
                            }
        ...
        // Post-check: revert if overall metrics worsened (jm/fg/ci priority)
        EvalMetrics mr_after=collect_metrics(m);
        if(!better_metrics(mr_after,mr_base)&&(mr_after.fg!=mr_base.fg||mr_after.jm!=mr_base.jm||mr_after.ci!=mr_base.ci)){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));
        }
    ...
    // Compute port preference for next job
    {
        static short cnt[MAX_LEAFS][MAX_LEAFS][MAX_PORTS];
        memset(cnt,0,g_l*g_l*MAX_PORTS*sizeof(short));
        for(int i=0;i<fl_count;++i){
            int sl=fl_sl[i],dl=fl_dl[i],p=fl_port[i];
            if(p>=0) cnt[sl][dl][p]++;
        }
        for(int sl=0;sl<g_l;++sl)
            for(int dl=0;dl<g_l;++dl){
                int bp=-1,bc=0;
                for(int pk=0;pk<g_p;++pk)
                    if(cnt[sl][dl][pk]>bc){bc=cnt[sl][dl][pk];bp=pk;}
                prev_port_pref[sl][dl]=(short)bp;
            }
        prev_port_valid=1;
    }
...
int main(){
    ...
    int n=fast_read_int();
    g_n=n; g_job_idx=0; g_hist_max_jm=0; prev_port_valid=0;
