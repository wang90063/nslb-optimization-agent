...
// local_w: weight on local max, global_w: weight on global, hardcap: 1=penalize >r, 2=penalize >=r, rev: reverse order
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            while(m2){
                ...
                if(hardcap==1&&(o>g_r||iv>g_r)) exceeds=1;
                if(hardcap==2&&(o>=g_r||iv>=g_r)) exceeds=1;
                m2&=m2-1;
            }
...
void solve_job(){
    ...
    // S7: strict hardcap (penalize >=r, not just >r) + swap
    run_greedy(m, 2, 1, 2);
    run_swap(m);
    TRY_STRATEGY();

    // S8: strict hardcap + reversed + swap
    run_greedy(m, 2, 1, 2, 1);
    run_swap(m);
    TRY_STRATEGY();
