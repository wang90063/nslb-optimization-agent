...
static int total_cbt_cards[MAX_FLOWS];
static unsigned char total_cbt_seen[MAX_CARDS];

inline int total_cbtphsc_from_card_state(int m){
    int card_cnt=0;
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
        int c=fl_src[i];
        if(total_cbt_seen[c]) continue;
        total_cbt_seen[c]=1;
        total_cbt_cards[card_cnt++]=c;
    }
    int total=0;
    for(int i=0;i<card_cnt;++i){
        int c=total_cbt_cards[i];
...
void run_cross_dest_swap(int m){
    ...
    int guard_p32_price=(g_p>=32&&g_r>=4&&fl_count<=7000);
    EvalMetrics guard_base;
    int guard_base_cbt=0;
    if(guard_p32_price){
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        guard_base=collect_metrics(m);
    }
    ...
    for(int iter=0;iter<cd_max_iter;++iter){
        ...
        if(guard_p32_price&&iter==0) guard_base_cbt=total_cbtphsc_from_card_state(m);
    ...
    if(guard_p32_price){
        EvalMetrics guard_after=collect_metrics(m);
        int guard_after_cbt=total_cbtphsc_from_card_state(m);
        int keep=0;
        if(guard_after.score_jm<guard_base.score_jm||
           guard_after.score_fg<guard_base.score_fg||
           guard_after.ci<guard_base.ci||
           guard_after.future_over<guard_base.future_over){
            keep=1;
        } else if(guard_after.score_jm==guard_base.score_jm&&
                  guard_after.score_fg==guard_base.score_fg&&
                  guard_after.ci==guard_base.ci&&
                  guard_after.future_over==guard_base.future_over&&
                  guard_after_cbt<guard_base_cbt){
            keep=1;
        } else if(better_metrics(guard_after,guard_base)){
            keep=1;
        }
        if(!keep){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
