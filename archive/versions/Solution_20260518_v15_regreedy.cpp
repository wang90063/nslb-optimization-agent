...
void solve_job(){
    ...
    // Pass 2: re-greedy - remove each flow and reassign with full context
    for(int i=0;i<fl_count;++i){
        int sl=fl_src[i]/g_pr,dl=fl_dst[i]/g_pr;
        if(sl==dl) continue;
        int old_port=fl_port[i];
        unsigned int mask=fl_pmask[i];
        // remove from current port
        unsigned int m2=mask;
        while(m2){int ph=__builtin_ctz(m2);out_load[sl][old_port][ph]--;in_load[dl][old_port][ph]--;m2&=m2-1;}
        // find best port with full context
        int bp=0,bc=0x7fffffff;
        for(int pk=0;pk<g_p;++pk){
            int cost=0;
            m2=mask;
