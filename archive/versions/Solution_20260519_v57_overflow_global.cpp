...
void solve_job(){
    ...
    // S6e2-S6e4: cheap global-aware candidates for overflow cases.
    // These reuse the more global-sensitive greedy costs without paying for
    // the expensive global_swap/LNS tail that is only worthwhile on balanced jobs.
    run_greedy(m, 1, 3, 1);
    run_swap(m);
    TRY_STRATEGY();
