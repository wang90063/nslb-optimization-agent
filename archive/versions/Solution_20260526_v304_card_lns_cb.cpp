...
void run_card_lns_cb(int m){
    static int card_flows[200];
    static short card_saved_port[200];
    static int card_fl_cnt;
    static int card_list[12800];
    static int card_cb[12800];
    int card_cnt=0;
    memset(cpm,0,sizeof(cpm));memset(cppc,0,sizeof(cppc));
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
    ...
    for(int c=0;c<g_l*g_p*g_r;++c){
        int cb=card_cbtphsc(c,m);
        if(cb>0){card_list[card_cnt]=c;card_cb[card_cnt]=cb;card_cnt++;}
    }
    if(!card_cnt) return;
    for(int ci=0;ci<card_cnt-1;++ci)
        for(int cj=ci+1;cj<card_cnt;++cj)
            if(card_cb[cj]>card_cb[ci]){
                int t=card_list[ci];card_list[ci]=card_list[cj];card_list[cj]=t;
                t=card_cb[ci];card_cb[ci]=card_cb[cj];card_cb[cj]=t;
            }
    // Compute MM-safe bounds ONCE from original state
    static short pre_mo[MAX_LEAFS][MAX_PORTS];
    static short pre_mi[MAX_LEAFS][MAX_PORTS];
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            short mo=0,mi=0;
            for(int ph=0;ph<m;++ph){
            ...
            pre_mo[leaf][pk]=mo;pre_mi[leaf][pk]=mi;
        }
    int max_cards=card_cnt<60?card_cnt:60;
    for(int ci=0;ci<max_cards;++ci){
        int card=card_list[ci];
        card_fl_cnt=0;
        for(int i=0;i<fl_count;++i){
            if(fl_src[i]==card&&fl_sl[i]!=fl_dl[i]&&fl_port[i]>=0){
                if(card_fl_cnt>=200) break;
                card_flows[card_fl_cnt]=i;
                card_saved_port[card_fl_cnt]=(short)fl_port[i];
                card_fl_cnt++;
            }
        }
        if(card_fl_cnt<2||card_fl_cnt>=200) continue;
        int before_cb=card_cbtphsc(card,m);
        if(before_cb==0) continue;
        for(int k=0;k<card_fl_cnt;++k){
            int fi=card_flows[k];
            int sl=fl_sl[fi],dl=fl_dl[fi],cp=fl_port[fi];
            unsigned int mk=fl_pmask[fi];
            while(mk){int ph=__builtin_ctz(mk);
                out_load[sl][cp][ph]--;in_load[dl][cp][ph]--;
                cppc[card][ph][cp]--;
                if(!cppc[card][ph][cp])cpm[card][ph]&=~(1u<<cp);
                mk&=mk-1;}
        }

        // Sort card flows by min_phase for phase-sequential reassignment
        for(int a=0;a<card_fl_cnt-1;++a)
            for(int b=a+1;b<card_fl_cnt;++b){
                int pa=__builtin_ctz(fl_pmask[card_flows[a]]);
                int pb=__builtin_ctz(fl_pmask[card_flows[b]]);
                if(pb<pa){int t=card_flows[a];card_flows[a]=card_flows[b];card_flows[b]=t;
                    short ts=card_saved_port[a];card_saved_port[a]=card_saved_port[b];card_saved_port[b]=ts;}
            }
        // Reassign with CB-aware cost, MM-safe constraint
        short card_ph_used[32];
        memset(card_ph_used,-1,sizeof(card_ph_used));
        int lns_aborted=0;
        for(int k=0;k<card_fl_cnt;++k){
            int fi=card_flows[k];
            int sl=fl_sl[fi],dl=fl_dl[fi];
            unsigned int mask=fl_pmask[fi];
            int min_ph=__builtin_ctz(mask);
            int max_ph=31-__builtin_clz(mask);
            int adj=-1;
            if(min_ph>0&&card_ph_used[min_ph-1]>=0) adj=card_ph_used[min_ph-1];
            if(adj<0&&max_ph<m-1&&card_ph_used[max_ph+1]>=0) adj=card_ph_used[max_ph+1];
            int bp=-1,bc=0x7fffffff;
            for(int pk=0;pk<g_p;++pk){
                int cost=0,ok=1;
                unsigned int m2=mask;
                while(m2){int ph=__builtin_ctz(m2);
                    if(out_load[sl][pk][ph]+1>pre_mo[sl][pk]){ok=0;break;}
                    if(in_load[dl][pk][ph]+1>pre_mi[dl][pk]){ok=0;break;}
                    int o=out_load[sl][pk][ph]+1;
                    int iv=in_load[dl][pk][ph]+1;
                    int v=o>iv?o:iv;if(v>cost)cost=v;
                    m2&=m2-1;}
                if(!ok) continue;
                if(adj>=0&&pk==adj) cost-=2;
                if(cost<bc){bc=cost;bp=pk;}
            }
            if(bp<0){
                // Can't reassign safely — revert all assigned so far and skip card
                for(int r=0;r<k;++r){
                    int rfi=card_flows[r];
                    int rsl=fl_sl[rfi],rdl=fl_dl[rfi],rcp=fl_port[rfi];
                    unsigned int rmk=fl_pmask[rfi];
                    while(rmk){int ph=__builtin_ctz(rmk);
                        out_load[rsl][rcp][ph]--;in_load[rdl][rcp][ph]--;
                        cppc[card][ph][rcp]--;
                        if(!cppc[card][ph][rcp])cpm[card][ph]&=~(1u<<rcp);
                        rmk&=rmk-1;}
                }
                // Restore all flows to original ports
                for(int r=0;r<card_fl_cnt;++r){
                    int rfi=card_flows[r];
                    int rsl=fl_sl[rfi],rdl=fl_dl[rfi];
                    int rp=card_saved_port[r];
                    fl_port[rfi]=rp;
                    unsigned int rmk=fl_pmask[rfi];
                    while(rmk){int ph=__builtin_ctz(rmk);
                        out_load[rsl][rp][ph]++;in_load[rdl][rp][ph]++;
                        cppc[card][ph][rp]++;cpm[card][ph]|=(1u<<rp);
                        rmk&=rmk-1;}
                }
                bp=-2; lns_aborted=1; break;
            }
            fl_port[fi]=(short)bp;
            unsigned int mk=mask;
            while(mk){int ph=__builtin_ctz(mk);
                out_load[sl][bp][ph]++;in_load[dl][bp][ph]++;
                cppc[card][ph][bp]++;cpm[card][ph]|=(1u<<bp);
                card_ph_used[ph]=bp;mk&=mk-1;}
        }
        if(lns_aborted) continue;
        int after_cb=card_cbtphsc(card,m);
        if(after_cb>=before_cb){
            // Revert
            for(int k=0;k<card_fl_cnt;++k){
                int fi=card_flows[k];
                int sl=fl_sl[fi],dl=fl_dl[fi],cp=fl_port[fi];
            ...
            for(int k=0;k<card_fl_cnt;++k){
                int fi=card_flows[k];
                int sl=fl_sl[fi],dl=fl_dl[fi];
                int rp=card_saved_port[k];
                fl_port[fi]=rp;
                unsigned int mk=fl_pmask[fi];
                while(mk){int ph=__builtin_ctz(mk);
                    out_load[sl][rp][ph]++;in_load[dl][rp][ph]++;
                    cppc[card][ph][rp]++;cpm[card][ph]|=(1u<<rp);
                    mk&=mk-1;}
            }
        }
