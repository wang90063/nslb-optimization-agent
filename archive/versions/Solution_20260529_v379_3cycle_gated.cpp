...
inline int card_phase_affinity_bias(int card, unsigned int mask, int pk){
    int bias=0;
    unsigned int prev_mask=mask>>1;
    unsigned int next_mask=mask<<1;
    unsigned int mk=prev_mask|next_mask;
    mk&=((1u<<MAX_PHASES)-1);
    while(mk){
        int ph=__builtin_ctz(mk);
        if(cppc[card][ph][pk]>0) bias++;
        mk&=mk-1;
    }
    return bias;
}

void run_greedy_cardphase_affinity(int m, int hardcap, int rev=0){
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            int gv=go>gi?go:gi;
            int aff=card_phase_affinity_bias(card,mask,pk);
            int cost=local_max*2+gv-aff*2;
            ...
            if(cost<bc){
                bc=cost;bp=pk;best_aff=aff;
                bt_over=cand_over;bt_sq=cand_sq;bt_sum=cand_sum;
            } else if(cost==bc){
                if(aff>best_aff){
                    bp=pk;best_aff=aff;
                    bt_over=cand_over;bt_sq=cand_sq;bt_sum=cand_sum;
                } else if(use_future_tie&&aff==best_aff){
...
void run_greedy_global_price(int m, int hardcap, int rev=0, int price_w=1, int local_w=2, int global_w=1){
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            int cost=local_max*local_w+gv*global_w+price*price_w+src_price+dst_price;
            if(exceeds) cost+=10000;
            int cand_over=0,cand_sum=0;
            long long cand_sq=0;
            if(use_future_tie){
...
static int cyc_edge_flows[MAX_PORTS][MAX_PORTS][64];
static int cyc_edge_cnt[MAX_PORTS][MAX_PORTS];
static int cyc_edge_cbt[MAX_PORTS][MAX_PORTS][64];

void run_3cycle_swap(int m){
    static short cy_max_out[MAX_LEAFS][MAX_PORTS];
    static short cy_max_in[MAX_LEAFS][MAX_PORTS];
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            short mo=0,mi=0;
            for(int ph=0;ph<m;++ph){
            ...
            cy_max_out[leaf][pk]=mo;cy_max_in[leaf][pk]=mi;
        }
    memset(cpm,0,sizeof(cpm));memset(cppc,0,sizeof(cppc));
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
    ...
    for(int iter=0;iter<2;++iter){
        int improved=0;
        for(int sl=0;sl<g_l;++sl){
            memset(cyc_edge_cnt,0,sizeof(cyc_edge_cnt));
            for(int i=0;i<fl_count;++i){
                if(fl_sl[i]!=sl||fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
                int ci=fl_src[i],cp=fl_port[i];
                int cb=card_cbtphsc(ci,m);
                if(cb==0) continue;
                int dp_cnt[32]={};
                for(int ph=0;ph<m;++ph)
                    for(int pk=0;pk<g_p;++pk) dp_cnt[pk]+=cppc[ci][ph][pk];
                int best_pk=-1,best_sc=-1;
                for(int pk=0;pk<g_p;++pk){
                    if(pk==cp) continue;
                    if(dp_cnt[pk]>best_sc){best_sc=dp_cnt[pk];best_pk=pk;}
                }
                if(best_pk<0||best_sc<=dp_cnt[cp]) continue;
                int idx=cyc_edge_cnt[cp][best_pk];
                if(idx>=64) continue;
                cyc_edge_flows[cp][best_pk][idx]=i;
                cyc_edge_cbt[cp][best_pk][idx]=cb;
                cyc_edge_cnt[cp][best_pk]++;
            }
            for(int px=0;px<g_p;++px)
            for(int py=0;py<g_p;++py){
                if(px==py||!cyc_edge_cnt[px][py]) continue;
                for(int pz=0;pz<g_p;++pz){
                    if(pz==px||pz==py) continue;
                    if(!cyc_edge_cnt[py][pz]||!cyc_edge_cnt[pz][px]) continue;
                    int ba=-1,bb=-1,bc=-1,best_tc=0;
                    int la=cyc_edge_cnt[px][py];if(la>8)la=8;
                    int lb=cyc_edge_cnt[py][pz];if(lb>8)lb=8;
                    int lc=cyc_edge_cnt[pz][px];if(lc>8)lc=8;
                    for(int ia=0;ia<la;++ia){
                      int fa=cyc_edge_flows[px][py][ia];
                      for(int ib=0;ib<lb;++ib){
                        int fb=cyc_edge_flows[py][pz][ib];
                        if(fb==fa||fl_src[fb]==fl_src[fa]) continue;
                        for(int ic=0;ic<lc;++ic){
                          int fc=cyc_edge_flows[pz][px][ic];
                          if(fc==fa||fc==fb) continue;
                          if(fl_src[fc]==fl_src[fa]||fl_src[fc]==fl_src[fb]) continue;
                          int tc=cyc_edge_cbt[px][py][ia]+cyc_edge_cbt[py][pz][ib]+cyc_edge_cbt[pz][px][ic];
                          if(tc>best_tc){best_tc=tc;ba=fa;bb=fb;bc=fc;}
                        }}}
                    if(ba<0) continue;
                    unsigned int ma=fl_pmask[ba],mb=fl_pmask[bb],mc=fl_pmask[bc];
                    int dla=fl_dl[ba],dlb=fl_dl[bb],dlc=fl_dl[bc];
                    int ok=1;
                    for(int ph=0;ph<m&&ok;++ph){
                        int ha=(ma>>ph)&1,hb=(mb>>ph)&1,hc=(mc>>ph)&1;
                        if(!ha&&!hb&&!hc) continue;
                        int dpx=-ha+hc,dpy=ha-hb,dpz=hb-hc;
                        if(dpx>0&&out_load[sl][px][ph]+dpx>cy_max_out[sl][px]){ok=0;break;}
                        if(dpy>0&&out_load[sl][py][ph]+dpy>cy_max_out[sl][py]){ok=0;break;}
                        if(dpz>0&&out_load[sl][pz][ph]+dpz>cy_max_out[sl][pz]){ok=0;break;}
                        if(ha&&in_load[dla][py][ph]+1>cy_max_in[dla][py]){ok=0;break;}
                        if(hb&&in_load[dlb][pz][ph]+1>cy_max_in[dlb][pz]){ok=0;break;}
                        if(hc&&in_load[dlc][px][ph]+1>cy_max_in[dlc][px]){ok=0;break;}
                    }
                    if(!ok) continue;
                    int ca=fl_src[ba],cbb=fl_src[bb],cc=fl_src[bc];
                    int cb0=card_cbtphsc(ca,m)+card_cbtphsc(cbb,m)+card_cbtphsc(cc,m);
                    unsigned int mk;
                    mk=ma;while(mk){int ph=__builtin_ctz(mk);
                      out_load[sl][px][ph]--;out_load[sl][py][ph]++;
                      in_load[dla][px][ph]--;in_load[dla][py][ph]++;
                      cppc[ca][ph][px]--;if(!cppc[ca][ph][px])cpm[ca][ph]&=~(1u<<px);
                      cppc[ca][ph][py]++;cpm[ca][ph]|=(1u<<py);mk&=mk-1;}
                    fl_port[ba]=py;
                    mk=mb;while(mk){int ph=__builtin_ctz(mk);
                      out_load[sl][py][ph]--;out_load[sl][pz][ph]++;
                      in_load[dlb][py][ph]--;in_load[dlb][pz][ph]++;
                      cppc[cbb][ph][py]--;if(!cppc[cbb][ph][py])cpm[cbb][ph]&=~(1u<<py);
                      cppc[cbb][ph][pz]++;cpm[cbb][ph]|=(1u<<pz);mk&=mk-1;}
                    fl_port[bb]=pz;
                    mk=mc;while(mk){int ph=__builtin_ctz(mk);
                      out_load[sl][pz][ph]--;out_load[sl][px][ph]++;
                      in_load[dlc][pz][ph]--;in_load[dlc][px][ph]++;
                      cppc[cc][ph][pz]--;if(!cppc[cc][ph][pz])cpm[cc][ph]&=~(1u<<pz);
                      cppc[cc][ph][px]++;cpm[cc][ph]|=(1u<<px);mk&=mk-1;}
                    fl_port[bc]=px;
                    int cb1=card_cbtphsc(ca,m)+card_cbtphsc(cbb,m)+card_cbtphsc(cc,m);
                    if(cb1<cb0){improved=1;}
                    else{
                      mk=ma;while(mk){int ph=__builtin_ctz(mk);
                        out_load[sl][py][ph]--;out_load[sl][px][ph]++;
                        in_load[dla][py][ph]--;in_load[dla][px][ph]++;
                        cppc[ca][ph][py]--;if(!cppc[ca][ph][py])cpm[ca][ph]&=~(1u<<py);
                        cppc[ca][ph][px]++;cpm[ca][ph]|=(1u<<px);mk&=mk-1;}
                      fl_port[ba]=px;
                      mk=mb;while(mk){int ph=__builtin_ctz(mk);
                        out_load[sl][pz][ph]--;out_load[sl][py][ph]++;
                        in_load[dlb][pz][ph]--;in_load[dlb][py][ph]++;
                        cppc[cbb][ph][pz]--;if(!cppc[cbb][ph][pz])cpm[cbb][ph]&=~(1u<<pz);
                        cppc[cbb][ph][py]++;cpm[cbb][ph]|=(1u<<py);mk&=mk-1;}
                      fl_port[bb]=py;
                      mk=mc;while(mk){int ph=__builtin_ctz(mk);
                        out_load[sl][px][ph]--;out_load[sl][pz][ph]++;
                        in_load[dlc][px][ph]--;in_load[dlc][pz][ph]++;
                        cppc[cc][ph][px]--;if(!cppc[cc][ph][px])cpm[cc][ph]&=~(1u<<px);
                        cppc[cc][ph][pz]++;cpm[cc][ph]|=(1u<<pz);mk&=mk-1;}
                      fl_port[bc]=pz;
                    }
                }}
        }
...
void solve_job(){
    ...
    // S6e6-S6e7: card-phase affinity greedy. Prefer reusing ports that
    // already appear on adjacent phases of the same source card.
    run_greedy_cardphase_affinity(m, 1);
    run_swap(m);
    run_jm_repair(m);
    TRY_STRATEGY();

    run_greedy_cardphase_affinity(m, 1, 1);
    run_swap(m);
    run_jm_repair(m);
    TRY_STRATEGY();
    ...
    // Gated 3-cycle swap at end of pipeline (only keep if metrics improve)
    if(!g_time_tight){
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        EvalMetrics cyc_base=collect_metrics(m);
        run_3cycle_swap(m);
        EvalMetrics cyc_after=collect_metrics(m);
        if(!better_metrics(cyc_after,cyc_base)){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));
        }
