...
// Cross-destination swap: swap flows across different dest leaves to improve Cbtphsc
// without increasing any port's max-phase-load (safe for p<16 / low-r cases)
...
void run_cross_dest_swap(int m){
    ...
    return; // DIAGNOSTIC: do nothing, just test if static arrays affect MM
    static short cd_bk_out[MAX_LEAFS][MAX_PORTS][MAX_PHASES];
    static short cd_bk_in[MAX_LEAFS][MAX_PORTS][MAX_PHASES];
    static short cd_bk_port[MAX_FLOWS];
    memcpy(cd_bk_out, out_load, g_l*sizeof(out_load[0]));
    memcpy(cd_bk_in, in_load, g_l*sizeof(in_load[0]));
    memcpy(cd_bk_port, fl_port, fl_count*sizeof(short));
    ...
    for(int iter=0;iter<3;++iter){
        ...
        for(int fi=0;fi<fl_count;++fi){
            ...
            int ci_before=card_cbtphsc(ci,m);
            unsigned int card_ports=0;
            for(int ph=0;ph<m;++ph) card_ports|=cpm[ci][ph];
            int best_net=0,best_j=-1,best_py=-1;
            unsigned int mk_p=card_ports&~(1u<<px);
            while(mk_p){
                int py=__builtin_ctz(mk_p);mk_p&=mk_p-1;
                // Quick check: would moving fi to py reduce card cbtphsc?
                unsigned int mk=mask_a;
                while(mk){int ph=__builtin_ctz(mk);
                    cppc[ci][ph][px]--;if(!cppc[ci][ph][px])cpm[ci][ph]&=~(1u<<px);
                    cppc[ci][ph][py]++;cpm[ci][ph]|=(1u<<py);mk&=mk-1;}
                int ci_after_move=card_cbtphsc(ci,m);
                mk=mask_a;
                while(mk){int ph=__builtin_ctz(mk);
                    cppc[ci][ph][py]--;if(!cppc[ci][ph][py])cpm[ci][ph]&=~(1u<<py);
                    cppc[ci][ph][px]++;cpm[ci][ph]|=(1u<<px);mk&=mk-1;}
                if(ci_after_move>=ci_before) continue;
                ...
                for(int bi=bstart;bi<bend;++bi){
                    ...
                    if(!ok) continue;
                    int before=ci_before+card_cbtphsc(cj,m);
                    ...
                    if(net>best_net){best_net=net;best_j=j;best_py=py;}
                }
            }
            if(best_j<0) continue;
            {
                int j=best_j,py=best_py,cj=fl_src[j],dl_b=fl_dl[j];
                unsigned int mask_b=fl_pmask[j];
                unsigned int only_a=mask_a&~mask_b,only_b=mask_b&~mask_a;
                unsigned int mk,m2;
                mk=mask_a;while(mk){int ph=__builtin_ctz(mk);
    ...
    // Safety: revert if total max-phase-load contribution increases
    int sum_before=0,sum_after=0;
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            short mo_b=0,mi_b=0,mo_a=0,mi_a=0;
            for(int ph=0;ph<m;++ph){
                if(cd_bk_out[leaf][pk][ph]>mo_b)mo_b=cd_bk_out[leaf][pk][ph];
                if(cd_bk_in[leaf][pk][ph]>mi_b)mi_b=cd_bk_in[leaf][pk][ph];
                if(out_load[leaf][pk][ph]>mo_a)mo_a=out_load[leaf][pk][ph];
                if(in_load[leaf][pk][ph]>mi_a)mi_a=in_load[leaf][pk][ph];
            }
            sum_before+=mo_b+mi_b;
            sum_after+=mo_a+mi_a;
        }
    if(sum_after>sum_before){
        memcpy(out_load, cd_bk_out, g_l*sizeof(out_load[0]));
        memcpy(in_load, cd_bk_in, g_l*sizeof(in_load[0]));
        memcpy(fl_port, cd_bk_port, fl_count*sizeof(short));
    }
}
