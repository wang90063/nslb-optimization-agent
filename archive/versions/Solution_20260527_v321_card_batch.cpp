...
void run_card_batch_consolidate(int m){
    ...
    for(int iter=0;iter<4;++iter){
        int improved=0;
        for(int fi=0;fi<fl_count;++fi){
            int ci=fl_src[fi],sl=fl_sl[fi],dl=fl_dl[fi],px=fl_port[fi];
            if(sl==dl||px<0) continue;
            if(card_cbtphsc(ci,m)==0) continue;
            unsigned int mask=fl_pmask[fi];
            int target_port=-1,best_cb_delta=0;
            for(int py=0;py<g_p;++py){
                if(py==px) continue;
                unsigned int m2=mask;int feasible=1;
                while(m2){int ph=__builtin_ctz(m2);
                    if(out_load[sl][py][ph]+1>cb_max_out[sl][py]){feasible=0;break;}
                    if(in_load[dl][py][ph]+1>cb_max_in[dl][py]){feasible=0;break;}
                    m2&=m2-1;}
                ...
                int cb_delta=card_cbtphsc(ci,m);
                m2=mask;
                while(m2){int ph=__builtin_ctz(m2);
                    cppc[ci][ph][py]--;if(!cppc[ci][ph][py])cpm[ci][ph]&=~(1u<<py);
                    cppc[ci][ph][px]++;cpm[ci][ph]|=(1u<<px);m2&=m2-1;}
                cb_delta-=card_cbtphsc(ci,m);
                if(cb_delta<best_cb_delta){
                    best_cb_delta=cb_delta;target_port=py;
                }
            }
            if(target_port>=0&&best_cb_delta<0){
                int py=target_port;
                unsigned int m2=mask;
                while(m2){int ph=__builtin_ctz(m2);
                    out_load[sl][px][ph]--;out_load[sl][py][ph]++;
                    in_load[dl][px][ph]--;in_load[dl][py][ph]++;
                    cppc[ci][ph][px]--;if(!cppc[ci][ph][px])cpm[ci][ph]&=~(1u<<px);
                    cppc[ci][ph][py]++;cpm[ci][ph]|=(1u<<py);m2&=m2-1;}
                fl_port[fi]=(short)py;
                improved++;
