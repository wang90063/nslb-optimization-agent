...
void solve_job(){
    ...
    #define TRY_STRATEGY() do { \
        ...
        if(cur_eval.score_jm<=g_r&&cur_eval.score_fg<=g_r&&cur_eval.ci==0){ \
            memset(cpm,0,sizeof(cpm)); \
            for(int _i=0;_i<fl_count;++_i){ \
                if(fl_sl[_i]==fl_dl[_i]||fl_port[_i]<0) continue; \
                int _c=fl_src[_i]; unsigned int _mk=fl_pmask[_i]; \
                while(_mk){int _ph=__builtin_ctz(_mk);cpm[_c][_ph]|=(1u<<fl_port[_i]);_mk&=_mk-1;} \
            } \
            int _tcb=0; \
            for(int _c=0;_c<g_l*g_p*g_r;++_c) \
                for(int _ph=0;_ph<m-1;++_ph) \
                    if(cpm[_c][_ph]&&cpm[_c][_ph+1]&&cpm[_c][_ph]!=cpm[_c][_ph+1]) _tcb++; \
            cur_eval.cb=_tcb; \
        } \
