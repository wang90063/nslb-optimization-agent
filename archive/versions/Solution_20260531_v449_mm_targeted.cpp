...
// MM-targeted greedy: penalizes the globally-hottest (leaf,port) pair
void run_greedy_mm_targeted(int m, int local_w, int global_w, int hardcap, int mm_penalty, int rev=0){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    // Find the globally-hottest (leaf, port) pairs
    int hot_leaf_out=-1,hot_port_out=-1,hot_val_out=0;
    int hot_leaf_in=-1,hot_port_in=-1,hot_val_in=0;
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            if(global_out[leaf][pk]>hot_val_out){
                hot_val_out=global_out[leaf][pk];hot_leaf_out=leaf;hot_port_out=pk;}
            if(global_in[leaf][pk]>hot_val_in){
                hot_val_in=global_in[leaf][pk];hot_leaf_in=leaf;hot_port_in=pk;}
        }
    if(rev>=2){
        unsigned int seed=rev*2654435761u;
        for(int a=fl_count-1;a>0;--a){
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            int cost=local_max*local_w+gv*global_w;
            if(exceeds) cost+=10000;
            // MM penalty: extra cost for the globally-hottest port
            if(sl==hot_leaf_out&&pk==hot_port_out) cost+=mm_penalty*local_max;
            if(dl==hot_leaf_in&&pk==hot_port_in) cost+=mm_penalty*local_max;
            if(cost<bc){bc=cost;bp=pk;}
        }
        fl_port[i]=(short)bp;
        unsigned int m2=mask;
...
void solve_job(){
    ...
    // MM-targeted strategies: only on later jobs where global info is meaningful
    if(g_job_idx>=g_n/2){
        run_greedy_mm_targeted(m, 2, 1, 1, 2);
        run_swap(m);
        run_jm_repair(m);
        TRY_STRATEGY();

        run_greedy_mm_targeted(m, 2, 1, 1, 4);
        run_swap(m);
        run_jm_repair(m);
        TRY_STRATEGY();

        run_greedy_mm_targeted(m, 2, 1, 1, 3, 1);
        run_swap(m);
        run_jm_repair(m);
        TRY_STRATEGY();

        run_greedy_mm_targeted(m, 1, 2, 1, 3);
        run_swap(m);
        run_jm_repair(m);
        TRY_STRATEGY();
    }
