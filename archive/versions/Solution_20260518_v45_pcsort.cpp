...
static int fl_count;
static int fl_order[MAX_FLOWS]; // sorted order for greedy
...
// rev: 0=normal, 1=reversed, 2=phase-count sorted (descending)
...
void solve_job(){
    ...
    // Sort fl_order by phase count (descending) using counting sort
    {
        int bcount[MAX_PHASES+1]={};
        for(int i=0;i<fl_count;++i)
            bcount[__builtin_popcount(fl_pmask[i])]++;
        int offsets[MAX_PHASES+1];
        offsets[m]=0;
        for(int b=m-1;b>=0;--b)
            offsets[b]=offsets[b+1]+bcount[b+1];
        for(int i=0;i<fl_count;++i){
    ...
    // S7: phase-count sorted (high-impact flows first) + hardcap + swap
    run_greedy(m, 2, 1, 1, 2);
    run_swap(m);
    TRY_STRATEGY();

    // S8: phase-count sorted + no hardcap + swap
    run_greedy(m, 2, 1, 0, 2);
    run_swap(m);
    TRY_STRATEGY();
