...
void run_relaxed_swap(int m){
    ...
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            ...
            int fo=global_out[leaf][pk]+mo;
            int fi=global_in[leaf][pk]+mi;
            if(fo>cur_fgmax)cur_fgmax=fo;
            if(fi>cur_fgmax)cur_fgmax=fi;
        }
    rs_valid_cnt=0;
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
            ...
            if(gsz>=2&&gsz<=300){
                for(int a=g_start;a<g_end;++a){
                    ...
                    for(int b=a+1;b<g_end;++b){
                        ...
                        int after=card_cbtphsc(ci,m)+card_cbtphsc(cj,m);
                        if(after<before){
                            // Check future_gmax constraint
                            int fg_ok=1;
                            if(dl_i!=dl_j){
                                // Check affected ports: (sl,pi),(sl,pj) out; (dl_i,pi),(dl_i,pj),(dl_j,pi),(dl_j,pj) in
                                short nmo_pi=0,nmo_pj=0;
                                for(int ph=0;ph<m;++ph){
                                    short opi=out_load[sl][pi][ph],opj=out_load[sl][pj][ph];
                                    if((only_i>>ph)&1){opi--;opj++;}
                                    if((only_j>>ph)&1){opj--;opi++;}
                                    if(opi>nmo_pi)nmo_pi=opi;
                                    if(opj>nmo_pj)nmo_pj=opj;
                                }
                                if(global_out[sl][pi]+nmo_pi>cur_fgmax||
                                   global_out[sl][pj]+nmo_pj>cur_fgmax) fg_ok=0;
                                if(fg_ok){
                                    short nmi_di_pi=0,nmi_di_pj=0;
                                    for(int ph=0;ph<m;++ph){
                                        short v1=in_load[dl_i][pi][ph],v2=in_load[dl_i][pj][ph];
                                        if((mi_mask>>ph)&1){v1--;v2++;}
                                        if(v1>nmi_di_pi)nmi_di_pi=v1;
                                        if(v2>nmi_di_pj)nmi_di_pj=v2;
                                    }
                                    if(global_in[dl_i][pi]+nmi_di_pi>cur_fgmax||
                                       global_in[dl_i][pj]+nmi_di_pj>cur_fgmax) fg_ok=0;
                                }
                                if(fg_ok){
                                    short nmi_dj_pi=0,nmi_dj_pj=0;
                                    for(int ph=0;ph<m;++ph){
                                        short v1=in_load[dl_j][pi][ph],v2=in_load[dl_j][pj][ph];
                                        if((mj_mask>>ph)&1){v2--;v1++;}
                                        if(v1>nmi_dj_pi)nmi_dj_pi=v1;
                                        if(v2>nmi_dj_pj)nmi_dj_pj=v2;
                                    }
                                    if(global_in[dl_j][pi]+nmi_dj_pi>cur_fgmax||
                                       global_in[dl_j][pj]+nmi_dj_pj>cur_fgmax) fg_ok=0;
                                }
                            }
                            if(fg_ok){
                            // Commit: update loads
