...
void run_sa_composite(int m){
    ...
    for(int iter=0;;++iter){
        ...
        // Compute CT delta: check if max_phase_load decreases on source ports
        int ct_delta=0;
        {
            int at_max_sl=(0),at_max_dl=(0);
            m2=mask;while(m2){int ph=__builtin_ctz(m2);
                if(out_load[sl][px][ph]==sa_max_out[sl][px]) at_max_sl=1;
                if(in_load[dl][px][ph]==sa_max_in[dl][px]) at_max_dl=1;
                m2&=m2-1;}
            if(at_max_sl){
                short new_max=0;
                for(int ph=0;ph<m;++ph){
                    short v=out_load[sl][px][ph];
                    if(mask&(1u<<ph)) v--;
                    if(v>new_max) new_max=v;
                }
                int old_ct_val=global_out[sl][px]+sa_max_out[sl][px];
                int new_ct_val=global_out[sl][px]+new_max;
                int old_ct=(old_ct_val>g_r)?(old_ct_val-g_r):0;
                int new_ct=(new_ct_val>g_r)?(new_ct_val-g_r):0;
                ct_delta+=new_ct-old_ct;
            }
            if(at_max_dl){
                short new_max=0;
                for(int ph=0;ph<m;++ph){
                    short v=in_load[dl][px][ph];
                    if(mask&(1u<<ph)) v--;
                    if(v>new_max) new_max=v;
                }
                int old_ct_val=global_in[dl][px]+sa_max_in[dl][px];
                int new_ct_val=global_in[dl][px]+new_max;
                int old_ct=(old_ct_val>g_r)?(old_ct_val-g_r):0;
                int new_ct=(new_ct_val>g_r)?(new_ct_val-g_r):0;
                ct_delta+=new_ct-old_ct;
            }
