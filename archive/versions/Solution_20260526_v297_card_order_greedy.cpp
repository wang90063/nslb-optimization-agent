...
static int card_order[MAX_FLOWS];
static int card_order_key[MAX_FLOWS];

static int card_order_cmp(const void *a, const void *b){
    return card_order_key[*(const int*)a]-card_order_key[*(const int*)b];
}

void run_greedy_card_order(int m, int hardcap=1){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    for(int i=0;i<fl_count;++i){
        card_order[i]=i;
        card_order_key[i]=fl_src[i];
    }
    qsort(card_order,fl_count,sizeof(int),card_order_cmp);
    for(int ii=0;ii<fl_count;++ii){
        int i=card_order[ii];
        int sl=fl_sl[i],dl=fl_dl[i];
        if(sl==dl){fl_port[i]=-1;continue;}
...
void solve_job(){
    ...
    if(!huge_job){
        ...
        // SCO: card-order greedy + swap (flows grouped by card)
        run_greedy_card_order(m);
        run_swap(m);
        run_jm_repair(m);
        TRY_STRATEGY();
