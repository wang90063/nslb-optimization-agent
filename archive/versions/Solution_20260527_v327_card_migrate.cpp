...
// Multi-card coordination: migrate cards off congested ports to enable consolidation
void run_card_migrate(int m){
    if(fl_count<10) return;
    clock_t mig_start=clock();
    double mig_budget=0.015;
    if(fl_count>5000) mig_budget=0.008;
    ...
    // For each card with CB > 0: try to consolidate by making room
    static int mig_card_done[MAX_CARDS];
    static int mig_epoch; mig_epoch++;
    for(int i=0;i<fl_count;++i){
        double el=(double)(clock()-mig_start)/CLOCKS_PER_SEC;
        if(el>=mig_budget) break;
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
        int card=fl_src[i];
        if(mig_card_done[card]==mig_epoch) continue;
        mig_card_done[card]=mig_epoch;
        int cb=card_cbtphsc(card,m);
        if(cb==0) continue;
        // Find dominant port for this card
        int port_cnt[MAX_PORTS]={};
        int card_flows[64]; int card_fcnt=0;
        for(int j=0;j<fl_count;++j){
            if(fl_src[j]!=card||fl_sl[j]==fl_dl[j]||fl_port[j]<0) continue;
            port_cnt[fl_port[j]]++;
            if(card_fcnt<64) card_flows[card_fcnt++]=j;
        }
        int dom_port=0;
        for(int p=1;p<g_p;++p) if(port_cnt[p]>port_cnt[dom_port]) dom_port=p;
        if(port_cnt[dom_port]==card_fcnt) continue; // already consolidated
        // Find flows from this card NOT on dominant port
        for(int k=0;k<card_fcnt;++k){
            int fi=card_flows[k];
            if(fl_port[fi]==dom_port) continue;
            int sl=fl_sl[fi],dl=fl_dl[fi];
            unsigned int mask=fl_pmask[fi];
            // Check if flow can move to dom_port directly (within MM)
            int can_move=1;
            unsigned int m2=mask;
            while(m2){int ph=__builtin_ctz(m2);
                if(out_load[sl][dom_port][ph]+1>pre_jm||in_load[dl][dom_port][ph]+1>pre_jm)
                    {can_move=0;break;}
                m2&=m2-1;}
            if(can_move){
                int old_p=fl_port[fi];
                // Compute local CI delta before moving
                int ci_delta=0;
                m2=mask;while(m2){int ph=__builtin_ctz(m2);
                    if(out_load[sl][old_p][ph]>g_r) ci_delta--;
                    if(out_load[sl][dom_port][ph]>=g_r) ci_delta++;
                    if(in_load[dl][old_p][ph]>g_r) ci_delta--;
                    if(in_load[dl][dom_port][ph]>=g_r) ci_delta++;
                    m2&=m2-1;}
                if(ci_delta>0) continue; // would increase CI
                int old_cb=card_cbtphsc(card,m);
                m2=mask;while(m2){int ph=__builtin_ctz(m2);
                    out_load[sl][old_p][ph]--;in_load[dl][old_p][ph]--;
                    cppc[card][ph][old_p]--;if(!cppc[card][ph][old_p])cpm[card][ph]&=~(1u<<old_p);
                    out_load[sl][dom_port][ph]++;in_load[dl][dom_port][ph]++;
                    cppc[card][ph][dom_port]++;cpm[card][ph]|=(1u<<dom_port);
                    m2&=m2-1;}
                int new_cb=card_cbtphsc(card,m);
                // Accept only if CB decreases AND weighted score improves
                // CB weight 5, CI weight 12: need 5*cb_decrease > 12*ci_increase
                int cb_delta=new_cb-old_cb;
                if(cb_delta<0 && 5*(-cb_delta)>12*ci_delta){
                    fl_port[fi]=dom_port;continue;
                }
                // Revert
                m2=mask;while(m2){int ph=__builtin_ctz(m2);
                    out_load[sl][dom_port][ph]--;in_load[dl][dom_port][ph]--;
                    cppc[card][ph][dom_port]--;if(!cppc[card][ph][dom_port])cpm[card][ph]&=~(1u<<dom_port);
                    out_load[sl][old_p][ph]++;in_load[dl][old_p][ph]++;
                    cppc[card][ph][old_p]++;cpm[card][ph]|=(1u<<old_p);
                    m2&=m2-1;}
                continue;
            }
        }
...
void solve_job(){
    ...
    // Multi-card coordination pass
    if(!g_time_tight){
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        int mig_ci0=get_cinphsc(m);
        run_card_migrate(m);
        int mig_ci1=get_cinphsc(m);
        if(mig_ci1>mig_ci0){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));
