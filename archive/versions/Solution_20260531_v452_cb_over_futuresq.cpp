...
void solve_job(){
    ...
    #define TRY_STRATEGY() do { \
        ...
        if(first||better_metrics(cur_eval,best_eval)){ \
            take_strategy=1; best_cb_cache=-1; \
        } else if(cur_eval.score_jm==best_eval.score_jm && \
                  cur_eval.score_fg==best_eval.score_fg && \
                  cur_eval.ci==best_eval.ci && \
                  cur_eval.future_over==best_eval.future_over){ \
            int cur_cb=total_cbtphsc_from_ports(fl_port,m); \
            if(best_cb_cache<0) best_cb_cache=total_cbtphsc_from_ports(sv_port,m); \
            if(cur_cb<best_cb_cache) take_strategy=1; \
        } \
        if(take_strategy){ \
