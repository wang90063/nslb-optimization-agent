...
inline int get_port_max_out(int leaf, int pk, int m){
    int mx=0;
    for(int ph=0;ph<m;++ph){int o=out_load[leaf][pk][ph];if(o>mx)mx=o;}
    return mx;
}
inline int get_port_max_in(int leaf, int pk, int m){
    int mx=0;
    for(int ph=0;ph<m;++ph){int o=in_load[leaf][pk][ph];if(o>mx)mx=o;}
    return mx;
}
...
void solve_job(){
    ...
    // Pass 2: Maxsingler safe swap (same as v33)
    ...
    // Pass 3: Global-max reduction swap (Maxmultir optimization)
    int job_max = get_job_max(m);
    for(int iter=0;iter<30;++iter){
        // Compute future global for all (leaf, port)
        int G_max=0;
        for(int leaf=0;leaf<g_l;++leaf)
            for(int pk=0;pk<g_p;++pk){
                int mo=get_port_max_out(leaf,pk,m);
                int mi=get_port_max_in(leaf,pk,m);
                int fo=global_out[leaf][pk]+mo;
                int fi=global_in[leaf][pk]+mi;
                int fv=fo>fi?fo:fi;
                if(fv>G_max)G_max=fv;
            }
        if(G_max<=g_r) break;
        int improved=0;
        for(int i=0;i<fl_count&&!improved;++i){
            ...
            int cp=fl_port[i];
            unsigned int mask=fl_pmask[i];
            // Check if this flow contributes to a port at G_max
            int fo_sl=global_out[sl][cp]+get_port_max_out(sl,cp,m);
            int fi_sl=global_in[sl][cp]+get_port_max_in(sl,cp,m);
            int fv_sl=fo_sl>fi_sl?fo_sl:fi_sl;
            int fo_dl=global_out[dl][cp]+get_port_max_out(dl,cp,m);
            int fi_dl=global_in[dl][cp]+get_port_max_in(dl,cp,m);
            int fv_dl=fo_dl>fi_dl?fo_dl:fi_dl;
            if(fv_sl!=G_max && fv_dl!=G_max) continue;
            int best_pk=-1, best_new_gmax=G_max;
            for(int pk=0;pk<g_p;++pk){
                if(pk==cp) continue;
                // Check job_max constraint
                int new_local=0, ok=1;
                unsigned int m2=mask;
                while(m2){
                    int ph=__builtin_ctz(m2);
                    int no=out_load[sl][pk][ph]+1;if(no>job_max){ok=0;break;}
                    int ni=in_load[dl][pk][ph]+1;if(ni>job_max){ok=0;break;}
                    if(no>new_local)new_local=no;
                    if(ni>new_local)new_local=ni;
                    m2&=m2-1;
                }
                if(!ok) continue;
                // Simulate move: compute new G_max
                // Affected ports: (sl,cp), (dl,cp), (sl,pk), (dl,pk)
                // Temporarily apply move
                unsigned int m3=mask;
                while(m3){int ph=__builtin_ctz(m3);out_load[sl][cp][ph]--;in_load[dl][cp][ph]--;out_load[sl][pk][ph]++;in_load[dl][pk][ph]++;m3&=m3-1;}
                // Recompute G_max for affected ports
                int new_gmax=0;
                for(int lf=0;lf<g_l;++lf)
                    for(int pp=0;pp<g_p;++pp){
                        int mo=get_port_max_out(lf,pp,m);
                        int mi=get_port_max_in(lf,pp,m);
                        int fo=global_out[lf][pp]+mo;
                        int fi=global_in[lf][pp]+mi;
                        int fv=fo>fi?fo:fi;
                        if(fv>new_gmax)new_gmax=fv;
                    }
                // Revert
                m3=mask;
                while(m3){int ph=__builtin_ctz(m3);out_load[sl][cp][ph]++;in_load[dl][cp][ph]++;out_load[sl][pk][ph]--;in_load[dl][pk][ph]--;m3&=m3-1;}
                if(new_gmax<best_new_gmax){best_new_gmax=new_gmax;best_pk=pk;}
            }
            if(best_pk>=0 && best_new_gmax<G_max){
                unsigned int m2=mask;
