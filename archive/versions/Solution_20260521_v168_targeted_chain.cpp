...
static unsigned char pc_src_stage1[MAX_CARDS];
static unsigned char pc_src_stage2[MAX_CARDS];
static unsigned char pc_src_stage3[MAX_CARDS];
static unsigned char pc_src_stage4[MAX_CARDS];
static unsigned char pc_src_stage5[MAX_CARDS];
...
int run_port_consistency(int m, const unsigned char *allow_src, unsigned char *moved_src){
    int total_moved=0;
    if(moved_src) memset(moved_src,0,MAX_CARDS*sizeof(unsigned char));
    for(int iter=0;iter<5;++iter){
        ...
        for(int i=0;i<fl_count;++i){
            ...
            int src=fl_src[i];
            if(allow_src&& !allow_src[src]) continue;
            int cnt=0;
...
int run_port_consistency(int m){
    return run_port_consistency(m, 0, 0);
}

int run_port_consistency_perport_refine(int m, const unsigned char *allow_src, unsigned char *moved_src){
    long long total_work=(long long)fl_count*(long long)m;
    if(total_work>100000) return 0;
    if(total_work>80000){
        if(!(g_p>=16&&m>=17&&fl_count<=5500)) return 0;
    }
    if(total_work>50000&&fl_count>5000) return 0;
...
int run_port_consistency_perport_refine(int m){
    return run_port_consistency_perport_refine(m, 0, 0);
}
...
void solve_job(){
    ...
    int pc1_moved=run_port_consistency(m, 0, pc_src_stage1);
    int refine1_moved=run_port_consistency_perport_refine(m, pc_src_stage1, pc_src_stage2);
    int revisit1_moved=run_port_consistency(m, pc_src_stage2, pc_src_stage3);
    if(allow_extra_pc_chain){
        if(pc1_moved>0&&refine1_moved>0&&revisit1_moved>0){
            int refine2_moved=run_port_consistency_perport_refine(m, pc_src_stage3, pc_src_stage4);
            if(refine2_moved>0){
                int revisit2_moved=run_port_consistency(m, pc_src_stage4, pc_src_stage5);
                if(revisit2_moved>0){
                    int refine3_moved=run_port_consistency_perport_refine(m, pc_src_stage5, pc_src_stage4);
                    if(refine3_moved>0) run_port_consistency(m, pc_src_stage4, 0);
                }
            }
        }
    }
