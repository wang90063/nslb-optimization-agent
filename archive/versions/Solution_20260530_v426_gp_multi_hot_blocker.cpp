...
static int global_price_out[MAX_LEAFS][MAX_PORTS];
static int global_price_in[MAX_LEAFS][MAX_PORTS];
static int global_price_src[MAX_LEAFS][MAX_PORTS];
static int global_price_dst[MAX_LEAFS][MAX_PORTS];
...
static int future_core_flows[MAX_FLOWS];
static int future_core_score[MAX_FLOWS];
static unsigned char future_core_seen[MAX_FLOWS];
struct EvalMetrics{
    int score_jm;
    int score_fg;
    int jm;
...
inline int price_term(int leaf, int pk){
    return global_price_out[leaf][pk] + global_price_in[leaf][pk];
}

inline int price_term_src(int leaf, int pk){
    return global_price_src[leaf][pk];
}

inline int price_term_dst(int leaf, int pk){
    return global_price_dst[leaf][pk];
}

inline int apply_price_term(int base, int leaf, int pk, int weight){
    return base + weight * price_term(leaf, pk);
}

inline int joint_price_bias(int sl, int dl, int pk){
    return price_term(sl, pk) + price_term(dl, pk) + price_term_src(sl, pk) + price_term_dst(dl, pk);
}

struct GlobalPriceChoice {
    int score_jm;
    int score_fg;
    int local_over;
    int future_over;
...
inline int better_global_price_choice_p32r4(const GlobalPriceChoice &a, const GlobalPriceChoice &b){
    if(a.score_jm!=b.score_jm) return a.score_jm<b.score_jm;
    if(a.score_fg!=b.score_fg) return a.score_fg<b.score_fg;
    if(a.local_over!=b.local_over) return a.local_over<b.local_over;
    if(a.future_over!=b.future_over) return a.future_over<b.future_over;
    if(a.price_bundle!=b.price_bundle) return a.price_bundle<b.price_bundle;
    if(a.future_sq!=b.future_sq) return a.future_sq<b.future_sq;
    if(a.future_sum!=b.future_sum) return a.future_sum<b.future_sum;
    if(a.local_max!=b.local_max) return a.local_max<b.local_max;
    if(a.gv!=b.gv) return a.gv<b.gv;
    return 0;
}
...
// Global-price greedy: keep the same local/global structure as the base
// hardcap greedy, but add a leaf-port price derived from previous jobs.
// The intent is to steer large p=16/32 jobs away from repeatedly choosing the
// same cheap-looking global ports that later create broad CB pressure.
void run_greedy_global_price(int m, int hardcap, int rev=0, int price_w=1, int local_w=2, int global_w=1){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    if(rev>=2){
        unsigned int seed=rev*2654435761u;
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        if(sl==dl){fl_port[i]=-1;continue;}
        unsigned int mask=fl_pmask[i];
        int use_p32_lex = (g_p>=32 && g_r>=4);
        int bp=0,bc=0x7fffffff;
        int bt_over=0x7fffffff,bt_sum=0x7fffffff;
        long long bt_sq=0x7fffffffffffffffLL;
        GlobalPriceChoice best_choice = {0x7fffffff,0x7fffffff,0x7fffffff,0x7fffffff,0x7fffffff,0x7fffffffffffffffLL,0x7fffffff,0x7fffffff,0x7fffffff};
        for(int pk=0;pk<g_p;++pk){
            int local_max=0;
            int exceeds=0;
            int use_future_tie=(hardcap&&g_p>=16&&g_hist_max_jm>g_r);
            ...
            while(m2){
                ...
                if(hardcap&&(o>g_r||iv>g_r)) exceeds=1;
                if(use_p32_lex){
                    if(o>g_r) local_over += o-g_r;
                    if(iv>g_r) local_over += iv-g_r;
                }
                if(hardcap&&g_p>=16&&v>g_r) use_future_tie=1;
                if(use_future_tie || use_p32_lex){
                    if(o>mo)mo=o;
                    if(iv>mi)mi=iv;
                }
                m2&=m2-1;
            ...
            int go=global_out[sl][pk],gi=global_in[dl][pk];
            int gv=go>gi?go:gi;
            int price=price_term(sl, pk)+price_term(dl, pk);
            int src_price=price_term_src(sl, pk);
            int dst_price=price_term_dst(dl, pk);
            int price_bundle=price*price_w+src_price+dst_price;
            int cost=local_max*local_w+gv*global_w+price_bundle;
            if(exceeds) cost+=10000;
            int cand_over=0,cand_sum=0;
            int cand_fg=gv;
            long long cand_sq=0;
            ...
            if(use_p32_lex){
                GlobalPriceChoice cur_choice;
                cur_choice.score_jm = local_max>g_r?local_max:g_r;
                cur_choice.score_fg = cand_fg>g_r?cand_fg:g_r;
                cur_choice.local_over = local_over;
                cur_choice.future_over = cand_over;
                cur_choice.price_bundle = price_bundle;
                cur_choice.future_sq = cand_sq;
                cur_choice.future_sum = cand_sum;
                cur_choice.local_max = local_max;
                cur_choice.gv = gv;
                if(pk==0 || better_global_price_choice_p32r4(cur_choice, best_choice)){
                    bp=pk;
...
static int cbt_eval_cards[MAX_FLOWS];
static unsigned char cbt_eval_seen[MAX_CARDS];

__attribute__((noinline,cold)) int total_cbtphsc_from_ports(const short *ports, int m){
    memset(cpm,0,sizeof(cpm));
    int card_cnt=0;
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||ports[i]<0) continue;
        int c=fl_src[i];
        ...
        if(!cbt_eval_seen[c]){
            cbt_eval_seen[c]=1;
            cbt_eval_cards[card_cnt++]=c;
        }
    }
    int total=0;
    for(int i=0;i<card_cnt;++i){
        int c=cbt_eval_cards[i];
        total+=card_cbtphsc(c,m);
        cbt_eval_seen[c]=0;
    }
    return total;
}
...
void run_cross_dest_swap(int m){
    ...
    for(int iter=0;iter<cd_max_iter;++iter){
        ...
        for(int fi=0;fi<fl_count;++fi){
            ...
            int cand_ports[3]={-1,-1,-1};
            int cand_score[3]={-0x7fffffff,-0x7fffffff,-0x7fffffff};
            int cand_limit=(g_p>=32)?3:((g_p>=16)?2:1);
            for(int pk=0;pk<g_p;++pk){
                int cnt=dp_cnt[pk];
                int score=cnt*256-joint_price_bias(sl,dl_a,pk);
                if(score>cand_score[0]){
                    cand_score[2]=cand_score[1];
                    cand_ports[2]=cand_ports[1];
                    cand_score[1]=cand_score[0];
                    cand_ports[1]=cand_ports[0];
                    cand_score[0]=score;
                    cand_ports[0]=pk;
                } else if(cand_limit>1&&score>cand_score[1]){
                    cand_score[2]=cand_score[1];
                    cand_ports[2]=cand_ports[1];
                    cand_score[1]=score;
                    cand_ports[1]=pk;
                } else if(cand_limit>2&&score>cand_score[2]){
                    cand_score[2]=score;
                    cand_ports[2]=pk;
                }
            }
            int best_wt=0,best_j=-1,best_py=-1;
            int best_price=-0x7fffffff;
            for(int dpk=0;dpk<cand_limit;++dpk){
                int py=cand_ports[dpk];
                if(py<0||py==px) continue;
                ...
                for(int bi=bstart;bi<bend;++bi){
                    ...
                    int price_delta=(joint_price_bias(sl,dl_a,px)-joint_price_bias(sl,dl_a,py))
                                   +(joint_price_bias(sl,dl_b,py)-joint_price_bias(sl,dl_b,px));
                    if(wt>best_wt||
                       (wt==best_wt&&price_delta>best_price)){
                        best_wt=wt;
                        best_price=price_delta;
                        best_j=j;
                        best_py=py;
                    }
                }
    ...
    for(int iter=0;iter<2;++iter){
        ...
        for(int fi=0;fi<fl_count;++fi){
            ...
            for(int pk=0;pk<g_p;++pk){
                int cnt=dp_cnt[pk];
                int score=cnt*256-joint_price_bias(sl,dl,pk);
                if(score>cand_score[0]){
                    cand_score[2]=cand_score[1];
                    cand_ports[2]=cand_ports[1];
                    cand_score[1]=cand_score[0];
            ...
            int best_wt=0,best_py=-1,best_price=-0x7fffffff;
            for(int dpk=0;dpk<cand_limit;++dpk){
                int py=cand_ports[dpk];
                if(py<0||py==px) continue;
                unsigned int m2=mask;
                ...
                int net=before-after;
                int wt=5*net-12*ov_delta;
                int price_delta=joint_price_bias(sl,dl,px)-joint_price_bias(sl,dl,py);
                mk=mask;while(mk){int ph=__builtin_ctz(mk);
                    cppc[ci][ph][py]--;if(!cppc[ci][ph][py])cpm[ci][ph]&=~(1u<<py);
                    cppc[ci][ph][px]++;cpm[ci][ph]|=(1u<<px);mk&=mk-1;}
                if(wt>best_wt||
            ...
            if(best_py<0||best_wt<=0) continue;
            {
                int py=best_py;
...
void run_sa_composite(int m){
    ...
    double sa_budget=0.017;
    if(fl_count>5000) sa_budget=0.01;
...
void run_sa_swap(int m){
    ...
    // Build per-leaf flow index for swap partner selection
    static int leaf_flows[MAX_LEAFS][13000];
    static int leaf_fcnt[MAX_LEAFS];
    memset(leaf_fcnt,0,g_l*sizeof(int));
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
        int sl=fl_sl[i];
        if(leaf_fcnt[sl]<13000) leaf_flows[sl][leaf_fcnt[sl]++]=i;
    }
    double T=2.0;
    double cool=0.99995;
    unsigned int rng=fl_count*2654435761u+g_job_idx*1013904223u+7u;
    for(int iter=0;;++iter){
        if((iter&0xFF)==0){
            double elapsed=(double)(clock()-sa_start)/CLOCKS_PER_SEC;
            if(elapsed>=sa_budget) break;
        ...
        int fi=rng%fl_count;
        if(fl_sl[fi]==fl_dl[fi]||fl_port[fi]<0){T*=cool;continue;}
        int sl=fl_sl[fi],px=fl_port[fi];
        if(leaf_fcnt[sl]<2){T*=cool;continue;}
        // Pick swap partner from same source leaf on different port
        rng=rng*1664525u+1013904223u;
        int bi=leaf_flows[sl][rng%leaf_fcnt[sl]];
        if(bi==fi||fl_port[bi]==px){T*=cool;continue;}
        int py=fl_port[bi];
        int dl_a=fl_dl[fi],dl_b=fl_dl[bi];
        unsigned int mask_a=fl_pmask[fi],mask_b=fl_pmask[bi];
        // Feasibility: check max constraints after swap
        int ok=1;
        unsigned int m2=mask_a;
        while(m2){int ph=__builtin_ctz(m2);
            int new_out_py=out_load[sl][py][ph]+1-(mask_b&(1u<<ph)?1:0);
            if(new_out_py>sa_max_out[sl][py]){ok=0;break;}
            int new_in_a=in_load[dl_a][py][ph]+1;
            if(new_in_a>sa_max_in[dl_a][py]){ok=0;break;}
            m2&=m2-1;}
        if(!ok){T*=cool;continue;}
        m2=mask_b;
        while(m2){int ph=__builtin_ctz(m2);
            int new_out_px=out_load[sl][px][ph]+1-(mask_a&(1u<<ph)?1:0);
            if(new_out_px>sa_max_out[sl][px]){ok=0;break;}
            int new_in_b=in_load[dl_b][px][ph]+1;
            if(new_in_b>sa_max_in[dl_b][px]){ok=0;break;}
            m2&=m2-1;}
        if(!ok){T*=cool;continue;}
        // Compute CI delta
        int ci_delta=0;
        m2=mask_a;
        while(m2){int ph=__builtin_ctz(m2);
            if(out_load[sl][py][ph]-(mask_b&(1u<<ph)?1:0)>=g_r)ci_delta++;
            if(out_load[sl][px][ph]>g_r)ci_delta--;
            if(in_load[dl_a][py][ph]>=g_r)ci_delta++;
            if(in_load[dl_a][px][ph]>g_r)ci_delta--;
            m2&=m2-1;}
        m2=mask_b;
        while(m2){int ph=__builtin_ctz(m2);
            if(out_load[sl][px][ph]-(mask_a&(1u<<ph)?1:0)>=g_r)ci_delta++;
            if(out_load[sl][py][ph]>g_r)ci_delta--;
            if(in_load[dl_b][px][ph]>=g_r)ci_delta++;
            if(in_load[dl_b][py][ph]>g_r)ci_delta--;
            m2&=m2-1;}
        // Compute CB delta
        int ci_a=fl_src[fi],ci_b=fl_src[bi];
        int before_cbt=card_cbtphsc(ci_a,m)+card_cbtphsc(ci_b,m);
        m2=mask_a;while(m2){int ph=__builtin_ctz(m2);
            cppc[ci_a][ph][px]--;if(!cppc[ci_a][ph][px])cpm[ci_a][ph]&=~(1u<<px);
            cppc[ci_a][ph][py]++;cpm[ci_a][ph]|=(1u<<py);m2&=m2-1;}
        m2=mask_b;while(m2){int ph=__builtin_ctz(m2);
            cppc[ci_b][ph][py]--;if(!cppc[ci_b][ph][py])cpm[ci_b][ph]&=~(1u<<py);
            cppc[ci_b][ph][px]++;cpm[ci_b][ph]|=(1u<<px);m2&=m2-1;}
        int after_cbt=card_cbtphsc(ci_a,m)+card_cbtphsc(ci_b,m);
        int cbt_delta=after_cbt-before_cbt;
        int obj_delta=12*ci_delta+5*cbt_delta;
        int accept=0;
        if(obj_delta<=0) accept=1;
        else{rng=rng*1664525u+1013904223u;double r=(rng&0xFFFF)/65536.0;
            if(r<__builtin_exp(-(double)obj_delta/T)) accept=1;}
        if(accept){
            m2=mask_a;while(m2){int ph=__builtin_ctz(m2);
                out_load[sl][px][ph]--;out_load[sl][py][ph]++;
                in_load[dl_a][px][ph]--;in_load[dl_a][py][ph]++;m2&=m2-1;}
            m2=mask_b;while(m2){int ph=__builtin_ctz(m2);
                out_load[sl][py][ph]--;out_load[sl][px][ph]++;
                in_load[dl_b][py][ph]--;in_load[dl_b][px][ph]++;m2&=m2-1;}
            fl_port[fi]=(short)py;fl_port[bi]=(short)px;
            // Update sa_max (may have decreased)
            short nmo=0;for(int ph=0;ph<m;++ph){if(out_load[sl][px][ph]>nmo)nmo=out_load[sl][px][ph];}
            sa_max_out[sl][px]=nmo;
            nmo=0;for(int ph=0;ph<m;++ph){if(out_load[sl][py][ph]>nmo)nmo=out_load[sl][py][ph];}
            sa_max_out[sl][py]=nmo;
            short nmi=0;for(int ph=0;ph<m;++ph){if(in_load[dl_a][px][ph]>nmi)nmi=in_load[dl_a][px][ph];}
            sa_max_in[dl_a][px]=nmi;
            nmi=0;for(int ph=0;ph<m;++ph){if(in_load[dl_a][py][ph]>nmi)nmi=in_load[dl_a][py][ph];}
            sa_max_in[dl_a][py]=nmi;
            nmi=0;for(int ph=0;ph<m;++ph){if(in_load[dl_b][py][ph]>nmi)nmi=in_load[dl_b][py][ph];}
            sa_max_in[dl_b][py]=nmi;
            nmi=0;for(int ph=0;ph<m;++ph){if(in_load[dl_b][px][ph]>nmi)nmi=in_load[dl_b][px][ph];}
            sa_max_in[dl_b][px]=nmi;
        } else {
            m2=mask_a;while(m2){int ph=__builtin_ctz(m2);
                cppc[ci_a][ph][py]--;if(!cppc[ci_a][ph][py])cpm[ci_a][ph]&=~(1u<<py);
                cppc[ci_a][ph][px]++;cpm[ci_a][ph]|=(1u<<px);m2&=m2-1;}
            m2=mask_b;while(m2){int ph=__builtin_ctz(m2);
                cppc[ci_b][ph][px]--;if(!cppc[ci_b][ph][px])cpm[ci_b][ph]&=~(1u<<px);
                cppc[ci_b][ph][py]++;cpm[ci_b][ph]|=(1u<<py);m2&=m2-1;}
        }
        T*=cool;
    }
}
...
static inline void insert_future_hot_port(FutureHotPort *hots,int &cnt,int cap,
                                          int leaf,int port,int dir,int future,
                                          int local_max,int score){
    if(cnt<cap){
        hots[cnt].leaf=(short)leaf;
        hots[cnt].port=(short)port;
        hots[cnt].future=(short)future;
        hots[cnt].local_max=(short)local_max;
        hots[cnt].dir=(unsigned char)dir;
        hots[cnt].score=score;
        cnt++;
    } else if(score<=hots[cnt-1].score){
        return;
    } else {
        hots[cnt-1].leaf=(short)leaf;
        hots[cnt-1].port=(short)port;
        hots[cnt-1].future=(short)future;
        hots[cnt-1].local_max=(short)local_max;
        hots[cnt-1].dir=(unsigned char)dir;
        hots[cnt-1].score=score;
    }
    for(int i=cnt-1;i>0&&hots[i].score>hots[i-1].score;--i){
        FutureHotPort t=hots[i];
        hots[i]=hots[i-1];
        hots[i-1]=t;
    }
}

static inline void insert_future_min_port(int *ports,int *scores,
                                          unsigned int *src_masks,unsigned int *dst_masks,
                                          int &cnt,int cap,int pk,int score,
                                          unsigned int src_mask,unsigned int dst_mask){
    if(cnt<cap){
        ports[cnt]=pk;
        scores[cnt]=score;
        src_masks[cnt]=src_mask;
        dst_masks[cnt]=dst_mask;
        cnt++;
    } else if(score>=scores[cnt-1]){
        return;
    } else {
        ports[cnt-1]=pk;
        scores[cnt-1]=score;
        src_masks[cnt-1]=src_mask;
        dst_masks[cnt-1]=dst_mask;
    }
    for(int i=cnt-1;i>0&&scores[i]<scores[i-1];--i){
        int tp=ports[i]; ports[i]=ports[i-1]; ports[i-1]=tp;
        int ts=scores[i]; scores[i]=scores[i-1]; scores[i-1]=ts;
        unsigned int tm=src_masks[i]; src_masks[i]=src_masks[i-1]; src_masks[i-1]=tm;
        tm=dst_masks[i]; dst_masks[i]=dst_masks[i-1]; dst_masks[i-1]=tm;
    }
}

static inline void insert_future_max_idx(int *idxs,int *scores,int &cnt,int cap,
                                         int idx,int score){
    if(cnt<cap){
        idxs[cnt]=idx;
        scores[cnt]=score;
...
// Multi-hot + blocker-aware future repair for p32/r4 GP seeds.
// Unlike the single-hot batch rebuild, this pass searches the top two future
// hot ports and allows a same-step blocker chain to free a better target port.
__attribute__((noinline,cold)) void run_p32_multi_hot_blocker_repair(int m){
    if(g_p<32||g_r<4) return;
    if(fl_count>7000||(long long)fl_count*m>140000) return;

    EvalMetrics base_eval=collect_metrics(m);
    if(base_eval.future_over<=0||base_eval.jm>g_r+1) return;
    ...
    int improved=0;
    int allow_jm=base_eval.jm>g_r?base_eval.jm:g_r;

    for(int round=0;round<3;++round){
        FutureHotPort hot_ports[6];
        int hot_cnt=0;
        for(int leaf=0;leaf<g_l;++leaf)
            for(int pk=0;pk<g_p;++pk){
                int mo=0,mi=0;
                ...
                int fi=global_in[leaf][pk]+mi;
                if(fo>g_r){
                    int score=(fo-g_r)*8192+mo*128+(fo==best_eval.fg?2048:0);
                    insert_future_hot_port(hot_ports,hot_cnt,6,leaf,pk,0,fo,mo,score);
                }
                if(fi>g_r){
                    int score=(fi-g_r)*8192+mi*128+(fi==best_eval.fg?2048:0);
                    insert_future_hot_port(hot_ports,hot_cnt,6,leaf,pk,1,fi,mi,score);
                }
            }
        if(hot_cnt<=0) break;

        memset(future_core_seen,0,fl_count*sizeof(unsigned char));
        int flow_cnt=0;
        int use_hot=hot_cnt<2?hot_cnt:2;
        for(int hi=0;hi<use_hot;++hi){
            FutureHotPort hp=hot_ports[hi];
            for(int fi=0;fi<fl_count;++fi){
                if(fl_sl[fi]==fl_dl[fi]) continue;
                if(fl_port[fi]!=hp.port||fl_port[fi]<0) continue;
                if((hp.dir==0&&fl_sl[fi]!=hp.leaf)||(hp.dir==1&&fl_dl[fi]!=hp.leaf)) continue;
                int score=hp.score+__builtin_popcount(fl_pmask[fi])*64;
                unsigned int mk=fl_pmask[fi];
                while(mk){
                    int ph=__builtin_ctz(mk);
                    int cur=(hp.dir==0)?out_load[hp.leaf][hp.port][ph]:in_load[hp.leaf][hp.port][ph];
                    if(cur==hp.local_max) score+=4096;
                    else if(cur+1>=hp.local_max) score+=1024;
                    mk&=mk-1;
                }
                if(!future_core_seen[fi]){
                    future_core_seen[fi]=1;
                    future_core_flows[flow_cnt++]=fi;
                    future_core_score[fi]=score;
                } else if(score>future_core_score[fi]){
                    future_core_score[fi]=score;
                }
            }
        }
        ...
        int flow_cap=flow_cnt;
        if(flow_cap>24) flow_cap=24;
        for(int i=0;i<flow_cap;++i){
            int best=i;
            for(int j=i+1;j<flow_cnt;++j){
                if(future_core_score[future_core_flows[j]]>
                   future_core_score[future_core_flows[best]])
                    best=j;
            }
            if(best!=i){
                int t=future_core_flows[i];
                future_core_flows[i]=future_core_flows[best];
                future_core_flows[best]=t;
            }
        }

        EvalMetrics round_best=best_eval;
        int best_fi=-1,best_fpk=-1,best_tpk=-1;
        int best_bfi=-1,best_bpk=-1,best_bap=-1;

        for(int idx=0;idx<flow_cap;++idx){
            int fi=future_core_flows[idx];
            int cp=fl_port[fi];
            ...
            int cand_ports[4];
            int cand_scores[4];
            unsigned int cand_src_masks[4];
            unsigned int cand_dst_masks[4];
            int cand_cnt=0;
            ...
            for(int pk=0;pk<g_p;++pk){
                ...
                int local_peak=0;
                int mo=0,mi=0;
                unsigned int need_src=0,need_dst=0;
                int invalid=0;
                for(int ph=0;ph<m;++ph){
                    int take=(mask>>ph)&1u;
                    int o=out_load[sl][pk][ph]+take;
                    int iv=in_load[dl][pk][ph]+take;
                    if(o>local_peak) local_peak=o;
                    if(iv>local_peak) local_peak=iv;
                    if(o>mo) mo=o;
                    if(iv>mi) mi=iv;
                    if(take){
                        if(o>allow_jm+1||iv>allow_jm+1){
                            invalid=1;
                            break;
                        }
                        if(o>allow_jm) need_src|=(1u<<ph);
                        if(iv>allow_jm) need_dst|=(1u<<ph);
                    }
                }
                if(invalid) continue;
                for(int ph=0;ph<m;++ph){
                    if(out_load[sl][pk][ph]>mo) mo=out_load[sl][pk][ph];
                    if(in_load[dl][pk][ph]>mi) mi=in_load[dl][pk][ph];
                }
                int fo=global_out[sl][pk]+mo;
                int fi_v=global_in[dl][pk]+mi;
                int future_fg=fo>fi_v?fo:fi_v;
                int future_over=0;
                if(fo>g_r) future_over+=fo-g_r;
                if(fi_v>g_r) future_over+=fi_v-g_r;
                int block_cnt=__builtin_popcount(need_src|need_dst);
                int score=block_cnt*200000+local_peak*4096+future_fg*256+
                          future_over*64+(global_out[sl][pk]+global_in[dl][pk])*4+
                          joint_price_bias(sl,dl,pk);
                insert_future_min_port(cand_ports,cand_scores,cand_src_masks,cand_dst_masks,
                                       cand_cnt,4,pk,score,need_src,need_dst);
            }

            for(int ci=0;ci<cand_cnt;++ci){
                int pk=cand_ports[ci];
                unsigned int need_src=cand_src_masks[ci];
                unsigned int need_dst=cand_dst_masks[ci];
                ...
                if(!need_src&&!need_dst){
                    ...
                    if(better_metrics(cand_eval,round_best)){
                        ...
                        best_fi=fi; best_fpk=cp; best_tpk=pk;
                        best_bfi=-1; best_bpk=-1; best_bap=-1;
                    }
                    continue;
                }
                ...
                int blocker_score[6];
                int blocker_cnt=0;
                for(int bj=0;bj<fl_count;++bj){
                    if(bj==fi||fl_sl[bj]==fl_dl[bj]||fl_port[bj]!=pk||fl_port[bj]<0) continue;
                    unsigned int overlap_src=0,overlap_dst=0;
                    if(fl_sl[bj]==sl) overlap_src=fl_pmask[bj]&need_src;
                    if(fl_dl[bj]==dl) overlap_dst=fl_pmask[bj]&need_dst;
                    if(!overlap_src&&!overlap_dst) continue;
                    int score=__builtin_popcount(overlap_src|overlap_dst)*4096;
                    if(fl_src[bj]==fl_src[fi]) score+=16384;
                    score-=__builtin_popcount(fl_pmask[bj])*32;
                    insert_future_max_idx(blocker_idx,blocker_score,blocker_cnt,6,bj,score);
                }

                for(int bi=0;bi<blocker_cnt;++bi){
                    int bfi=blocker_idx[bi];
                    int bcp=pk;
                    int bsl=fl_sl[bfi],bdl=fl_dl[bfi];
                    ...
                    for(int ap=0;ap<g_p;++ap){
                        ...
                        int invalid=0;
                        for(int ph=0;ph<m;++ph){
                            int take=(bmask>>ph)&1u;
                            int o=out_load[bsl][ap][ph]+take;
                            int iv=in_load[bdl][ap][ph]+take;
                            if(o>local_peak) local_peak=o;
                            if(iv>local_peak) local_peak=iv;
                            if(o>mo) mo=o;
                            if(iv>mi) mi=iv;
                            if(take&&(o>allow_jm||iv>allow_jm)){
                                invalid=1;
                                break;
                            }
                        }
                        if(invalid) continue;
                        for(int ph=0;ph<m;++ph){
                            if(out_load[bsl][ap][ph]>mo) mo=out_load[bsl][ap][ph];
                            if(in_load[bdl][ap][ph]>mi) mi=in_load[bdl][ap][ph];
                        }
                        int fo=global_out[bsl][ap]+mo;
                        int fi_v=global_in[bdl][ap]+mi;
                        int future_fg=fo>fi_v?fo:fi_v;
                        int future_over=0;
                        if(fo>g_r) future_over+=fo-g_r;
                        if(fi_v>g_r) future_over+=fi_v-g_r;
                        int score=local_peak*4096+future_fg*256+future_over*64+
                                  (global_out[bsl][ap]+global_in[bdl][ap])*4+
                                  joint_price_bias(bsl,bdl,ap);
                        if(ap_cnt<3){
                            ap_ports[ap_cnt]=ap;
                            ap_scores[ap_cnt]=score;
                            ap_cnt++;
                        } else if(score<ap_scores[ap_cnt-1]){
                            ap_ports[ap_cnt-1]=ap;
                            ap_scores[ap_cnt-1]=score;
                        } else continue;
                        for(int i=ap_cnt-1;i>0&&ap_scores[i]<ap_scores[i-1];--i){
                            int tp=ap_ports[i]; ap_ports[i]=ap_ports[i-1]; ap_ports[i-1]=tp;
                            int ts=ap_scores[i]; ap_scores[i]=ap_scores[i-1]; ap_scores[i-1]=ts;
                        }
                    }

                    for(int ai=0;ai<ap_cnt;++ai){
                        int ap=ap_ports[ai];
                        apply_flow_move(bfi,bcp,ap);

                        int ok=1;
                        for(int ph=0;ph<m&&ok;++ph){
                            if(!((mask>>ph)&1u)) continue;
                            if(out_load[sl][pk][ph]+1>allow_jm) ok=0;
                            if(in_load[dl][pk][ph]+1>allow_jm) ok=0;
                        }
                        if(ok){
                            apply_flow_move(fi,cp,pk);
                            EvalMetrics cand_eval=collect_metrics(m);
                            ...
                            if(better_metrics(cand_eval,round_best)){
                                round_best=cand_eval;
                                best_fi=fi; best_fpk=cp; best_tpk=pk;
                                best_bfi=bfi; best_bpk=bcp; best_bap=ap;
                            }
                        }

                        apply_flow_move(bfi,ap,bcp);
                    }
                }
            }
        }
        ...
        if(best_fi<0) break;
        if(best_bfi>=0) apply_flow_move(best_bfi,best_bpk,best_bap);
        apply_flow_move(best_fi,best_fpk,best_tpk);
...
void solve_job(){
    ...
    int first=1;
    int any_jm_le_r=0;
    int p32_gp_tiebreak=(g_p>=32&&g_r>=4&&fl_count<=7000);
    int cur_uses_global_price=0;
    int best_uses_global_price=0;
    ...
    // S6k-S6l: global-price candidates for p=32 structures
    if(g_p>=32&&g_r>=4&&fl_count<=7000){
        cur_uses_global_price=1;
        run_greedy_global_price(m, 1, 0, 1, 2, 1);
        run_swap(m);
        run_p32_multi_hot_blocker_repair(m);
        TRY_STRATEGY();

        cur_uses_global_price=1;
        run_greedy_global_price(m, 1, 1, 1, 2, 1);
        run_swap(m);
        TRY_STRATEGY();
        cur_uses_global_price=0;
    }
    ...
    // Two-pass SA: split budget, CB recovery between passes
    if(!g_time_tight){
        for(int sa_pass=0;sa_pass<3;++sa_pass){
            memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
            memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
            memcpy(bk_port, fl_port, fl_count*sizeof(short));
    ...
    // SA swap pass: 2-opt neighborhood
    if(!g_time_tight){
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        EvalMetrics swap_base=collect_metrics(m);
        run_sa_swap(m);
        EvalMetrics swap_after=collect_metrics(m);
        if(swap_after.fg>swap_base.fg||(swap_after.fg==swap_base.fg&&swap_after.jm>swap_base.jm)){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));
        } else {
