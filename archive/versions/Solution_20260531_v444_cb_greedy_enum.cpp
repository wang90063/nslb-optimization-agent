...
void run_cb_greedy_descent(int m){
    if(fl_count<10) return;
    static short gd_max_out[MAX_LEAFS][MAX_PORTS];
    static short gd_max_in[MAX_LEAFS][MAX_PORTS];
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            short mo=0,mi=0;
            for(int ph=0;ph<m;++ph){
            ...
            gd_max_out[leaf][pk]=mo;gd_max_in[leaf][pk]=mi;
        }
    memset(cpm,0,sizeof(cpm));memset(cppc,0,sizeof(cppc));
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
    ...
    for(int iter=0;iter<5;++iter){
        ...
        for(int fi=0;fi<fl_count;++fi){
            ...
            int best_py=-1,best_delta=0;
            for(int py=0;py<g_p;++py){
                if(py==px) continue;
                unsigned int m2=fl_pmask[fi]; int ok=1;
                while(m2){int ph=__builtin_ctz(m2);
                    if(out_load[sl][py][ph]+1>gd_max_out[sl][py]){ok=0;break;}
                    if(in_load[dl][py][ph]+1>gd_max_in[dl][py]){ok=0;break;}
                    m2&=m2-1;}
                if(!ok) continue;
                int ci_delta=0;
                m2=fl_pmask[fi];
                ...
                int cb_delta=cur_cb-new_cb;
                mk=fl_pmask[fi];
                while(mk){int ph=__builtin_ctz(mk);
                    cppc[ci][ph][py]--;if(!cppc[ci][ph][py])cpm[ci][ph]&=~(1u<<py);
                    cppc[ci][ph][px]++;cpm[ci][ph]|=(1u<<px);mk&=mk-1;}
                int wt=5*cb_delta-12*ci_delta;
                if(wt>best_delta){best_delta=wt;best_py=py;}
            }
            if(best_py<0) continue;
            int py=best_py;
            unsigned int mk=fl_pmask[fi];
            ...
            gd_max_out[sl][px]=nmo;gd_max_in[dl][px]=nmi;
            any_move++;
        }
        if(!any_move) break;
        for(int leaf=0;leaf<g_l;++leaf)
...
void solve_job(){
    ...
    if(!g_time_tight){
        ...
        // CB greedy descent: systematically find all feasible CB-improving moves
    if(!g_time_tight){
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        EvalMetrics gd_base=collect_metrics(m);
        run_cb_greedy_descent(m);
        EvalMetrics gd_after=collect_metrics(m);
        if(gd_after.fg>gd_base.fg||(gd_after.fg==gd_base.fg&&gd_after.jm>gd_base.jm)){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));
        } else {
