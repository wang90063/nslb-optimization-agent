...
void run_cb_swap_consolidate(int m){
    if(fl_count<10) return;
    static short sw_max_out[MAX_LEAFS][MAX_PORTS];
    static short sw_max_in[MAX_LEAFS][MAX_PORTS];
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            short mo=0,mi=0;
            for(int ph=0;ph<m;++ph){
            ...
            sw_max_out[leaf][pk]=mo;sw_max_in[leaf][pk]=mi;
        }
    memset(cpm,0,sizeof(cpm));memset(cppc,0,sizeof(cppc));
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
    ...
    // Build index: flows on each (leaf, port) as source
    static int port_flows[MAX_LEAFS][MAX_PORTS][256];
    static int port_flow_cnt[MAX_LEAFS][MAX_PORTS];
    memset(port_flow_cnt,0,sizeof(port_flow_cnt));
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
        int sl=fl_sl[i],p=fl_port[i];
        if(port_flow_cnt[sl][p]<256)
            port_flows[sl][p][port_flow_cnt[sl][p]++]=i;
    }
    for(int iter=0;iter<2;++iter){
        int any_move=0;
        for(int fa=0;fa<fl_count;++fa){
            int sl_a=fl_sl[fa],dl_a=fl_dl[fa],px=fl_port[fa],ci_a=fl_src[fa];
            if(sl_a==dl_a||px<0) continue;
            int cur_cb_a=card_cbtphsc(ci_a,m);
            if(cur_cb_a<=0) continue;
            for(int py=0;py<g_p;++py){
                if(py==px) continue;
                // Check if moving fa from px to py is blocked by sa_max
                unsigned int m2=fl_pmask[fa]; int blocked=0;
                while(m2){int ph=__builtin_ctz(m2);
                    if(out_load[sl_a][py][ph]+1>sw_max_out[sl_a][py]){blocked=1;break;}
                    if(in_load[dl_a][py][ph]+1>sw_max_in[dl_a][py]){blocked=1;break;}
                    m2&=m2-1;}
                if(!blocked) continue; // Already feasible — SA handles this
                // Find a flow_b on port_y (same source leaf) that can move to px
                int best_fb=-1,best_wt=0;
                int cnt=port_flow_cnt[sl_a][py];
                if(cnt==0) continue;
                for(int bi=0;bi<cnt&&bi<32;++bi){
                    int fb=port_flows[sl_a][py][bi];
                    if(fb==fa) continue;
                    if(fl_sl[fb]!=sl_a) continue;
                    // Check: after removing fb from py, can fa fit on py?
                    // And can fb fit on px (within sa_max)?
                    unsigned int mask_b=fl_pmask[fb];
                    // Check fb can go to px
                    unsigned int m3=mask_b; int ok_b=1;
                    while(m3){int ph=__builtin_ctz(m3);
                        if(out_load[sl_a][px][ph]-((fl_pmask[fa]>>ph)&1)+1>sw_max_out[sl_a][px]){ok_b=0;break;}
                        m3&=m3-1;}
                    if(!ok_b) continue;
                    // Check fa can go to py after fb leaves
                    unsigned int m4=fl_pmask[fa]; int ok_a=1;
                    while(m4){int ph=__builtin_ctz(m4);
                        int new_load=out_load[sl_a][py][ph]-((mask_b>>ph)&1)+1;
                        if(new_load>sw_max_out[sl_a][py]){ok_a=0;break;}
                        m4&=m4-1;}
                    if(!ok_a) continue;
                    // Check in_load constraints for both
                    int dl_b=fl_dl[fb];
                    m3=mask_b; ok_b=1;
                    while(m3){int ph=__builtin_ctz(m3);
                        if(in_load[dl_b][px][ph]+1>sw_max_in[dl_b][px]){ok_b=0;break;}
                        m3&=m3-1;}
                    if(!ok_b) continue;
                    m4=fl_pmask[fa]; ok_a=1;
                    while(m4){int ph=__builtin_ctz(m4);
                        int new_in=in_load[dl_a][py][ph]+1;
                        if(new_in>sw_max_in[dl_a][py]){ok_a=0;break;}
                        m4&=m4-1;}
                    if(!ok_a) continue;
                    // Compute CB delta for both cards
                    int ci_b=fl_src[fb];
                    // Simulate: move fa px→py, fb py→px
                    unsigned int mk;
                    mk=fl_pmask[fa];
                    while(mk){int ph=__builtin_ctz(mk);
                        cppc[ci_a][ph][px]--;if(!cppc[ci_a][ph][px])cpm[ci_a][ph]&=~(1u<<px);
                        cppc[ci_a][ph][py]++;cpm[ci_a][ph]|=(1u<<py);mk&=mk-1;}
                    mk=mask_b;
                    while(mk){int ph=__builtin_ctz(mk);
                        cppc[ci_b][ph][py]--;if(!cppc[ci_b][ph][py])cpm[ci_b][ph]&=~(1u<<py);
                        cppc[ci_b][ph][px]++;cpm[ci_b][ph]|=(1u<<px);mk&=mk-1;}
                    int new_cb_a=card_cbtphsc(ci_a,m);
                    int new_cb_b=(ci_b!=ci_a)?card_cbtphsc(ci_b,m):new_cb_a;
                    // Undo
                    mk=mask_b;
                    while(mk){int ph=__builtin_ctz(mk);
                        cppc[ci_b][ph][px]--;if(!cppc[ci_b][ph][px])cpm[ci_b][ph]&=~(1u<<px);
                        cppc[ci_b][ph][py]++;cpm[ci_b][ph]|=(1u<<py);mk&=mk-1;}
                    mk=fl_pmask[fa];
                    while(mk){int ph=__builtin_ctz(mk);
                        cppc[ci_a][ph][py]--;if(!cppc[ci_a][ph][py])cpm[ci_a][ph]&=~(1u<<py);
                        cppc[ci_a][ph][px]++;cpm[ci_a][ph]|=(1u<<px);mk&=mk-1;}
                    int orig_cb_b=(ci_b!=ci_a)?card_cbtphsc(ci_b,m):cur_cb_a;
                    int cb_gain=(cur_cb_a-new_cb_a)+(orig_cb_b-new_cb_b);
                    if(cb_gain<=0) continue;
                    int wt=cb_gain;
                    if(wt>best_wt){best_wt=wt;best_fb=fb;}
                }
                if(best_fb<0) continue;
                // Execute the swap
                int fb=best_fb;
                int ci_b=fl_src[fb];
                unsigned int mask_b=fl_pmask[fb];
                ...
                mk=fl_pmask[fa];
                while(mk){int ph=__builtin_ctz(mk);
                    out_load[sl_a][px][ph]--;out_load[sl_a][py][ph]++;
                    in_load[dl_a][px][ph]--;in_load[dl_a][py][ph]++;mk&=mk-1;}
                mk=mask_b;
                int dl_b=fl_dl[fb];
                while(mk){int ph=__builtin_ctz(mk);
                    out_load[sl_a][py][ph]--;out_load[sl_a][px][ph]++;
                    in_load[dl_b][py][ph]--;in_load[dl_b][px][ph]++;mk&=mk-1;}
                fl_port[fa]=(short)py;
                fl_port[fb]=(short)px;
                // Update sw_max (may have decreased)
                short nmo,nmi;
                nmo=0;nmi=0;
                for(int ph=0;ph<m;++ph){
                    if(out_load[sl_a][px][ph]>nmo)nmo=out_load[sl_a][px][ph];
                    if(in_load[dl_a][px][ph]>nmi)nmi=in_load[dl_a][px][ph];
                }
                sw_max_out[sl_a][px]=nmo;sw_max_in[dl_a][px]=nmi;
                nmo=0;nmi=0;
                for(int ph=0;ph<m;++ph){
                    if(out_load[sl_a][py][ph]>nmo)nmo=out_load[sl_a][py][ph];
                    if(in_load[dl_b][py][ph]>nmi)nmi=in_load[dl_b][py][ph];
                }
                sw_max_out[sl_a][py]=nmo;sw_max_in[dl_b][py]=nmi;
                any_move++;
                break; // Move to next flow_a
            }
        }
        if(!any_move) break;
    }
...
void solve_job(){
    ...
    if(!g_time_tight){
        ...
        // CB swap consolidation: find 2-opt moves that enable blocked CB improvements
    if(!g_time_tight){
        memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        EvalMetrics sw_base=collect_metrics(m);
        run_cb_swap_consolidate(m);
        EvalMetrics sw_after=collect_metrics(m);
        if(sw_after.fg>sw_base.fg||(sw_after.fg==sw_base.fg&&sw_after.jm>sw_base.jm)){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));
        } else {
