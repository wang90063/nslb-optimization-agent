...
static int core_card_flowcnt[MAX_CARDS];
static int core_card_slot[MAX_CARDS];
...
__attribute__((noinline,cold)) void run_r4_leaf_bundle_rebuild(int m){
    if(g_r!=4||g_p<16) return;
    long long total_work=(long long)fl_count*(long long)m;
    if(g_time_tight||fl_count>6000||total_work>90000) return;
    ...
    static short cap_out[MAX_LEAFS][MAX_PORTS];
    static short cap_in[MAX_LEAFS][MAX_PORTS];
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            short mo=0,mi=0;
            for(int ph=0;ph<m;++ph){
                if(bk_out[leaf][pk][ph]>mo) mo=bk_out[leaf][pk][ph];
                if(bk_in[leaf][pk][ph]>mi) mi=bk_in[leaf][pk][ph];
            }
            cap_out[leaf][pk]=mo;
            cap_in[leaf][pk]=mi;
        }
    ...
    for(int i=0;i<fl_count;++i){
        ...
        if(!core_card_seen[c]){
            core_card_seen[c]=1;
            core_cards[cand_cnt++]=c;
            core_card_score[c]=card_cbtphsc(c,m);
            core_card_flowcnt[c]=0;
        }
        if(core_card_score[c]>0) core_card_flowcnt[c]++;
    }
    for(int i=0;i<cand_cnt;++i){
        int c=core_cards[i];
        if(core_card_score[c]<=0) continue;
        int leaf=c/g_pr;
        int wt=core_card_flowcnt[c];
        if(wt>12) wt=12;
        leaf_score[leaf]+=core_card_score[c]*wt;
    }

    EvalMetrics best_eval=base_eval;
    int improved=0;
    unsigned char leaf_used[MAX_LEAFS]={};
    for(int leaf_try=0;leaf_try<2;++leaf_try){
        int best_leaf=-1;
        for(int leaf=0;leaf<g_l;++leaf){
            if(leaf_used[leaf]) continue;
            if(best_leaf<0||leaf_score[leaf]>leaf_score[best_leaf]) best_leaf=leaf;
        }
        if(best_leaf<0||leaf_score[best_leaf]<=0) break;
        leaf_used[best_leaf]=1;

        int selected_cards[6];
        ...
        for(int pick=0;pick<6;++pick){
            ...
            for(int i=0;i<cand_cnt;++i){
                int c=core_cards[i];
                if(c/g_pr!=best_leaf||core_card_score[c]<=0||core_card_slot[c]) continue;
                int score=core_card_score[c]*1024+core_card_flowcnt[c]*32;
                if(score>best_score){
                    best_score=score;
                    best_card=c;
                }
            }
            if(best_card<0) break;
            if(selected_flows+core_card_flowcnt[best_card]>160&&sel_card_cnt>=2) break;
            core_card_slot[best_card]=sel_card_cnt+1;
            selected_cards[sel_card_cnt++]=best_card;
            selected_flows+=core_card_flowcnt[best_card];
            if(selected_flows>=160) break;
        }
        if(sel_card_cnt<=1){
            for(int i=0;i<sel_card_cnt;++i) core_card_slot[selected_cards[i]]=0;
            continue;
        }

        int cnt=0;
        for(int i=0;i<fl_count;++i){
            int slot=core_card_slot[fl_src[i]];
            if(slot<=0||fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
            pc_flows[cnt++]=i;
        }
        if(cnt<=1||cnt>160){
            for(int i=0;i<sel_card_cnt;++i) core_card_slot[selected_cards[i]]=0;
        ...
        for(int i=0;i<cnt;++i){
            int fi=pc_flows[i];
            int c=fl_src[fi];
            cell_core_score[fi]=core_card_score[c]*1024+
                                core_card_flowcnt[c]*16+
                                __builtin_popcount(fl_pmask[fi])*64;
        }
        for(int i=0;i<cnt;++i){
            int best=i;
            for(int j=i+1;j<cnt;++j){
        ...
        unsigned int src_masks[6][MAX_PHASES];
        memset(src_masks,0,sizeof(src_masks));
        for(int i=0;i<cnt;++i){
            int fi=pc_flows[i];
            int sl=fl_sl[fi],dl=fl_dl[fi],cur_pk=bk_port[fi];
            int slot=core_card_slot[fl_src[fi]]-1;
            unsigned int mask=fl_pmask[fi];
            ...
            for(int pk=0;pk<g_p;++pk){
                ...
                unsigned int tmp_masks[MAX_PHASES];
                memcpy(tmp_masks,src_masks[slot],m*sizeof(unsigned int));
                unsigned int mk=mask;
                while(mk){
                    int ph=__builtin_ctz(mk);
                    int o=out_load[sl][pk][ph]+1;
                    int iv=in_load[dl][pk][ph]+1;
                    if(o>cap_out[sl][pk]||iv>cap_in[dl][pk]){
                        ok=0;
                        break;
                    }
                    if(o>local_peak) local_peak=o;
                ...
                int fo=global_out[sl][pk]+mo;
                int fi_fg=global_in[dl][pk]+mi;
                int future_fg=fo>fi_fg?fo:fi_fg;
                long long future_sq=(long long)fo*fo+(long long)fi_fg*fi_fg;
                int press=global_out[sl][pk]+global_in[dl][pk];
                int change=(pk!=cur_pk);
...
void solve_job(){
    ...
    if(!g_time_tight){
        ...
        memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
        memcpy(bk_port, fl_port, fl_count*sizeof(short));
        EvalMetrics bundle_base=collect_metrics(m);
        run_r4_leaf_bundle_rebuild(m);
        EvalMetrics bundle_after=collect_metrics(m);
        if(!better_metrics(bundle_after,bundle_base)){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));
        }
