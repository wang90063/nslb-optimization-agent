...
void run_card_phase_group(int m){
    ...
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            ...
            gp_max_out[leaf][pk]=mo;gp_max_in[leaf][pk]=mi;
        }
    memset(cpm,0,sizeof(cpm));memset(cppc,0,sizeof(cppc));
    for(int i=0;i<fl_count;++i){
        if(fl_sl[i]==fl_dl[i]||fl_port[i]<0) continue;
    ...
    for(int iter=0;iter<3;++iter){
        int any_improved=0;
        for(int ci=0;ci<g_l*g_p*g_r;++ci){
            ...
            for(int gi=0;gi<gcnt;++gi){
                ...
                int dl=fl_dl[fi];
                unsigned int mk=fl_pmask[fi];
                while(mk){int ph=__builtin_ctz(mk);gp_delta_in[dl][ph]++;mk&=mk-1;}
            }
            int ok=1;
            for(int ph=0;ph<m&&ok;++ph){
                if(gp_delta_out[ph]>0){
                    if(out_load[sl_card][dp][ph]+gp_delta_out[ph]>gp_max_out[sl_card][dp]) ok=0;
                }
            }
            for(int d=0;d<n_dl&&ok;++d){
                int dl=gp_dls[d];
                for(int ph=0;ph<m&&ok;++ph){
                    if(gp_delta_in[dl][ph]>0){
                        if(in_load[dl][dp][ph]+gp_delta_in[dl][ph]>gp_max_in[dl][dp]) ok=0;
                    }
                }
            }
            if(!ok) continue;
