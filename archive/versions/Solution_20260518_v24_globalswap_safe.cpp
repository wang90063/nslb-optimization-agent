...
void solve_job(){
    ...
    // PASS2_HERE
    // Pass 2: local bottleneck swap (same as v22)
    int jmx=0;
    ...
    // Pass 3: conservative global swap — only move if local max doesn't increase
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            int mo=0,mi=0;
            for(int ph=0;ph<m;++ph){
    ...
    for(int giter=0;giter<50;++giter){
        int gmx=0;
        for(int leaf=0;leaf<g_l;++leaf)
            for(int pk=0;pk<g_p;++pk){
        ...
        for(int i=0;i<fl_count&&!improved;++i){
            ...
            for(int pk=0;pk<g_p;++pk){
                ...
                while(m2){
                    int ph=__builtin_ctz(m2);
                    if(out_load[sl][pk][ph]+1>jmx||in_load[dl][pk][ph]+1>jmx){local_ok=0;break;}
                    m2&=m2-1;
                }
                if(!local_ok) continue;
                // compute new projected global if we move flow i from cp to pk
                int new_mo_cp=0,new_mi_cp=0,new_mo_pk=0,new_mi_pk=0;
                for(int ph=0;ph<m;++ph){
                    int oc=out_load[sl][cp][ph]-((mask>>ph)&1);
                    int ic=in_load[dl][cp][ph]-((mask>>ph)&1);
                    int op=out_load[sl][pk][ph]+((mask>>ph)&1);
                    int ip=in_load[dl][pk][ph]+((mask>>ph)&1);
                    if(oc>new_mo_cp)new_mo_cp=oc;
                    if(ic>new_mi_cp)new_mi_cp=ic;
                    if(op>new_mo_pk)new_mo_pk=op;
                    if(ip>new_mi_pk)new_mi_pk=ip;
                }
                ...
                int cur_mo_cp=proj_out[sl][cp]-global_out[sl][cp];
                int cur_mi_cp=proj_in[dl][cp]-global_in[dl][cp];
                int npo=global_out[sl][cp]+new_mo_cp;
                int npi=global_in[dl][cp]+new_mi_cp;
                int npop=global_out[sl][pk]-old_mo_pk+new_mo_pk;
                int npip=global_in[dl][pk]-old_mi_pk+new_mi_pk;
                int ns=npo;if(npi>ns)ns=npi;if(npop>ns)ns=npop;if(npip>ns)ns=npip;
