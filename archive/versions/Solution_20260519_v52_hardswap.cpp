...
void run_global_swap(int m){
    ...
    int post_gmax=get_future_gmax(m);
    int post_jmax=get_job_max(m);
    if(post_jmax>pre_jmax){
        memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
        memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
        memcpy(fl_port, bk_port, fl_count*sizeof(short));
    } else if(post_gmax>=pre_gmax){
        if(pre_jmax<=g_r){
            int post_ci=get_cinphsc(m);
            if(post_ci>bk_ci){
                memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
                memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
                memcpy(fl_port, bk_port, fl_count*sizeof(short));
    ...
    } else if(pre_jmax<=g_r){
        int post_ci=get_cinphsc(m);
        double mm_gain = 40.0*g_r/(double)post_gmax - 40.0*g_r/(double)pre_gmax;
        double ci_cost = 12.0*(post_ci - bk_ci)/(double)(fl_count>0?fl_count:1);
        if(mm_gain - ci_cost < 0){
            memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
            memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
            memcpy(fl_port, bk_port, fl_count*sizeof(short));
        }
...
void run_lns(int m){
    ...
    int post_gmax_lns=get_future_gmax(m);
    int post_jmax_lns=get_job_max(m);
    int rollback_lns=0;
    if(post_gmax_lns>=pre_gmax||post_jmax_lns>pre_jmax) rollback_lns=1;
    else if(pre_jmax<=g_r&&get_cinphsc(m)>pre_ci_lns) rollback_lns=1;
    if(rollback_lns){
        memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
        memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
        memcpy(fl_port, bk_port, fl_count*sizeof(short));
...
// Job-max targeted LNS: destroy all flows on bottleneck (leaf,port,phase), rebuild
static int jlns_idx[MAX_FLOWS];
void run_jm_lns(int m){
    int pre_jmax=get_job_max(m);
    if(pre_jmax<=g_r) return;
    memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
    memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
    memcpy(bk_port, fl_port, fl_count*sizeof(short));
    for(int round=0;round<10;++round){
        int mx=get_job_max(m);
        if(mx<=g_r) break;
        // Find bottleneck (leaf, port, phase, direction)
        int bn_leaf=-1,bn_pk=-1,bn_ph=-1,bn_dir=0;
        for(int leaf=0;leaf<g_l&&bn_leaf<0;++leaf)
            for(int pk=0;pk<g_p;++pk)
                for(int ph=0;ph<m;++ph){
                    if(out_load[leaf][pk][ph]==mx){bn_leaf=leaf;bn_pk=pk;bn_ph=ph;bn_dir=0;goto found_bn;}
                    if(in_load[leaf][pk][ph]==mx){bn_leaf=leaf;bn_pk=pk;bn_ph=ph;bn_dir=1;goto found_bn;}
                }
        found_bn:
        if(bn_leaf<0) break;
        // Collect all flows on this bottleneck
        int cnt=0;
        for(int i=0;i<fl_count;++i){
            if(fl_port[i]!=bn_pk) continue;
            if(!(fl_pmask[i]&(1u<<bn_ph))) continue;
            if(bn_dir==0&&fl_sl[i]!=bn_leaf) continue;
            if(bn_dir==1&&fl_dl[i]!=bn_leaf) continue;
            jlns_idx[cnt++]=i;
        }
        if(cnt==0) break;
        // Remove all these flows
        for(int k=0;k<cnt;++k){
            int i=jlns_idx[k];
            int sl=fl_sl[i],dl=fl_dl[i];
            unsigned int m2=fl_pmask[i];
        ...
        // Re-assign with greedy that penalizes bottleneck port
        for(int k=0;k<cnt;++k){
            int i=jlns_idx[k];
            int sl=fl_sl[i],dl=fl_dl[i];
            unsigned int mask=fl_pmask[i];
        ...
        if(get_job_max(m)>=mx) break; // no improvement this round
    }
    if(get_job_max(m)>=pre_jmax){
        memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
        memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
        memcpy(fl_port, bk_port, fl_count*sizeof(short));
    }
...
void solve_job(){
    ...
    #define TRY_STRATEGY() do { \
        if(get_job_max(m)<=g_r||get_job_max(m)<=g_hist_max_jm) any_jm_le_r=1; \
    ...
    // S-extra: more random restarts for hard cases (seeds 6-13)
    if(!any_jm_le_r){
        for(int seed=6;seed<=13;++seed){
            for(int i=0;i<fl_count;++i) fl_order[i]=i;
            run_greedy(m, 2, 1, 1, seed);
            run_swap(m);
            TRY_STRATEGY();
        }
        // Also try FTRL variants with more shuffles
    ...
    // Final global refinement: try to reduce fg without increasing jm
    // This runs for ALL cases (not just jm<=r)
    run_global_swap(m);

    // Reduce Cbtphsc: consolidate same-card flows onto same port
