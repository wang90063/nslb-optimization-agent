...
void solve_job(){
    ...
    g_job_flow_units=0;
    for(int i=0;i<fl_count;++i) g_job_flow_units += __builtin_popcount(fl_pmask[i]);
    ...
    #define TRY_STRATEGY() do { \
        ...
        if(first||better_metrics(cur_eval,best_eval)){ \
            ...
            int cur_cbt=total_cbtphsc_from_ports(fl_port,m); \
            int best_cbt=total_cbtphsc_from_ports(sv_port,m); \
            double cur_proxy=(40.0/(double)cur_eval.score_fg) - \
                ((5.0*(double)cur_cbt + 3.0*(double)cur_eval.future_over)/(double)g_job_flow_units); \
            double best_proxy=(40.0/(double)best_eval.score_fg) - \
                ((5.0*(double)best_cbt + 3.0*(double)best_eval.future_over)/(double)g_job_flow_units); \
            if(cur_proxy>best_proxy) take_strategy=1; \
        } \
        if(take_strategy){ \
