...
static int gp_order_src_peak[MAX_LEAFS];
static int gp_order_dst_peak[MAX_LEAFS];

int gp_pressure_order_cmp(const void *a, const void *b){
    int ia=*(const int*)a;
    int ib=*(const int*)b;
    int pa=gp_order_src_peak[fl_sl[ia]]+gp_order_dst_peak[fl_dl[ia]];
    int pb=gp_order_src_peak[fl_sl[ib]]+gp_order_dst_peak[fl_dl[ib]];
    if(pa!=pb) return pb-pa;
    int sa=__builtin_popcount(fl_pmask[ia]);
    int sb=__builtin_popcount(fl_pmask[ib]);
    if(sa!=sb) return sb-sa;
    if(fl_sl[ia]!=fl_sl[ib]) return fl_sl[ia]-fl_sl[ib];
    if(fl_dl[ia]!=fl_dl[ib]) return fl_dl[ia]-fl_dl[ib];
    return ia-ib;
}

void build_global_price_pressure_order(){
    for(int leaf=0;leaf<g_l;++leaf){
        int src_peak=0,dst_peak=0;
        for(int pk=0;pk<g_p;++pk){
            if(global_out[leaf][pk]>src_peak) src_peak=global_out[leaf][pk];
            if(global_in[leaf][pk]>dst_peak) dst_peak=global_in[leaf][pk];
        }
        gp_order_src_peak[leaf]=src_peak;
        gp_order_dst_peak[leaf]=dst_peak;
    }
    for(int i=0;i<fl_count;++i) fl_order[i]=i;
    qsort(fl_order,fl_count,sizeof(fl_order[0]),gp_pressure_order_cmp);
}
...
void solve_job(){
    ...
    if(g_p>=32&&g_r>=4&&fl_count<=7000){
        ...
        cur_uses_global_price=1;
        build_global_price_pressure_order();
        run_greedy_global_price(m, 1, 2, 1, 2, 1);
        run_swap(m);
        TRY_STRATEGY();
