...
static double g_sa_budget_override=0.017;
...
void run_sa_composite(int m){
    ...
    double sa_budget=g_sa_budget_override;
    if(fl_count>5000&&sa_budget>0.01) sa_budget=0.01;
...
void solve_job(){
    ...
    // Adaptive SA budget: measure CB, allocate more time to high-CB jobs
    {
        int total_cb=0;
        for(int c=0;c<12800;++c) total_cb+=card_cbtphsc(c,m);
        double cb_ratio=(fl_count>0)?(double)total_cb/fl_count:0.0;
        if(cb_ratio>0.15) g_sa_budget_override=0.028;
        else if(cb_ratio>0.08) g_sa_budget_override=0.022;
        else if(cb_ratio>0.03) g_sa_budget_override=0.017;
        else if(cb_ratio>0.01) g_sa_budget_override=0.010;
        else g_sa_budget_override=0.006;
    }
