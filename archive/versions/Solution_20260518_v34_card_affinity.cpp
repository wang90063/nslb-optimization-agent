...
// Card-port affinity: track preferred port for each source card
static short card_port[MAX_CARDS];  // -1 = no preference yet
...
void solve_job(){
    ...
    memset(card_port,-1,g_pr*sizeof(short));
    ...
    // Pass 1: greedy with global penalty + card-port affinity tiebreaker
    for(int i=0;i<fl_count;++i){
        ...
        int pref_port=card_port[src_card];  // preferred port for this card
        for(int pk=0;pk<g_p;++pk){
            int cost=0;
            ...
            int gv=go>gi?go:gi;
            int total_cost=cost*2+gv;
            // Card affinity: slight preference for card's existing port
            // Use *4 to make affinity a sub-unit tiebreaker
            total_cost=total_cost*4+(pk!=pref_port && pref_port>=0 ? 1 : 0);
            if(total_cost<bc){bc=total_cost;bp=pk;}
        }
        fl_port[i]=(short)bp;
        if(card_port[src_card]<0) card_port[src_card]=(short)bp;
        unsigned int m2=mask;
