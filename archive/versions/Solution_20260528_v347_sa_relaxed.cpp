...
void run_sa_composite(int m){
    ...
    for(int iter=0;;++iter){
        ...
        int fg_penalty=0;
        while(m2){int ph=__builtin_ctz(m2);
            if(out_load[sl][py][ph]>=g_r){ok=0;break;}
            if(in_load[dl][py][ph]>=g_r){ok=0;break;}
            if(out_load[sl][py][ph]+1>sa_max_out[sl][py]) fg_penalty++;
            if(in_load[dl][py][ph]+1>sa_max_in[dl][py]) fg_penalty++;
            m2&=m2-1;}
        ...
        int obj_delta=12*ci_delta+5*cbt_delta+8*fg_penalty;
        int accept=0;
        ...
        if(accept){
            ...
            nmo=0;nmi=0;
            for(int ph=0;ph<m;++ph){
                if(out_load[sl][py][ph]>nmo)nmo=out_load[sl][py][ph];
                if(in_load[dl][py][ph]>nmi)nmi=in_load[dl][py][ph];
            }
            if(nmo>sa_max_out[sl][py])sa_max_out[sl][py]=nmo;
            if(nmi>sa_max_in[dl][py])sa_max_in[dl][py]=nmi;
            sa_accepted++;
        } else {
            m2=mask;
            while(m2){int ph=__builtin_ctz(m2);
