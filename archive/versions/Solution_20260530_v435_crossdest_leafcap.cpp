...
void run_cross_dest_swap(int m){
    ...
    static short cd_leafmax_out[MAX_LEAFS];
            ...
            for(int dpk=0;dpk<cand_limit;++dpk){
                ...
                for(int bi=bstart;bi<bend;++bi){
                    ...
                    m2=only_a;while(m2){int ph=__builtin_ctz(m2);
                        if(out_load[sl][py][ph]+1>cd_leafmax_out[sl]){ok=0;break;}m2&=m2-1;}
                    if(!ok) continue;
                    m2=only_b;while(m2){int ph=__builtin_ctz(m2);
                        if(out_load[sl][px][ph]+1>cd_leafmax_out[sl]){ok=0;break;}m2&=m2-1;}
                    if(!ok) continue;
                    if(dl_a==dl_b){
                        m2=only_a;while(m2){int ph=__builtin_ctz(m2);
