...
static unsigned int cbt_mask[MAX_CARDS][MAX_PHASES];
static unsigned char cbt_seen[MAX_CARDS];
static int cbt_cards[MAX_FLOWS];
struct EvalMetrics{
    int score_jm;
    int score_fg;
    int jm;
...
inline int get_job_cbtphsc(int m){
    int card_cnt=0;
    for(int i=0;i<fl_count;++i){
        int port=fl_port[i];
        if(port<0||fl_sl[i]==fl_dl[i]) continue;
        int src=fl_src[i];
        if(!cbt_seen[src]){
            cbt_seen[src]=1;
            cbt_cards[card_cnt++]=src;
            for(int ph=0;ph<m;++ph) cbt_mask[src][ph]=0;
        }
        unsigned int phase_mask=fl_pmask[i];
        while(phase_mask){
            int ph=__builtin_ctz(phase_mask);
            cbt_mask[src][ph]|=(1u<<port);
            phase_mask&=phase_mask-1;
        }
    }
    int score=0;
    for(int idx=0;idx<card_cnt;++idx){
        int src=cbt_cards[idx];
        for(int ph=0;ph+1<m;++ph){
            unsigned int a=cbt_mask[src][ph];
            unsigned int b=cbt_mask[src][ph+1];
            if(a&&b&&a!=b) ++score;
        }
        cbt_seen[src]=0;
    }
...
inline int better_fo_proxy(
    int fg_a,int over_a,int cbt_a,long long sq_a,
    int fg_b,int over_b,int cbt_b,long long sq_b){
    if(fg_a!=fg_b) return fg_a<fg_b;
    long long pen_a=3LL*over_a+5LL*cbt_a;
    long long pen_b=3LL*over_b+5LL*cbt_b;
    if(pen_a!=pen_b) return pen_a<pen_b;
    return sq_a<sq_b;
}
...
void run_fo_swap(int m){
    EvalMetrics pre_eval=collect_metrics(m);
    if(pre_eval.jm>g_r||pre_eval.future_over<=0) return;
    int pre_cbt=get_job_cbtphsc(m);
    memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
    memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
    memcpy(bk_port, fl_port, fl_count*sizeof(short));
    int cur_jm=pre_eval.jm;
    int cur_gmax=pre_eval.fg;
    int improved=1;
    for(int round=0;round<3&&improved;++round){
        ...
        for(int i=0;i<fl_count;++i){
            ...
            unsigned int mask=fl_pmask[i];
            int cur_mo_s=0,cur_mi_d=0;
            for(int ph=0;ph<m;++ph){
                if(out_load[sl][cp][ph]>cur_mo_s)cur_mo_s=out_load[sl][cp][ph];
                if(in_load[dl][cp][ph]>cur_mi_d)cur_mi_d=in_load[dl][cp][ph];
            }
            int cur_fo_s=global_out[sl][cp]+cur_mo_s;
            int cur_fi_d=global_in[dl][cp]+cur_mi_d;
            int cur_contrib=0;
            if(cur_fo_s>g_r) cur_contrib+=(cur_fo_s-g_r);
            if(cur_fi_d>g_r) cur_contrib+=(cur_fi_d-g_r);
            int best_pk=-1, best_gain=0;
            for(int pk=0;pk<g_p;++pk){
                if(pk==cp) continue;
                ...
                while(m2){
                    ...
                    int o=out_load[sl][pk][ph]+1;
                    int iv=in_load[dl][pk][ph]+1;
                    if(o>cur_jm||iv>cur_jm){ok=0;break;}
                    m2&=m2-1;
                }
                if(!ok) continue;
                // Check fg constraint
                int new_mo_pk=0,new_mi_pk=0;
                for(int ph=0;ph<m;++ph){
                    int o=out_load[sl][pk][ph];
                    if(mask&(1u<<ph)) o++;
                    if(o>new_mo_pk) new_mo_pk=o;
                    int iv=in_load[dl][pk][ph];
                    if(mask&(1u<<ph)) iv++;
                    if(iv>new_mi_pk) new_mi_pk=iv;
                }
                int new_fo_pk=global_out[sl][pk]+new_mo_pk;
                int new_fi_pk=global_in[dl][pk]+new_mi_pk;
                if(new_fo_pk>cur_gmax||new_fi_pk>cur_gmax) continue;
                int new_contrib=0;
                if(new_fo_pk>g_r) new_contrib+=(new_fo_pk-g_r);
                if(new_fi_pk>g_r) new_contrib+=(new_fi_pk-g_r);
                // Recompute old port after removal
                int rem_mo_s=0,rem_mi_d=0;
                for(int ph=0;ph<m;++ph){
                    int o=out_load[sl][cp][ph];
                    if(mask&(1u<<ph)) o--;
                    if(o>rem_mo_s) rem_mo_s=o;
                    int iv=in_load[dl][cp][ph];
                    if(mask&(1u<<ph)) iv--;
                    if(iv>rem_mi_d) rem_mi_d=iv;
                }
                int rem_fo_s=global_out[sl][cp]+rem_mo_s;
                int rem_fi_d=global_in[dl][cp]+rem_mi_d;
                int rem_contrib=0;
                if(rem_fo_s>g_r) rem_contrib+=(rem_fo_s-g_r);
                if(rem_fi_d>g_r) rem_contrib+=(rem_fi_d-g_r);
                int gain=cur_contrib-rem_contrib-new_contrib;
                if(gain>best_gain){best_gain=gain;best_pk=pk;}
            }
            if(best_pk>=0){
                unsigned int m2=mask;
    ...
    EvalMetrics post_eval=collect_metrics(m);
    int post_cbt=get_job_cbtphsc(m);
    if(!better_fo_proxy(
            post_eval.fg,post_eval.future_over,post_cbt,post_eval.future_sq,
            pre_eval.fg,pre_eval.future_over,pre_cbt,pre_eval.future_sq)){
        memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
        memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
        memcpy(fl_port, bk_port, fl_count*sizeof(short));
    }
...
void solve_job(){
    ...
    if(!huge_job){
        ...
        // Extra random restarts targeting future_over diversity
        if(any_jm_le_r && fl_count<=3000){
            for(int seed=30;seed<=49;++seed){
                for(int i=0;i<fl_count;++i) fl_order[i]=i;
                run_greedy(m, 2, 1, 1, seed);
                run_swap(m);
                run_fo_swap(m);
    ...
    // Minimize future_over via local search (swap flows to reduce global excess)
    if(get_job_max(m)<=g_r) run_fo_swap(m);
