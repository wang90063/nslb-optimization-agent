...
void run_greedy_peak(int m, int hardcap, int rev=0){
    memset(out_load,0,g_l*sizeof(out_load[0]));
    memset(in_load,0,g_l*sizeof(in_load[0]));
    if(rev>=2){
        unsigned int seed=rev*2654435761u;
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        for(int pk=0;pk<g_p;++pk){
            ...
            while(m2){
                ...
                int o=out_load[sl][pk][ph]+1;
                int iv=in_load[dl][pk][ph]+1;
                int v=o>iv?o:iv;if(v>active_max)active_max=v;
                if(o>mo)mo=o;
                if(iv>mi)mi=iv;
                if(hardcap&&(o>g_r||iv>g_r)) exceeds=1;
                m2&=m2-1;
            ...
            int peak=mo>mi?mo:mi;
            int fo=global_out[sl][pk]+mo;
            int fi=global_in[dl][pk]+mi;
            int gv=fo>fi?fo:fi;
            int cost=peak*1000 + active_max*10 + gv;
            if(exceeds) cost+=10000;
            if(cost<bc){bc=cost;bp=pk;}
        }
        fl_port[i]=(short)bp;
...
void solve_job(){
    ...
    // S6e2-S6e3: peak-aware greedy, directly optimize per-port all-phase peak.
    run_greedy_peak(m, 1);
    run_swap(m);
    TRY_STRATEGY();

    run_greedy_peak(m, 1, 1);
    run_swap(m);
    TRY_STRATEGY();
