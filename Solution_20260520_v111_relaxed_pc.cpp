...
void run_port_consistency(int m){
    ...
    for(int i=0;i<fl_count;++i){
        ...
        for(int k=0;k<cnt;++k){
            ...
            while(m2){
                int ph=__builtin_ctz(m2);
                if(out_load[sl][best_p][ph]+1>g_r||in_load[dl][best_p][ph]+1>g_r){ok=0;break;}
                m2&=m2-1;
            }
            if(!ok) continue;
            int new_mo=0,new_mi=0;
            for(int ph=0;ph<m;++ph){
                int o=out_load[sl][best_p][ph];
                if(mask&(1u<<ph)) o++;
                if(o>new_mo) new_mo=o;
                int iv=in_load[dl][best_p][ph];
                if(mask&(1u<<ph)) iv++;
                if(iv>new_mi) new_mi=iv;
            }
            if(global_out[sl][best_p]+new_mo>cur_gmax) continue;
            if(global_in[dl][best_p]+new_mi>cur_gmax) continue;
            m2=mask;
