...
// Backup arrays for all-or-nothing swap
...
void solve_job(){
    ...
    // Pass 1: greedy with global penalty
    for(int i=0;i<fl_count;++i){
    ...
    // Save state before swap attempt
    ...
    if(pre_swap_max > g_r){
        ...
        // Check: did swap actually reduce the global job max?
        int post_swap_max = get_job_max(m);
        if(post_swap_max >= pre_swap_max){
            // Swap didn't help - revert to greedy solution
