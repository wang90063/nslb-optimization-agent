...
__attribute__((noinline,cold)) void run_random_start_repair(int m){
    ...
    int n_restarts=(fl_count<=1000)?18:((fl_count<=2500)?12:8);
    ...
    for(int restart=0;restart<n_restarts;++restart){
        ...
        if(perturb_budget>24) perturb_budget=24;
        ...
        if(iter_cap>2200) iter_cap=2200;
        for(int iter=0;iter<iter_cap&&viol_cnt>0;++iter){
            if(no_progress>150+base_viol_cnt*3) break;
            rng=rng*1664525u+1013904223u;
            int fi=viol_flows[rng%viol_cnt];
            if(mc_tabu[fi]>iter){no_progress++;continue;}
            int cp=fl_port[fi];
...
void solve_job(){
    ...
    // Low-r perturb-restart repair: only for tight `jm==r+1` cases with
    // moderate job size, so we strengthen small-search without reviving the
    // old expensive random-restart branch.
    int lowr_jm=get_job_max(m);
    if(g_r<=3&&g_p>=16&&fl_count<=4500&&job_work<=70000&&lowr_jm==g_r+1)
