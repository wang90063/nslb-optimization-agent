...
inline unsigned int xorshift(){rng_state^=rng_state<<13;rng_state^=rng_state>>17;rng_state^=rng_state<<5;return rng_state;}

int run_greedy(int m){
...
void shuffle_order(){
    for(int i=fl_count-1;i>0;--i){
        int j=xorshift()%(i+1);
        int t=fl_order[i];fl_order[i]=fl_order[j];fl_order[j]=t;
    }
}

void sort_by_popcount_desc(){
    int buckets[32]={};
    ...
    for(int i=0;i<fl_count;++i)
        tmp[offsets[__builtin_popcount(fl_pmask[i])]++]=i;
    memcpy(fl_order, tmp, fl_count*sizeof(int));
}
...
void solve_job(){
    ...
    // Multi-start: try different orderings, keep best
    long time_per_job = 4500 / g_n;  // ms budget per job
    long job_deadline = elapsed_ms() + time_per_job;

    // Start 1: popcount descending (heavy flows first)
    sort_by_popcount_desc();
    int best_max = run_greedy(m);
    memcpy(best_port, fl_port, fl_count*sizeof(short));

    // Start 2: original order (if time allows)
    if(elapsed_ms() < job_deadline){
        for(int i=0;i<fl_count;++i) fl_order[i]=i;
        int mx = run_greedy(m);
        if(mx < best_max){best_max=mx; memcpy(best_port, fl_port, fl_count*sizeof(short));}
    }

    // Start 3+: random shuffles (if time allows)
    int attempts = 0;
    while(elapsed_ms() < job_deadline && attempts < 5){
        shuffle_order();
        int mx = run_greedy(m);
        if(mx < best_max){best_max=mx; memcpy(best_port, fl_port, fl_count*sizeof(short));}
        attempts++;
    ...
    // Restore best result and rebuild load arrays
    ...
    for(int i=0;i<fl_count;++i){
        ...
        while(m2){int ph=__builtin_ctz(m2);out_load[sl][fl_port[i]][ph]++;in_load[dl][fl_port[i]][ph]++;m2&=m2-1;}
    }
