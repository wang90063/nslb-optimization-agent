...
// Relaxed swap: allow swaps between flows with same (sl,dl) but different masks
// Constraint: don't increase any cell's load beyond current max_phase_load per (leaf,port)
    ...
    for(int iter=0;iter<5;++iter){
        ...
        while(g_start<rs_valid_cnt){
            ...
            while(g_end<rs_valid_cnt&&rs_keys[g_end].sl==rs_keys[g_start].sl
                  &&rs_keys[g_end].dl==rs_keys[g_start].dl) g_end++;
            int gsz=g_end-g_start;
            if(gsz>=2&&gsz<=150){
                for(int a=g_start;a<g_end;++a){
                    int i=rs_keys[a].idx;
                    int pi=fl_port[i],ci=fl_src[i],sl=fl_sl[i],dl=fl_dl[i];
                    if(pi<0) continue;
                    ...
                    for(int b=a+1;b<g_end;++b){
                        ...
                        int pj=fl_port[j],cj=fl_src[j];
                        if(pj<0||pi==pj||ci==cj) continue;
                        if(fl_pmask[i]==fl_pmask[j]) continue; // already handled by neutral swap
                        unsigned int mj_mask=fl_pmask[j];
                        // Check feasibility: phases in mi only (not mj)
                        unsigned int only_i=mi_mask&~mj_mask;
                        unsigned int only_j=mj_mask&~mi_mask;
                        int ok=1;
                        unsigned int m2=only_i;
                        while(m2){int ph=__builtin_ctz(m2);
                            if(out_load[sl][pj][ph]+1>max_out[sl][pj]||
                               in_load[dl][pj][ph]+1>max_in[dl][pj]){ok=0;break;}
                            if(out_load[sl][pi][ph]-1<0||in_load[dl][pi][ph]-1<0){ok=0;break;}
                            m2&=m2-1;}
                        if(!ok){g_start=g_end;continue;} // skip rest? no, just this pair
                        if(!ok) continue;
                        m2=only_j;
                        while(m2){int ph=__builtin_ctz(m2);
                            if(out_load[sl][pi][ph]+1>max_out[sl][pi]||
                               in_load[dl][pi][ph]+1>max_in[dl][pi]){ok=0;break;}
                            if(out_load[sl][pj][ph]-1<0||in_load[dl][pj][ph]-1<0){ok=0;break;}
                            m2&=m2-1;}
                        if(!ok) continue;
                        ...
                        if(after<before){
                            ...
                            m2=only_i;while(m2){int ph=__builtin_ctz(m2);
                                out_load[sl][pi][ph]--;in_load[dl][pi][ph]--;
                                out_load[sl][pj][ph]++;in_load[dl][pj][ph]++;m2&=m2-1;}
                            m2=only_j;while(m2){int ph=__builtin_ctz(m2);
                                out_load[sl][pj][ph]--;in_load[dl][pj][ph]--;
                                out_load[sl][pi][ph]++;in_load[dl][pi][ph]++;m2&=m2-1;}
                            fl_port[i]=(short)pj;fl_port[j]=(short)pi;
                            pi=pj;improved++;
                        } else {
                            // Revert cpm/cppc
...
static int lowr_beam_score[MAX_FLOWS];
static unsigned char lowr_beam_used[MAX_FLOWS];
...
static inline LowrHotStats collect_lowr_hot_stats(int m,int hot_level){
    LowrHotStats st;
    st.hot_cnt=0;
    st.overflow_sum=0;
    st.hot_pressure=0;
    ...
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk){
            ...
            for(int ph=0;ph<m;++ph){
                int o=out_load[leaf][pk][ph];
                if(o>g_r) st.overflow_sum+=(o-g_r);
                if(o>=hot_level){
                    st.hot_cnt++;
                    st.hot_pressure+=(global_out[leaf][pk]+o)*16+ph;
                }
                if(o>mo) mo=o;
                int iv=in_load[leaf][pk][ph];
                if(iv>g_r) st.overflow_sum+=(iv-g_r);
                if(iv>=hot_level){
                    st.hot_cnt++;
                    st.hot_pressure+=(global_in[leaf][pk]+iv)*16+ph;
                }
                if(iv>mi) mi=iv;
            }
            int fo=global_out[leaf][pk]+mo;
            int fi=global_in[leaf][pk]+mi;
            if(fo>st.future_fg) st.future_fg=fo;
            if(fi>st.future_fg) st.future_fg=fi;
        }
    return st;
}

static inline int better_lowr_hot_stats(const LowrHotStats &a,const LowrHotStats &b){
    if(a.hot_cnt!=b.hot_cnt) return a.hot_cnt<b.hot_cnt;
    if(a.overflow_sum!=b.overflow_sum) return a.overflow_sum<b.overflow_sum;
    if(a.hot_pressure!=b.hot_pressure) return a.hot_pressure<b.hot_pressure;
    if(a.future_fg!=b.future_fg) return a.future_fg<b.future_fg;
    return 0;
}

static inline int better_lowr_beam_move(const LowrBeamMove &a,const LowrBeamMove &b){
    if(a.stats.hot_cnt!=b.stats.hot_cnt) return a.stats.hot_cnt<b.stats.hot_cnt;
    if(a.stats.overflow_sum!=b.stats.overflow_sum) return a.stats.overflow_sum<b.stats.overflow_sum;
    if(a.stats.hot_pressure!=b.stats.hot_pressure) return a.stats.hot_pressure<b.stats.hot_pressure;
    if(a.local_peak!=b.local_peak) return a.local_peak<b.local_peak;
    if(a.stats.future_fg!=b.stats.future_fg) return a.stats.future_fg<b.stats.future_fg;
    if(a.press!=b.press) return a.press<b.press;
    return 0;
}

static inline int collect_lowr_hot_cells(LowrHotCell *cells,int cap,int hot_level,int m){
    int cnt=0;
    for(int leaf=0;leaf<g_l;++leaf)
        for(int pk=0;pk<g_p;++pk)
            for(int ph=0;ph<m;++ph){
                int o=out_load[leaf][pk][ph];
                if(o>=hot_level){
                    int score=(o-hot_level+1)*8192+(global_out[leaf][pk]+o)*16+(m-ph);
                    insert_hot_cell(cells,cnt,cap,leaf,pk,ph,0,o,score);
                }
                int iv=in_load[leaf][pk][ph];
                if(iv>=hot_level){
                    int score=(iv-hot_level+1)*8192+(global_in[leaf][pk]+iv)*16+(m-ph);
                    insert_hot_cell(cells,cnt,cap,leaf,pk,ph,1,iv,score);
                }
            }
    return cnt;
}

static inline int collect_lowr_beam_ports(int fi,int m,int hot_level,short *ports,int cap){
    struct PortCand{
        short pk;
        short new_hot;
        short new_over;
    ...
    for(int pk=0;pk<g_p;++pk){
        if(pk==cp) continue;
        int new_hot=0,new_over=0,local_peak=0;
        int mo=0,mi=0;
        ...
        while(mk){
            ...
            int o=out_load[sl][pk][ph]+1;
            int iv=in_load[dl][pk][ph]+1;
            if(o>hot_level||iv>hot_level){
                ok=0;
                break;
            }
            if(o>g_r) new_over++;
            if(iv>g_r) new_over++;
            if(o>=hot_level) new_hot++;
            if(iv>=hot_level) new_hot++;
        ...
        PortCand cand;
        cand.pk=(short)pk;
        cand.new_hot=(short)new_hot;
        cand.new_over=(short)new_over;
        cand.local_peak=(short)local_peak;
        cand.future_fg=global_out[sl][pk]+mo;
        int fi_fg=global_in[dl][pk]+mi;
        if(fi_fg>cand.future_fg) cand.future_fg=fi_fg;
        cand.press=global_out[sl][pk]+global_in[dl][pk];
        if(cnt<cap){
            best[cnt++]=cand;
        } else {
            int worst=cnt-1;
            if(best[worst].new_hot<cand.new_hot||
               (best[worst].new_hot==cand.new_hot&&best[worst].new_over<cand.new_over)||
               (best[worst].new_hot==cand.new_hot&&best[worst].new_over==cand.new_over&&best[worst].local_peak<cand.local_peak)||
               (best[worst].new_hot==cand.new_hot&&best[worst].new_over==cand.new_over&&best[worst].local_peak==cand.local_peak&&best[worst].future_fg<cand.future_fg)||
               (best[worst].new_hot==cand.new_hot&&best[worst].new_over==cand.new_over&&best[worst].local_peak==cand.local_peak&&best[worst].future_fg==cand.future_fg&&best[worst].press<=cand.press))
                continue;
            best[worst]=cand;
        }
        for(int i=cnt-1;i>0;--i){
            PortCand &a=best[i],&b=best[i-1];
            if(a.new_hot>b.new_hot) break;
            if(a.new_hot==b.new_hot&&a.new_over>b.new_over) break;
            if(a.new_hot==b.new_hot&&a.new_over==b.new_over&&a.local_peak>b.local_peak) break;
            if(a.new_hot==b.new_hot&&a.new_over==b.new_over&&a.local_peak==b.local_peak&&a.future_fg>b.future_fg) break;
            if(a.new_hot==b.new_hot&&a.new_over==b.new_over&&a.local_peak==b.local_peak&&a.future_fg==b.future_fg&&a.press>=b.press) break;
            PortCand t=best[i];
            best[i]=best[i-1];
            best[i-1]=t;
        }
    }
    for(int i=0;i<cnt;++i) ports[i]=best[i].pk;
    return cnt;
}

static void dfs_lowr_hotcell_beam(int m,int hot_level,int depth,int depth_cap,
                                  const LowrHotStats &cur_stats,
                                  EvalMetrics &best_eval,int &improved){
    if(depth>=depth_cap||cur_stats.hot_cnt<=0) return;

    LowrHotCell hot_cells[8];
    int hot_cnt=collect_lowr_hot_cells(hot_cells,8,hot_level,m);
    if(hot_cnt<=0) return;

    int cand_cnt=0;
    for(int i=0;i<hot_cnt;++i){
        LowrHotCell hc=hot_cells[i];
        for(int fi=0;fi<fl_count;++fi){
            if(lowr_beam_used[fi]) continue;
            if(fl_sl[fi]==fl_dl[fi]||fl_port[fi]!=hc.port) continue;
            if(((fl_pmask[fi]>>hc.ph)&1u)==0) continue;
            if(hc.dir==0){
                if(fl_sl[fi]!=hc.leaf) continue;
            } else {
                if(fl_dl[fi]!=hc.leaf) continue;
            }
            if(!cell_core_seen[fi]){
                cell_core_seen[fi]=1;
                lowr_beam_score[fi]=0;
                pc_flows[cand_cnt++]=fi;
            }
            lowr_beam_score[fi]+=hc.score+lowr_core_flow_priority(fi);
        }
    }
    ...
    for(int i=0;i<cand_cnt;++i){
        ...
        for(int j=i+1;j<cand_cnt;++j){
            ...
            if(lowr_beam_score[fj]>lowr_beam_score[fb]||
               (lowr_beam_score[fj]==lowr_beam_score[fb]&&
    ...
    int max_flows=(g_r==2)?8:10;
    if(cand_cnt>max_flows) cand_cnt=max_flows;

    LowrBeamMove moves[40];
    int move_cnt=0;
    int branch_cap=(g_r==2)?4:5;
    for(int i=0;i<cand_cnt;++i){
        int fi=pc_flows[i];
        short port_list[4];
        int port_cnt=collect_lowr_beam_ports(fi,m,hot_level,port_list,4);
        int from_pk=fl_port[fi];
        for(int pi=0;pi<port_cnt;++pi){
            int to_pk=port_list[pi];
            apply_flow_move(fi,from_pk,to_pk);
            LowrHotStats next_stats=collect_lowr_hot_stats(m,hot_level);
            int allow=0;
            if(next_stats.hot_cnt<cur_stats.hot_cnt) allow=1;
            else if(next_stats.hot_cnt==cur_stats.hot_cnt&&next_stats.overflow_sum<=cur_stats.overflow_sum) allow=1;
            else if(depth+1<depth_cap&&next_stats.hot_cnt<=cur_stats.hot_cnt+1&&next_stats.overflow_sum<cur_stats.overflow_sum) allow=1;

            if(allow){
                EvalMetrics next_eval;
                int need_eval=(next_stats.hot_cnt<cur_stats.hot_cnt)||depth+1==depth_cap||next_stats.hot_cnt==0;
                if(need_eval){
                    next_eval=collect_metrics(m);
                    if(better_metrics(next_eval,best_eval)){
                        best_eval=next_eval;
                        improved=1;
                ...
                while(mk){
                    int ph=__builtin_ctz(mk);
                    int o=out_load[sl][to_pk][ph];
                    int iv=in_load[dl][to_pk][ph];
                    if(o>local_peak) local_peak=o;
                    if(iv>local_peak) local_peak=iv;
                    mk&=mk-1;
                }
                LowrBeamMove mv;
                mv.fi=fi;
                mv.from_pk=(short)from_pk;
                mv.to_pk=(short)to_pk;
                mv.local_peak=(short)local_peak;
                mv.press=global_out[sl][to_pk]+global_in[dl][to_pk];
                mv.stats=next_stats;
                if(move_cnt<40){
                    moves[move_cnt++]=mv;
                }
            }
            apply_flow_move(fi,to_pk,from_pk);
        }
    }
    ...
    for(int i=0;i<move_cnt;++i){
        ...
        for(int j=i+1;j<move_cnt;++j)
            if(better_lowr_beam_move(moves[j],moves[best]))
                best=j;
        if(best!=i){
            LowrBeamMove t=moves[i];
            moves[i]=moves[best];
            moves[best]=t;
        }
    }
    if(move_cnt>branch_cap) move_cnt=branch_cap;

    for(int i=0;i<move_cnt;++i){
        LowrBeamMove mv=moves[i];
        apply_flow_move(mv.fi,mv.from_pk,mv.to_pk);
        lowr_beam_used[mv.fi]=1;
        dfs_lowr_hotcell_beam(m,hot_level,depth+1,depth_cap,mv.stats,best_eval,improved);
        lowr_beam_used[mv.fi]=0;
        apply_flow_move(mv.fi,mv.to_pk,mv.from_pk);
    }
}
...
__attribute__((noinline,cold)) void run_lowr_hotcell_beam(int m){
    if(g_r>3||g_p<16) return;
    if(fl_count>1800||(long long)fl_count*m>30000) return;
    ...
    memset(lowr_beam_used,0,fl_count*sizeof(lowr_beam_used[0]));
    LowrHotStats base_stats=collect_lowr_hot_stats(m,base_eval.jm);
    if(base_stats.hot_cnt<2||base_stats.hot_cnt>8) return;
    EvalMetrics best_eval=base_eval;
    int improved=0;
    int depth_cap=(g_r==2)?2:3;
    dfs_lowr_hotcell_beam(m,base_eval.jm,0,depth_cap,base_stats,best_eval,improved);
...
void solve_job(){
    ...
    // First try exact single-flow escapes out of the hottest `jm=r+1` cells.
    run_lowr_hotcell_exact(m);

    // If a small multi-hot-cell residue remains, try a tiny coordinated beam
    // before broader rebuild operators.
    run_lowr_hotcell_beam(m);
