...
static int rk_local[MAX_PORTS],rk_exceeds[MAX_PORTS],rk_gv[MAX_PORTS],rk_fgv[MAX_PORTS];
static int rk_over[MAX_PORTS],rk_sum[MAX_PORTS],rk_delta[MAX_PORTS];
static long long rk_sq[MAX_PORTS];
static int rk_cand[MAX_PORTS],rk_tmp[MAX_PORTS];
...
inline int better_rank_expert(int pa,int pb,int expert){
    switch(expert){
        case 0:
            if(rk_fgv[pa]!=rk_fgv[pb]) return rk_fgv[pa]<rk_fgv[pb];
            if(rk_over[pa]!=rk_over[pb]) return rk_over[pa]<rk_over[pb];
            if(rk_sq[pa]!=rk_sq[pb]) return rk_sq[pa]<rk_sq[pb];
            if(rk_delta[pa]!=rk_delta[pb]) return rk_delta[pa]<rk_delta[pb];
            return pa<pb;
        case 1:
            if(rk_over[pa]!=rk_over[pb]) return rk_over[pa]<rk_over[pb];
            if(rk_fgv[pa]!=rk_fgv[pb]) return rk_fgv[pa]<rk_fgv[pb];
            if(rk_sq[pa]!=rk_sq[pb]) return rk_sq[pa]<rk_sq[pb];
            if(rk_sum[pa]!=rk_sum[pb]) return rk_sum[pa]<rk_sum[pb];
            return pa<pb;
        case 2:
            if(rk_sq[pa]!=rk_sq[pb]) return rk_sq[pa]<rk_sq[pb];
            if(rk_fgv[pa]!=rk_fgv[pb]) return rk_fgv[pa]<rk_fgv[pb];
...
inline int choose_rank_port(int m,int sl,int dl,unsigned int mask,int hardcap,int local_slack){
    int best_exceeds=2;
    int best_local=0x7fffffff;
    int best_fgv=0x7fffffff;
    int best_over=0x7fffffff;
    for(int pk=0;pk<g_p;++pk){
        int local_max=0;
        int exceeds=0;
        int mo=0,mi=0;
        ...
        int fgv=fo>fi?fo:fi;
        int over=0;
        if(fo>g_r) over+=(fo-g_r);
        if(fi>g_r) over+=(fi-g_r);
        rk_local[pk]=local_max;
        rk_exceeds[pk]=exceeds;
        rk_gv[pk]=gv;
        rk_fgv[pk]=fgv;
        rk_over[pk]=over;
        rk_sum[pk]=fo+fi;
        rk_delta[pk]=fgv-gv;
        rk_sq[pk]=(long long)fo*fo+(long long)fi*fi;
        if(exceeds<best_exceeds) best_exceeds=exceeds;
    }
    for(int pk=0;pk<g_p;++pk){
        if(rk_exceeds[pk]!=best_exceeds) continue;
        if(rk_local[pk]<best_local) best_local=rk_local[pk];
    }
    int allow_local=best_local;
    if(local_slack>0&&best_local>g_r) allow_local+=local_slack;
    int cand_cnt=0;
    for(int pk=0;pk<g_p;++pk){
        if(rk_exceeds[pk]!=best_exceeds||rk_local[pk]>allow_local) continue;
        rk_cand[cand_cnt++]=pk;
        if(rk_fgv[pk]<best_fgv) best_fgv=rk_fgv[pk];
        if(rk_over[pk]<best_over) best_over=rk_over[pk];
    }
    if(cand_cnt>10){
        int keep=0;
        int fgv_slack=(allow_local>g_r)?1:0;
        for(int i=0;i<cand_cnt;++i){
            int pk=rk_cand[i];
            if(rk_fgv[pk]<=best_fgv+fgv_slack) rk_tmp[keep++]=pk;
        }
        if(keep>0){
            cand_cnt=keep;
            for(int i=0;i<cand_cnt;++i) rk_cand[i]=rk_tmp[i];
        }
    }
    if(cand_cnt>8){
        int keep=0;
        for(int i=0;i<cand_cnt;++i){
            int pk=rk_cand[i];
            if(rk_over[pk]==best_over) rk_tmp[keep++]=pk;
        }
        if(keep>0){
            cand_cnt=keep;
            for(int i=0;i<cand_cnt;++i) rk_cand[i]=rk_tmp[i];
    ...
    if(cand_cnt<=1) return rk_cand[0];
    int best_pk=rk_cand[0];
    ...
    for(int i=0;i<cand_cnt;++i){
        ...
        for(int expert=0;expert<5;++expert){
            ...
            for(int j=0;j<cand_cnt;++j){
                int pb=rk_cand[j];
                if(pa!=pb&&better_rank_expert(pb,pa,expert)) rank++;
            }
            rank_sum+=rank;
        }
        if(rank_sum<best_rank){
            best_rank=rank_sum;
            best_pk=pa;
        }else if(rank_sum==best_rank){
            if(rk_fgv[pa]!=rk_fgv[best_pk]){ if(rk_fgv[pa]<rk_fgv[best_pk]) best_pk=pa; }
            else if(rk_over[pa]!=rk_over[best_pk]){ if(rk_over[pa]<rk_over[best_pk]) best_pk=pa; }
            else if(rk_sq[pa]!=rk_sq[best_pk]){ if(rk_sq[pa]<rk_sq[best_pk]) best_pk=pa; }
            else if(rk_delta[pa]!=rk_delta[best_pk]){ if(rk_delta[pa]<rk_delta[best_pk]) best_pk=pa; }
            else if(pa<best_pk) best_pk=pa;
        }
    }
    return best_pk;
}

void run_greedy_rank(int m, int hardcap, int rev=0, int local_slack=0){
    ...
    for(int ii=0;ii<fl_count;++ii){
        ...
        int bp=choose_rank_port(m, sl, dl, mask, hardcap, local_slack);
        fl_port[i]=(short)bp;
        unsigned int m2=mask;
...
void solve_job(){
    ...
    // S6r-S6s: constrained rank aggregation on locally best ports.
    // Keep jm/hardcap guard first, then combine several global-aware experts.
    run_greedy_rank(m, 1);
    run_swap(m);
    TRY_STRATEGY();

    run_greedy_rank(m, 1, 1);
    run_swap(m);
    TRY_STRATEGY();
