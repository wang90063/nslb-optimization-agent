...
// Sort flows by (src_leaf, dst_leaf) to group same leaf-pair flows together
void sort_by_leafpair(){
    // counting sort by src_leaf*MAX_LEAFS + dst_leaf
    static int count[MAX_LEAFS*MAX_LEAFS];
    static int tmp[MAX_FLOWS];
    int num_pairs = g_l * g_l;
    memset(count, 0, num_pairs * sizeof(int));
    for(int i=0;i<fl_count;++i){
        int sl=fl_src[i]/g_pr, dl=fl_dst[i]/g_pr;
        count[sl*g_l+dl]++;
    }
    int offsets[MAX_LEAFS*MAX_LEAFS];
    offsets[0]=0;
    for(int i=1;i<num_pairs;++i) offsets[i]=offsets[i-1]+count[i-1];
    for(int i=0;i<fl_count;++i){
        int sl=fl_src[i]/g_pr, dl=fl_dst[i]/g_pr;
        int key=sl*g_l+dl;
        tmp[offsets[key]++]=i;
