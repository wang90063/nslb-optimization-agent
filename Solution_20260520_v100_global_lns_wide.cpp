...
static int glns_idx[MAX_FLOWS];
void run_global_lns_wide(int m){
    int pre_gmax=get_future_gmax(m);
    int pre_jmax=get_job_max(m);
    if(pre_gmax<=g_r) return;
    memcpy(bk_out, out_load, g_l*sizeof(out_load[0]));
    memcpy(bk_in, in_load, g_l*sizeof(in_load[0]));
    memcpy(bk_port, fl_port, fl_count*sizeof(short));
    int best_gmax=pre_gmax;
    static short bn_out[MAX_LEAFS][MAX_PORTS];
    static short bn_in[MAX_LEAFS][MAX_PORTS];
    for(int round=0;round<8;++round){
        memcpy(out_load, bk_out, g_l*sizeof(out_load[0]));
        memcpy(in_load, bk_in, g_l*sizeof(in_load[0]));
        memcpy(fl_port, bk_port, fl_count*sizeof(short));
        int cur_gmax=get_future_gmax(m);
        memset(bn_out,0,g_l*sizeof(bn_out[0]));
        memset(bn_in,0,g_l*sizeof(bn_in[0]));
        for(int leaf=0;leaf<g_l;++leaf)
            for(int pk=0;pk<g_p;++pk){
                int mo=0,mi=0;
                for(int ph=0;ph<m;++ph){
                ...
                if(global_out[leaf][pk]+mo>=cur_gmax) bn_out[leaf][pk]=1;
                if(global_in[leaf][pk]+mi>=cur_gmax) bn_in[leaf][pk]=1;
            }
        int cnt=0;
        for(int i=0;i<fl_count;++i){
            int sl=fl_sl[i],dl=fl_dl[i];
            ...
            if(bn_out[sl][cp]||bn_in[dl][cp]) glns_idx[cnt++]=i;
        }
        if(cnt==0||cnt>6000) break;
        for(int k=0;k<cnt;++k){
            int i=glns_idx[k];
        ...
        unsigned int rng=round*2654435761u+cnt*31+m*7;
        for(int a=cnt-1;a>0;--a){
            rng=rng*1664525u+1013904223u;
            int b=rng%(a+1);
            int t=glns_idx[a];glns_idx[a]=glns_idx[b];glns_idx[b]=t;
        }
        for(int k=0;k<cnt;++k){
            int i=glns_idx[k];
            int sl=fl_sl[i],dl=fl_dl[i];
            ...
            for(int pk=0;pk<g_p;++pk){
                ...
                while(m2){int ph=__builtin_ctz(m2);
                    ...
                    int v=o>iv?o:iv;if(v>local_max)local_max=v;
                    if(o>mo)mo=o;if(iv>mi)mi=iv;
                    if(o>pre_jmax||iv>pre_jmax){local_max=10000;break;}
                    m2&=m2-1;}
                if(local_max>=10000){if(10000<bc){bc=10000;bp=pk;}continue;}
                for(int ph=0;ph<m;++ph){
                    int o=out_load[sl][pk][ph];if(o>mo)mo=o;
                    int iv=in_load[dl][pk][ph];if(iv>mi)mi=iv;
                }
        ...
        int ng=get_future_gmax(m);
        if(ng<best_gmax&&get_job_max(m)<=pre_jmax){
            best_gmax=ng;
...
void solve_job(){
    ...
    // Multi-target global LNS: reduce Maxmultir by re-assigning all flows on near-max ports
    if(restored_jm<=g_r && fl_count<=6000) run_global_lns_wide(m);
